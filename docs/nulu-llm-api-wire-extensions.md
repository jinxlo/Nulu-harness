...
This reference defines every Nulu Harness-specific HTTP header and additive JSON field sent by [`@worldapptechnologies/nulu-llm-gateway`](../packages/llm/llm-gateway/README.md) on `worldapp-gateway` chat-completion requests. It does not redefine fields owned by the upstream Nulu API. The provider-neutral LLM interface and `llm-pi-ai` do not implement these additions.
...
| Nulu request-body extension fields | Snake case with the reserved `nulu_` prefix | `nulu_plugin_packages`, `nulu_session_log` |
| NULU-owned nested JSON members | Camel case | `afterSeq`, `throughSeq`, `sessionId` |
| Tagged values | Kebab-case strings; durable events use `domain/action` | `session-log-gateway/delivery-accepted` |
...
The [`NuluLlmApiExtensionRegistry`](../packages/llm/llm-api-extensions/README.md) reserves one provider per top-level extension name. Empty or whitespace-padded names, duplicate registrations, and collisions with the base Nulu request fail before HTTP dispatch.
...
[`@worldapptechnologies/nulu-plugin-package-inventory`](../packages/llm/plugin-package-inventory/README.md) contributes the complete active Loader-backed plugin package inventory. The field is enabled by default.
