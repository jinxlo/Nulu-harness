...
This opt-in overlay adds a signed GitHub endpoint to `nulu web`. When a pull request in the configured repository changes from draft to ready for review, the rule creates a titled root Session under the repository's Web Workspace and starts a read-only review prompt.

## Prerequisites

- A local checkout that NULU may register as a Web Workspace.
- A high-entropy GitHub webhook secret available through the `NULU_GITHUB_WEBHOOK_SECRET` credential reference.
...
The overlay defaults the Workspace to the launch directory and the listener to `127.0.0.1:3081`. Override them with `NULU_GITHUB_REVIEW_WORKSPACE` and `NULU_GITHUB_WEBHOOK_PORT`.
...
An installed NULU uses the same overlay through an absolute path:

```sh
nulu web --patch /absolute/path/to/github-review/cordis.yml
```

For a permanent profile, place `github-ready-review-rule.mjs` beside `$NULU_HOME/profiles/web/cordis.patch.yml`, append the rows from `cordis.yml` to that patch, and start with `nulu web`. The shipped CLI already contains both webhook packages; the overlay alone activates them.
...
The rule accepts only source `primary-github`, repository `nulu-harness/nulu-harness`, event `pull_request`, and action `ready_for_review`. It passes the exact head SHA plus selected PR fields to the review prompt, labeling the JSON as untrusted metadata and forbidding file, branch, PR, or GitHub mutation.
