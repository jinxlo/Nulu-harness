...
Run `pwd` from the repository root, then create `scratch-plugin/cordis.yml` as a Web overlay that inserts the local plugin. Replace `/absolute/path/to/nulu-harness` below with the printed path:

```yaml
- insert:
    - id: hello
      name: '/absolute/path/to/nulu-harness/scratch-plugin/src/my-plugin.ts'
```
...
```sh
pnpm nulu web --patch ./scratch-plugin/cordis.yml
```
