...
3. Propagate the fixed layer through every affected child in order:
   ...
   - **Native cascading rebase:** use `gh stack rebase`, validate the rewritten layers, then publish with `gh stack push`; or use `gh stack sync`, which may publish first and therefore requires immediate post-sync validation under [nulu-pre-push-checks](../../.agents/skills/nulu-pre-push-checks/SKILL.md).
