...
中性词汇定义在 `nulu-tools` 中；工具绝不导入 UI 或传输类型。使用该 API 的消费方把每个 `card` 映射到自己的视图。设计与原因见[渲染意图联合体 Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.zh.md)；`nulu-tool-fs`（generic/diff）和 `nulu-tool-bash`（terminal）是参考实现。
