...
评审意见可能同时针对一条依赖堆叠（`A ← B ← C …`）中的多个 PR（Pull Request）。请通过 GitHub 官方的堆叠 PR 功能保持这条链的关联。本指南负责评审修复的归属与传播；[nulu-merging-stacked-prs](../../.agents/skills/nulu-merging-stacked-prs/SKILL.md) skill（技能）负责检查关联关系和落地。
...
3. 将修复后的层按顺序传播到每个受影响的子 PR：
   ...
   - **原生级联 rebase：** 使用 `gh stack rebase`，验证所有已改写的层，然后通过 `gh stack push` 发布；也可以使用 `gh stack sync`，该命令可能先发布，因此必须按照 [nulu-pre-push-checks](../../.agents/skills/nulu-pre-push-checks/SKILL.md) 在同步后立即验证。
