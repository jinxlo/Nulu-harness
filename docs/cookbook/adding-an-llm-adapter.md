...
How to connect a new model provider. Reference implementations: `packages/llm/llm-gateway` (direct HTTP, SSE framed by `eventsource-parser`) and `packages/llm/llm-pi-ai` (wrapping an LLM library). Read the `StreamChunk` doc in `packages/llm/llm/src/types.ts` first — it records the protocol conventions both adapters were verified against.
...
Keep wire types, request serialization, transport parsing, chunk translation, and the adapter class as separate responsibilities; [`llm-gateway`](../../packages/llm/llm-gateway/README.md) is the reference layout.
