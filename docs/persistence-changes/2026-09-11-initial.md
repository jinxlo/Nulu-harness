...
# Initial persistence type baseline

English | [中文](2026-09-11-initial.zh.md)

## Summary

This baseline records the complete persistence type inventory declared by the current source and gives later transitions a starting point stored in this tree.
...
This record initializes type history without asserting relationships to earlier formats or changing the Session writer version or runtime behavior. Each root starts without a predecessor and retains its complete normalized schema; successor records reference its after schema.

<a id="verification"></a>
## Verification

Ran `pnpm exec tsx scripts/gen-persistence-catalog.ts` to generate the current inventory and `pnpm run persistence-changes --baseline 2026-09-11-initial` to create this baseline and its schema snapshot.
