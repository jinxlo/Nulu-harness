...
查阅 `nulu-v0.1.3-alpha.2` 对应的 V2 持久化类型。Assistant 流内嵌在 message 或 attempt 事件中，物理 header 要求 `isSeeded`，不再存储 `seedLength`。本快照保留 V3 系统消息与信封变更之前的 V2 声明。
...
来源为已发布 tag `nulu-v0.1.3-alpha.2`，其写入器常量为 2。[tag 记录](../releases/nulu-v0.1.3-alpha.2.zh.md)说明相邻发布之间的变化。本完整目录同时包含未变化的根类型和该 tag 变化的根类型。
