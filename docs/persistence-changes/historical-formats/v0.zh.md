...
查阅 `nulu-v0.1.2-rc.1` 对应的 V0 持久化类型。Assistant chunk 是独立事件，物理 header 以可选的 `seedLength` 存储继承事件数。完整 schema 保留该检查点声明的类型，包括宽泛的数值 header 版本。
...
来源为已发布 tag `nulu-v0.1.2-rc.1`，其写入器常量为 0。这是[已收录预发布归档](../releases/README.zh.md)中最后一个 V0 tag；[tag 记录](../releases/nulu-v0.1.2-rc.1.zh.md)保留其发布背景。更早的 V0 tag 还存在其他类型差异，本快照不替代那些记录。
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:session-log-gateway/delivery-accepted[0].data`](#persistence-type-eventsession-log-gatewaydelivery-accepted0data) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
| `type` | 必需 | `"session-log-gateway/delivery-accepted"` |
...
来源：`packages/session/session-log-gateway/src/types.ts`
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`packages/web/web-search-gateway/src/provider.ts#NuluSearchLlmRequest`](#persistence-type-packageswebweb-search-gatewaysrcprovidertsnulusearchllmrequest) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
| `type` | 必需 | `"web/nulu-search-llm-request"` |
...
来源：`packages/web/web-search-gateway/src/provider.ts`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `max_tokens` | 必需 | `number` |
| `messages` | 必需 | [`event:web/nulu-search-llm-request[0].data.body.messages`](#persistence-type-eventwebnulu-search-llm-request0databodymessages) |
| `model` | 必需 | `string` |
| `tools` | 必需 | [`event:web/nulu-search-llm-request[0].data.body.tools`](#persistence-type-eventwebnulu-search-llm-request0databodytools) |
...
| 位置 | 存在性 | 类型 |
|---|---|---|
| 0 | 必需 | [`event:web/nulu-search-llm-request[0].data.body.messages[0]`](#persistence-type-eventwebnulu-search-llm-request0databodymessages0) |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `content` | 必需 | [`event:web/nulu-search-llm-request[0].data.body.messages[0].content`](#persistence-type-eventwebnulu-search-llm-request0databodymessages0content) |
| `role` | 必需 | `"user"` |
...
| 位置 | 存在性 | 类型 |
|---|---|---|
| 0 | 必需 | [`event:web/nulu-search-llm-request[0].data.body.messages[0].content[0]`](#persistence-type-eventwebnulu-search-llm-request0databodymessages0content0) |
...
来源：`packages/llm/llm/src/types.ts` · `packages/web/web-search-gateway/src/provider.ts`
...
| 位置 | 存在性 | 类型 |
|---|---|---|
| 0 | 必需 | [`event:web/nulu-search-llm-request[0].data.body.tools[0]`](#persistence-type-eventwebnulu-search-llm-request0databodytools0) |
...
| `body` | 必需 | [`event:web/nulu-search-llm-request[0].data.body`](#persistence-type-eventwebnulu-search-llm-request0databody) |
| `endpoint` | 必需 | `string` |
