...
Inspect the V2 persistence types captured at `nulu-v0.1.3-alpha.2`. Assistant streams are embedded in message or attempt events, and the physical header requires `isSeeded` instead of storing `seedLength`. This snapshot retains V2 declarations before the V3 system-message and envelope changes.
...
The source is the published tag `nulu-v0.1.3-alpha.2`, whose writer constant is 2. The [tag record](../releases/nulu-v0.1.3-alpha.2.md) describes the adjacent release changes. This complete inventory includes unchanged roots as well as that tag’s changed roots.
...
roots:
  ...
  "event:session-log-gateway/delivery-accepted": d63b8b8ffad9c02fd80c43a17df4f240c1fe8118ecca9de34f9d5871838ab5b9
