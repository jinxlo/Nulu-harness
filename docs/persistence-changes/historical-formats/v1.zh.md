...
来源为 `nulu-v0.1.3-alpha.1` 祖先历史中首次 V2 写入器变更之前的最后一个 V1 源码树，关联于 PR #3349（分支 `worktree/session-format-03-v0-v1-migration`）。其源码变更日期为 2026-09-02，标题为 `fix(session): close live assistant framing races`。这标识中间源码树，而非 PR 最终合并的源码树。[预发布归档](../releases/README.zh.md)没有 V1 tag，本记录不会为此虚构产品发布。
