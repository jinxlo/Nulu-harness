...
> This document describes the architecture of the Nulu Harness — the foundation of **DeepSeek Code**. The governing principle, from the microkernel design discussion: **everything is a plugin**. The core is deliberately tiny — a handful of abstract services plus one concrete loop plugin (`nulu-agent-loop`) — and every product feature is a plugin against the extension API described here, without modifying the loop.

本文介绍 Nulu Harness 整体架构，它是 **DeepSeek Code** 的底层基座。微内核设计讨论中确立了核心设计准则：**一切皆插件**。内核刻意做得极精简，仅包含少量抽象服务，外加一个实体循环插件 `nulu-agent-loop`。所有产品功能均基于本文定义的扩展接口开发为独立插件，无需改动主循环逻辑。

> Dependency rule: extension plugins depend on interfaces, never on `nulu-agent-loop` (the loop is swappable); composition bundles such as `nulu-base` and `nulu-sdk-minimal` may assemble the concrete loop.

依赖约束规范：各类扩展插件仅依赖抽象接口，严禁直接依赖 `nulu-agent-loop`（该主循环支持替换实现）；`nulu-base` 与 `nulu-sdk-minimal` 等组合包可以组装具体循环。
...
- 代码体标识符（事件名 `agent/status`、状态值 `running`、包名 `nulu-bash-local` 等）在译文中保留 code span 原文，不得口语化改写；Pass 2 必须逐句核验。
