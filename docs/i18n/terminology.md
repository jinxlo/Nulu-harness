...
| RAG | RAG | RAG（检索增强生成） | | |
| SDK | SDK | | | 只指受支持的 Python 与 TypeScript SDK 所使用的 JSON-RPC 客户端／服务器协议；Nulu Harness 项目本身不是 SDK |
...
| Typert | Typert | | TypeRT、typeRT、Type RT | Nulu Harness 类型图、生成器、loader 与运行时 registry 的产品拼写 |
...
| package | 包 | | | 指 npm 包（`@worldapptechnologies/nulu-*`）；`package.json` 等代码标识保持原样 |
| pairing | 配对 | | | |
