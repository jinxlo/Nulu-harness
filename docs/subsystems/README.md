...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `NULU_*` environment vocabulary |
...
| [client-modules.md](client-modules.md) | the web plugin table: `nulu.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
...
| [client-resources.md](client-resources.md) | the client resource model: `nulu-resource://<type>/…` addresses, protocol providers and `ResourceProtocolMap`, the `useResource` global hook and its states, pins and release |
