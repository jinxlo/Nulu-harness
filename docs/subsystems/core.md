...
|---|---|---|---|
| `ContentBlockMap` | nulu-llm | `ContentBlock` | [llm-streaming.md](llm-streaming.md#content-blocks-and-messages) |
| `MessageSourceMap` | nulu-llm | `MessageSource` | [llm-streaming.md](llm-streaming.md#content-blocks-and-messages) |
| `FinishReasonMap` | nulu-llm | `FinishReason` | [llm-streaming.md](llm-streaming.md#the-model-request-and-result) |
| `TurnEndReasonMap` | nulu-session | `TurnEndReason` | [session.md](session.md) |
| `SessionEventMap` | nulu-session | `SessionEvent` | [session.md](session.md) |
...
The `Branded<B>` primitive and stateless constructor live in [nulu-brand](../../packages/util/brand), which has no harness capability dependency. `brandString<T>()` applies a compile-time-only string brand.
...
The two core IDs are `ToolCallId` (correlates a tool call with its result; nulu-llm) and `SessionId` (the shared live agent and durable session identity; nulu-session). Capability packages brand their own ids too, such as `JobId` in [jobs.md](jobs.md).
