...
Nulu Harness 是实验性的开发者预览软件。它尚未接受安全审计，不得视为安全或可用于生产环境的软件。
...
不要把 Nulu Harness 当作不可信工作负载唯一的安全控制措施。
...
请在充分了解相关风险的前提下使用 Nulu Harness。本软件依照 [MIT License](LICENSE) 提供，不附带任何保证。在适用法律允许的最大范围内，对于使用本项目造成的计算机损坏、数据丢失或泄露、文件丢失或其他损害，作者和版权持有人不承担责任。
