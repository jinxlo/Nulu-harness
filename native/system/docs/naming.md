...
The public package family belongs to the `@worldapptechnologies` scope and uses the `node-addon-system` package prefix; platform packages append platform information only:

```text
@worldapptechnologies/node-addon-system
@worldapptechnologies/node-addon-system-<platform>
```
