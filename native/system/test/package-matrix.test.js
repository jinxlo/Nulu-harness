...
test('the real Landlock subpath imports without platform packages or dlopen, while the root is unexported', { timeout: 120_000 }, (t) => {
  ...
  const installed = path.join(dir, 'node_modules', '@worldapptechnologies', 'node-addon-system');
  ...
  const result = spawnSync(process.execPath, ['--no-addons', '--input-type=module', '--eval', `
    ...
    try {
      ...
      const api = await import('@worldapptechnologies/node-addon-system/landlock-run');
      ...
      assert.throws(() => createRequire(import.meta.url).resolve('@worldapptechnologies/node-addon-system'), {
        code: 'ERR_PACKAGE_PATH_NOT_EXPORTED',
      });
    } finally {
