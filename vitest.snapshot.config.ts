...
if (process.env.NULU_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.NULU_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.NULU_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
