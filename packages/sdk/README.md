...
The SDK family lets another process drive a complete Nulu Harness runtime over newline-delimited JSON-RPC. Its protocol package defines the public messages, the TypeScript client launches `nulu` with a named profile and ordered patches, and the server accepts SDK requests over stdio. Clients can open sessions, send prompts, and observe session events, agent status changes, and subagent completions. The TypeScript client and [Python SDK](../../python/README.md) use the same protocol, and these packages do not create developer projects or define another application.
...
- [SDK application bundle](../bundle/sdk-app/README.md) — the `nulu --profile sdk` application that boots the JSON-RPC server.
...
- [SDK subagent provider](../subagent/subagent-nulu-sdk/README.md) — a harness-internal consumer of the TypeScript client.
