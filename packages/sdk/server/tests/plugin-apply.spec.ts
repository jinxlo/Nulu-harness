...
describe('nulu-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-init-'))
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 'init-1', method: 'initialize', params: { cwd: storageDir, provider: 'worldapp-gateway', model: 'apply-model' } })

      const response = await harness.waitForFrame(frame => frame.id === 'init-1', 'initialize response')
      expect(response).toEqual({
        jsonrpc: '2.0',
        id: 'init-1',
        result: { serverInfo: { name: 'nulu-harness-sdk-runtime', version: '0.0.1' } },
      })
  ...
  it('waits for Loader-owned adapter registration before initialize', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-readiness-'))
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-prompt-'))
    const llmServer = await mockCompletionServer()
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_BASE_URL', llmServer.url)
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { cwd: storageDir, provider: 'worldapp-gateway', model: 'dsagent-model' } })
  ...
  it('answers shutdown before exiting 0 exactly once, even against a racing second shutdown', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-shutdown-'))
    ...
    try {
      ...
      harness.send({ jsonrpc: '2.0', id: 'after-exit', method: 'initialize', params: { cwd: storageDir, provider: 'worldapp-gateway', model: 'x' } })
      await settle()
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-flush-failure-'))
    ...
    try {
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'after-flush-failure', method: 'initialize', params: { cwd: storageDir, provider: 'worldapp-gateway', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
    } finally {
      await harness.dispose()
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-apply-dispose-'))
    const harness = await mountPlugin(storageDir)
    try {
      ...
      expect(error.error).toMatchObject({
        code: -32603,
        message: 'unknown Nulu Harness SDK runtime method: nope/unknown',
      })
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'probe-2', method: 'initialize', params: { cwd: storageDir, provider: 'worldapp-gateway', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
      expect(harness.exits()).toEqual([])
    } finally {
