...
import { createUserMessage, LlmAdapter, ReasoningEffortId } from '@worldapptechnologies/nulu-llm'
...
import AgentRegistry, { type Agent, type AgentHandle } from '@worldapptechnologies/nulu-agent'
...
import SubagentRuntime, { type SubagentResult, type SubagentRunEndInfo } from '@worldapptechnologies/nulu-subagent'
import type { JsonRpcTransportPeer } from '@worldapptechnologies/nulu-sdk-protocol'
...
describe('HarnessSdkJsonRpcServer', () => {
  it('creates a harness agent and calls the configured OpenAI-compatible endpoint', { timeout: 15_000 }, async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-'))
    const llmServer = await mockCompletionServer()
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_BASE_URL', llmServer.url)
    ...
    try {
      ...
      expect(init.serverInfo.name).toBe('nulu-harness-sdk-runtime')
      ...
      const orphanHandle = await ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { provider: 'worldapp-gateway', model: 'dsagent-model' },
      })
  ...
  it('notifies the host when a child session is created with parent lineage', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
  ...
  it('creates an SDK session without an optional system prompt', { timeout: 15_000 }, async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-no-system-'))
    const llmServer = await mockCompletionServer()
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_BASE_URL', llmServer.url)
    const ctx = await makeHarness(storageDir)
    try {
      ...
      await server.initialize({ cwd: storageDir, provider: 'worldapp-gateway', model: 'plain-model' })
      await server.prompt({
        sessionId: 'plain',
  ...
  it('notifies the host when a subagent run settles', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-end-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
      ...
      const parentHandle = await ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { provider: 'worldapp-gateway', model: 'worldapp-gateway' },
      })
      ...
      const parentlessHandle = await parentHandle.agent.ctx.agents.create({
        ...
        meta: { cwd: storageDir },
        agentOptions: { model: 'worldapp-gateway' },
  ...
  it('ignores a remote run id that collides with a local child of the same parent', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-remote-collision-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
  ...
  it('retains locality across continuation runs on one live child', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-continuation-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
  ...
  it('correlates reused local ids by parent scope when runs settle out of order', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-reuse-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
  ...
  it('keeps locality bound to the accepted run across provider re-registration', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-provider-reuse-'))
    const ctx = await makeHarness(storageDir)
    try {
      const transport = new FakeTransport()
      const server = new HarnessSdkJsonRpcServer(ctx, transport)
  ...
  it('uses the recorded local flag when start was missed and ignores remote runs', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-subagent-fallback-'))
    const ctx = await makeHarness(storageDir)
  ...
  it('does not re-register an LLM adapter whose provider already has an owner', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-existing-llm-'))
    const ctx = await makeHarness(storageDir)
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    ...
    try {
      ...
      expect(inspect.hasAdapterFor('worldapp-gateway')).toBe(true)
      expect(inspect.hasAdapterFor('missing-provider')).toBe(false)
      await server.initialize({ cwd: storageDir, provider: 'worldapp-gateway', model: 'preinstalled-model' })

      expect(ctx.get('llm')?.listProviders().filter(provider => provider.id === 'worldapp-gateway')).toEqual([{ id: 'worldapp-gateway', name: 'Nulu' }])
      await server.shutdown()
    } finally {
      await ctx.fiber.dispose()
      await rm(storageDir, { recursive: true, force: true })
  ...
  it('rejects a missing non-Nulu provider when an LLM service already exists', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-new-llm-'))
    const ctx = await makeHarness(storageDir)
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    await ctx.plugin(LlmGateway)
    try {
      ...
      expect(ctx.get('llm')?.listProviders()).toEqual([{ id: 'worldapp-gateway', name: 'Nulu' }])
      await server.shutdown()
    } finally {
      await ctx.fiber.dispose()
      await rm(storageDir, { recursive: true, force: true })
  ...
  it.each([0, -1, 1.5, Number.NaN, Number.MAX_SAFE_INTEGER + 1])(
    ...
    async (maxTokens) => {
      const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-invalid-max-tokens-'))
      const ctx = await makeHarness(storageDir)
      try {
        const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
  ...
  it('rejects an unavailable exact model during initialize', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-invalid-route-'))
    const ctx = await makeHarness(storageDir)
  ...
  it('rejects prompts while exact-route initialization is pending', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-pending-route-'))
    const ctx = await makeHarness(storageDir)
  ...
  it('rejects an unsupported reasoning effort during initialize', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-unsupported-reasoning-'))
    const ctx = await makeHarness(storageDir)
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    try {
      const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
  ...
  it('rejects unknown JSON-RPC runtime methods', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'nulu-jsonrpc-unknown-'))
    const ctx = await makeHarness(storageDir)
    try {
      const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
      ...
      await expect(server.handleRequest('does/not/exist', {}))
        .rejects
        .toThrow('unknown Nulu Harness SDK runtime method: does/not/exist')
