/**
 * Low-level JSON-RPC client for a Nulu Harness SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@worldapptechnologies/nulu-sdk-protocol` wire over the child's stdio, fans
 ...
 * spawns directly rather than through the `nulu-subprocess` service — the
 * seam's documented exception for SDK-managed transports.
 *
 * @module @worldapptechnologies/nulu-sdk-client/client
 */
...
import { resolveNuluLaunch, type RuntimeProcessOptions } from './launch.ts'
...
/**
 * JSON-RPC client for the Nulu Harness SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
  /** Original public nulu launch and timeout options for this client. */
  ...
  /** @param options - nulu profile, patch, home, process, environment, and timeout options. */
  constructor(options?: HarnessClientOptions)
  constructor(options: HarnessClientOptions = {}, runtime?: RuntimeProcessOptions) {
    this.options = options
    this.runtime = runtime ?? resolveNuluLaunch(options)
  }
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('Nulu Harness runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('Nulu Harness runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('Nulu Harness runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('Nulu Harness runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('Nulu Harness runtime is not running')
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('Nulu Harness runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('Nulu Harness runtime closed'))
  }
