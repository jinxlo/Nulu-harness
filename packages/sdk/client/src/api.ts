/**
 * High-level run API over {@link HarnessClient}: `NuluHarness` owns one
 ...
 *
 * @module @worldapptechnologies/nulu-sdk-client/api
 */

import { randomUUID } from 'node:crypto'
import { resolve } from 'node:path'
import type { SessionEvent, TurnEndReason } from '@worldapptechnologies/nulu-session'
...
import type { ContentBlock, NuluHarnessOptions, HarnessNotification, RunResult, SdkPromptContentBlock } from './types.ts'

/**
 * Reusable SDK for running Nulu Harness agent turns in a runtime
 ...
 * child is reaped.
 */
export class NuluHarness implements AsyncDisposable {
  private clientInstance: HarnessClient
  private readonly createClient: () => HarnessClient
  private readonly cwd: string
  private readonly provider: string
  private readonly model: string
  private readonly reasoningEffort: NuluHarnessOptions['reasoningEffort']
  private readonly maxTokens: number | undefined
  private initialized: Promise<void> | undefined
  private closed = false

  /** @param options - nulu launch configuration plus the session route, effort, and output cap. */
  constructor(options?: NuluHarnessOptions)
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, cleanupError],
            'Nulu Harness initialization and cleanup failed',
          )
        }
...
export function createProcessNuluHarness(
  runtime: RuntimeProcessOptions,
  options: NuluHarnessOptions = {},
): NuluHarness {
  const Constructor = NuluHarness as unknown as new (
    publicOptions: NuluHarnessOptions,
