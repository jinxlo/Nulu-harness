---
description: "面向以子进程方式启动 Nulu Harness 运行时、并通过 stdio JSON-RPC 驱动 agent（智能体）轮次的调用方的 TypeScript SDK 客户端：NuluHarness 运行 API 与低层 HarnessClient。"
...
`nulu-sdk-client` 让 TypeScript 程序通过 stdio JSON-RPC 启动并驱动完整的 Nulu Harness 运行时。使用 `NuluHarness` 可打开会话、发送文本或图像提示词、收集事件与通知流，并在运行时进入 idle 后取得最后提交的助手响应；使用 `HarnessClient` 可直接发送协议请求和订阅通知。调用方可以提供 `nuluBin`；否则客户端解析同版本的 `@worldapptechnologies/nulu` 可执行文件。客户端跨多次运行持有子进程，公开类型化的传输与协议错误，并在 `close()` 或 `await using` 时回收进程。它适用于调用方能够选择运行时 profile 和启动设置的场景。
...
当 TypeScript 代码需要从另一进程驱动完整 Harness 运行时、且你能显式指名运行时可执行文件时，使用本客户端。常用路径极简：用启动规格构造 `NuluHarness`，运行提示词，然后关闭它，使子进程总能被回收。

### 用 NuluHarness 运行 agent 轮次
...
客户端是同一协议上的两层：`NuluHarness`（自有运行）叠加在 `HarnessClient`（协议客户端）之上，与 Python SDK 的分层一致。它运行在任何 harness 上下文之外，因此直接 spawn 运行时而非经由 `nulu-subprocess` 服务——即该 seam 记录的 SDK 托管传输例外——其关闭阶梯也位于本包。运行时对上下文内每个会话都发通知；会话树范围限定是客户端对 `subagent.started` 血缘边的过滤。

### 源码地图

| 文件 | 职责 |
|---|---|
| [`src/api.ts`](src/api.ts) | `NuluHarness` + `HarnessSession`：自有运行、从回执到 idle 的收集、`finalResponse` |
...
- [SDK subagent 后端](../../subagent/subagent-nulu-sdk/README.zh.md) — harness 内部消费本客户端的例子。
- [SDK 应用组合包](../../bundle/sdk-app/README.zh.md) — 本客户端启动的 `nulu --profile sdk` 运行时应用。
...
- **无捆绑运行时解析**——客户端解析同版本 `@worldapptechnologies/nulu` 包（或调用方提供的 `nuluBin`）；打包可执行文件的发现留在 Python 侧，直到出现 TypeScript 发行版消费方。
