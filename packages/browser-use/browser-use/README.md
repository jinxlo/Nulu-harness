...
# @worldapptechnologies/nulu-browser-use
...
The service has no configuration. Provider plugins inject `browserUse` and call `ctx.browserUse.register(BrowserUseProviderName(name))`; the brand is exported from `@worldapptechnologies/nulu-browser-use/brand`. The returned effect disposer releases that registration.
