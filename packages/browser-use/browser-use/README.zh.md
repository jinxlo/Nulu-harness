...
服务没有配置。提供方 插件注入 `browserUse` 并调用 `ctx.browserUse.register(BrowserUseProviderName(name))`；品牌类型从 `@worldapptechnologies/nulu-browser-use/brand` 导出。返回的 effect 清理器释放该注册。
