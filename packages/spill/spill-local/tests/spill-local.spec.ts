...
import type { SaveTextSpill } from '@worldapptechnologies/nulu-spill'
import LocalSpillStore, {
  ...
  sessionDir,
  sweepSpillRoots,
} from '@worldapptechnologies/nulu-spill-local'
import type { SweepRoot } from '@worldapptechnologies/nulu-spill-local'
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'nulu-spill-test-'))
})
afterEach(() => {
...
describe('LocalSpillStore service', () => {
  ...
  it('defaults cleanupPeriodDays to 30', async () => {
    ...
    const emptyBase = mkdtempSync(join(tmpdir(), 'nulu-empty-'))
...
describe('startup cleanup sweep', () => {
  ...
  it('prunes an emptied DISCOVERED default root but never the active root', async () => {
    ...
    const prior = mkdtempSync(join(tmpdir(), 'nulu-spill-'))
  ...
  it('covers the configured root AND discovered default roots (real gatherRoots)', async () => {
    ...
    const fakeTmp = mkdtempSync(join(tmpdir(), 'nulu-faketmp-'))
...
describe('discoverDefaultRoots', () => {
  it('returns only real nulu-spill-* directories, excluding symlinks and non-matches', async () => {
    const base = mkdtempSync(join(tmpdir(), 'nulu-disc-'))
    try {
      // A real backend-shaped root (nulu-spill-<6>) via mkdtemp — the only match.
