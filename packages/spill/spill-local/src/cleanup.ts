...
/**
 * A backend-generated default root name: `nulu-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `nulu-spill-test-*` fixture
 * or a foreign tool's differently-shaped `nulu-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `nulu-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `nulu-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `nulu-spill-test-*` fixture or a foreign
