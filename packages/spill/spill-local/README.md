...
Mount this backend in a composition that spills text to the local filesystem. It registers as the `ctx.spillStore` service that the `nulu-spill-policy` plugin and other callers use.
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-spill-local) is the exhaustive source for every accepted field.
...
- [nulu-spill-policy](../spill-policy/README.md) — the policy that calls this backend when a result is too large.
