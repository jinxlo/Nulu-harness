 ...
 * `@worldapptechnologies/nulu-spill-local` (host filesystem) is the first.
 *
 * The Service Definition is deliberately minimal: `saveText` and nothing else. It owns NO
 * retention policy (that is `@worldapptechnologies/nulu-output-retention`), NO tool-result
 * replacement (that is `@worldapptechnologies/nulu-spill-policy`), and NO retrieval or
 * search API. The backend supplies the locator and retrieval hint appropriate
 * for its storage substrate.
 *
 * @module @worldapptechnologies/nulu-spill
 */
