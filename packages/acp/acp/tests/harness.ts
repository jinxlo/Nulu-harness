...
import AttachmentStore, { AttachmentError, AttachmentId } from '@worldapptechnologies/nulu-attachment'
...
import { type GenerateOptions, LlmAdapter, ReasoningEffortId, type LlmResolvedModelInfo, type StreamChunk } from '@worldapptechnologies/nulu-llm'
...
export async function makeBridgeHarness(options: {
  ...
  const persistenceRoot = options.persistenceRoot ?? await mkdtemp(join(tmpdir(), 'nulu-acp-test-'))
  ...
  const clientApp = createAcpClientApp({ name: 'nulu-acp-test-client' })
