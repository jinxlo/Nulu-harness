...
import { ToolCallId, type StreamChunk } from '@worldapptechnologies/nulu-llm'
import { SESSION_FORMAT_VERSION, SessionId } from '@worldapptechnologies/nulu-session'
import type { SessionHeader } from '@worldapptechnologies/nulu-session'
import { SessionPersistenceRevision, type SessionPersistenceSnapshot } from '@worldapptechnologies/nulu-session-persistence'
...
describe('automation-only ACP bridge', () => {
  ...
  it('lists and resumes persisted sessions after an equivalent process restart', async () => {
    const persistenceRoot = await mkdtemp(join(tmpdir(), 'nulu-acp-restart-'))
    try {
