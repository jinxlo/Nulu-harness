---
description: "Automation-only ACP (Agent Client Protocol) server for programmatic clients and maintainers driving Nulu Harness agents over JSON-RPC stdio."
...
`nulu-acp` lets trusted programs automate persistent Nulu Harness agents through the standard [ACP](https://agentclientprotocol.com): create or resume sessions, select a model and reasoning effort, attach MCP servers, submit or cancel work, receive semantic updates, and close sessions independently. Choose it for out-of-process subagents, test runners, and scripted controllers; it intentionally omits NULU-specific presentation data and interactive UI features. Persistence supports listing, resuming, and closing sessions across process restarts, but deletion, forks, transcript replay, and additional directories are unsupported. Run `pnpm nulu --profile acp` to start the server; use `nulu-subagent-acp` as the repository client.
...
```yaml
- name: '@worldapptechnologies/nulu-acp'
  config:
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-acp) is the exhaustive source for every accepted field and its JSDoc.

### Start a server

`pnpm nulu --profile acp` starts the shipped stdio server. The `acp` profile mounts session persistence, so clients can list, resume, and close persistent sessions. [`@worldapptechnologies/nulu-subagent-acp`](../../subagent/subagent-acp/README.md) starts the same profile for out-of-process delegation.
...
- **Standard semantic updates only.** The wire carries committed messages and thoughts, generic tool lifecycle, configuration, and context usage; raw provider deltas, retry attempts, NULU presentation data, and unsupported content stay off the wire.
...
- [nulu-subagent-acp](../../subagent/subagent-acp/README.md) — the out-of-process ACP client that spawns and drives this server.
...
- **MCP tools only** — MCP resources and prompts have no NULU consumer.
