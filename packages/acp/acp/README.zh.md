---
description: "面向程序化客户端与维护者的仅自动化 ACP（Agent Client Protocol）服务器，用于通过 JSON-RPC stdio 驱动 Nulu Harness agent（智能体）。"
...
`nulu-acp` 让受信程序通过标准 [ACP](https://agentclientprotocol.com) 自动操作持久 Nulu Harness agent：创建或恢复会话、选择模型与推理强度、挂载 MCP 服务器、提交或取消工作、接收语义更新，并独立关闭会话。进程外 subagent、测试运行器与脚本化控制器适合选择它；它刻意不提供 NULU 专用呈现数据与交互式 UI 功能。持久化支持跨进程重启列出、恢复与关闭会话，但不支持删除、fork、transcript（文本记录）回放与附加目录。运行 `pnpm nulu --profile acp` 可启动服务器；仓库客户端使用 `nulu-subagent-acp`。
...
当自动化应拥有交互时选择它：管理持久会话、工具、模型选择与权限的进程外 subagent、测试运行器或脚本化控制器。当人类需要 NULU 专用呈现卡片、计划、标题、todo、终端视图或 elicitation 时请避开；本服务器刻意只提供标准 ACP v1 界面。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-acp)是每个受支持字段及其 JSDoc 的穷尽式真源。

### 启动服务器

`pnpm nulu --profile acp` 会启动随附的 stdio 服务器。`acp` profile 会挂载会话持久化，因此客户端可以列出、恢复和关闭持久会话。[`@worldapptechnologies/nulu-subagent-acp`](../../subagent/subagent-acp/README.zh.md) 会启动同一 profile 来执行进程外委派。
...
- **只发送标准语义更新。** 协议承载已提交消息与 thought、通用工具生命周期、配置与上下文用量；原始提供方增量、重试尝试、NULU 呈现数据与不受支持内容不会进入协议。
...
- [nulu-subagent-acp](../../subagent/subagent-acp/README.zh.md)——spawn 并驱动本服务器的进程外 ACP 客户端。
...
- **仅 MCP 工具**——MCP resource 与 prompt 没有 NULU 消费方。
