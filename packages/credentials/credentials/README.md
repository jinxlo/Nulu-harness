...
- **A key the launching environment supplies cannot be overwritten** — `DEEPSEEK_API_KEY=… nulu` (or a CI secret, a container `-e`) wins for this run and is reported read-only; clear the variable in the launching shell before storing a different value.
