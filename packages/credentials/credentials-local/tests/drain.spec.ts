...
import { credentialKey, credentialRef } from '@worldapptechnologies/nulu-credentials'
...
vi.mock('@worldapptechnologies/nulu-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@worldapptechnologies/nulu-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@worldapptechnologies/nulu-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('NULU_CRED_DRAIN_A')
const OTHER = credentialRef('NULU_CRED_DRAIN_B')
...
describe('write-drain teardown', () => {
  it('lets the in-flight write land and fails the queued one after disposal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nulu-credentials-drain-'))
  ...
  it('fails a queued record write after disposal on the same terms', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nulu-credentials-drain-record-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const ctx = new Context()
    const fiber = ctx.plugin(LocalCredentialProvider, { path: join(dir, '.credentials.yaml'), watch: false })
    await fiber
