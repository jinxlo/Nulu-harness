...
const KEY = credentialRef('NULU_CRED_TEST')
const OTHER = credentialRef('NULU_CRED_OTHER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nulu-credentials-local-'))
...
describe('resolveSpec', () => {
  it('defaults to .credentials.yaml under the harness home with watching on', () => {
    const spec = resolveSpec({ nuluHome: '/custom/home' })
  ...
  it('lets an explicit path win over the home', () => {
    const spec = resolveSpec({ path: '/etc/nulu/creds.yaml', nuluHome: '/ignored', watch: false, debounceMs: 5 })
    expect(spec).toEqual({ filename: resolve('/etc/nulu/creds.yaml'), watch: false, debounceMs: 5 })
  })
})
...
describe('layering and reads', () => {
  ...
  it('serves file entries alongside comments and quoted values', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  # notes\n  NULU_CRED_TEST: plain\n  NULU_CRED_OTHER: "with space"\n')
  ...
  it('lets a non-empty process environment win read-only over the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  NULU_CRED_TEST: from-file\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('NULU_CRED_TEST', 'from-env')
  ...
  it('treats an empty environment value as absent, falling through to the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  NULU_CRED_TEST: stored\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('NULU_CRED_TEST', '')
...
describe('layer ladder', () => {
  // inherited process env > .credentials.yaml > $NULU_HOME/.env, and the
  ...
  async function bootLayered(
    ...
    const ctx = new Context()
    ctx.provide(NULU_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
  ...
  it('lets the stored value beat the user .env, so a UI write takes effect immediately', async () => {
    ...
    const ctx = await bootLayered(path, [
      ...
      { source: 'user-env', path: '/home/.nulu/.env', values: { NULU_CRED_TEST: 'older-user-env' } },
    ])
    expect(await ctx.credentials.resolve(KEY)).toEqual({ value: 'stored', source: 'file' })
  ...
  it('serves the user .env only when nothing is stored', async () => {
    ...
    const ctx = await bootLayered(join(dir, '.credentials.yaml'), [
      { source: 'process', values: {} },
      { source: 'user-env', path: '/home/.nulu/.env', values: { NULU_CRED_TEST: 'from-user-env' } },
    ])
  ...
  it('serves the invoking project .env over the user one, but never over the store', async () => {
    ...
    const layers = [
      ...
      { source: 'project-env' as const, path: '/work/.env', values: { NULU_CRED_TEST: 'from-project' } },
      { source: 'user-env' as const, path: '/home/.nulu/.env', values: { NULU_CRED_TEST: 'from-user' } },
    ]
  ...
  it('lets only the inherited environment shadow the store, read-only', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  NULU_CRED_TEST: stored\n')
    const ctx = await bootLayered(path, [
      { source: 'process', values: { NULU_CRED_TEST: 'from-shell' } },
      { source: 'user-env', path: '/home/.nulu/.env', values: { NULU_CRED_TEST: 'from-user-env' } },
    ])
...
describe('document validation', () => {
  ...
  it.each([
    ...
    ['a sequence root', '- NULU_CRED_TEST\n', /must be a mapping/],
    ...
    ['the flat layout with a non-string value', 'NULU_CRED_TEST: [nope]\n', /nest the existing 1 entry under/],
    ['the flat layout with several entries and a non-string value', 'NULU_CRED_TEST: a\nNULU_CRED_OTHER: [b]\n',
      /nest the existing 2 entries under/],
    ['the flat layout with an empty value', 'NULU_CRED_TEST: ""\n', /pre-release flat layout/],
    ...
    ['malformed yaml', 'NULU_CRED_TEST: "unterminated\n', /invalid document/],
  ...
  it('never puts a credential value in a diagnostic', async () => {
    ...
    await writeCredentials(path, `NULU_CRED_TEST: "${secret}\n`)
    let failure: unknown
    try {
...
describe('document writes', () => {
  ...
  it('rejects empty values and writes the environment would shadow', async () => {
    ...
    vi.stubEnv('NULU_CRED_TEST', 'shadowing')
  ...
  it('fails a write loud when the on-disk document became invalid', async () => {
    ...
    await writeCredentials(path, 'NULU_CRED_TEST: "unterminated\n')
