...
const ALPHA = credentialRef('NULU_REVIEW_ALPHA')
const BETA = credentialRef('NULU_REVIEW_BETA')
const INNER = credentialRef('NULU_REVIEW_INNER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nulu-cred-review-'))
...
describe('document editor', () => {
  it('leaves a sibling multi-line value untouched while patching one entry', async () => {
    ...
    expect(await readFile(path, 'utf8'))
      .toBe(`version: 1\nrefs:\n  NULU_REVIEW_WRAPPED: |-\n    line1\n    line2\n  ${ALPHA}: b\n`)
    expect(await ctx.credentials.resolve(credentialRef('NULU_REVIEW_WRAPPED')))
      .toEqual({ value: 'line1\nline2', source: 'file' })
  })
