...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nulu-cred-migration-'))
...
describe('flat-layout boot migration', () => {
  it('upgrades the flat document in place, byte for byte, and serves its keys', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('NULU_CRED_TEST'))).toEqual({ value: 'stored', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('NULU_CRED_OTHER')))
      .toEqual({ value: 'quoted value', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('NULU_CRED_BLOCK')))
  ...
  it('yields to a concurrent migrator under the writer lock', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('NULU_CRED_TEST'))).toEqual({ value: 'winner', source: 'file' })
  })
  ...
  it('leaves an empty flow mapping alone', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('NULU_CRED_TEST'))).toBeUndefined()
  })
  ...
  it('renders a final newline for a document that lacks one', () => {
    expect(renderFlatLayoutMigration('NULU_CRED_TEST: bare')).toBe('version: 1\nrefs:\n  NULU_CRED_TEST: bare\n')
  })
})
