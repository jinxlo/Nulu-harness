...
import { credentialKey, credentialKeyScope, credentialRef, parseCredentialKey } from '@worldapptechnologies/nulu-credentials'
import type { CredentialKey, CredentialRecord } from '@worldapptechnologies/nulu-credentials'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nulu-cred-records-'))
...
describe('record storage', () => {
  ...
  it('keeps references and records in one document without either disturbing the other', async () => {
    ...
    await ctx.credentials.set(credentialRef('NULU_RECORDS_KEY'), 'sk-live')
    await put(ctx, CODEX, { kind: 'grant', payload: { token: 't' } })

    const text = await readFile(path, 'utf8')
    expect(text).toBe(
      'version: 1\nrefs:\n  NULU_RECORDS_KEY: sk-live\nrecords:\n'
      + '  llm-pi-ai/openai-codex:\n    kind: grant\n    payload:\n      token: t\n',
    )
    expect(await ctx.credentials.resolve(credentialRef('NULU_RECORDS_KEY'))).toEqual({ value: 'sk-live', source: 'file' })
  })
