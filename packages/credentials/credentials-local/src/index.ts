/**
 * File-backed credentials provider over `$NULU_HOME/.credentials.yaml`, layered
 ...
 * > $NULU_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `WORLD_APP_TECHNOLOGIES_API_KEY=… nulu`, a CI
 ...
 * @module @worldapptechnologies/nulu-credentials-local
 */
...
import { canonicalizeWatchPath, resolveNuluHome } from '@worldapptechnologies/nulu-home-paths'
import { launchEnvironmentOf } from '@worldapptechnologies/nulu-launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@worldapptechnologies/nulu-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@worldapptechnologies/nulu-credentials'
import type { LaunchEnvironmentEntry } from '@worldapptechnologies/nulu-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveNuluHome(config.nuluHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `nulu-atomic-write`, this is a
...
/** File-backed credentials provider (`$NULU_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start nulu from instead',
      )
    }
  }
