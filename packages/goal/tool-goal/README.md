...
Mount `nulu-tool-goal` beside the goal service when the model should create and update persisted goals itself. The tools are the model-facing half of the goal surface; the `/goal` command is the human-facing half, and the continuation driver uses the same tools to complete or block goals at the end of autonomous rounds.
...
```yaml
- id: tool-goal
  name: '@worldapptechnologies/nulu-tool-goal'
  config:
    blockedAfterConsecutiveRounds: 3
```

The value must be a positive safe integer. It supplies both the hard lower bound on model self-blocking and the number named in model guidance. The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-goal) is the exhaustive source for every accepted field.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-goal) — the exact schemas the model receives.
...
- **No scheduling or direct human rendering** — these tools mutate state only; the same-session driver and `nulu-command-goal` are independent consumers of the same domain.
