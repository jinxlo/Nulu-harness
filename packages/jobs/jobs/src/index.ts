 ...
 * process-local registry lives in `@worldapptechnologies/nulu-jobs-local`.
 * @module @worldapptechnologies/nulu-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@worldapptechnologies/nulu-jobs is the abstract job registry seam; load an implementation such as @worldapptechnologies/nulu-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
