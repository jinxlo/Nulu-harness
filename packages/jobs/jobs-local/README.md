...
`nulu-jobs-local` runs background jobs inside the harness process: work keeps running while the agent moves on, and the owning agent can read, wait on, list, and cancel it, with completion delivered as an in-session notice when `nulu-tool-jobs` is also mounted. It implements the `nulu-jobs` contract with in-memory records handed out as fresh snapshots, never live state. A per-owner concurrency limit (default 10) bounds how many jobs one agent can have running or stopping at once; jobs die with the harness process and are not durable across restarts.
...
Load this plugin when a composition needs in-process background jobs: long-running tools register their work, and the owning agent reads, waits on, lists, and cancels it without blocking its own turn. It implements the [`nulu-jobs`](../jobs/README.md) contract; the model-facing `job_output`, `job_list`, and `job_kill` tools come from [`nulu-tool-jobs`](../tool-jobs/README.md).
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-jobs-local) is the exhaustive source for the accepted field.
...
Indirectly, through producer plugins and `nulu-tool-jobs`, to which the registry backend delegates all model rendering.
