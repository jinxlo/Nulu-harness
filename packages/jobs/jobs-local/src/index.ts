 ...
 * @module @worldapptechnologies/nulu-jobs-local
 */

import { Context } from '@worldapptechnologies/cordis'
import z from '@worldapptechnologies/schemastery'
import type { Agent } from '@worldapptechnologies/nulu-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@worldapptechnologies/nulu-scope'
import type { ScopeLayer } from '@worldapptechnologies/nulu-scope'
import { deadline, timeoutOf } from '@worldapptechnologies/nulu-timeout'
import { JobRegistry, JobId } from '@worldapptechnologies/nulu-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@worldapptechnologies/nulu-jobs'
 ...
 * `@worldapptechnologies/nulu-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @worldapptechnologies/nulu-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @worldapptechnologies/nulu-agent)')
    }
