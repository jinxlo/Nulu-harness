...
import type { FileSystem } from '@worldapptechnologies/nulu-fs'
...
export function bootstrapArgs(fs: FileSystem, config: LaunchConfig, maxMessageBytes: number): string[] {
  ...
  const subprocess = dirname(fileURLToPath(import.meta.resolve('@worldapptechnologies/nulu-subprocess/package.json')))
