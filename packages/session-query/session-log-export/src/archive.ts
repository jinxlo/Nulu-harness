...
import type { SessionLineageNode, SessionQueryEngine } from '@worldapptechnologies/nulu-session-query'
import { SESSION_FORMAT_VERSION } from '@worldapptechnologies/nulu-session'
import { sessionFormatLogFilename } from '@worldapptechnologies/nulu-session-format'
import type { SessionEvent, SessionHeader, SessionId, SessionStore } from '@worldapptechnologies/nulu-session'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `nulu-session-${safeSessionIdSegment(sessionId)}.zip`
}
