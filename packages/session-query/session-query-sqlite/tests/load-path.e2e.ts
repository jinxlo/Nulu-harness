 ...
 *
 * @module @worldapptechnologies/nulu-session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@worldapptechnologies/cordis'
import Loader from '@worldapptechnologies/cordis-plugin-loader'
import SessionStore, { SessionSeq, SESSION_FORMAT_VERSION, SessionId } from '@worldapptechnologies/nulu-session'
import SessionProjectionRegistry from '@worldapptechnologies/nulu-session-projection'
import JsonlSessionPersistence from '@worldapptechnologies/nulu-session-persistence-jsonl'
import SqliteSessionQueryEngine, * as queryModule from '@worldapptechnologies/nulu-session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'nulu-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('nulu-session-query-sqlite real Loader path', () => {
