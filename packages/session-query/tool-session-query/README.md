...
Mount this package when the agent should be able to search its own prior sessions and inspect their relationships and events. The common path is explicit: mount the plugin over `ctx.sessionQuery` (backed by `nulu-session-query-sqlite`), then let the model call the tools.
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-session-query) is the exhaustive source for every accepted field and its JSDoc.
...
Every trusted query-service call crosses one error sanitizer: caller cancellation is preserved exactly, corpus and provider diagnostics go to the internal log, and unsafe or unprintable failures fall back to the fixed `SESSION_QUERY_TOOL_FAILED` code and message. Local argument-validation and authorization errors keep their precise tool-owned messages (`SESSION_QUERY_TOOL_UNAUTHORIZED` for a target outside the caller workspace). The package performs no byte or character truncation and does not import a spill backend; deployments that need bounded inline output mount `@worldapptechnologies/nulu-spill-policy`, which can replace oversized rendered text while retaining the complete result.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-session-query) — the five tool schemas as the model sees them.
- [nulu-session-query](../session-query/README.md) — the service these tools call.
- [nulu-session-query-sqlite](../session-query-sqlite/README.md) — the full-text backend behind the two search tools.
...
The model sees the generated [`session_search`, `session_event_search`, `session_trace`, `session_event_trace`, and `session_event_read` schemas](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-session-query). Search filters add fixed schema tokens, while cursors, workspace paths, output pagination, and model-controlled result limits remain absent.
