...
Mount this package next to `nulu-compaction-basic` when tool output regularly dominates the conversation window. Trimming changes what the model sees — shorter results — and gives compaction less history to condense.
...
All settings are optional; the defaults trim any result with more than 8,192 text characters to its first 4,096 plus its last 1,024, joined by the marker. The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-compaction-tool-result-pruner) is the exhaustive source.
...
Trimming only runs when a compaction trigger qualifies: `nulu-compaction-basic` invokes it after pressure or overflow is confirmed, before it selects what to condense. Below pressure nothing is trimmed, and trimming itself makes no model call.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-compaction-tool-result-pruner) — every accepted config field and its source declaration.
