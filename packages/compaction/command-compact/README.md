...
Type `/compact` in a chat UI when the conversation has grown long and you want to condense it immediately. The shipped `nulu` base mounts the command next to the default backend, so it is usually already available.
...
The shipped `nulu` base mounts it beside the default backend, and the Web client provides the command adapter. Automation surfaces that compose no command adapter keep automatic condensation only.
