...
```yaml
- name: '@worldapptechnologies/nulu-plan-mode'
  config:
    section: |
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-plan-mode) is the exhaustive source for every accepted field and its JSDoc.
...
- [exit_plan_mode tool catalog entry](../../../docs/tool-catalog.md#worldapptechnologiesnulu-plan-mode) — the exact schema the model receives.
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-plan-mode) — every accepted config field and its meaning.
...
The [`exit_plan_mode` schema](../../../docs/tool-catalog.md#worldapptechnologiesnulu-plan-mode) remains available in both states; execution outside plan mode fails, while an approved in-mode review returns the canonical `{ approved: true }` value and renders the existing confirmation text. Rejection remains a failed call carrying review feedback, and a dismissed review a failed call naming the user's takeover.
