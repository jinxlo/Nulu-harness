...
function stageDist(): string {
  dist = mkdtempSync(join(tmpdir(), 'nulu-web-app-'))
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(NULU_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(log).toHaveBeenCalledWith('nulu web: http://127.0.0.1:4567/?token=test-token (LAN: http://192.168.1.5:4567/?token=test-token)')
    expect(log).toHaveBeenCalledWith('nulu web: opening the default browser; pass --no-open to disable')
    expect(openBrowser).toHaveBeenCalledWith('http://127.0.0.1:4567/?token=test-token')
    expect(lifecycle).toEqual([
      'nulu web: http://127.0.0.1:4567/?token=test-token (LAN: http://192.168.1.5:4567/?token=test-token)',
      'nulu web: opening the default browser; pass --no-open to disable',
      'open:http://127.0.0.1:4567/?token=test-token',
    ])
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('Nulu Harness implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ NULU_WEB_URL: 'http://127.0.0.1:4567' })
  ...
  it.each([
    ...
    expect(diagnostic).toHaveBeenCalledWith(
      `web-app: could not open the default browser because ${reason}; use the nulu web URL printed at startup`,
    )
  ...
  it('scrubs the helper environment and reports helper spawn or exit failures', async () => {
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'must-not-reach-browser')
    vi.stubEnv('NULU_HOME', '/must-not-reach-browser')
    ...
    expect(options?.env).not.toHaveProperty('WORLD_APP_TECHNOLOGIES_API_KEY')
    expect(options?.env).not.toHaveProperty('NULU_HOME')
