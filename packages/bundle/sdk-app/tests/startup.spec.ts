...
import { internals, provideCmdline } from '@worldapptechnologies/nulu-cmdline'
...
describe('SDK app startup', () => {
  ...
  it('prints app help without publishing readiness or binding stdin', () => {
    ...
    expect(out()).toContain('nulu --profile sdk')
  ...
  it('renders the selected SDK profile name in help', () => {
    ...
    expect(out()).toContain('Usage: nulu --profile sdk-minimal')
    expect(out()).toContain('nulu --profile sdk-minimal')
  })
})
