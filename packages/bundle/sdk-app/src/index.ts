 ...
 * @module @worldapptechnologies/nulu-sdk-app
 */

import { Command } from 'commander'
import type { Context } from '@worldapptechnologies/cordis'
import z from '@worldapptechnologies/schemastery'
import { exitOnStdinEnd, parseCmdline } from '@worldapptechnologies/nulu-cmdline'
...
function sdkCommand(profile: string): Command {
  return new Command()
    .name(`nulu --profile ${profile}`)
    .description('Serve Nulu Harness SDK clients over stdio JSON-RPC.')
    .helpOption('-h, --help', 'show this help')
    .addHelpText('after', `
Example:
  nulu --profile ${profile}     serve one SDK runtime until its client disconnects
`)
}
