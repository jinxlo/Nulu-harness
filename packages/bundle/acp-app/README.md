...
# `@worldapptechnologies/nulu-acp-app`


## Summary

The automation-only ACP stdio application as a `nulu` profile bundle over [`nulu-base`](../base/README.md). It inherits the base's disabled module-HMR policy; its patch sets the coding-agent persona and default model route, mounts an app-owned zero-option command provider, and starts [`nulu-acp`](../../acp/acp/README.md) only after that provider accepts the invocation. `nulu --profile acp --help` therefore writes help and exits without claiming stdin or stdout.
...
The shipped row creates sessions with `worldapp-gateway` and `nulu-5`; a later patch can replace that row's complete config. The base profile owns adapters, tools, persistence, policy, settings, credentials, and the per-session workspace supplied by the ACP client.
...
- **A profile can omit the ACP bridge** — a custom ACP launch profile must retain this bundle or another `nulu-acp` row; otherwise no peer answers the client.
