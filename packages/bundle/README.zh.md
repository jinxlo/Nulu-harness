---
description: "共享核心、浏览器 GUI、一次性任务、ACP（Agent Client Protocol）与 SDK 应用表层的现成 nulu profile 组合包。"
kind: "package-group"
---
...
本组列出 `nulu --profile` 使用的可安装 patch 层。每个包都声明 `nulu.bundle.patch`；启动器会叠放这些 patch 文档来组装具名 profile。`web`、`headless`、`acp` 与 `sdk` profile 以 `nulu-base` 为基础，`sdk-minimal` 则由一个组合包提供完整配置树。领域包也可以在本目录之外声明附加层。
...
内置组合包从 nulu 安装目录解析；树外（out-of-tree）组合包通过 `nulu plugin --profile <name> add <package>` 安装进 profile。

<a id="related-documentation"></a>
## 相关文档

- [nulu 应用](../../apps/cli/README.zh.md)——启动 profile 的 `nulu` 命令。
