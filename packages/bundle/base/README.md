---
description: "The shared nulu core: model access, tools, durable sessions, and safety defaults for every nulu --profile surface, for users composing or customizing a profile."
kind: "package-bundle"
---
...
Every base-backed `nulu --profile` surface runs on `nulu-base`, so those surfaces share a model connection, the full tool set, durable session history, and workspace safety defaults. The shipped `sdk-minimal` profile deliberately uses a complete standalone tree instead. You rarely touch this bundle directly — shipped base-backed profiles already include it, and a custom base-backed profile names it first. When you need different defaults, change your profile patch or add a later bundle; this package is not a library you import.
...
You get the nulu core automatically: the shipped `web`, `headless`, `sdk`, and `acp` profiles already include it, and a custom profile names it as its first bundle. After that, everything works with no further configuration.

### A minimal custom profile

To build a profile on the shared core, create a profile with a `package.json` that names `@worldapptechnologies/nulu-base` first:
...
```json
{
  ...
  "nulu": {
    "profile": {
      "bundles": ["@worldapptechnologies/nulu-base"]
    }
  }
}
```

Run `nulu --profile my-profile "your task"` and you get a working agent with model access, tools, persistence, and the default permission policy. The shipped `web`, `headless`, `sdk`, and `acp` profiles are created for you on first use. To add more bundles, run `nulu plugin --profile <name> add <package>`; in-box bundles resolve from the nulu installation. The profile contract is documented in the [app-boot profile section](../../boot/app-boot/README.md).

### What you get

Out of the box, every profile built on this core provides: a Nulu model connection (the provider and model are configurable, and you can enable extra providers from your settings), the full tool set — file editing, shell commands, web search, public HTTP(S) fetch, subagents, task and goal tracking — durable sessions that survive restarts, and the default permission policy that confines file writes to your workspace and asks before risky actions. Web fetch runs without per-call approval; its provider rejects non-public destinations. Feedback stays in the Session log. [OTel session upload](../../session/session-telemetry-otel/README.md) defaults to `FEEDBACK_ONLY` for all users, including `nulu-official`: new text feedback, message ratings, edits, and withdrawals release the complete canonical prefix through that event, including context. Later records wait for the next explicit feedback; sending an authorized batch needs no further interaction or model call. `DISABLED` prevents OTel capture. The default-on [Nulu session-log contributor](../../session/session-log-gateway/README.md) remains a separate request path.
...
The patch gates the two shell stacks by platform on its own rows: `bash-sandbox` and `tool-bash` carry `disabled: !!js process.platform === 'win32'`, and their twins `pwsh-sandbox` and `tool-pwsh` mount on win32 only with the inverted expression. The permission surface stays identical to POSIX: the sandbox policy executes the same file-effect policy through the Windows ACL restricted-token runner (`nulu-sandbox-local` → `@worldapptechnologies/nulu-sandbox-windows-acl`), and `fs-sandbox` keeps fencing `ctx.fs` writes — mounting `nulu-fs-local` alongside it would double-register `ctx.fs` and fail the load.
...
- **Windows temp grants are private per-session subdirectories** — `workspace-write` confines writes to the workspace plus the session's own temp subdirectory (`<temp>\nulu-<hash>`, TMP/TEMP rewritten for confined children); `read-only` grants nothing. See `@worldapptechnologies/nulu-sandbox-windows-acl`.
