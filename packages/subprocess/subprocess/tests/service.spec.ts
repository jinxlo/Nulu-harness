...
import { scrubbedParentEnv, SubprocessRuntime } from '@worldapptechnologies/nulu-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and NULU_ names (case-insensitively) but keeps PATH', () => {
    process.env.NULU_SCRUB_PROBE = 'stale'
    process.env.nulu_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.NULU_SCRUB_PROBE).toBeUndefined()
      expect(env.nulu_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.NULU_SCRUB_PROBE
      delete process.env.nulu_scrub_probe_lower
