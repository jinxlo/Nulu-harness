...
You reach this package through a provider backend, not a composition: `nulu-subagent-spawn-in-process` and `nulu-subagent-fork-in-process` each call `startInProcessRun(request, options)` and own everything around it. This page documents the lifecycle both share so you can read one backend's behavior and reason about the other.
...
- [nulu-subagent-spawn-in-process](../subagent-spawn-in-process/README.md) — the fresh-child backend built on this driver.
- [nulu-subagent-fork-in-process](../subagent-fork-in-process/README.md) — the seeded-child backend built on this driver.
