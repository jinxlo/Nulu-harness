...
/**
 * With-key cross-process boundary proof: the backend spawns the real nulu ACP profile, speaks ACP over
 * stdio, and returns its real model answer. This is the out-of-process counterpart to in-process
 * spawn coverage and self-skips without `WORLD_APP_TECHNOLOGIES_API_KEY`.
 */

// The real ACP profile: nulu plus the example's live Nulu patch.
...
// How to launch the child ACP profile (src via tsx / lib via plain node, per NULU_EXAMPLE_MODE).
...
function resolveChildLaunch(nuluHome: string) {
  const modelKey = process.env.WORLD_APP_TECHNOLOGIES_API_KEY
  const baseUrl = process.env.WORLD_APP_TECHNOLOGIES_BASE_URL
  return resolveExampleLaunch({
    ...
    env: {
      ...modelKey !== undefined ? { WORLD_APP_TECHNOLOGIES_API_KEY: modelKey } : {},
      ...baseUrl !== undefined ? { WORLD_APP_TECHNOLOGIES_BASE_URL: baseUrl } : {},
      NULU_HOME: nuluHome,
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
  it('drives the real acp-agent example process to answer a prompt', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-subagent-acp-e2e-'))
    const childLaunch = resolveChildLaunch(join(workdir, '.nulu-child'))
    ctx = new Context()
