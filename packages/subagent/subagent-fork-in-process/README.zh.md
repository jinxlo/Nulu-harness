...
`nulu-subagent-fork-in-process` 是一个进程内 subagent 后端：它以父级已完成的对话轮次作为每个子 agent 的初始内容——子 agent 能看到所有已完成轮次，但看不到进行中的轮次，因此后续工作可以在对话基础上继续，而无需重复提供对话内容。委派工具以 `fork` 提供方名称找到它，其行为与 spawn 后端一致，唯一差异是会话初始内容。当子任务延续当前对话时选择它；当子 agent 必须独立运行时选择 spawn。初始内容是 fork 时的一次性快照：此后父级记录的任何内容都不会到达子 agent。
...
当委派的工作必须建立在父级对话之上时，挂载此后端。常用路径与 spawn 相同：加载 subagent 服务与本后端，再把 `nulu-tool-subagent` 之类的委派工具指向 `fork` 提供方。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-subagent-fork-in-process)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [nulu-subagent-in-process-driver](../subagent-in-process-driver/README.zh.md)——本后端调用的共享运行驱动器。
- [nulu-subagent-spawn-in-process](../subagent-spawn-in-process/README.zh.md)——全新子级的兄弟后端。
- [nulu-tool-subagent](../tool-subagent/README.zh.md)——指向该提供方的面向模型委派工具。
- [生成配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-subagent-fork-in-process)——每个受支持配置字段及其源声明。
...
父级只通过 `nulu-tool-subagent` 接收子 agent 自身的最终输出，不接收继承的前缀或中间工作。
