...
Mount this backend when delegated work must build on the parent's conversation. The common path mirrors spawn: load the subagent service and this backend, then point a delegation tool such as `nulu-tool-subagent` at the `fork` provider.
...
- name: '@worldapptechnologies/nulu-subagent-fork-in-process'
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-subagent-fork-in-process) is the exhaustive source for every accepted field and its JSDoc.
...
- [nulu-subagent-spawn-in-process](../subagent-spawn-in-process/README.md) — the fresh-child sibling backend.
- [nulu-tool-subagent](../tool-subagent/README.md) — the model-facing delegation tool that reaches this provider.
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-subagent-fork-in-process) — every accepted config field and its source declaration.
...
The parent receives only the child's own final output through `nulu-tool-subagent`, not the inherited prefix or intermediate work.
