...
Mount this provider when a delegation should run as a complete Harness runtime in its own process. The common path is explicit: mount the seam, mount this provider, configure `nuluBin` when needed, and select the child `profile` and ordered `patches`.
...
The provider advertises `agentOptions: true`, with `outputSchema`/`depthLimit`/`toolFilter`/`persona` false, and `inheritsParentContext: false`. Its immutable `agentRouteDefaults` publish the configured provider/model baseline to `nulu-tool-subagent` before model overrides and exact-route preflight; `start()` independently applies the same configuration defaults for direct callers and `maxTokens`. Agent route values cross the SDK wire as an explicit whitelist; the child remains a fresh runtime in another process, and the only value derived from the parent agent itself is the workspace cwd. `nulu-tool-subagent` deployments over this provider set `maxDepth: 'provider-managed'` — the child harness owns its own recursion budget.

### Configuration

| Field | Default | Meaning |
|---|---|---|
| `providerName` | `nulu-sdk` | Registry name on `ctx.subagents` |
| `nuluBin` | SDK dependency | Explicit nulu CLI module, resolved and checked at plugin load; omission uses the SDK dependency |
| `profile` | `sdk` | Named child profile |
| `patches` | `[]` | Ordered per-launch profile patch files, resolved and checked at plugin load |
| `nuluHome` | required | Absolute isolated Harness home for every nested child process |
| `cwd` | parent session cwd | Working-directory override for the child process and its SDK session |
| `provider` | `worldapp-gateway` | Provider route sent in the child's `initialize` |
| `model` | `nulu-5` | Model sent in the child's `initialize` |
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-subagent-nulu-sdk) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-subagent-nulu-sdk) — every accepted config field and its source declaration.
