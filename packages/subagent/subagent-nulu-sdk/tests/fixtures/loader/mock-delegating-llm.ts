...
import { ToolCallId, LlmAdapter, ReasoningEffortId } from '@worldapptechnologies/nulu-llm'
...
class MockDelegatingAdapter extends LlmAdapter {
  override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
    if (process.env.NULU_TEST_PARENT_MODEL_RECORD !== undefined) {
      appendFileSync(process.env.NULU_TEST_PARENT_MODEL_RECORD, `${provider}/${model}\n`)
    }
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResultText.length === 0) {
      const selectedRoute = process.env.NULU_TEST_CHILD_DEFAULT_ROUTE === '1'
...
export function apply(ctx: Context): void {
  const providers = process.env.NULU_TEST_PARENT_PROVIDER === 'worldapp-gateway'
    ? ['worldapp-gateway', 'mock']
    : ['mock']
