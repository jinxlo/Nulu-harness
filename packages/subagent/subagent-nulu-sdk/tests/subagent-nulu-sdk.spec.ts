...
import {
  ...
  HarnessSession,
  SdkProtocolError,
} from '@worldapptechnologies/nulu-sdk-client'
import { createProcessNuluHarness } from '../../../sdk/client/src/api.ts'
import type { RuntimeProcessOptions } from '../../../sdk/client/src/launch.ts'
import type { NuluHarnessOptions } from '@worldapptechnologies/nulu-sdk-client'
...
let createdHarnessOptions: NuluHarnessOptions[] = []
...
beforeEach(() => {
  ...
  runInternals.createHarness = (options) => {
    ...
    return createProcessNuluHarness(runtime, options)
  }
})
...
async function setup(fakeEnv: Record<string, string> = {}, config: Partial<sdk.Config> = {}) {
  ...
  await ctx.plugin(sdk, {
    providerName: 'nulu-sdk',
    profile: 'sdk',
    patches: [],
    nuluHome: process.cwd(),
...
function expectedFailure(fields: string): string {
  return `Subagent failure (provider: NULU SDK; ${fields})`
}
...
describe('nulu-subagent-nulu-sdk provider', () => {
  it('constructs the production nulu-backed harness lazily', async () => {
    const harness = defaultCreateHarness({})
    expect(harness).toBeInstanceOf((await import('@worldapptechnologies/nulu-sdk-client')).NuluHarness)
    await harness.close()
  })

  it('runs a child turn end to end with a parent-unique run id', async () => {
    const ctx = await setup({ FAKE_TEXT: 'hello from sdk child' })
    const run = await ctx.subagents.start('nulu-sdk', request('do X'))
    ...
    const nextRun = await ctx.subagents.start('nulu-sdk', request('again'))
  ...
  it('resolves relative launch files at load and forwards absolute paths', async () => {
    const ctx = await setup({ FAKE_TEXT: 'explicit nulu child' }, {
      nuluBin: relative(process.cwd(), fakeRuntime),
      patches: [relative(process.cwd(), existingPatch)],
    })
    const run = await ctx.subagents.start('nulu-sdk', request())
    expect(text((await run.result).output)).toBe('explicit nulu child')
  ...
  it('initializes the child with the configured provider/model/maxTokens and the parent cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-nulu-sdk-init-'))
  ...
  it('preserves instance defaults around a partial request override', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-nulu-sdk-partial-route-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const run = await ctx.subagents.start('nulu-sdk', request('partial', new AbortController().signal, {
  ...
  it('isolates complete per-run route overrides on concurrent children', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-nulu-sdk-routes-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const runs = await Promise.all([
        ctx.subagents.start('nulu-sdk', request('first', new AbortController().signal, {
          provider: 'provider-a',
          model: 'model-a',
          reasoningEffort: ReasoningEffortId('high'),
          maxTokens: 111,
        })),
        ctx.subagents.start('nulu-sdk', request('second', new AbortController().signal, {
          provider: 'provider-b',
          model: 'model-b',
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.NULU_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'NULU_TEST_AMBIENT_SECRET_KEY,WORLD_APP_TECHNOLOGIES_API_KEY',
        WORLD_APP_TECHNOLOGIES_API_KEY: 'explicit-child-key',
        FAKE_TEXT: 'done',
      })
      const run = await ctx.subagents.start('nulu-sdk', request())
      const result = await run.result
      const answer = text(result.output)
      expect(answer).toContain('NULU_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('WORLD_APP_TECHNOLOGIES_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.NULU_TEST_AMBIENT_SECRET_KEY
    }
  })
  ...
  it('aggregates safe initialize and shutdown facts when startup rollback fails', async () => {
    ...
    try {
      ...
      const error = await ctx.subagents.start('nulu-sdk', request()).catch((cause: unknown) => cause)
      expect(error).toBeInstanceOf(AggregateError)
      expect((error as Error).message).toBe(
        `subagent-nulu-sdk: ${expectedFailure('stage: initialize; category: protocol')}; `
        + `subagent-nulu-sdk: ${expectedFailure('stage: shutdown; category: unknown')}`,
      )
  ...
  it('reports only safe shutdown facts when cancelled startup rollback fails', async () => {
    ...
    const spy = vi.spyOn(NuluHarness.prototype, 'close').mockImplementation(async function (this: NuluHarness) {
      spy.mockRestore()
      await this.close()
      throw new Error(rawCleanup)
    })
  ...
  it('keeps an initialize failure authoritative when a later abort flag is already set', async () => {
    ...
    const start = vi.spyOn(NuluHarness.prototype, 'start').mockRejectedValue(rawFailure)
    const close = vi.spyOn(NuluHarness.prototype, 'close').mockResolvedValue()
    try {
      const controller = new AbortController()
      const pending = startSdkRun(request('p', controller.signal), {
        profile: 'sdk',
  ...
  it('aborting the required signal settles a hung child as aborted', async () => {
    ...
    const controller = new AbortController()
    const run = await ctx.subagents.start('nulu-sdk', request('p', controller.signal))
  ...
  it('rejects WITHOUT spawning when the signal is already aborted', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-nulu-sdk-preabort-'))
  ...
  it('rejects a pre-aborted request through the registered provider before cwd resolution', async () => {
    ...
    await expect(ctx.subagents.start('nulu-sdk', {
      label: 'p',
  ...
  it('rejects after reaping when the child dies before the hannuluake', async () => {
    ...
    expect(String(failure)).toBe(
      `SdkRunFailure: subagent-nulu-sdk: ${expectedFailure('stage: initialize; category: transport')}`,
    )
  ...
  it.each([
    ...
    await expect(ctx.subagents.start('nulu-sdk', request())).rejects.toThrow(
      `subagent-nulu-sdk: ${expectedFailure(`stage: initialize; category: ${category}`)}`,
    )
    await ctx.fiber.dispose()
  })
  ...
  it('routes provider-level onError through ctx.logger.warn', async () => {
    ...
    expect(warnings[0]).toContain('subagent-nulu-sdk "nulu-sdk": child run failed (error)')
    await run.dispose()
    await ctx.fiber.dispose()
  })
  ...
  it('rejects non-positive timing bounds at load', async () => {
    ...
    await ctx.plugin(SessionProjectionRegistry)
    await ctx.plugin(SubagentRuntime)
    const base = { providerName: 'sdk', profile: 'sdk', patches: [], nuluHome: process.cwd(), provider: 'p', model: 'm', env: {} }
  ...
  it('requires an explicit absolute Harness home for nested nulu runtimes', async () => {
    const ctx = new Context()
    await ctx.plugin(SubagentRuntime)
    await expect(ctx.plugin(sdk, {
      ...
      profile: 'sdk',
      patches: [],
      nuluHome: './personal-home',
      provider: 'p',
      model: 'm',
      env: {},
    })).rejects.toThrow('nuluHome must be an absolute path')
    await ctx.fiber.dispose()
  })

  it.each([
    { field: 'nuluBin', override: { nuluBin: './missing-nulu-bin' } },
    { field: 'nuluBin', override: { nuluBin: '.' } },
  ...
  it('uses a validated config cwd override instead of the parent session cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-nulu-sdk-cwd-'))
    try {
  ...
  it('fails loud when neither config cwd nor parent session cwd exists', async () => {
    ...
    await expect(ctx.subagents.start('nulu-sdk', {
      ...
      .rejects.toThrow(
        `subagent-nulu-sdk: ${expectedFailure('stage: initialize; category: configuration')}`,
      )
    await ctx.fiber.dispose()
  })

  it('keeps named plugin exports with no default export (loader shape)', () => {
    expect(sdk.name).toBe('subagent-nulu-sdk')
