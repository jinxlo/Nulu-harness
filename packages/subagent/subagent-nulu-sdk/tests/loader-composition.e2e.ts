...
const configPath = fileURLToPath(new URL('nulu-sdk.patch.yml', fixtureDir))
...
async function childLaunch(failure = false): Promise<{
  ...
  const childHome = await mkdtemp(join(tmpdir(), 'nulu-sdk-subagent-home-'))
  ...
  return {
    ...
    env: {
      ...
      ...(failure ? { NULU_TEST_CHILD_FAILURE: '1' } : {}),
    },
  }
}
...
describe('SDK subagent routing and diagnostics through the production profile', () => {
  it('runs the selected child route in the parent session workspace', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'nulu-sdk-subagent cwd composition smoke',
        tempDirPrefix: 'nulu-sdk-subagent-cwd-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        ...
        env: {
          ...child.env,
          NULU_TEST_CHILD_DEFAULT_ROUTE: '1',
          NULU_TEST_PARENT_MODEL_RECORD: '.parent-model-routes',
        },
        inspect: async (cwd) => {
  ...
  it('presents the child error diagnostic separately from partial output', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'nulu-sdk-subagent diagnostic composition smoke',
        tempDirPrefix: 'nulu-sdk-subagent-diagnostic-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        tsconfigPath: repoTsconfig,
      ...
      expect(toolResultText(events)).toBe(
        ...
        + 'Diagnostic: Subagent failure (provider: NULU SDK; stage: session-run; category: child-error)\n'
