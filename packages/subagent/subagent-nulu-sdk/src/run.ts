 ...
 *
 * @module @worldapptechnologies/nulu-subagent-nulu-sdk/run
 */
...
import type { SessionEvent, SessionId, TurnEndReason } from '@worldapptechnologies/nulu-session'
...
export interface SdkRunSpec {
  /** Explicit nulu CLI module; omission resolves the SDK client's same-version dependency. */
  nuluBin?: string
  /** Named child profile. */
  profile: string
   ...
   * runtime's own `WORLD_APP_TECHNOLOGIES_API_KEY`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `NULU_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-nulu-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: NuluHarnessOptions): NuluHarness } = {
  createHarness: options => new NuluHarness(options),
}
 ...
 * @returns an Error whose message contains only the fixed NULU SDK failure line.
 */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.nuluBin === undefined ? {} : { nuluBin: spec.nuluBin },
    profile: spec.profile,
    patches: spec.patches,
    nuluHome: spec.nuluHome,
    processCwd: spec.cwd,
