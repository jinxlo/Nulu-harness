...
/** One running test-only Responses-to-Nulu bridge. */
export interface NuluResponsesBridge {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk: string) => {
      ...
      if (Buffer.byteLength(body) > MAX_REQUEST_BYTES) {
        request.destroy(new Error('Nulu bridge request exceeded its byte limit'))
      }
    })
...
async function completeWithNulu(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Nulu bridge upstream returned HTTP ${response.status}`)
  }
  ...
  if (typeof content !== 'string' || content.trim().length === 0) {
    throw new Error('Nulu bridge upstream returned no text')
  }
  return content
}
 ...
 */
export async function startNuluResponsesBridge(
  nonce: string,
): Promise<NuluResponsesBridge> {
  let seenRequests = 0
  ...
  const server = createServer((request, response) => {
    ...
    void (async () => {
      ...
      if (
        ...
        throw new Error('Codex Nulu bridge received no bearer credential')
      }
      const body = JSON.parse(await readRequest(request)) as Record<string, unknown>
      const task = taskText(body)
      if (!task.includes(nonce)) {
        throw new Error('Codex Nulu bridge request omitted the expected nonce')
      }
      const text = await completeWithNulu(authorization, task)
      completedRequests += 1
      response.writeHead(200, {
        'content-type': 'text/event-stream',
        'cache-control': 'no-cache',
        connection: 'keep-alive',
        'x-request-id': 'req_nulu_e2e',
      })
      ...
      response.end(JSON.stringify({ error: { message: 'Nulu bridge request failed' } }))
    })
  })
  ...
  if (address === null || typeof address === 'string') {
    throw new Error('Nulu bridge did not acquire a TCP port')
  }
  return {
