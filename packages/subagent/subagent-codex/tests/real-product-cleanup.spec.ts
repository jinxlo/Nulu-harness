...
it('closes its real HTTP server and removes its root after context disposal rejects', async () => {
  const root = mkdtempSync(join(tmpdir(), 'nulu-codex-cleanup-rejected-'))
...
it('removes sibling roots after an earlier root removal fails', async () => {
  const root = mkdtempSync(join(tmpdir(), 'nulu-codex-cleanup-sibling-'))
  try {
...
it('keeps resources registered during pending cleanup for their own cleanup', async () => {
  const oldRoot = mkdtempSync(join(tmpdir(), 'nulu-codex-cleanup-old-'))
  const nextRoot = mkdtempSync(join(tmpdir(), 'nulu-codex-cleanup-next-'))
