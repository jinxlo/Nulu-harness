...
有了 `nulu-web-search-perplexity`，harness 可以通过 Perplexity 搜索 web，一次调用同时获得模型生成的答案与可引用来源。当部署持有 Perplexity API 密钥、并希望获得生成答案时选择它。Perplexity 没有结果数量控制，因此返回的来源会在事后被截断到请求的上限。Perplexity 省略结构化结果元数据时，来源回退为只含 URL 的引用。面向模型的 `web_search` 工具位于 `nulu-tool-web`。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-web-search-perplexity)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-web-search-perplexity)——每个受支持配置字段及其源声明。
...
通过 `nulu-tool-web`，会话模型会看到生成答案及结构化结果元数据，或只含 URL 的引用。该提供方确切的错误消息为 `Perplexity search aborted`、`Perplexity search request failed: <error>` 和 `Perplexity returned an unprocessable response body: <error>`；HTTP 失败保留提供方消息。错误包装层属于消费方。
...
- **按错误形状分类中止**——只有名为 `AbortError` 的 `DOMException` 才映射为 `WEB_ABORTED`；携带自定义原因的中止（例如 `nulu-timeout` 的 `TimeoutReason`）呈现为 `WEB_PROVIDER_ERROR`。
