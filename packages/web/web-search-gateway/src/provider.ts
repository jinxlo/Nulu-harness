/**
 * Nulu search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @worldapptechnologies/nulu-web-search-gateway/provider
 */

import { WebError } from '@worldapptechnologies/nulu-web'
import type {
  WebSearchProvider,
  WebSearchRequest,
  WebSearchResult,
  WebSearchSource,
} from '@worldapptechnologies/nulu-web'
...
export const GATEWAY_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const GATEWAY_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'nulu-harness/0.0.1'

/**
 * Exact secret-free Nulu Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface NuluSearchLlmRequest {
...
export interface NuluSearchProviderOptions {
  /** Literal Nulu API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current Nulu API key for one search operation. */
   ...
   */
  recordRequest?: (request: NuluSearchLlmRequest) => void
}
...
/**
 * Map a Nulu Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'Nulu returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/**
 * The Nulu-backed search provider. HTTP redirects fail as `WEB_PROVIDER_ERROR`;
 * failures after dispatch name the endpoint and tell the model how the user can configure it.
 */
export class NuluSearchProvider implements WebSearchProvider {
   ...
   */
  constructor(private readonly resolveOptions: () => NuluSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: NuluSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official Nulu expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw searchEndpointError(
        endpoint,
        `Nulu search request failed: ${String(error)}`,
        error,
      )
    }
    ...
    if (!response.ok) {
      ...
      let message = `Nulu API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      const message = error instanceof WebError
        ? error.message
        : `Nulu returned an unprocessable response body: ${String(error)}`
   ...
   */
  private async apiKey(options: NuluSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `Nulu search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'WORLD_APP_TECHNOLOGIES_API_KEY'
    throw new WebError(
      `Nulu search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-gateway config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('Nulu search aborted', 'WEB_ABORTED', {
...
/** True for Nulu request limits that can be sent to the Messages API. */
