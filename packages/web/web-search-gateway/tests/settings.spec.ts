/** The `web-search-gateway` settings section layered over the composition entry. */
...
import { WEB_SEARCH_GATEWAY_SETTINGS_NAMESPACE } from '@worldapptechnologies/nulu-web-search-gateway'
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(nuluPlugin, { apiKey: 'ds-key', baseURL: 'https://search.entry.test/v1' })
...
describe('web-search-gateway settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    await bench.ctx.settings.update(WEB_SEARCH_GATEWAY_SETTINGS_NAMESPACE, {
  ...
  it('keeps the literal key out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_GATEWAY_SETTINGS_NAMESPACE, { apiKey: 'ds-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-gateway')
  ...
  it('releases the namespace when the plugin unloads', async () => {
    const bench = await boot()
    expect(bench.ctx.settings.describe().map(row => String(row.ns))).toContain('web-search-gateway')

    await bench.pluginFiber.dispose()

    expect(bench.ctx.settings.describe().map(row => String(row.ns))).not.toContain('web-search-gateway')
    await bench.ctx.fiber.dispose()
  })
})
