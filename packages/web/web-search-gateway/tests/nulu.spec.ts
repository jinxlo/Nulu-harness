...
import WebRuntime, { WebError } from '@worldapptechnologies/nulu-web'
import {
  NuluSearchProvider,
  GATEWAY_PROVIDER_ID,
} from '@worldapptechnologies/nulu-web-search-gateway'
import * as nuluPlugin from '@worldapptechnologies/nulu-web-search-gateway'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@worldapptechnologies/nulu-web-search-gateway/src/types.ts'
...
const options = {
  apiKey: 'ds-key',
  baseURL: 'https://api.worldapp-gateway.test/anthropic/v1',
  model: 'nulu-5',
...
describe('NuluSearchProvider availability', () => {
...
describe('NuluSearchProvider request mapping', () => {
  it('records and posts the same Anthropic Messages request with the web_search server tool', async () => {
    ...
    expect(url).toBe('https://api.worldapp-gateway.test/anthropic/v1/messages')
...
describe('NuluSearchProvider settings changes mid-search', () => {
  it('serves one search from one section even when settings land during credential resolution', async () => {
    ...
    const provider = new NuluSearchProvider(() => ({ ...current, resolveApiKey }))
...
describe('NuluSearchProvider error handling', () => {
  ...
  it('maps a credential resolver rejection under an active signal to WEB_PROVIDER_ERROR', async () => {
    ...
    await expect(searchProvider({
      ...
      .rejects.toThrow(expect.objectContaining({
        ...
        message: 'Nulu search credential resolution failed: Error: credential backend failed',
      }))
  })
  ...
  it('uses the default credential reference when no resolver is configured', async () => {
    await expect(searchProvider({ ...options, apiKey: '' }).search({ query: 'q' }))
      .rejects.toThrow('Nulu search has no API key for "WORLD_APP_TECHNOLOGIES_API_KEY"')
  })
  ...
  it('maps an HTTP error to WEB_PROVIDER_ERROR with the provider message', async () => {
    ...
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({
        code: 'WEB_PROVIDER_ERROR',
        message: 'Nulu API error (HTTP 429): rate limited\n\n'
          + 'The web search request used endpoint "https://api.worldapp-gateway.test/anthropic/v1/messages". '
          ...
          + 'WORLD_APP_TECHNOLOGIES_SEARCH_BASE_URL or configure web-search-gateway.baseURL to a trusted '
  ...
  it('handles a string-form error body', async () => {
    ...
    expect(error.message).toContain('Nulu API error (HTTP 400): bad request')
  })

  it('keeps a status-line message when the error body is not JSON', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response('upstream error', { status: 503 })))
    const error = await rejectedWebError(searchProvider(options).search({ query: 'q' }))
    expect(error.message).toContain('Nulu API error (HTTP 503)')
  })

  it('keeps the status-line message when the JSON error body carries no detail', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({}, { status: 500 })))
    const error = await rejectedWebError(searchProvider(options).search({ query: 'q' }))
    expect(error.message).toContain('Nulu API error (HTTP 500)')
  })
  ...
  it('maps a network failure to WEB_PROVIDER_ERROR', async () => {
    ...
    expect(error.message).toContain('The web search request used endpoint "https://api.worldapp-gateway.test/anthropic/v1/messages".')
  })
...
describe('web-search-gateway plugin registration', () => {
  it('registers the provider into ctx.web (HMR-safe)', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(searchResponse())))
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
    const fiber = await ctx.plugin(nuluPlugin, { apiKey: 'ds-key' })
  ...
  it('rejects maxTokens: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
    await expect(ctx.plugin(nuluPlugin, { apiKey: 'ds-key', maxTokens: 0 }))
  ...
  it('rejects maxUses: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
    await expect(ctx.plugin(nuluPlugin, { apiKey: 'ds-key', maxUses: 0 }))
  ...
  it('rejects a fractional maxUses at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
    await expect(ctx.plugin(nuluPlugin, { apiKey: 'ds-key', maxUses: 1.5 }))
      .rejects.toThrow(/maxUses expected number multiple of 1/)
  })

  it('has no default export (namespace plugin export shape)', () => {
    expect('default' in nuluPlugin).toBe(false)
  })
  ...
  it('survives the real Loader unwrapExports path keeping name/inject/Config', () => {
    ...
    const unwrapped = loader.unwrapExports(nuluPlugin) as Record<string, unknown>
    expect(unwrapped).toBe(nuluPlugin)
    expect(unwrapped.name).toBe('web-search-gateway')
  ...
  it('boots over ctx.web through the unwrapped module without an inject error', async () => {
    ...
    await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
    const loader = Object.create(Loader.prototype) as Loader
    const unwrapped = loader.unwrapExports(nuluPlugin) as Parameters<Context['plugin']>[0]
  ...
  it('falls back to the env key and defaults when config omits them', async () => {
    const prev = process.env.WORLD_APP_TECHNOLOGIES_API_KEY
    process.env.WORLD_APP_TECHNOLOGIES_API_KEY = 'env-key'
    try {
      const fetchMock = vi.fn(async () => jsonResponse(searchResponse()))
      vi.stubGlobal('fetch', fetchMock)
      const ctx = new Context()
      await ctx.plugin(WebRuntime, { searchProvider: GATEWAY_PROVIDER_ID })
      nuluPlugin.apply(ctx, {})
      await ctx.web.search({ query: 'q' })
      const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit]
      expect(url).toBe('https://platform.worldapptechnologies.com/api/v1/messages')
      expect((init.headers as Record<string, string>)['x-api-key']).toBe('env-key')
      expect(JSON.parse(init.body as string)).toMatchObject({ model: 'nulu-5' })
      await ctx.fiber.dispose()
    } finally {
      if (prev === undefined) delete process.env.WORLD_APP_TECHNOLOGIES_API_KEY
      else process.env.WORLD_APP_TECHNOLOGIES_API_KEY = prev
    }
  })

  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    const previous = process.env.WORLD_APP_TECHNOLOGIES_API_KEY
    delete process.env.WORLD_APP_TECHNOLOGIES_API_KEY
    const dir = await mkdtemp(join(tmpdir(), 'nulu-web-search-credentials-'))
    ...
    try {
      ...
      await ctx.plugin(nuluPlugin, { baseURL: 'https://api.worldapp-gateway.test/anthropic/v1' })

      await expect(ctx.web.search({ query: 'missing' }))
        .rejects.toThrow(expect.objectContaining({ code: 'WEB_PROVIDER_CREDENTIAL_MISSING' }))

      const ref = credentialRef('WORLD_APP_TECHNOLOGIES_API_KEY')
      ...
      if (previous === undefined) delete process.env.WORLD_APP_TECHNOLOGIES_API_KEY
      else process.env.WORLD_APP_TECHNOLOGIES_API_KEY = previous
    }
  })
  ...
  it('reports an actionable credential error when neither config nor env supplies a key', async () => {
    ...
    try {
      ...
      if (prev !== undefined) process.env.WORLD_APP_TECHNOLOGIES_API_KEY = prev
    }
  })
})
