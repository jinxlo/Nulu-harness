 ...
 * @module @worldapptechnologies/nulu-web
 */
...
export class WebRuntime extends Service {
   ...
   * `$NULU_WEB_SEARCH_PROVIDER` / `$NULU_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.NULU_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.NULU_WEB_FETCH_PROVIDER
  }
