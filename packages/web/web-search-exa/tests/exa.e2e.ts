...
import { ExaSearchProvider, EXA_DEFAULT_BASE_URL, EXA_DEFAULT_HIGHLIGHTS_PER_RESULT, EXA_DEFAULT_SEARCH_TYPE } from '@worldapptechnologies/nulu-web-search-exa'
...
maybe('ExaSearchProvider real API', () => {
  it('returns sources for a live query', async () => {
    ...
    const result = await provider.search({ query: 'Nulu Harness', maxResults: 5 })
