...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-web-search-exa) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-web-search-exa) — every accepted config field and its source declaration.
...
Indirectly, through `nulu-tool-web`, which retains this provider's `maxResults`-bounded URLs, titles, first highlights, and publication dates or its exact `Exa search aborted`, `Exa search request failed: <error>`, and `Exa returned an unprocessable response body: <error>` failures under the consumer's error wrapper.
