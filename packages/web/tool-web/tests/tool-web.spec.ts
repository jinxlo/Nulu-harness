...
import type { WebSearchProvider, WebSearchResult } from '@worldapptechnologies/nulu-web'
...
} from '@worldapptechnologies/nulu-tool-web'
import type { ContentBlock } from '@worldapptechnologies/nulu-llm'
import type { ToolResult } from '@worldapptechnologies/nulu-tools'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@worldapptechnologies/nulu-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by nulu-tool-call-timeout-policy over exec.signal.
