/**
 * Integration: the real fetch backend (`nulu-web-fetch-http`) + a real search provider
 * (`nulu-web-search-exa`) + the real seam (`nulu-web`) + the model tool (`nulu-tool-web`) + the
 * tool-call timeout policy (`nulu-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@worldapptechnologies/nulu-tools'
...
import * as TimeoutPolicy from '@worldapptechnologies/nulu-tool-call-timeout-policy'
...
describe('web_search integration over the real Exa provider', () => {
  it('runs web_search end-to-end and formats the provider result', async () => {
    ...
    const out = await call('web_search', { queries: ['worldapp-gateway'] })
    expect(out.isError).toBe(false)
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by nulu-tool-call-timeout-policy,
