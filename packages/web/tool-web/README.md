...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-web) is the exhaustive source for every accepted field and its JSDoc. `searchMaxQueries` bounds the accepted array before exact-string deduplication and provider fan-out; validation rejects an oversized array before any search starts. The timeout budgets attach to each tool definition and are enforced by [`@worldapptechnologies/nulu-tool-call-timeout-policy`](../../guard/timeout-policy/README.md); the model-facing schemas expose no timeout argument.
...
```text
web_search({ queries: ['nulu harness documentation'] })
```
...
- [nulu-web](../web/README.md) — the web service the tools execute through.
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-web) — the exact `web_search` and `web_fetch` schemas.
- [nulu-tool-call-timeout-policy](../../guard/timeout-policy/README.md) — the deployment policy that enforces each tool's timeout budget.
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-web) — every accepted config field and its source declaration.
...
The model sees the generated [`web_search` and `web_fetch` schemas](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-web). Result-count and timeout budgets are deployment settings, not model arguments.
