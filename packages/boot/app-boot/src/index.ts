/**
 * Shared boot glue for `nulu` profiles, including the CLI packaged by the Python runtime wheel: load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.nulu`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @worldapptechnologies/nulu-app-boot
 */
...
import { Context, type FiberState } from '@worldapptechnologies/cordis'
...
import { nuluHomePath, resolveNuluHome } from '@worldapptechnologies/nulu-home-paths'
...
declare module '@worldapptechnologies/cordis' {
  interface Context {
    ...
    nuluHomePath?: typeof nuluHomePath
  }
}
 ...
 * @param snapshotMode - the bin's `$NULU_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_NAMES = new Set([
  ...
  'WORLD_APP_TECHNOLOGIES_BASE_URL', 'WORLD_APP_TECHNOLOGIES_SEARCH_BASE_URL',
...
const BOOTSTRAP_PREFIXES = ['NULU_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * them; the home file is the user's own, and `NULU_HOME` is itself bootstrap-only, so no `.env` can
 ...
 * entries (`@worldapptechnologies/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@worldapptechnologies/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@worldapptechnologies/cordis-plugin-group`
 ...
 * directory. The self-referential `nulu-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    text: `The Nulu Harness implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend NULU itself.`,
  })
}
