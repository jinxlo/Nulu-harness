...
const tmp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'nulu-app-boot-'))
  tempRoots.push(dir)
  return dir
}
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'NULU_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['NULU_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['NULU_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'NULU_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['NULU_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['NULU_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'NULU_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'NULU_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@worldapptechnologies', 'nulu-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@worldapptechnologies', 'nulu-system-prompt')
  ...
  it('exposes nuluHomePath to Loader config expressions', async () => {
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js nuluHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(nuluHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir after reusable instructions', async () => {
    ...
    try {
      ...
      // through `-1 < n`.
      const identityAt = rendered.indexOf('You are an AI agent powered by Nulu Harness.')
