 ...
 * the argv shape nulu-sandbox-local's confine() builds), with piped stdio
...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  beforeAll(() => {
    scratchRoot = mkdtempSync(join(tmpdir(), 'nulu-acl-runner-'))
    writableDir = join(scratchRoot, 'writable')
    mkdirSync(writableDir)
    isolatedTemp = mkdtempSync(join(tmpdir(), 'nulu-acl-runner-temp-'))
    ...
    try {
      publicProbeDir = mkdtempSync(join(process.env.PUBLIC ?? 'C:\\Users\\Public', 'nulu-acl-public-'))
    } catch {
