...
import SessionStore, { type Session, type SessionEvent } from '@worldapptechnologies/nulu-session'
import InvariantRegistry, { InvariantError } from '@worldapptechnologies/nulu-invariants'
import * as SandboxPolicyInvariant from '@worldapptechnologies/nulu-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@worldapptechnologies/nulu-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@worldapptechnologies/nulu-sandbox-policy',
    })
  })
})
