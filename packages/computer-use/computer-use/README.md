...
# @worldapptechnologies/nulu-computer-use
...
The service has no configuration. Provider plugins inject `computerUse` and call `ctx.computerUse.register(ComputerUseProviderName(name))`; the brand is exported from `@worldapptechnologies/nulu-computer-use/brand`. The returned effect disposer releases that registration.
