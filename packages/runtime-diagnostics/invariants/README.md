...
`nulu-invariants` runs package-owned runtime checks — invariants — inside a Nulu Harness composition: any package can ship a `./invariant` companion that verifies its own durable relationships (authoritative event streams and mutable snapshots) while the composition runs. Checks run automatically, and a failed check reports an `InvariantError` attributed to the package that owns the violated relationship. Choose it for compositions that want self-checking diagnostics with a global switch and package-name filters; the standard agent composition already mounts it with the four core companions, and loading the service alone installs no checks.
...
Use the registry for compositions that want live diagnostics. [`nulu-sdk-minimal`](../../bundle/sdk-minimal/README.md) mounts it with the four core stateful companions — `nulu-session`, `nulu-agent`, `nulu-scope`, and `nulu-agent-loop`; `nulu-base` deliberately omits runtime diagnostics. Custom compositions mount the registry and add companions for any other loaded package whose contracts they want checked. Loading the registry alone installs no checks: it ships no product checks of its own, so a composition that never mounts a companion observes no diagnostic behavior.
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-invariants) is the exhaustive source for every accepted field and its JSDoc.
...
| Companion | Checks |
|---|---|
| `nulu-session`, `nulu-agent`, `nulu-scope`, `nulu-agent-loop` | Session log enclosure and call/result trace, agent-status transitions, scope-filtered dispatch subjects, loop-built request reconstruction |
| `nulu-llm`, `nulu-llm-retry`, `nulu-tools`, `nulu-system-prompt` | LLM stream grammar, retry-failure shape, tool-pipeline stage pairing and frozen results, prompt-assembly section names |
| `nulu-compaction`, `nulu-hook-protocol`, `nulu-sandbox-policy` | Compaction stream pairing, hook invocation/result pairing, sandbox mode values |
| `nulu-fs`, `nulu-subagent`, `nulu-workflow`, `nulu-tool-workflow` | Filesystem event identity, subagent provider and start/end pairing, workflow lifecycle identity, workflow record shape |
| `nulu-goal`, `nulu-goal-round-driver` | Durable goal-stream folds and reconstructed continuation prompts |
| `nulu-permission-presets`, `nulu-user-approval`, `nulu-commands` | Preset references to live presets, approval asked/decided pairing, command run/done pairing |
| `nulu-jobs`, `nulu-tool-todo`, `nulu-time-context` | Job snapshot field relationships, whole-list todo shape, durable clock readings |
| `nulu-credentials`, `nulu-settings`, `nulu-storage-domain`, `nulu-workspace` | Commit events against the live service or memory state, entity-cache mirroring |
| `nulu-agent-presets`, `nulu-session-title`, `nulu-plan-mode`, `nulu-schedule` | Preset mount placement, title source citation, plan-mode payload, schedule stream |
| `nulu-client-hmr`, `nulu-client-modules`, `nulu-client-runtime` | Browser/node-half stat-watcher lifecycle, boot entry graph, slot mutation versioning |
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-invariants) — every accepted config field and its source declaration.
...
- **Request reconstruction covers loop-built requests only** — the `nulu-agent-loop` companion reconstructs requests explicitly built by the loop; direct one-shot LLM calls remain outside that contract even when callers freeze them or attach a session id.
