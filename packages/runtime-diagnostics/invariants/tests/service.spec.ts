...
import InvariantRegistry, {
  InvariantError,
  type Config,
} from '@worldapptechnologies/nulu-invariants'
...
describe('InvariantRegistry selection', () => {
  it('applies defaults when constructed directly without schema normalization', async () => {
    ...
    const probe = vi.fn()
    const registration = runtimeRegistration(service.register('@worldapptechnologies/nulu-session', (child) => {
      child.on('invariants-test/ping', probe, { global: true })
    }))
    await registration
  ...
  it('enables registrations by default and treats empty lists as admit-all and exclude-none', async () => {
    for (const config of [{}, { package_allowlist: [], package_blocklist: [] }]) {
      ...
      const probe = vi.fn()
      await registerProbe(ctx, '@worldapptechnologies/nulu-session', probe)
      ctx.emit('invariants-test/ping')
      expect(probe).toHaveBeenCalledOnce()
    }
  })
  ...
  it('disables every installer while still reserving package ownership', async () => {
    ...
    const probe = vi.fn()
    const registration = await registerProbe(ctx, '@worldapptechnologies/nulu-session', probe)
    expect(() => ctx.invariants.register('@worldapptechnologies/nulu-session', () => {}))
  ...
  it('uses unanchored, case-sensitive JavaScript regex sources', async () => {
    ...
    await registerProbe(unanchored.ctx, '@worldapptechnologies/nulu-session-extra', unanchoredProbe)
    unanchored.ctx.emit('invariants-test/ping')
    expect(unanchoredProbe).toHaveBeenCalledOnce()

    const anchored = await setup({ package_allowlist: ['^@worldapptechnologies/nulu-session$'] })
    const anchoredProbe = vi.fn()
    await registerProbe(anchored.ctx, '@worldapptechnologies/nulu-session-extra', anchoredProbe)
    ...
    await registerProbe(caseSensitive.ctx, '@worldapptechnologies/nulu-session', caseProbe)
  ...
  it('lets the blocklist override an allowlist match', async () => {
    const { ctx } = await setup({
      package_allowlist: ['^@worldapptechnologies/nulu-'],
      package_blocklist: ['session'],
    })
    const sessionProbe = vi.fn()
    const agentProbe = vi.fn()
    await registerProbe(ctx, '@worldapptechnologies/nulu-session', sessionProbe)
    await registerProbe(ctx, '@worldapptechnologies/nulu-agent', agentProbe)
    ctx.emit('invariants-test/ping')
  ...
  it('accepts zero-match patterns for packages registered later', async () => {
    ...
    const now = vi.fn()
    const later = vi.fn()
    await registerProbe(ctx, '@worldapptechnologies/nulu-session', now)
  ...
  it('allows the same source in both lists and applies blocklist precedence', async () => {
    ...
    const probe = vi.fn()
    await registerProbe(ctx, '@worldapptechnologies/nulu-agent', probe)
    ctx.emit('invariants-test/ping')
    expect(probe).not.toHaveBeenCalled()
  })
})
...
describe('InvariantRegistry lifecycle', () => {
  it('honors the installer dependency API in its child fiber', async () => {
    ...
    await ctx.plugin({
      ...
      apply(child: Context) {
        ...
        registration = runtimeRegistration(child.invariants.register('@worldapptechnologies/nulu-probe', installer))
  ...
  it('attributes failures to the registering package with the stable code', async () => {
    const { ctx } = await setup()
    const registration = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-session', (child, fail) => {
    ...
    expect(caught).toMatchObject({
      name: 'InvariantError',
      code: 'INVARIANT',
      packageName: '@worldapptechnologies/nulu-session',
      message: 'invariant violated by "@worldapptechnologies/nulu-session": seq must strictly increase',
    })
  })
  ...
  it('disposes the child fiber completely and permits HMR re-registration', async () => {
    const { ctx } = await setup()
    const first = vi.fn()
    const firstRegistration = await registerProbe(ctx, '@worldapptechnologies/nulu-session', first)
    ctx.emit('invariants-test/ping')
    ...
    const second = vi.fn()
    await registerProbe(ctx, '@worldapptechnologies/nulu-session', second)
    ctx.emit('invariants-test/ping')
    expect(first).toHaveBeenCalledOnce()
  ...
  it('reserves ownership until asynchronous child disposal completes', async () => {
    ...
    const registration = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-session', (child) => {
    ...
    const replacement = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-session', () => {}))
    await replacement
    await replacement()
  })
  ...
  it('rolls back listeners and ownership atomically when an installer fails', async () => {
    const { ctx } = await setup()
    const leaked = vi.fn()
    const failed = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-session', (child) => {
    ...
    const retry = vi.fn()
    await registerProbe(ctx, '@worldapptechnologies/nulu-session', retry)
    ctx.emit('invariants-test/ping')
  ...
  it('rolls back publication effects and ownership when child-fiber publication fails', async () => {
    ...
    const failed = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-publication-probe', () => {}))
    await expect(Promise.resolve(failed)).rejects.toThrow('publication failed')
    ctx.emit('invariants-test/ping')
    expect(leaked).not.toHaveBeenCalled()
    stopRejecting()

    const retry = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-publication-probe', () => {}))
    await retry
    await retry()
  })
  ...
  it('joins asynchronous checks and rolls back their effects on failure', async () => {
    const { ctx } = await setup()
    const leaked = vi.fn()
    const failed = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-async-probe', async (child, fail) => {
      child.on('invariants-test/ping', leaked, { global: true })
    ...
    const retry = runtimeRegistration(ctx.invariants.register('@worldapptechnologies/nulu-async-probe', async () => {
      await Promise.resolve()
    }))
    await retry
    await retry()
  ...
  it('releases a synchronous reservation if the service fiber is already inactive', async () => {
    ...
    await fiber.dispose()
    expect(() => service.register('@worldapptechnologies/nulu-session', () => {})).toThrow(/inactive/i)
  })
})
