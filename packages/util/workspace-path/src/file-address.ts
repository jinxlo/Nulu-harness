/**
 * The `nulu-resource://file/…` address grammar: how a file is named across the
 ...
 *
 * Every resource address is `nulu-resource://<type>/…`, the URI host naming the
 * resource protocol; for `file` the path opens with the scope:
 *
 * - `nulu-resource://file/session/<sessionId>/<path>` names a file by its path,
 *   relative to that Session's workspace root or absolute; the
 *   Host resolves it against the root it holds for the Session.
 * - `nulu-resource://file/absolute/<path>` names a file by its absolute path with
 *   the leading `/` dropped (`nulu-resource://file/absolute/home/ys/notes.txt`;
 *   Windows `nulu-resource://file/absolute/C:/x/y.txt`; a UNC path keeps an empty
 *   first segment, `nulu-resource://file/absolute//server/share/x.txt`). It carries
 *   no Session.
 *
...
const FILE_ADDRESS_PREFIX = 'nulu-resource://file/'
 ...
 * @returns the `nulu-resource://file/session/<sessionId>/<path>` address.
 */
 ...
 * @returns the `nulu-resource://file/absolute/<path>` address.
 */
 ...
 * @returns the parts, or `undefined` when the string is not a `nulu-resource://file/` URI in a known scope with a path, or a segment is not validly encoded.
 */
