...
```ts
import { brandString, type Branded } from '@worldapptechnologies/nulu-brand'
...
```ts
import { brandNumber, type BrandedNumber } from '@worldapptechnologies/nulu-brand'
...
Brand values that cross package boundaries and could plausibly be confused — `ToolCallId` in `nulu-llm`, the shared agent/session `SessionId` in `nulu-session`, `JobId` in `nulu-jobs`, and `SessionSeq` versus `SessionLogOffset` in `nulu-session`. Values that stay local or cannot be confused do not need this abstraction.
...
Keeping these helpers in their own package means `nulu-jobs` can brand `JobId` without importing an unrelated capability package, while each capability still owns the meaning and validation of its concrete ids.
