...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-fs) is the exhaustive source for every accepted field and its JSDoc.
...
- [nulu-fs](../fs/README.md) — the `ctx.fs` contract these tools consume.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-fs) — the exhaustive schemas this package registers.
...
- **No model-facing directory listing ships** — `ctx.fs.listDir` serves provider code such as skill discovery, while the sibling `nulu-tool-fs-search` package supplies ripgrep-backed `glob` and `grep` rather than extending the filesystem seam.
