...
使用 `nulu-tool-fs` 可让模型带行号读取 UTF-8 文件、读取受支持的图片、创建或原子地替换文件，以及执行有针对性的字面量编辑。结果都有上限，失败会提供稳定错误码与恢复指令。当写入和编辑必须在成功读取后执行时，请添加 `nulu-fs-observation-policy`；省略它时，变更仍是原子的，但不受此条件约束。图片读取需要持久附件存储和支持图片输入的路由模型。glob 或 grep 搜索请选择同级的发现工具包。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-tool-fs)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [nulu-fs](../fs/README.zh.md)——这些工具消费的 `ctx.fs` 约定。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-fs)——本包注册的穷尽式 schema。
...
模型会看到已生成的 [`read`、`read_image`、`write` 和 `edit` schema](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-fs)，参数使用 snake_case。图片工具只在持久附件存储已挂载时出现；schema 本身与路由无关，严格门禁在执行时拒绝。作用域工具限制可以为某个 agent 移除任一定义。
...
- **未交付面向模型的目录列表工具**：`ctx.fs.listDir` 服务于 skill（技能）发现等提供方代码，同级 `nulu-tool-fs-search` 包则提供基于 ripgrep 的 `glob` 与 `grep`，而不是扩展文件系统 seam。
