...
// --------------------------------------------------------------------------
describe('default deployment (with nulu-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nulu-tool-fs-'))
    ctx = new Context()
...
// --------------------------------------------------------------------------
describe('bare provider (no nulu-fs-observation-policy)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nulu-tool-fs-bare-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
...
// caller-selected session workspace wins, matching nulu-tool-bash.
describe('per-session cwd', () => {
  let sessionDir: string
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'nulu-tool-fs-cfg-'))
    sessionDir = await mkdtemp(join(tmpdir(), 'nulu-tool-fs-session-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
