...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('fs tools with-key smoke', () => {
  it('creates, reads, then edits a file — verified on disk', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-fs-e2e-'))
    ...
    const agent = await ctx.agentLoop.create(SessionId('fs-e2e'), { provider: 'worldapp-gateway', model: 'nulu-5' })
  ...
  it('resolves a relative path against the per-session cwd (factory meta.cwd)', async () => {
    ...
    const configDir = await mkdtemp(join(tmpdir(), 'nulu-fs-e2e-cfg-'))
    workdir = configDir
    const sessionDir = await mkdtemp(join(tmpdir(), 'nulu-fs-e2e-session-'))
    try {
