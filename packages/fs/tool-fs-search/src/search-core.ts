 ...
 *
 * @module @worldapptechnologies/nulu-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@worldapptechnologies/nulu-output-retention'
import type { RetainedItems } from '@worldapptechnologies/nulu-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@worldapptechnologies/nulu-subprocess'
 ...
 * `@worldapptechnologies/nulu-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`nulu-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@worldapptechnologies/nulu-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@worldapptechnologies/nulu-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@worldapptechnologies/nulu-tool-fs-search/presentation} `grepSearchMeta`)
