 ...
 * model-facing render consumes ({@link module:@worldapptechnologies/nulu-tool-fs-search/search-core}
 ...
 * final output budget (`nulu-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @worldapptechnologies/nulu-tool-fs-search/presentation
 */
 ...
 * ({@link module:@worldapptechnologies/nulu-tool-fs-search/grep} `formatGrepMatches`), so
