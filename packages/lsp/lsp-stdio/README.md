...
Mount this provider when a deployment has local language servers — for example `typescript-language-server` — and wants the harness to navigate code through them. It needs filesystem and subprocess providers for the same execution world, plus the `nulu-lsp` seam and, for model access, `nulu-tool-lsp`.
...
| `env` | `{}` | Extra env merged over the credential-scrubbed ambient env; variables matching `KEY`/`PASSWORD`/`SECRET`/`TOKEN` and all `NULU_*` names are not forwarded |
...
`servers` must contain at least one entry with non-empty ids; timer budgets must be positive integers within Node's timer range, and byte caps must be positive. The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-lsp-stdio) is the exhaustive source for every accepted field.
...
- [nulu-lsp](../lsp/README.md) — the seam this provider registers against.
- [nulu-tool-lsp](../tool-lsp/README.md) — the model-facing tool over the seam.
...
Indirectly, through `nulu-tool-lsp`, which surfaces this provider's normalized results while this host contributes no prompt or schema itself.
