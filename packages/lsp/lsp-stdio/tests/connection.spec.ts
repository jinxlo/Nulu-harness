...
import { LspConnection } from '@worldapptechnologies/nulu-lsp-stdio'
import type { ConnectionWriter } from '@worldapptechnologies/nulu-lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit NULU_* env entries to the child', async () => {
    // A configured NULU_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'NULU_LSP_TEST_FACT', NULU_LSP_TEST_FACT: 'managed' })
