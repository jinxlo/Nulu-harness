...
/**
 * Keyless built-artifact smoke: plain Node imports `@worldapptechnologies/nulu-lsp` and
 * `@worldapptechnologies/nulu-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@worldapptechnologies/nulu-lsp')
      const LspLocal = await import('@worldapptechnologies/nulu-lsp-stdio')
      const { default: LocalFileSystem } = await import('@worldapptechnologies/nulu-fs-local')
