...
describe('real Loader composition', () => {
  it('registers on a real WebServer and dispatches a signed request', { timeout: 60_000 }, async () => {
    root = await mkdtemp(join(tmpdir(), 'nulu-webhook-github-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "    host: '127.0.0.1'",
      '    port: 0',
      "- name: '@worldapptechnologies/nulu-webhook-github'",
      '  config:',
      '    source: loader',
      '    path: /github',
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@worldapptechnologies/nulu-webhook-github', GitHubAdapter],
    ])
