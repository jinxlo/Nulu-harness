...
function copyFixture(sourceRoot = fixtureRoot): string {
  const root = mkdtempSync(join(tmpdir(), 'nulu-typert-remote-model-'))
...
`
  ...
  writeFileSync(configPath, JSON.stringify({
    ...
    compilerOptions: {
      ...
      skipLibCheck: false,
      paths: {
        '@worldapptechnologies/nulu-typert-protocol': ['./typert-protocol.d.ts'],
...
import type { TypertRemoteNamespaceMap } from '@worldapptechnologies/nulu-typert-protocol'
declare const ctx: { remote: TypertRemoteNamespaceMap }
