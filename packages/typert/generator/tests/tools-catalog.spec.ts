...
import type { TypertContribution } from '@worldapptechnologies/nulu-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@worldapptechnologies/nulu-tool-cordis/src/api-catalog.ts'
...
describe('model-driven nulu-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@worldapptechnologies/nulu-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('nulu-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@worldapptechnologies/nulu-tools')
    ...
    const record = ctx.typert.getPackage('@worldapptechnologies/nulu-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@worldapptechnologies/nulu-tools', 'host')).toBeUndefined()
  })
})
