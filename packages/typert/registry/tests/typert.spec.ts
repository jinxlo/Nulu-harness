...
} from '@worldapptechnologies/nulu-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@worldapptechnologies/nulu-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@worldapptechnologies/nulu-tools', 'ToolInput')).toBe('@worldapptechnologies/nulu-tools#ToolInput')
    expect(typertPackageKey('@worldapptechnologies/nulu-tools', 'host')).toBe('@worldapptechnologies/nulu-tools#host')
    expect(ctx.typert.get('@worldapptechnologies/nulu-tools#ToolInput')).toMatchObject({
      package: '@worldapptechnologies/nulu-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@worldapptechnologies/nulu-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@worldapptechnologies/nulu-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@worldapptechnologies/nulu-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@worldapptechnologies/nulu-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@worldapptechnologies/nulu-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.toJSONSchema('@worldapptechnologies/nulu-tools#ToolInput')).not.toBe(projected)
  })
