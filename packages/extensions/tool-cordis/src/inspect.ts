 ...
 * `name` adds.
 * @module @worldapptechnologies/nulu-tool-cordis/inspect
 */
...
import type {} from '@worldapptechnologies/nulu-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so a NULU restart clears them.']
  }
