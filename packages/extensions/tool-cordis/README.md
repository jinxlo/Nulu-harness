...
`nulu-tool-cordis` lets a model inspect the live Cordis runtime and create, run, stop, update, or remove temporary dynamic packages with host code, browser code, or both. Package versions are immutable, so a failed package can be corrected by adding a new version and updating the active one. Definitions exist only in process memory and disappear when NULU restarts; the package does not write repository files, install dependencies, or change `cordis.yml`. It also teaches the model this workflow. Compose it with `@worldapptechnologies/nulu-cordis-host-runner`, which provides the sandbox and run round trip.
...
```yaml
- name: '@worldapptechnologies/nulu-cordis-host-runner'
  config:
    vmTimeoutMs: 5000
- name: '@worldapptechnologies/nulu-tool-cordis'
```
...
Definitions are session-scoped and process-local: a package is visible and controllable only in the session that defined it, stays active across later turns, and can affect other sessions in the same process while running. Stopping, removing, unloading the toolset, or restarting NULU clears it. The sandbox isolates globals but is not a security boundary — treat a dynamic package like bash access, and load this plugin as deliberately as you would grant one.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-cordis) — the exact schemas the model receives.
...
The conversation model sees the generated [`cordis_inspect_list`, `cordis_inspect_query`, `cordis_inspect_self`, `cordis_define`, `cordis_run`, `cordis_stop`, and `cordis_undefine` schemas](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-cordis) whenever this plugin is visible.
...
Dynamic Cordis plugins temporarily extend the current NULU process. A Plugin uses apply(ctx) to consume Services, listen to Events, provide Services, register model Tools, or register browser UI in Slots.
```
