...
`nulu-cordis-host-runner` 让动态包在本进程中可运行：模型用 `cordis_define` 记录的定义留在这里，host 半在 `node:vm` 沙箱中运行，带浏览器半的包会等待人在页面上批准或拒绝，模型也可以在这里检查实时运行时及其定义。面向模型的工具在 `@worldapptechnologies/nulu-tool-cordis` 中，浏览器半经 `@worldapptechnologies/nulu-cordis-client-runner` 装载。定义只存在于进程内存中，因此 NULU 重启即清空，也不会向磁盘写任何东西。唯一的配置字段 `vmTimeoutMs` 限制同步沙箱求值的时长。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-cordis-host-runner)是每个受支持字段的穷尽式真源。
...
定义以会话为界、以进程为本：包只对定义它的会话可见，其他会话读取时视其为不存在，NULU 重启后一切都消失。会话日志保留一次 define 调用的参数——包括它提交的代码——以及回执；解析出的定义只存于内存注册表。浏览器半只能经一次运行到达页面，因此刷新后的页面手上什么都没有，直到有人再次运行该包。
...
- [生成的配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-cordis-host-runner)——每个受支持配置字段。
