...
# @worldapptechnologies/nulu-file-reference


## Summary

Host-backed user interfaces use `nulu-file-reference` to offer `@file` completion: a UI asks for path candidates for the addressed agent, the model types `@path` or `@"path with spaces"`, and picking a candidate inserts the matching mention as ordinary prompt text. The seam itself owns no filesystem access — a concrete provider such as `@worldapptechnologies/nulu-file-reference-local` supplies candidates, ranking, caching, and invalidation. Selecting a candidate never reads or attaches file contents; the model must call a filesystem tool to inspect a file. Session Controller exposes the same discovery to browser consumers through the `fileReferences/list` Remote.
...
- [Filesystem tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-fs) — the `read` tool that referenced paths are meant for.
