...
```yaml
- name: '@worldapptechnologies/nulu-agent-instructions'
  config:
    maxBytes: 65536
```
...
| `nuluHome` | `$NULU_HOME` or `~/.nulu` | Directory containing the user-global `AGENTS.md` |

The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-agent-instructions) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-agent-instructions) — every accepted config field and its source declaration.
...
Instructions from: ~/.nulu/AGENTS.md
...
- **Candidate semantics stay intentionally small** — lowercase names, `.claude/rules/`, and `@path` imports are not interpreted; project scopes load `AGENTS.local.md`/`CLAUDE.local.md` overlays by default, but the user-global `$NULU_HOME` scope has no local overlay and other custom names require explicit candidate configuration.
