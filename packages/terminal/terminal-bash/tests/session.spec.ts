...
import { LocalPtySession } from '@worldapptechnologies/nulu-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@worldapptechnologies/nulu-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@worldapptechnologies/nulu-terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from '@worldapptechnologies/nulu-subprocess'
import { TerminalError } from '@worldapptechnologies/nulu-terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07nulu> ')
...
describe('LocalPtySession readiness and output', () => {
  it('answers split cursor-position queries before publishing prompt readiness', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07nulu> \x1b[')
    terminal.emitData('6n')
    await vi.advanceTimersByTimeAsync(20)

    expect(terminal.writes).toContain('\x1b[1;7R')
  ...
  it('drains terminal replies before caller input and re-inspects after concurrent output', async () => {
    ...
    terminal.write = async (data) => {
      terminal.writes.push(data)
      if (data === '\x1b[1;7R') await responseGate.promise
    }
    ...
    expect(terminal.writes).toEqual(['\x1b[1;7R'])
    responseGate.resolve(undefined)
    await vi.advanceTimersByTimeAsync(0)
    ...
    expect(terminal.writes).toEqual(['\x1b[1;7R', '\x1b[1;7R', 'caller input\r'])
  ...
  it('drains a terminal reply that is pending when caller input starts', async () => {
    ...
    responseGate.resolve(undefined)
    await vi.advanceTimersByTimeAsync(0)
    expect(terminal.writes).toEqual(['\x1b[1;7R', 'caller input\r'])
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07nulu> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07nulu> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
