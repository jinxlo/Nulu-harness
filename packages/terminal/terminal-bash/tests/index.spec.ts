...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId } from '@worldapptechnologies/nulu-session'
...
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@worldapptechnologies/nulu-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@worldapptechnologies/nulu-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@worldapptechnologies/nulu-terminal-bash'
import { ENCODING_PREAMBLE } from '@worldapptechnologies/nulu-pwsh-local'
import * as ptyLocal from '@worldapptechnologies/nulu-terminal-bash'
import type { ResolvedConfig } from '@worldapptechnologies/nulu-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@worldapptechnologies/nulu-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'nulu> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'nulu> \'',
        NULU_SHELL: '1', NULU_SESSION_ID: 'agent', NULU_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07nulu> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo nulu> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo nulu> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', NULU_SHELL: '1', NULU_SESSION_ID: 'agent', NULU_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'nulu> ' : "function prompt { 'nulu> ' }\n",
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'nulu> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
