...
import { normalizeTerminalText, TerminalSanitizer } from '@worldapptechnologies/nulu-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07nulu> ')).toEqual({ text: 'nulu> ', prompt: true, promptTail: 'nulu> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('nulu> ')).toEqual({ text: 'nulu> ', prompt: false, promptTail: 'nulu> ' })
  })
