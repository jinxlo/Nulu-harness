...
当 agent 需要跨调用保留终端状态或提供交互式输入时，使用 `nulu-tool-terminal`。它可以打开、发送、读取、传递信号、关闭和列出终端会话，同时防止一个 agent 操作其他 agent 的会话。发送可以等待有界的前台输出，也可以返回供后续收集或中断的后台任务 job id。`maxResultBytes` 限制每个结果的大小，而结果会保留在会话历史中直到压缩（compaction）。指引会让模型对有界工作优先使用单次工具。
...
工具需要 `ctx.terminals`——必须挂载一个后端——以及用于指引章节的系统提示词服务。后台发送还额外要求任务服务及其面向模型的控制器（`@worldapptechnologies/nulu-tool-jobs`）。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-tool-terminal)与[工具目录](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-terminal)是配置字段与 schema 的穷尽式真源。
...
- [工具目录](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-terminal)——6 个生成的 schema 与结果形态。
...
6 个生成的 schema 列在 [`nulu-tool-terminal` 目录章节](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-terminal)中。此插件活跃时，请求中会包含它们的固定 schema token；按 agent 范围过滤工具时可能隐藏这些 schema。
...
- **后台模式要求任务接口面**——`run_in_background` 同时需要 `@worldapptechnologies/nulu-jobs` 及其面向模型的控制器（`@worldapptechnologies/nulu-tool-jobs`）；缺少时会拒绝该参数。
