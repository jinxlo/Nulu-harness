...
The tools need `ctx.terminals` — a backend must be mounted — and the system-prompt service for the guidance section. Background sends additionally require the jobs service and its model-facing controller (`@worldapptechnologies/nulu-tool-jobs`).
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-terminal) and [tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-terminal) are the exhaustive sources for config fields and schemas.
...
- [Tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-terminal) — the six generated schemas and result shapes.
...
The six generated schemas are listed in the [`nulu-tool-terminal` catalog section](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- **Background mode requires the jobs surface** — `run_in_background` needs both `@worldapptechnologies/nulu-jobs` and its model-facing controller (`@worldapptechnologies/nulu-tool-jobs`); without them the argument is rejected.
