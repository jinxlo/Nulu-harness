...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@worldapptechnologies/nulu-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@worldapptechnologies/nulu-hook-protocol'
import type { RunHookOptions } from '@worldapptechnologies/nulu-hook-protocol'
 ...
 * executor (nulu-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what nulu-bash-local's resolve does for the fields runHook sets.
      return {
