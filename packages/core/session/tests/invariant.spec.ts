...
import { createScope, scopeTarget } from '@worldapptechnologies/nulu-scope'
import { createSystemMessage, createUserMessage, ToolCallId, createMessage, createToolResultMessage, freezeMessage } from '@worldapptechnologies/nulu-llm'
import SessionStore, { SessionId, SessionSeq, TOOL_NOT_STARTED } from '@worldapptechnologies/nulu-session'
...
describe('session-log invariants', () => {
  ...
  it('requires a system/message to name the open step', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const message = createSystemMessage('You are terse.', '@worldapptechnologies/nulu-system-prompt')
