...
Mount `nulu-tools` wherever agents call tools: it provides `ctx.tools`, the registry every tool plugin registers into and the loop dispatches through. Registering a tool is enough to make it visible — the registry feeds its schemas into the system-prompt assembly automatically.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tools) — the shipped tools' schemas the model receives.
...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@worldapptechnologies/nulu-tool-call-timeout-policy` wrapper.
