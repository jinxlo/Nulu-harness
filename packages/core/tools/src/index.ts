...
import type { ToolProviderResult } from '@worldapptechnologies/nulu-system-prompt'
import type { CodeRuntime } from '@worldapptechnologies/nulu-code-runtime'
 ...
 * instead of deriving them: the seam's `nulu-code-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@worldapptechnologies/nulu-tools`
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@worldapptechnologies/nulu-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `nulu-agent-loop`'s parallel scheduler. */
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`nulu-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @worldapptechnologies/nulu-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`nulu-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `nulu-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('nulu.tool.execution') as ToolExecutionToken
}
