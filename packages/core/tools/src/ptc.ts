...
import type { ContentBlock, ToolCallId } from '@worldapptechnologies/nulu-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@worldapptechnologies/nulu-code-runtime'
import { snapshotJsonValue, type JsonValue } from '@worldapptechnologies/nulu-util-values'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`nulu-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
