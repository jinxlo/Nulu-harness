 ...
 *
 * @module @worldapptechnologies/nulu-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@worldapptechnologies/cordis'
...
import type { SessionEvent, SessionId, SessionLogOffset } from '@worldapptechnologies/nulu-session'
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in nulu-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `nulu-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `nulu-agent-loop` package.
 */
 ...
 * (`@worldapptechnologies/nulu-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@worldapptechnologies/nulu-agent#Agent',
