...
import LlmRuntime, { createUserMessage, LlmError, ReasoningEffortId  } from '@worldapptechnologies/nulu-llm'
import type { GenerateOptions, LlmModelReasoningInfo, LlmResolvedModelInfo, StreamChunk } from '@worldapptechnologies/nulu-llm'
import SessionStore, { Session, SessionId, foldRequestHeader } from '@worldapptechnologies/nulu-session'
...
describe('request stability across the loop', () => {
  ...
  it('rematerializes the selected adapter maxTokens default after a provider switch', async () => {
    const nulu = new MockAdapter([textResponse('nulu')], undefined, 256_000)
    ...
    expect(nulu.requests[0]?.maxTokens).toBe(256_000)
  ...
  it('preserves an explicit agent maxTokens cap across a provider switch', async () => {
    ...
    expect(nulu.requests[0]?.maxTokens).toBe(4_096)
