...
import LlmRuntime, { createUserMessage, ToolCallId, LlmError, MessageSource, ProviderRequestId, StreamChunk  } from '@worldapptechnologies/nulu-llm'
import SessionStore, { Session, SessionEvent, SessionId, TurnEndReason, type UserMessage } from '@worldapptechnologies/nulu-session'
import SystemPrompt from '@worldapptechnologies/nulu-system-prompt'
import ToolRuntime, { defineContentToolFixture, type PostToolDecision } from '@worldapptechnologies/nulu-tools'
...
describe('turn and step boundary recovery', () => {
  ...
  it('a step/end validation failure surfaces the resulting open-step invariant', async () => {
    ...
    expect(errors.map(error => error.message)).toEqual([
      ...
      'invariant violated by "@worldapptechnologies/nulu-session": turn/end 1 while step 1 is still open',
    ])
    expect(boundaryCounts(agent)).toMatchObject({
      turnStart: 1,
      turnEnd: 0,
