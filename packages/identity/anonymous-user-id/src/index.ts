 ...
 * harness home resolved by {@link resolveNuluHome} (`$NULU_HOME` > `~/.nulu`),
 ...
 * machine: every process sharing one `$NULU_HOME` reports the same id, and
 ...
 *
 * @module @worldapptechnologies/nulu-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `NULU_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveNuluHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
