---
description: "Anonymous per-harness-home identity for users and maintainers tracing how telemetry, feedback acknowledgement, and Nulu provider requests correlate records."
...
When you want the records your installation sends out to be recognizable as coming from the same harness home — telemetry, feedback, and Nulu requests all carry one shared id — this package is what provides it. There is nothing to install or configure: the id appears automatically, and the shipped feedback, telemetry, and Nulu features already use it. Do not use it to identify a user or to join records across different homes; it is anonymous and home-scoped.
...
- **Nulu requests** — every provider request carries the `x-nulu-harness-user-id` header, so usage can be attributed per installation.
...
When you build a feature that should share the installation's anonymous id, import the value once and reuse it — telemetry, feedback, and Nulu already use the same id, so your records line up with theirs:
...
- [nulu-llm-gateway](../../llm/llm-gateway/README.md) — sends `x-nulu-harness-user-id` on provider requests.
...
None, as the shared identifier reaches Nulu only as model-hidden HTTP metadata and registers nothing model-facing.
...
- **Configured Nulu gateways receive the id** — `nulu-llm-gateway` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
