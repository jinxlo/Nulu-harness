...
import type { ShellRunResult, CollectedOutput } from '@worldapptechnologies/nulu-shell'
import SessionProjectionRegistry from '@worldapptechnologies/nulu-session-projection'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@worldapptechnologies/nulu-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@worldapptechnologies/nulu-sandbox'
...
import type { Config } from '@worldapptechnologies/nulu-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'nulu-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'NULU_WRAP=1', 'bash', '-c', 'printf "%s" "$NULU_WRAP"']
    ...
    const spawn = vi.spyOn(ctx.subprocess, 'spawn')
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$NULU_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nulu-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.NULU_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'nulu-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const deniedRoot = mkdtempSync(join(tmpdir(), 'nulu-sandbox-denied-'))
    try {
