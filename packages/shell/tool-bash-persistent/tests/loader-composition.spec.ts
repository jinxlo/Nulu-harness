...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'nulu-persistent-bash-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@worldapptechnologies/nulu-tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('NULU_PERSISTENT_BASH')
    ...
    expect(multiline).not.toContain('NULU_PERSISTENT_BASH')
