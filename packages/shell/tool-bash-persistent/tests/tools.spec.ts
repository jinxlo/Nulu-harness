...
class StubPtySession implements TerminalBackendSession {
  ...
  startSend(request: TerminalSendRequest): TerminalSendOperation {
    ...
    if (this.mode === 'wait-for-abort' || this.mode === 'end-on-abort') {
      const done = new Promise<ReturnType<StubPtySession['result']>>((resolve) => {
        request.signal?.addEventListener('abort', () => {
          const start = /__NULU_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(request.text)?.[0]
          const end = /__NULU_PERSISTENT_BASH_END_[^:]+:/.exec(request.text)?.[0]
    ...
    this.pendingText = ''
    const start = /__NULU_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(sent)?.[0]
    const end = /__NULU_PERSISTENT_BASH_END_[^:]+:/.exec(sent)?.[0]
...
describe('tool-bash-persistent', () => {
  ...
  it('returns a stdin_read fallback reached after multiple polling rounds', async () => {
    ...
    expect(result).not.toContain('NULU_PERSISTENT_BASH_START')
  })
