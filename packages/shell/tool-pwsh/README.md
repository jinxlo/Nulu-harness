...
`nulu-tool-pwsh` gives the agent a `pwsh` tool that runs PowerShell commands through the mounted shell executor — the Windows counterpart of `nulu-tool-bash`, mirroring it call-for-call. Each call runs in a fresh pwsh process, so no state survives; `run_in_background` turns long-running commands into background jobs. Commands are PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables, with no dialect translation. Every call runs with the managed `NULU_*` environment, and under a sandboxing executor the tool teaches and enforces the Windows-specific language-mode and named-pipe contracts. Mount it with a PowerShell executor such as `nulu-pwsh-local` and the `nulu-shell-env` plugin.
...
Load this plugin in any composition where the agent should run PowerShell commands — typically a Windows composition whose `ctx.shell` is backed by a PowerShell executor. It registers the `pwsh` tool once the executor provider and the `nulu-shell-env` registry are mounted.

### When to choose it

Choose the pwsh tool when commands must be written in PowerShell — native paths and `$env:` variables — or when the deployment is Windows-native. Choose `nulu-tool-bash` when the command set is bash-dialect; there is no translation between the two. When work needs cross-call state (cwd, variables), the persistent counterpart [`nulu-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md) keeps one owner-scoped shell alive.
...
- name: '@worldapptechnologies/nulu-shell-env'
- name: '@worldapptechnologies/nulu-tool-pwsh'
```
...
A composition with no PowerShell executor never activates the tool, and the injected services (`tools`, `shell`, `systemPrompt`, `shellEnv`) must all exist. Background calls without the job runtime fail with `background jobs unavailable: load @worldapptechnologies/nulu-jobs and @worldapptechnologies/nulu-tool-jobs`, and `sandbox_permissions` without a sandboxing executor fails with `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`.
...
The renderer shares the bash tool's structure and the `parseExitStatus` marker contract from `nulu-shell`: a clean exit (0, no signal) produces no marker; the UI card consumes the exit marker as its exit-status pill. Windows forced termination settles as exit 1 without a signal, so `[killed by signal: …]` is POSIX-only there. The `tool:pwsh` prompt section (first-party order 1010) teaches the exit-marker convention and the Windows exit-1-after-interruption reading.
...
- [shell-env](../shell-env/README.md) — the managed `NULU_*` environment every call receives.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-pwsh) — the exact `pwsh` argument schema.
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-pwsh) — every accepted config field and its source declaration.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@worldapptechnologies/nulu-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls.
