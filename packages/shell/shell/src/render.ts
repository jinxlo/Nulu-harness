/**
 * Shared rendering helpers for the shell tools (`nulu-tool-bash`,
 * `nulu-tool-pwsh`): the exit-status marker contract the tools' renderers emit,
 * Host `presentResult` implementations parse here, and the Web terminal card
 * model mirrors without importing Host code.
 * @module @worldapptechnologies/nulu-shell/render
 */
