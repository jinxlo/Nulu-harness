 ...
 */
async function harness(adapter: MockAdapter, sessionRoot?: string, nuluHome?: string) {
...
describe('bash tool through the agent loop', () => {
  it('first-turn bash receives session identity in a scrubbed NULU_* namespace', async () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-bash-session-env-'))
    dirs.push(root)
    const nuluHome = join(root, 'nulu-home')
    vi.stubEnv('NULU_STALE_PARENT', 'stale')
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'bash', {
        command: 'printf \'%s\\n%s\\n%s\\n%s\\n\' "$NULU_HOME" "$NULU_SHELL" "$NULU_SESSION_ID" "${NULU_STALE_PARENT-unset}"',
        description: 'inspect session environment',
      }),
      textResponse('Session environment inspected.'),
    ])
    const ctx = await harness(adapter, root, nuluHome)
    ...
    expect(resultText(result)).toBe(`${nuluHome}\n1\nsession-env-id\nunset\n`)
  ...
  it('background: start ack → completion wakes the idle agent → job_output collects it', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'nulu-bg-'))
    dirs.push(dir)
