...
import type { ConfinedArgv, RunnerFailureRule, SandboxExecutionPolicy, SandboxPolicy } from '@worldapptechnologies/nulu-sandbox'
...
const spillDir = mkdtempSync(join(tmpdir(), 'nulu-pwsh-sandbox-spec-'))
...
describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'nulu-pwsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'nulu-pwsh-sandbox-ro-'))
