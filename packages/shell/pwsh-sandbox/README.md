...
Mount this executor instead of `nulu-pwsh-local` when PowerShell commands must not run with the harness process's full file authority. It registers as `ctx.shell`, inherits `nulu-pwsh-local`'s process mechanics, and requires a `ctx.sandbox` provider plus `ctx.sandboxPolicy`.
...
On Windows, mount the ACL restricted-token provider; on Linux and macOS, mount the local runner provider instead. The executor's own config is the local pwsh executor's knobs verbatim; the generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-pwsh-sandbox) is the exhaustive source.

```yaml
- id: sandbox
  name: '@worldapptechnologies/nulu-sandbox-windows-acl'
- id: sandbox-policy
...
- id: bash
  name: '@worldapptechnologies/nulu-pwsh-sandbox'
```
...
- **Reads are unrestricted on Windows** — the ACL runner restricts writes only; the read boundary is documented in `@worldapptechnologies/nulu-sandbox-windows-acl`.
