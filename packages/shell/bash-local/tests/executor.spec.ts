...
const spillDir = mkdtempSync(join(tmpdir(), 'nulu-bash-exec-spec-'))
...
describe('LocalBashExecutor.run', () => {
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'true', workdir: '/nonexistent-nulu' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/nuluEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: 'cat; echo "[$SEAM_VAR][$NULU_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
...
describe('LocalBashExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = await bash.start(bash.resolve({
      command: 'cat; echo "[$BG_VAR][$NULU_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      nuluEnv: { NULU_BG_VAR: 'bg-nulu-env' },
    }))
    const output = await readUntil(proc, '[bg-env][bg-nulu-env]')
  ...
  it('an asynchronous creation failure settles as killed with a stage-neutral note', async () => {
    const { bash } = await setup()
    const proc = await bash.start(bash.resolve({ command: 'true', workdir: '/nonexistent-nulu' }))
