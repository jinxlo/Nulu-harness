...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-persona) is the exhaustive source for every accepted field and its JSDoc.
...
Use this row when a preset must change an agent's identity and not only its tools. The deployment-wide persona itself is configured on the `nulu-system-prompt` row, not here; this row exists only to shadow or replace it for one agent.
...
`nulu-system-prompt` owns the global persona as its own config and registers `deployment:persona-prefix` unconditionally, so a process has exactly one. This row collides with that registration outside an agent scope, by design: the row exists because a preset cannot mount the prompt registry itself.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-persona) — every accepted config field and its source declaration.
