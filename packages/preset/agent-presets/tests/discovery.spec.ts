...
import { COMPOSITION_FILE, discoverPresets, scanRoot } from '@worldapptechnologies/nulu-agent-presets'
...
describe('display order', () => {
  it('puts declared order first, then everything else by id', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-order-'))
    roots.push(root)
  ...
  it('breaks a tie between equal declared orders by id', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-order-tie-'))
    roots.push(root)
...
describe('preset discovery', () => {
  ...
  it('skips a directory whose name no preset id could ever claim', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-oddname-'))
    roots.push(root)
  ...
  it('ignores a plain file sitting beside the preset directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-'))
    roots.push(root)
  ...
  it('expands a leading tilde in a root path', async () => {
    ...
    const found = await scanRoot({ path: '~/.nulu-agent-presets-absent', trust: 'user' }, HARNESS)
...
describe('composition health', () => {
  ...
  async function scanned(composition: string): Promise<string | undefined> {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-health-'))
    roots.push(root)
  ...
  it('reports a composition that stats but cannot be read', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-unreadable-'))
    roots.push(root)
...
describe('rows naming a plugin that cannot be resolved', () => {
  /** One directory under a fresh root holding `composition`, scanned. */
  async function scanned(composition: string): Promise<string | undefined> {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-resolve-'))
    roots.push(root)
    await mkdir(join(root, 'probe'))
    await writeFile(join(root, 'probe', COMPOSITION_FILE), composition)
    const [preset] = await scanRoot({ path: root, trust: 'user' }, HARNESS)
  ...
  it('reports a package the harness cannot resolve, with the row and the name', async () => {
    ...
    // compose a session.
    expect(await scanned('- id: stale\n  name: \'@worldapptechnologies/nulu-no-such-package\'\n'))
      .toBe('row "stale" names a plugin that cannot be resolved: @worldapptechnologies/nulu-no-such-package')
  })
  ...
  it('resolves a preset-relative row against the preset\'s own directory', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-presets-relative-'))
    roots.push(root)
    await mkdir(join(root, 'probe'))
  ...
  it('reports a file: URL whose target is not there', async () => {
    ...
    const missing = pathToFileURL(join(tmpdir(), 'nulu-presets-absent', 'nope.mjs')).href
  ...
  it('reads an installed package off disk without asking the resolver', async () => {
    ...
    const home = await mkdtemp(join(tmpdir(), 'nulu-presets-installed-'))
    roots.push(home)
  ...
  it('reports a package whose install link dangles', async () => {
    ...
    const home = await mkdtemp(join(tmpdir(), 'nulu-presets-dangling-'))
    roots.push(home)
