...
import AgentPresets, { COMPOSITION_FILE, SETTINGS_NAMESPACE } from '@worldapptechnologies/nulu-agent-presets'
...
async function harness(
  ...
  const home = await mkdtemp(join(tmpdir(), 'nulu-preset-settings-'))
  roots.push(home)
...
describe('the default preset as a user setting', () => {
  ...
  it('clears a user default it has just deleted', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-preset-authored-'))
    roots.push(root)
