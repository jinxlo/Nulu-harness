 ...
 * @module @worldapptechnologies/nulu-agent-presets
 */
...
import type SettingsService from '@worldapptechnologies/nulu-settings'
import type { SettingsScope } from '@worldapptechnologies/nulu-settings'
...
export class AgentPresets extends TypertRemoteService {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = [
      ...
      ...config.roots,
      ...config.includeUserRoot ? [{ path: nuluHomePath(USER_PRESET_DIR), trust: 'user' } satisfies PresetRoot] : [],
    ]
   ...
   * scope key. The binding is nulu-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `nulu-skill-filesystem` watches its
