...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-tool-ralph)是每个受支持字段的穷尽式真源。配置的提供方必须存在、支持结构化输出，并报告 `inheritsParentContext: false`；针对违反此要求的提供方发起调用时，会在任何 Round 开始前直接报错。
...
已生成的 [`ralph` schema](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-ralph) 公开一个必填 `objective` 字符串与一个可选 `maxRounds` 数字。提供方选择、交接大小、报告 schema、工作流脚本与编排行为均由部署侧控制，不在调用 schema 中。
