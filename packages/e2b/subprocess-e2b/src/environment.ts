...
import { e2bControlEnvs } from '@worldapptechnologies/nulu-e2b'
import type { Sandbox } from '@worldapptechnologies/nulu-e2b'
import { SENSITIVE_ENV_PATTERN } from '@worldapptechnologies/nulu-subprocess'
...
export async function readRemoteEnvironment(sandbox: Sandbox, signal?: AbortSignal): Promise<string> {
  ...
  const result = await sandbox.commands.run(
    'set -o pipefail; nulu_e2b_passwd="$(getent passwd "$(id -u)")"; IFS=: read -r _ _ _ _ _ nulu_e2b_home _ <<<"$nulu_e2b_passwd"; test -n "$nulu_e2b_home" -a -d "$nulu_e2b_home"; printf \'%s\' "$nulu_e2b_home" | base64 -w 0; printf \'\\n\'; env -0 | base64 -w 0',
...
export function scrubRemoteEnvironment(raw: string): Map<string, string> {
  ...
  for (const [name, value] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('NULU_') || SENSITIVE_ENV_PATTERN.test(name)) continue
...
export function bootstrapEnvironment(raw: string): Record<string, string> {
  ...
  for (const [name] of remoteEnvironmentEntries(raw)) {
    if (name.startsWith('NULU_') || SENSITIVE_ENV_PATTERN.test(name)) environment[name] = ''
  }
  return environment
}
