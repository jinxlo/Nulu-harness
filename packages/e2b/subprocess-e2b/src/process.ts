...
import type { CommandHandle, CommandResult, Sandbox } from '@worldapptechnologies/nulu-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$nulu_e2b_env_bin" -i "$nulu_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$nulu_e2b_tee" --output-error=warn-nopipe >("$nulu_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$nulu_e2b_tee" --output-error=warn-nopipe >("$nulu_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'nulu_e2b_rm=$7',
    'shift 7',
    'nulu_e2b_pgid="$("$nulu_e2b_ps" -o pgid= -p "$$" | "$nulu_e2b_tr" -d " ")"',
    `printf '%s\\n' "$nulu_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t nulu_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$nulu_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$nulu_e2b_env_bin" -i -- "\${nulu_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'nulu_e2b_status=$?',
    `printf '%s\\n' "$nulu_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$nulu_e2b_status"',
  ].join('\n')
  const argv = spec.argv.map(quoteE2BShellArg).join(' ')
  const bootstrap = [
    `mapfile -d '' -t nulu_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'nulu_e2b_env_bin="$(command -v env)"',
    'nulu_e2b_setsid="$(command -v setsid)"',
    'nulu_e2b_bash="$(command -v bash)"',
    'nulu_e2b_node="$(command -v node)"',
    'nulu_e2b_ps="$(command -v ps)"',
    'nulu_e2b_tr="$(command -v tr)"',
    'nulu_e2b_tee="$(command -v tee)"',
    'nulu_e2b_head="$(command -v head)"',
    'nulu_e2b_rm="$(command -v rm)"',
    'for nulu_e2b_tool in "$nulu_e2b_env_bin" "$nulu_e2b_setsid" "$nulu_e2b_bash" "$nulu_e2b_node" "$nulu_e2b_ps" "$nulu_e2b_tr" "$nulu_e2b_tee" "$nulu_e2b_head" "$nulu_e2b_rm"; do',
    '  [[ "$nulu_e2b_tool" == /* && -x "$nulu_e2b_tool" ]] || exit 125',
    'done',
    `exec "$nulu_e2b_env_bin" -i -- "\${nulu_e2b_env[@]}" "$nulu_e2b_setsid" --wait -- "$nulu_e2b_bash" -c ${quoteE2BShellArg(inner)} nulu-e2b "$nulu_e2b_env_bin" "$nulu_e2b_node" "$nulu_e2b_ps" "$nulu_e2b_tr" "$nulu_e2b_tee" "$nulu_e2b_head" "$nulu_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
