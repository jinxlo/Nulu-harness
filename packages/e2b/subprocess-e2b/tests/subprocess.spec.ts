...
import type E2BRuntime from '@worldapptechnologies/nulu-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0NULU_STALE=old\0BROKEN\0=bad\0'
...
function runtime(fake: FakeSandbox, getSandbox: () => Promise<Sandbox> = async () => fake.sandbox): E2BRuntime {
  return {
    cwd: '/workspace',
    runtimeRoot: '/workspace/.nulu-e2b',
    getSandbox,
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    const handle = testHandle(runtime(fake), spec({
      ...
      env: {
        ...
        WORLD_APP_TECHNOLOGIES_API_KEY: 'explicit-secret',
        NULU_MODE: 'test',
        // The seam's tombstone: an explicit undefined removes the ambient entry.
        KEEP: undefined,
      },
    }), '/workspace/.nulu-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.nulu-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$nulu_e2b_env_bin" -i'))!
    expect(command).toContain('"$nulu_e2b_setsid" --wait -- "$nulu_e2b_bash" -c')
    expect(command).not.toContain('WORLD_APP_TECHNOLOGIES_API_KEY')
    expect(command).not.toContain('NULU_MODE')
    ...
    expect(command).not.toContain('${!nulu_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$nulu_e2b_home" -a -d "$nulu_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('nulu_e2b_node="$(command -v node)"')
    expect(command).toContain('"$nulu_e2b_env_bin" -i "$nulu_e2b_node" -e')
    expect(command).toContain('"$nulu_e2b_env_bin" -i -- "${nulu_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$nulu_e2b_env_bin" -i -- "${nulu_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.nulu-e2b/processes/one/pid',
      '/workspace/.nulu-e2b/processes/one/exit-code',
      '/workspace/.nulu-e2b/processes/one/environment',
      '/workspace/.nulu-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.nulu-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0WORLD_APP_TECHNOLOGIES_API_KEY=explicit-secret\0NULU_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.nulu-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('nulu_e2b_tee='))!
    expect(command).toContain('"$nulu_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$nulu_e2b_tee" --output-error=warn-nopipe')
