...
import type { SubprocessTerminalSpawnSpec } from '@worldapptechnologies/nulu-subprocess'
...
class FakeTerminalSandbox {
  ...
  ambient = 'KEEP=visible\0UNICODE=你好\0NPM_TOKEN=secret\0NULU_STALE=old\0BROKEN\0=bad\0'
  sessionId = '123\n'
  foreground = '456\n'
  groups = [123]
  zombieGroups: number[] = []
...
function spec(overrides: Partial<SubprocessTerminalSpawnSpec> = {}): SubprocessTerminalSpawnSpec {
  return {
    ...
    cols: 80,
    graceMs: 5,
    env: { TERM: 'dumb', NULU_SESSION_ID: 'owner', TOKEN_EXPLICIT: 'kept' },
    ...overrides,
  }
}
...
describe('E2B terminal allocation', () => {
  it('hides bootstrap-shell bytes and preserves requested-shell bytes across the output boundary', async () => {
    ...
    expect(fake.writes.get('/runtime/terminal-one/environment')).not.toContain('NULU_STALE')
    ...
    expect(marker).toMatch(/^nulu-e2b-bootstrap:/)
    expect(fake.inputs[0]?.data.toString()).not.toContain(marker)
    const runner = fake.writes.get('/runtime/terminal-one/runner.bash') ?? ''
    expect(runner).toContain('if (( ${#nulu_argv[@]} == 0 )); then')
    expect(runner).toContain('printf \'%s\' "$nulu_output_marker"')
    expect(runner).toContain('exec env -i -- "${nulu_env[@]}" "${nulu_argv[@]}"')
  ...
  it('inherits only safe ambient values and limits the allocation signal to setup', async () => {
    ...
    expect(environment).not.toContain('NULU_STALE')
...
describe('E2B subprocess terminal service', () => {
  ...
  it('resolves remote executables', async () => {
    ...
    expect(commandOptions?.envs?.HOME).toMatch(/^\/\.nulu-e2b-control-/)
