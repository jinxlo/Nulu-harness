...
`nulu-e2b` 让 agent（智能体）的文件操作、shell 命令与终端在一个共享的远程 Linux 沙箱内运行，而不是在你的机器上。应用启动时会创建沙箱，并在配置的生命周期到期或应用关闭时删除它，因此其中保存的一切都是短暂的。请配置 API 密钥、绝对远程工作目录与沙箱生命周期。请与 `nulu-fs-e2b`、`nulu-subprocess-e2b` 一起使用；单独使用它不会带来任何用户可见的能力。它不会向模型发送任何内容，而且任何已发布的组合都不会默认启用 E2B。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-e2b)完整列出了每个受支持字段及其 JSDoc，是这些信息的真源。
...
- [生成配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-e2b)——每个受支持配置字段及其源声明。
