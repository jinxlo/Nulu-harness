...
describe('e2b control-plane URL', () => {
  ...
  it('keeps the loopback debug plane direct instead of sending its API key to a proxy', async () => {
    const { e2bApiUrl } = await import('../src/api-url.ts')
    const { proxyRouteFor } = await import('@worldapptechnologies/nulu-http-proxy')
    const { createLaunchEnvironmentSnapshot } = await import('@worldapptechnologies/nulu-launch-environment')
