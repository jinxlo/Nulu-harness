...
import { boot } from '@worldapptechnologies/nulu-app-boot'
import { type Agent } from '@worldapptechnologies/nulu-agent'
import { Session, SessionId } from '@worldapptechnologies/nulu-session'
import type {} from '@worldapptechnologies/nulu-fs-e2b'
import type {} from '@worldapptechnologies/nulu-bash-local'
import type {} from '@worldapptechnologies/nulu-lsp-stdio'
import type {} from '@worldapptechnologies/nulu-terminal-bash'
...
try {
  ...
  const outputDrainHandle = ctx.subprocess.spawn({
    argv: ['bash', '-c', "bash -c 'exec -a nulu-output-drain-descendant sleep 30' & printf 'leader-done\\n'"],
    cwd: process.cwd(),
  ...
  const outputDrainClean = !outputDrainProcesses.some(processInfo =>
    JSON.stringify([processInfo.cmd, processInfo.args]).includes('nulu-output-drain-descendant'),
  )
  ...
  const sleeping = ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "printf 'NULU_SLEEP_%s\\n' READY; sleep 30",
    submit: true,
  })
  let sleepReadyOutput = ''
  const sleepReadyDeadline = Date.now() + 5_000
  while (!sleepReadyOutput.includes('NULU_SLEEP_READY\n')) {
    sleepReadyOutput += sleeping.readOutput().delta
    if (sleepReadyOutput.includes('NULU_SLEEP_READY\n')) break
  ...
  const stubborn = await ctx.terminals.startSend(owner, terminal.sessionId, {
    text: "bash -c 'trap \"\" TERM; exec sleep 30' & printf 'NULU_STUBBORN_PID=%s\\n' \"$!\"",
    submit: true,
  }).done
  const stubbornMatch = /NULU_STUBBORN_PID=([1-9][0-9]*)/.exec(stubborn.viewport)
