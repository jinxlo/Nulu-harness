...
import { FsTargetKey, FsVersion } from '@worldapptechnologies/nulu-fs'
import E2BFileSystem from '@worldapptechnologies/nulu-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.nulu-e2b-control-/)
...
describe('E2BFileSystem identity, metadata, and reads', () => {
  ...
  it('projects canonical process paths, file URLs, and containment', async () => {
    ...
    expect(fs.processPathFromHostPath('/Users/alice/.nulu/attachments/object')).toBeUndefined()
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['nulu-version']).toBeDefined()
