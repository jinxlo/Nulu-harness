...
import { isUserInvocable } from '@worldapptechnologies/nulu-skill'
...
export class SessionSkillCatalog extends TypertRemoteService {
  ...
  async list(request: SkillListRequest, signal: AbortSignal): Promise<SkillListValue> {
    ...
    if (skillRegistry === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @worldapptechnologies/nulu-skill',
        {},
      )
    }
