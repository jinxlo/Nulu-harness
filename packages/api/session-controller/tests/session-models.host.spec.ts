...
import LlmRuntime, { LlmAdapter, ReasoningEffortId } from '@worldapptechnologies/nulu-llm'
import type {
...
async function harness(logged?: {
  ...
  ctx.llm.registerAdapter(['worldapp-gateway'], new CatalogAdapter('Nulu', [
    { provider: 'worldapp-gateway', id: 'nulu-5', name: 'Nulu Chat' },
    { provider: 'worldapp-gateway', id: 'nulu-5-ultra', name: 'Nulu Reasoner', description: 'Reasoning model' },
  ], REASONING))
...
describe('Web session model selection', () => {
  it('validates an ordered image batch before persisting any member', async () => {
    ...
    const remote = createSessionTestRemote(ctx, {
      defaultModelSelection: () => ({ provider: 'worldapp-gateway', model: 'nulu-5' }),
      cwd: '/tmp',
    })
  ...
  it('groups successful providers and leaves an unlisted current selection out of the catalog', async () => {
    ...
    const remote = createSessionTestRemote(ctx, { defaultModelSelection: () => ({ provider: 'worldapp-gateway', model: 'nulu-5' }), cwd: '/tmp' })
    ...
    expect(catalog.groups).toEqual([{
      ...
      name: 'Nulu',
      models: [
        { id: 'nulu-5', name: 'Nulu Chat', reasoning: REASONING },
        {
          id: 'nulu-5-ultra',
          name: 'Nulu Reasoner',
  ...
  it('accepts an advisory-unlisted model, rejects an unavailable provider, and switches only after the next assembly', async () => {
    ...
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'worldapp-gateway', model: 'nulu-5' })
    ...
    expect((await ctx.systemPrompt.assemble()).variables)
      .toMatchObject({ provider: 'worldapp-gateway', model: 'private-preview' })
    await expect(agentEvents(ctx, agent).waterfall(
    ...
    expect(unsupported).toMatchObject({
      ...
      error: {
        ...
        message: 'provider "worldapp-gateway" model "private-preview" does not support reasoning effort "medium"',
      },
    })
    ...
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'worldapp-gateway', model: 'private-preview', reasoningEffort: 'max' })
    await ctx.fiber.dispose()
  })

  it('reads the Agent default live for a session whose log names no selection', async () => {
    const { ctx, sessionId } = await harness()
    let stored = { provider: 'worldapp-gateway', model: 'nulu-5' }
    createSessionTestRemote(ctx, {
    ...
    stored = { provider: 'worldapp-gateway', model: 'nulu-5-ultra' }
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'worldapp-gateway', model: 'nulu-5-ultra' })
    await ctx.fiber.dispose()
  })
  ...
  it('saves an accepted selection as the default and survives a storage failure', async () => {
    ...
    expectValue(await remote.selectModel(request({
      sessionId, provider: 'worldapp-gateway', model: 'nulu-5-ultra', reasoningEffort: 'max',
    })))
    expect(saved).toEqual([
      { provider: 'worldapp-gateway', model: 'nulu-5-ultra', reasoningEffort: 'max' },
    ])
    ...
    const stillAccepted = expectValue(await remote.selectModel(request({
      sessionId, provider: 'worldapp-gateway', model: 'nulu-5',
    })))
    expect(stillAccepted.selected).toEqual({ provider: 'worldapp-gateway', model: 'nulu-5', reasoningEffort: 'high' })
    expect(currentSelection(ctx, sessionId))
      .toEqual({ provider: 'worldapp-gateway', model: 'nulu-5', reasoningEffort: 'high' })
    await ctx.fiber.dispose()
  })
  ...
  it('refuses a prompt no adapter can route, and reports it on the directory', async () => {
    ...
    expectValue(await remote.selectModel(request({
      sessionId, provider: 'worldapp-gateway', model: 'unlisted-but-served',
    })))
    const catalog = await buildModelCatalog(ctx)
