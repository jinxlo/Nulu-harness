...
import SessionStore, { SESSION_FORMAT_VERSION, SessionLogOffset, SessionId } from '@worldapptechnologies/nulu-session'
...
describe('ApiSession create or adoption', () => {
  it('shares one in-flight creation between concurrent callers', async () => {
    const { ctx, agents } = await harness()
    const cwd = mkdtempSync(join(tmpdir(), 'nulu-session-controller-concurrent-'))
    tempDirs.push(cwd)
  ...
  it('accepts a raced ordinary creation and rejects a raced attached child', async () => {
    const ordinary = await harness()
    const cwd = mkdtempSync(join(tmpdir(), 'nulu-session-controller-create-'))
    tempDirs.push(cwd)
    ...
    const child = await harness()
    const childCwd = mkdtempSync(join(tmpdir(), 'nulu-session-controller-child-'))
  ...
  it('validates ownership and cwd on the Agent returned by creation', async () => {
    const child = await harness()
    const childCwd = mkdtempSync(join(tmpdir(), 'nulu-session-controller-returned-child-'))
    tempDirs.push(childCwd)
    const childMeta = {
    ...
    const requestedCwd = mkdtempSync(join(tmpdir(), 'nulu-session-controller-wrong-cwd-'))
  ...
  it('surfaces directory creation failure', async () => {
    const { agents } = await harness()
    const parent = mkdtempSync(join(tmpdir(), 'nulu-session-controller-file-'))
