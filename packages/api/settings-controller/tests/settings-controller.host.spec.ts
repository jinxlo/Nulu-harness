...
import type { SettingsDescriptor } from '@worldapptechnologies/nulu-settings'
import { RemoteError, remoteErrorOf, remoteMethods } from '@worldapptechnologies/nulu-typert-protocol'
  ...
  it('reports the actionable configuration error while no settings provider is mounted', async () => {
    ...
    for (const call of calls) {
      ...
      expect(remoteErrorOf(failure)).toMatchObject({
        ...
        message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @worldapptechnologies/nulu-settings-file) in its composition',
        details: {},
      })
    }
  })
