 ...
 *
 * @module @worldapptechnologies/nulu-api-settings-controller/src/credentials.ts
 */

import { Context } from '@worldapptechnologies/cordis'
import { credentialRef } from '@worldapptechnologies/nulu-credentials'
import type { CredentialProvider } from '@worldapptechnologies/nulu-credentials'
  ...
  private provider(): CredentialProvider {
    ...
    if (credentials === undefined) {
      throw new RemoteError(
        'gateway/internal',
        'credentials service is absent: this deployment does not mount a credential provider (e.g. @worldapptechnologies/nulu-credentials-local) in its composition',
        {},
      )
    }
    return credentials
