...
import { SESSION_FORMAT_VERSION, SessionId, SessionSeq } from '@worldapptechnologies/nulu-session'
...
vi.mock('@worldapptechnologies/node-addon-system/flock', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@worldapptechnologies/node-addon-system/flock')>()
  return {
...
async function freshRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'nulu-jsonl-lease-'))
  dirs.push(root)
  return root
}
