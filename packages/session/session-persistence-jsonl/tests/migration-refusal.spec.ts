...
import type { SessionFormatJsonObject } from '@worldapptechnologies/nulu-session-format'
...
const nativePrefix: readonly SessionFormatJsonObject[] = [
  ...
  { type: 'system/message', surfaceOp: 'append', data: {
    turn: 1, step: 1, message: {
      id: 'native-system', role: 'system', source: { kind: 'plugin', plugin: '@worldapptechnologies/nulu-system-prompt' },
...
const migrationRefusals = [
  ...
  }))),
  {
    ...
    tail: { type: 'session-log-gateway/delivery-accepted', data: {
      sessionId: id, throughSeq: prefix.length - 1, sessionFormatVersion: 3,
    } },
    diagnostic: '@worldapptechnologies/nulu-session-format-v2-to-v3 refuses this format v2 Session: format v2 delivery marker claims target format v3',
  },
  {
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'nulu-migration-refusal-'))
})
