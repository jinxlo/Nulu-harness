...
用于 DeepSeek 官方 LLM（大语言模型）API 请求的增量规范会话日志上传。该函数插件注入 `ctx.sessions` 与 `ctx.deepseekLlmApiExtensions`，并拥有 `nulu_session_log` 请求字段以及用于派生接受水位的持久 `session-log-deepseek/delivery-accepted` 事件。仅当官方 API 需要接收会话日志后缀时才启用它。
...
| 配置键 | 默认值 | 含义 |
|---|---:|---|
| `enabled` | `false` | 注册 `nulu_session_log` 贡献。将其设为 `true` 可选择启用会话日志上传。 |
...
缺少存活会话的直接请求会省略 `nulu_session_log`。普通 agent（智能体）、压缩（compaction）与会话标题调用都会携带存活会话 id。
...
无。`nulu_session_log` 是 DeepSeek 请求中模型输入字段的同级字段，不会插入 `messages`、系统提示词或工具 schema。
