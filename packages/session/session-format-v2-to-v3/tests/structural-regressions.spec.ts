...
const carriers = [
  ...
  ['title', (value: SessionFormatJsonObject) => row('session/title-llm-request', { titleProvider: 'mock', messageSeqs: [2], route: config, system: 'title', messages: [{ ...value, source: { kind: 'plugin', plugin: 'nulu-session-title-llm' } }], maxTokens: 20 })],
...
describe('structural inherited ownership and reference regressions', () => {
  ...
  it('remaps references on both sides of a prompt insertion without reinterpreting same-Session captures or framed title input', () => {
    ...
    const titleInput = { ...message('title-input'), source: { kind: 'plugin', plugin: 'nulu-session-title-llm' }, content: [{ type: 'text', text: 'Generate the session title from this JSON array of human messages:\n[{"seq":2,"text":"a"},{"seq":4,"text":"b"}]' }] }
    const input = dense([
