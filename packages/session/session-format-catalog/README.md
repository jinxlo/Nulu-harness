...
The catalog contains all supported historical readers directly. A profile cannot add, remove, or reorder an edge by mounting a feature plugin. Its peer dependency on `nulu-session` supplies the installed current event vocabulary and current restoration rules, while historical edge validators remain frozen.
