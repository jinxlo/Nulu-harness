...
describe('Nulu session-log acceptance invariant', () => {
  it('accepts a watermark naming an earlier event in its containing Session', async () => {
    ...
    expect(() => session.append('session-log-gateway/delivery-accepted', {
  ...
  it.each([-1, 0.5])('rejects malformed acceptance format version %s', async (sessionFormatVersion) => {
    ...
    expect(() => session.append('session-log-gateway/delivery-accepted', {
      ...
      code: 'INVARIANT',
      packageName: '@worldapptechnologies/nulu-session-log-gateway',
    }))
  })
  ...
  it('rejects negative-zero format versions restored across the owned invariant boundary', async () => {
    ...
    const session = {
      ...
      snapshotEvents: () => [{
        type: 'session-log-gateway/delivery-accepted' as const,
        seq: SessionSeq(0),
        time: 1,
  ...
  it('rejects current-generation mismatches and leaves v0 watermarks inert', async () => {
    ...
    expect(() => wrongId.append('session-log-gateway/delivery-accepted', {
    ...
    expect(() => wrongSeq.append('session-log-gateway/delivery-accepted', {
    ...
    expect(() => invalidSeq.append('session-log-gateway/delivery-accepted', {
