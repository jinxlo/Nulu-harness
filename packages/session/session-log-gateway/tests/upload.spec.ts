...
import { createAssistantMessage, createSystemMessage, createUserMessage } from '@worldapptechnologies/nulu-llm'
import type { JsonValue } from '@worldapptechnologies/nulu-util-values'
import * as SessionLogGateway from '../src/index.ts'
import type { NuluSessionLogExtension, NuluSessionLogWireEvent, NuluSessionLogWireSurfaceOp } from '../src/types.ts'
...
async function harness(
  ...
  const upload = ctx.plugin(SessionLogGateway, { enabled: true })
  await upload
...
describe('incremental Nulu session-log upload', () => {
  it('publishes raw numeric sequence fields on its external wire DTO', () => {
    expectTypeOf<NuluSessionLogExtension['sessionFormatVersion']>().toEqualTypeOf<number>()
    expectTypeOf<NuluSessionLogExtension['afterSeq']>().toEqualTypeOf<number>()
    expectTypeOf<NuluSessionLogExtension['throughSeq']>().toEqualTypeOf<number>()
    expectTypeOf<NuluSessionLogExtension['events'][number]['seq']>().toEqualTypeOf<number>()
    expectTypeOf<NuluSessionLogExtension['events'][number]['data']>().toEqualTypeOf<JsonValue>()
    expectTypeOf<NuluSessionLogExtension['session']['seedLength']>()
      .toEqualTypeOf<number | undefined>()
  })

  it('requires surface placement and restricts sources to non-assistant surface wire events', () => {
    type System = Extract<NuluSessionLogWireEvent, { type: 'system/message' }>
    type Assistant = Extract<NuluSessionLogWireEvent, { type: 'assistant/message' }>
    type User = Extract<NuluSessionLogWireEvent, { type: 'user/message' }>
    type Tool = Extract<NuluSessionLogWireEvent, { type: 'tool/result' }>
    type LogOnly = Extract<NuluSessionLogWireEvent, { type: 'turn/start' }>
    type Replace = Exclude<NuluSessionLogWireSurfaceOp, 'append'>
    expectTypeOf<System['surfaceOp']>().toEqualTypeOf<NuluSessionLogWireSurfaceOp>()
    expectTypeOf<System['sourceEventSeqs']>().toEqualTypeOf<readonly number[] | undefined>()
    expectTypeOf<Assistant['surfaceOp']>().toEqualTypeOf<NuluSessionLogWireSurfaceOp>()
    expectTypeOf<User['surfaceOp']>().toEqualTypeOf<NuluSessionLogWireSurfaceOp>()
    expectTypeOf<Tool['surfaceOp']>().toEqualTypeOf<NuluSessionLogWireSurfaceOp>()
  ...
  it('uploads assistant provenance only through its embedded stream', async () => {
    ...
    const prepared = await ctx.nuluLlmApiExtensions.prepare({
      body: body(), signal: SIGNAL, sessionId: session.id,
    })
    expect(prepared.fields.nulu_session_log?.events).toEqual([{
      type: assistant.type,
  ...
  it('uploads system append and replacement placement with unchanged data and provenance', async () => {
    ...
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(prepared.fields.nulu_session_log?.events).toEqual(session.snapshotEvents())
  })
  ...
  it.each(['extension/event', 'tool/code-dispatch', 'tool/code-dispatch-start'])('uploads opaque ignorable %s without interpreting its metadata', async (type) => {
    for (const metadata of [
      ...
      const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
      expect(prepared.fields.nulu_session_log?.events[0]).toStrictEqual(event)
  ...
  it('does not contribute the session log under its default configuration', async () => {
    ...
    await ctx.plugin(SessionStore)
    await ctx.plugin(NuluLlmApiExtensionRegistry)
    await ctx.plugin(SessionLogGateway)
    ...
    expect(prepared.fields).not.toHaveProperty('nulu_session_log')
  })
  ...
  it('uploads the full first prefix, records acceptance, then sends only the appended suffix', async () => {
    ...
    const first = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    const firstPayload = first.fields.nulu_session_log
    ...
    const second = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(second.fields.nulu_session_log).toMatchObject({ afterSeq: 1, throughSeq: 3 })
    expect(second.fields.nulu_session_log?.events).toHaveLength(2)
    expect(second.fields.nulu_session_log?.events[0]).toMatchObject({
  ...
  it('reconstructs a persisted cursor and ignores an inherited parent watermark in a fork', async () => {
    ...
    const prepared = await first.ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: first.session.id })
    await prepared.accept()
    const seed = first.session.snapshotEvents()

    const resumed = await harness('parent', seed)
    expect(SessionLogGateway.acceptedThrough(resumed.session)).toBe(0)
    const resumedPayload = await resumed.ctx.nuluLlmApiExtensions.prepare({
      body: body(), signal: SIGNAL, sessionId: resumed.session.id,
    })
    expect(resumedPayload.fields.nulu_session_log?.afterSeq).toBe(0)

    const fork = await harness('child', seed, {
      inheritedEventCount: SessionLogOffset(seed.length),
      meta: { parentSession: first.session.id, isSeeded: true },
    })
    expect(SessionLogGateway.acceptedThrough(fork.session)).toBe(-1)
    const forkPayload = await fork.ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: fork.session.id })
    expect(forkPayload.fields.nulu_session_log).toMatchObject({ afterSeq: -1, throughSeq: fork.session.seq - 1 })
  })
  ...
  it('takes the maximum watermark when concurrent acceptances settle out of order', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const earlier = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    session.append('step/start', { turn: 1, step: 1 })
    const later = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
  ...
  it('folds only events appended after the cached acceptance scan', () => {
    ...
    expect(SessionLogGateway.acceptedThrough(session)).toBe(2)
    expect(reads).toBe(2)
  })
  ...
  it('rejects a missing event below the captured Session length', () => {
    ...
    expect(() => SessionLogGateway.acceptedThrough(session))
      .toThrow('session-log-gateway: missing event 0 below captured length 1')
  })
  ...
  it('ignores another format generation before interpreting its frozen sequence', () => {
    ...
    expect(SessionLogGateway.acceptedThrough(migrated)).toBe(2)
  })
  ...
  it.each([-1, -0, 0.5])('rejects malformed acceptance format version %s', (sessionFormatVersion) => {
    ...
    expect(() => SessionLogGateway.acceptedThrough(session)).toThrow(/malformed acceptance format version/)
  })
  ...
  it('omits the field for direct or stale requests and uploads the prior acceptance marker next', async () => {
    ...
    await expect(ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL }))
      .resolves.toMatchObject({ fields: {} })
    await expect(ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: 'missing' }))
      .resolves.toMatchObject({ fields: {} })
    await expect(ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id }))
      .resolves.toMatchObject({ fields: {} })
    session.append('turn/start', { turn: 1 })
    const first = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    await first.accept()
    const current = await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })
    expect(current.fields.nulu_session_log).toMatchObject({
      afterSeq: 0,
      throughSeq: 1,
      events: [{ type: 'session-log-gateway/delivery-accepted' }],
    })
  })
  ...
  it('contributes complete events without reading request messages', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: {}, signal: SIGNAL, sessionId: session.id })
    expect(prepared.fields.nulu_session_log?.events).toEqual(session.snapshotEvents())
  })
  ...
  it('translates logical brands and isSeeded into the raw upload DTO', async () => {
    ...
    const wire = JSON.parse(JSON.stringify(prepared.fields.nulu_session_log)) as Record<string, unknown>
  ...
  it('translates ignorable and surface event envelopes to raw wire values', async () => {
    ...
    const events = prepared.fields.nulu_session_log?.events ?? []
  ...
  it('fails closed on a malformed persisted acceptance watermark', async () => {
    ...
    expect(() => SessionLogGateway.acceptedThrough(session)).toThrow(/malformed acceptance watermark/)
  })
  ...
  it('withdraws its request field when the contributing plugin reloads', async () => {
    ...
    session.append('turn/start', { turn: 1 })
    expect((await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .toHaveProperty('nulu_session_log')
    await disposeUpload()
    expect((await ctx.nuluLlmApiExtensions.prepare({ body: body(), signal: SIGNAL, sessionId: session.id })).fields)
      .not.toHaveProperty('nulu_session_log')
  })
})
