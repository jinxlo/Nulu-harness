...
import NuluLlmApiExtensions from '@worldapptechnologies/nulu-llm-api-extensions'
...
import type { NuluSessionLogExtension } from '../src/types.ts'
...
it('uploads freeform feedback and message put/edit/delete through the unchanged provider route', async () => {
  ...
  vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'feedback-test-key')
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@worldapptechnologies/nulu-llm-api-extensions', NuluLlmApiExtensions],
  ...
  try {
    ...
    const assistant = createAssistantMessage({ content: [{ type: 'text', text: 'Answer' }], source: { provider: 'worldapp-gateway', model: 'nulu-5' } })
    ...
    const request = async () => {
      const chunks = []
      for await (const chunk of ctx!.llm.stream({ provider: 'worldapp-gateway', model: 'nulu-5', sessionId: session.id, messages: session.deriveMessages() })) chunks.push(chunk)
      return chunks.at(-1)
    }
    ...
    const first = (server.requests[0]!.body as { nulu_session_log: NuluSessionLogExtension }).nulu_session_log
    const retry = (server.requests[1]!.body as { nulu_session_log: NuluSessionLogExtension }).nulu_session_log
    ...
    expect(SessionLogGateway.acceptedThrough(session)).toBe(first.throughSeq)
    ...
    expect(await request()).toMatchObject({ type: 'finish', reason: { kind: 'stop' } })
    const suffix = (server.requests[2]!.body as { nulu_session_log: NuluSessionLogExtension }).nulu_session_log
    ...
    expect(SessionLogGateway.acceptedThrough(session)).toBe(suffix.throughSeq)
    ...
    for (const wire of server.requests) {
      ...
      expect(wire.body).toMatchObject({ model: 'nulu-5', messages: [
