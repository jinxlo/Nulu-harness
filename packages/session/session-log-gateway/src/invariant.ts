/** Package-owned invariants for Nulu session-log acceptance watermarks. */
...
const PACKAGE_NAME = '@worldapptechnologies/nulu-session-log-gateway'

/** Cordis companion plugin name. */
export const name = 'session-log-gateway-invariant'
/** Service required before the companion can reserve package ownership. */
export const inject = ['invariants']

/** Validate one acceptance watermark against its containing event and session. */
function validateDeliveryAccepted(session: Session, event: SessionEvent<'session-log-gateway/delivery-accepted'>, fail: InvariantFailure): void {
  ...
  if (!Number.isSafeInteger(acceptedFormatVersion)
    ...
    fail(
      'session-log-gateway/delivery-accepted sessionFormatVersion must be a non-negative safe integer'
  ...
  if (sessionId !== session.id && !inherited) {
    fail('a non-inherited session-log-gateway/delivery-accepted event must name its containing session')
  }
  let acceptedSeq: ReturnType<typeof SessionSeq>
  try {
    acceptedSeq = SessionSeq(throughSeq)
  } catch {
    fail(`session-log-gateway/delivery-accepted throughSeq must identify an earlier event, got ${throughSeq} at seq ${event.seq}`)
  }
...
function validateSession(session: Session, fail: InvariantFailure): void {
  ...
  for (const event of session.snapshotEvents()) {
    if (event.type === 'session-log-gateway/delivery-accepted') validateDeliveryAccepted(session, event, fail)
  }
}
