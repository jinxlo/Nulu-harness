/**
 * Incremental session-log contribution for official Nulu LLM API requests.
 ...
 * @module @worldapptechnologies/nulu-session-log-gateway
 */
...
import type {
  NuluSessionLogExtension,
  NuluSessionLogWireEvent,
  NuluSessionLogWireHeader,
  NuluSessionLogWireSurfaceOp,
} from './types.ts'

export type * from './types.ts'

/** Cordis plugin name. */
export const name = 'session-log-gateway'
/** Services required to resolve sessions and contribute the provider request field. */
export const inject = ['nuluLlmApiExtensions', 'sessions']
...
function wireHeader(session: Session): NuluSessionLogWireHeader {
...
function wireEvent(event: SessionEvent): NuluSessionLogWireEvent {
  const common = {
...
function wireSurfaceOp(op: SurfaceOp): NuluSessionLogWireSurfaceOp {
  return op === 'append'
    ? op
...
export function acceptedThrough(session: Session): SessionSeqCursor {
  ...
  for (let index = start; index < length; index++) {
    ...
    if (event === undefined) {
      throw new Error(`session-log-gateway: missing event ${String(index)} below captured length ${String(length)}`)
    }
    if (event.type !== 'session-log-gateway/delivery-accepted') continue
    ...
    if (!Number.isSafeInteger(acceptedFormatVersion)
      ...
      throw new Error(`session-log-gateway: malformed acceptance format version at seq ${event.seq}`)
    }
    ...
    try {
      ...
      throw new Error(`session-log-gateway: malformed acceptance watermark at seq ${event.seq}`)
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.nuluLlmApiExtensions.register('nulu_session_log', {
    prepare: (request) => {
      ...
      const value: NuluSessionLogExtension = {
        version: 1,
      ...
      return {
        value,
        accept: () => {
          session.append('session-log-gateway/delivery-accepted', {
