...
缓存通过存储栈打开自己的域，因此 base 先挂 `storage`、`storage-json`（根 `nuluHomePath('storages')`）与 `storage-domain`（`backend: json`）：
...
本插件注入 `storageDomain`、`sessionProjections` 与 `sessions`。生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-session-projection-cache)是每个受支持字段及其 JSDoc 的穷尽式真源。
