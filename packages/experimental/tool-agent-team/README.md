...
Add this package on top of `@worldapptechnologies/nulu-experimental-agent-team` when the model should run a team through tools. Once mounted, every team member — the Lead and each teammate — gets the same nine tools plus a policy paragraph that states its own role and name.
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-experimental-tool-agent-team) is the exhaustive source for every accepted field and its JSDoc.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-experimental-tool-agent-team) — every tool schema the model receives.
