...
const bootstrap: InspectorClientBootstrap = {
  ...
  protocol: 'nulu-inspector-v0-token',
  maxQueuedRecords: 16,
...
describe('experimental Inspector Client plugin', () => {
  ...
  afterEach(() => {
    ...
    delete globalThis.__NULU_INSPECTOR__
    Reflect.deleteProperty(globalThis, '__NULU_BOOT__')
  })

  it('provides ctx.inspector and sends observations after the Worker accepts the source', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__NULU_INSPECTOR__ = bootstrap
  ...
  it('rotates a copied session identity while its original page remains live', async () => {
    ...
    try {
      ...
      sessionStorage.setItem('nulu.experimental-inspector.client-source-id.v0', first.sourceId)
  ...
  it('does not report queue loss again after a replacement absorbs it', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__NULU_INSPECTOR__ = { ...bootstrap, maxQueuedRecords: 1 }
    const ctx = new Context()
    const fiber = ctx.plugin({ apply })
    await fiber.await()
    const socket = FakeWebSocket.sockets[0]!
  ...
  it('discovers and serves its built Client bundle through the source protocol', async () => {
    globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket
    globalThis.__NULU_INSPECTOR__ = bootstrap
    Reflect.set(globalThis, '__NULU_BOOT__', {
      rev: 'graph',
      entries: [{
        id: '@worldapptechnologies/nulu-experimental-inspector',
        url: '/plugins/@worldapptechnologies/nulu-experimental-inspector/client.js?rev=bundle-rev',
        rev: 'bundle-rev',
      }],
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(script?.url).toContain('/plugins/@worldapptechnologies/nulu-experimental-inspector/client.js?rev=bundle-rev')
      expect(script?.sourceMapUrl)
        .toContain('/plugins/@worldapptechnologies/nulu-experimental-inspector/client.js.map?rev=bundle-rev')
