...
describe('Cordis tree inspection', () => {
  ...
  it('projects Host and Client trees and resolves both node kinds to RemoteObjects', async (test) => {
    ...
    const stored = await cdp.call('NULUInspector.getCordisTree')
    ...
    const disconnectedTree = (await cdp.call('NULUInspector.getCordisTree')).result?.tree as {
  ...
  it('emits only node-level DOM changes for Client snapshots', async (test) => {
    ...
    const firstTree = (await cdp.call('NULUInspector.getCordisTree')).result?.tree as {
    ...
    await vi.waitFor(async () => {
      const tree = (await cdp!.call('NULUInspector.getCordisTree')).result?.tree as {
        clients: Array<{ revision: number }>
      }
  ...
  it('restores a disconnected Client tree from a new transport generation', async (test) => {
    ...
    const initialTree = (await cdp.call('NULUInspector.getCordisTree')).result?.tree as {
