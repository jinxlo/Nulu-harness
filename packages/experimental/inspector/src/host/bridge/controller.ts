...
export async function startInspector(options: InspectorOptions = {}): Promise<InspectorHandle> {
  ...
  const clientProtocol = `nulu-inspector-v${String(INSPECTOR_PROTOCOL_VERSION)}-${randomBytes(32).toString('base64url')}`
  ...
  lifecycle.markRunning((error) => {
    try {
      ...
      console.error('nulu inspector: Host source cleanup after Worker failure failed', closeError)
    }
    void fetchObserver?.stop().catch((stopError: unknown) => {
      console.error('nulu inspector: fetch cleanup after Worker failure failed', stopError)
    })
    console.error('nulu inspector: Worker stopped unexpectedly', error)
  })
