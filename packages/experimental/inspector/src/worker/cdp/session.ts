...
export class CdpSession implements NetworkSink {
  ...
  constructor(
    ...
    this.unsubscribeSources = sources.subscribeStatus(() => {
      if (this.diagnosticsEnabled) this.sendEvent('NULUInspector.sourcesChanged', { sources: this.sources.describe() })
    })
  }
  ...
  receive(value: unknown): void {
    ...
    try {
      ...
      } else if (request.method === 'NULUInspector.enable') {
        this.diagnosticsEnabled = true
        result = { sources: this.sources.describe() }
      } else if (request.method === 'NULUInspector.disable') {
        this.diagnosticsEnabled = false
        result = {}
      } else if (request.method === 'NULUInspector.getSources') {
        result = { sources: this.sources.describe() }
      } else if (request.method === 'NULUInspector.getCordisTree') {
