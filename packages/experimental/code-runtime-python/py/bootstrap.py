"""CPython bootstrap for nulu-code-runtime-python.
...
async def _run(channel: ProtocolChannel) -> None:
    ...
    # point between create_task and `await __nulu_main__`), so a body-local
    ...
    try:
        ...
        exec(code, ns)  # noqa: S102 -- defines __nulu_main__; executing model code is the point
        value = await ns["__nulu_main__"]()
...
async def _pump_replies(
    ...
    # `_run` reaches `await __nulu_main__` (whose top-level statements run first)
...
def _model_traceback(exc: BaseException, max_bytes: int) -> str:
    ...
    def emit():
        ...
        if truncated:
            yield f"[nulu-code-runtime-python] exception chain truncated at {_MAX_TRACEBACK_CHAIN} links\n"
