---
description: "CPython-subprocess code runtime: the nulu-code-runtime seam implementation for Python model code, with the fd-3 wire protocol it speaks."
...
Indirectly, through PTC mode in `nulu-tools` when an explicit source-checkout composition mounts this provider; it renders the program's completion value or failure into a retained `run_code` result, and no shipped profile mounts this private package.
...
- **The truncation-marker text and the tempdir prefix keep the pre-rename short names** — the marker `[nulu-code-runtime-python] log capture truncated at <N> bytes` and the `nulu-code-runtime-python-` tempdir prefix are byte-anchored by tests and are independent of the npm package name; promotion (dropping the `experimental-` prefix) does not rename them.
