---
description: "CPython 子进程代码 runtime：为 Python 模型代码实现 nulu-code-runtime seam，及其使用的 fd-3 wire 协议。"
...
间接地，通过 `nulu-tools` 中的 PTC mode；当显式的源码 checkout 组合挂载本提供方时，它会把程序的完成值或失败渲染成保留的 `run_code` 结果，且已发布 profile 均不挂载这个私有包。
...
- **截断标记文本与临时目录前缀保留改名前的短名**——标记 `[nulu-code-runtime-python] log capture truncated at <N> bytes` 与 `nulu-code-runtime-python-` 临时目录前缀被测试逐字节锚定，且独立于 npm 包名；promotion（去掉 `experimental-` 前缀）不会重命名它们。
