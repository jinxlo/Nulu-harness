 ...
 * @module @worldapptechnologies/nulu-experimental-code-runtime-python
 */
...
import { CodeRuntime, DUNDER_MEMBER, PORTABLE_RESERVED_WORDS, RESERVED_BINDING_GLOBALS, RESERVED_ERROR_MEMBERS } from '@worldapptechnologies/nulu-code-runtime'
import type { CodeBindingErrorClass, CodeBindingFunction, CodeJsonValue, CodeRunFailure, CodeRunRequest, CodeRunResult } from '@worldapptechnologies/nulu-code-runtime'
...
function materializePyScripts(): string {
  const dir = mkdtempSync(join(tmpdir(), 'nulu-code-runtime-python-'))
 ...
 * `[nulu-code-runtime-python] log capture truncated at <N> bytes` — 51 fixed
 * characters (the bracketed prefix `[nulu-code-runtime-python] log capture
...
/**
 * Fixed host-heap headroom reserved for the application itself (the nulu
...
function validatePythonBin(bin: string): void {
  ...
  try {
    ...
    throw new Error(`nulu-code-runtime-python: config.pythonBin ${JSON.stringify(bin)} failed the CPython version probe: ${messageOf(error)}`)
  }
  const match = /^(\S+) (\d+) (\d+) (\d+)$/.exec(output)
  if (match === null) {
    throw new Error(`nulu-code-runtime-python: config.pythonBin ${JSON.stringify(bin)} did not report a CPython version`)
  }
  const [, implementation, majorText, minorText, patchText] = match
  const major = Number(majorText)
  const minor = Number(minorText)
  if (implementation !== 'cpython') {
    throw new Error(`nulu-code-runtime-python: config.pythonBin ${JSON.stringify(bin)} must be CPython, got ${implementation}`)
  }
  if (major < MIN_CPYTHON.major || (major === MIN_CPYTHON.major && minor < MIN_CPYTHON.minor)) {
    throw new Error(`nulu-code-runtime-python: config.pythonBin ${JSON.stringify(bin)} must be CPython ${MIN_CPYTHON.major}.${MIN_CPYTHON.minor} or newer, got ${implementation} ${majorText}.${minorText}.${patchText}`)
  }
}
...
export class PythonCodeRuntime extends CodeRuntime {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (process.platform === 'win32') {
      throw new Error('nulu-code-runtime-python: this backend requires a Unix platform (POSIX rlimits, fd-3 stdio, process-group signals); it cannot run on Windows')
    }
    this.config = config as ResolvedConfig
    for (const [key, value] of Object.entries(this.config)) {
      if (typeof value === 'number' && !(Number.isFinite(value) && value > 0)) {
        throw new Error(`nulu-code-runtime-python: config.${key} must be a positive number, got ${String(value)}`)
      }
    }
    ...
    if (!Number.isInteger(this.config.cpuSeconds)) {
      throw new Error(`nulu-code-runtime-python: config.cpuSeconds must be a positive integer, got ${String(this.config.cpuSeconds)}`)
    }
    ...
    if (!Number.isSafeInteger(this.config.cpuSeconds + 1)) {
      throw new Error(`nulu-code-runtime-python: config.cpuSeconds must be at most ${Number.MAX_SAFE_INTEGER - 1} (it and its +1 hard limit cross to setrlimit as exact integers), got ${String(this.config.cpuSeconds)}`)
    }
    ...
    if (!Number.isSafeInteger(this.config.addressSpaceMb * 1024 * 1024)) {
      throw new Error(`nulu-code-runtime-python: config.addressSpaceMb must be at most ${Math.floor(Number.MAX_SAFE_INTEGER / (1024 * 1024))} (its byte count crosses the wire as an exact integer), got ${String(this.config.addressSpaceMb)}`)
    }
    ...
    if (this.config.pythonBin === '' || this.config.pythonBin.includes('\0')) {
      throw new Error(`nulu-code-runtime-python: config.pythonBin must be a non-empty path without NUL bytes, got ${JSON.stringify(this.config.pythonBin)}`)
    }
    ...
    if (this.config.maxWallMs > MAX_TIMER_DELAY_MS) {
      throw new Error(`nulu-code-runtime-python: config.maxWallMs must not exceed ${MAX_TIMER_DELAY_MS} (setTimeout clamps a larger delay to 1ms), got ${String(this.config.maxWallMs)}`)
    }
    if (this.config.graceMs + CLOSE_REAP_MARGIN_MS > MAX_TIMER_DELAY_MS) {
      throw new Error(`nulu-code-runtime-python: config.graceMs must not exceed ${MAX_TIMER_DELAY_MS - CLOSE_REAP_MARGIN_MS} (its close deadline adds ${CLOSE_REAP_MARGIN_MS}ms, and setTimeout clamps a larger delay to 1ms), got ${String(this.config.graceMs)}`)
    }
    ...
    for (const key of ['maxLogBytes', 'maxValueBytes'] as const) {
      ...
      if (!Number.isInteger(this.config[key])) {
        throw new Error(`nulu-code-runtime-python: config.${key} must be a positive integer (the child reads it as an int, so a float diverges from the host), got ${String(this.config[key])}`)
      }
      ...
      if (key === 'maxLogBytes' && this.config[key] < MIN_LOG_BYTES) {
        throw new Error(`nulu-code-runtime-python: config.maxLogBytes must be at least ${MIN_LOG_BYTES} (a smaller budget cannot serialize the truncation marker itself, so a marker-only truncated run would return more than the configured cap), got ${String(this.config[key])}`)
      }
    }
    ...
    if (budgetableBytes <= 0) {
      throw new Error(`nulu-code-runtime-python: config.addressSpaceMb must exceed the ${INTERPRETER_BASELINE_BYTES}-byte interpreter baseline with room for the output budgets, so the child has address space left to build and encode them; got ${String(this.config.addressSpaceMb)} MiB (${addressSpaceBytes} bytes)`)
    }
    ...
    if (pythonBin === undefined) {
      ...
      throw new Error(`nulu-code-runtime-python: config.pythonBin ${JSON.stringify(this.config.pythonBin)} ${explicit ? 'is not an executable regular file' : 'does not resolve on PATH'}`)
    }
  ...
  async run(request: CodeRunRequest): Promise<CodeRunResult> {
    if (this.disposed) throw new Error('nulu-code-runtime-python: run() after disposal')
  ...
  private validateBindings(request: CodeRunRequest): Map<string, ValidatedNamespace> {
    ...
    const claimGlobal = (name: string, role: string): void => {
      if (RUNTIME_OWNED_GLOBALS.has(name)) {
        throw new Error(`nulu-code-runtime-python: ${role} ${JSON.stringify(name)} collides with a runtime-owned global`)
      }
      if (injectedGlobals.has(name)) {
        throw new Error(`nulu-code-runtime-python: ${role} ${JSON.stringify(name)} collides with another injected global`)
      }
    ...
    for (const namespace of request.bindings) {
      ...
      if (!IDENTIFIER.test(global) || RESERVED_NAMES.has(global)) {
        throw new Error(`nulu-code-runtime-python: binding global ${JSON.stringify(global)} is not a usable Python identifier`)
      }
      if (bindings.has(global)) {
        throw new Error(`nulu-code-runtime-python: duplicate binding global ${JSON.stringify(global)}`)
      }
      ...
      if (errorClass) {
        ...
        if (!IDENTIFIER.test(name) || RESERVED_NAMES.has(name)) {
          throw new Error(`nulu-code-runtime-python: errorClass.name ${JSON.stringify(name)} is not a usable Python identifier`)
        }
        ...
        if (memberNameProperty.length === 0) {
          throw new Error('nulu-code-runtime-python: errorClass.memberNameProperty must be a non-empty attribute name')
        }
        if (EXCEPTION_RESERVED_MEMBERS.has(memberNameProperty) || DUNDER.test(memberNameProperty)) {
          throw new Error(`nulu-code-runtime-python: errorClass.memberNameProperty ${JSON.stringify(memberNameProperty)} is a reserved error member and cannot be assigned`)
        }
  ...
  private execute(
    ...
    try {
      ...
      if (proto === null) {
        throw new Error('nulu-code-runtime-python: python subprocess spawned without a fd-3 pipe')
      }
