...
import { SESSION_FORMAT_VERSION, Session, SessionId, type SessionEvent } from '@worldapptechnologies/nulu-session'
import {
  generationLogFilename,
  scanLog,
} from '@worldapptechnologies/nulu-session-persistence-jsonl/src/format.ts'
import { foldSubagentDescriptor } from '@worldapptechnologies/nulu-subagent'
...
function readSession(id: string): ReturnType<typeof scanLog> {
  const path = `home/sessions/--nulu-workspace--/${id}/${generationLogFilename(SESSION_FORMAT_VERSION, 'none')}`
...
describe('WebWorker preview VFS example', () => {
  it('matches its deterministic source byte for byte', () => {
    ...
    const predecessors = Object.values(VFS_EXAMPLE_SESSION_IDS)
      .map(id => `home/sessions/--nulu-workspace--/${id}/session.v2.jsonl`)
