...
describe('tar codec', () => {
  ...
  it('mounts as a VFS with directories synthesized along file paths', () => {
    ...
    expect(vfs.existsSync('/nulu/config/cordis.yml')).toBe(true)
    expect(vfs.readFileSync('/nulu/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
    expect(vfs.existsSync('/nulu/config')).toBe(true)
    expect(vfs.existsSync('/nulu/workspace')).toBe(true)
    expect(vfs.existsSync('/nulu/absent')).toBe(false)
  })
  ...
  it('applies ordered data overlays without exposing runtime paths', () => {
    ...
    expect(vfs.readFileSync('/nulu/workspace/status.txt', 'utf8')).toBe('fixture')
    expect(vfs.readFileSync('/nulu/home/sessions/example/session.jsonl', 'utf8')).toBe('{}\n')
    expect(() => loadVfsOverlay(packTar({
      'config/cordis.yml': encoder.encode('replaced'),
    }), '/nulu', vfs)).toThrow(/overlay entry must stay under home\/ or workspace/)
    expect(vfs.readFileSync('/nulu/config/cordis.yml', 'utf8')).toBe('- id: subject\n')
  })
})
