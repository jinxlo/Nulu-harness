...
describe('image gzip envelope', () => {
  ...
  it('mounts an inflated image through the real VFS loader', async () => {
    const vfs = loadVfsImage(await inflateImage(gzipSync(archive()), 'the spec image'), '/nulu')
    expect(vfs.existsSync('/nulu/node_modules/@scope/pkg/lib/index.js')).toBe(true)
    expect(decoder.decode(vfs.readFileSync('/nulu/config/cordis.yml') as Uint8Array)).toBe('- id: subject\n')
  })
