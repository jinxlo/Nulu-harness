...
# `@worldapptechnologies/nulu-experimental-webworker-runtime`
...
- **PTC Node programs are unavailable**: the process shim exposes `/nulu/bin/node` as its executable identity so the provider can activate, but the Worker has neither a Node executable nor `stripTypeScriptTypes`. Program execution fails before launching a child.
