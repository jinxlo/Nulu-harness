 ...
 * @module @worldapptechnologies/nulu-experimental-webworker-runtime/src/compile/transform
 */
import { parse } from 'acorn'

const HELPER_SOURCE: Record<string, string> = {
  def: 'const __nulu$def=(t,k,get)=>Object.defineProperty(t,k,{enumerable:true,configurable:true,get});',
  default: 'const __nulu$default=(m)=>(m&&m.__esModule?m.default:m);',
  ns: 'const __nulu$ns=(m)=>(m&&m.__esModule?m:Object.assign({},m,{default:m}));',
  exportAll: 'const __nulu$exportAll=(t,m)=>{for(const k of Object.keys(m))if(k!=="default"&&!(k in t))__nulu$def(t,k,()=>m[k]);};',
  dynImport: 'const __nulu$dynImport=(s)=>Promise.resolve().then(()=>__nulu$ns(require(s)));',
}
...
class Transformer {
  ...
  private visit(node: unknown, context: {
    ...
    switch (record.type) {
      ...
      case 'MetaProperty': {
        ...
        if (meta.name === 'import') {
          this.moduleSyntax = true
          this.replace(record.start, record.end, '__nulu$meta')
        }
        break
      }
  ...
  run(): string {
    ...
    for (const { exported, local } of this.bindings) {
      prologue.push(`__nulu$def(exports,${JSON.stringify(exported)},()=>${local});`)
    }
