...
import { NULU_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return NULU_TMP
  if (absolute.startsWith('/tmp/')) return `${NULU_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `nulu-bash-sandbox`. */
