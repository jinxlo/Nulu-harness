 ...
 * @module @worldapptechnologies/nulu-experimental-webworker-runtime/src/node/globals/process
 */
...
export interface ProcessShimOptions {
  ...
  readonly cwd: string
  /** Environment the tree reads; `NULU_HOME` belongs here. */
...
export function installProcessGlobal(options: ProcessShimOptions): ProcessShim {
  ...
  const shim: ProcessShim = {
    env: { ...options.env },
    argv: [...(options.argv ?? ['node', 'nulu-webworker'])],
    execArgv: [],
    execPath: '/nulu/bin/node',
    title: 'nulu-webworker',
    platform: 'linux',
    arch: 'x64',
    pid: 1,
    version: 'v0.0.0',
