...
interface KoffiType {
  readonly __nuluKoffiType: string
...
const token = (label: string, size: number, alignment = Math.min(size, 8) || 1): KoffiType =>
  ({ __nuluKoffiType: label, size, alignment })
...
const typeOf = (target: unknown): KoffiType => {
  ...
  if (descriptor?.__nuluKoffiType === undefined) {
...
const describe = (target: unknown): string =>
  typeof target === 'string' ? target : (target as KoffiType | undefined)?.__nuluKoffiType ?? 'anonymous'
...
function array(target: unknown, length: number): KoffiType {
  ...
  return token(`array(${element.__nuluKoffiType}, ${String(length)})`, element.size * length, element.alignment)
}
