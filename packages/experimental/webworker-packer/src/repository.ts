 ...
 * assumptions about pnpm workspaces or the `nulu` CLI.
 * @module @worldapptechnologies/nulu-experimental-webworker-packer/src/repository
 */
import { execFileSync } from 'node:child_process'
import { existsSync, mkdtempSync, readFileSync, readdirSync, rmSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join, relative } from 'node:path'
import { NULU_HOME_ENV } from '@worldapptechnologies/nulu-home-paths'
...
/** Composition entry point package: the `nulu` CLI, run from source. */
const CLI_PACKAGE = 'apps/cli'

/** Composition entry point: the `nulu` CLI, run from source. */
 ...
 * layers only, so the image is the shipped profile: the machine's `$NULU_HOME`
...
export function composeProfile(repoRoot: string, profile: string): string {
  const home = mkdtempSync(join(tmpdir(), 'nulu-pack-home-'))
  try {
    return execFileSync(
      process.execPath,
      ['--import', 'tsx/esm', join(repoRoot, CLI_ENTRY), '--profile', profile, '--dump-default-config'],
      { cwd: repoRoot, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024, env: { ...process.env, [NULU_HOME_ENV]: home } },
    )
  } finally {
 ...
 * (`nulu.configTrees` in its package.json): `path` is relative to the CLI
...
export function configTrees(repoRoot: string): ConfigTree[] {
  ...
  const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as {
    nulu?: { configTrees?: unknown }
  }
  const declared = manifest.nulu?.configTrees
  if (declared === undefined) return []
  if (!Array.isArray(declared)) {
    throw new Error(`vfs image: ${CLI_PACKAGE} nulu.configTrees must be an array`)
  }
  const mounts = new Set<string>()
  return declared.map((entry, index) => {
    const tree = entry as Partial<ConfigTreeDeclaration> | null
    const at = `${CLI_PACKAGE} nulu.configTrees[${String(index)}]`
