 ...
 *
 * Usage: nulu-pack-vfs-image --out <file> [--profile web] [--root /nulu]
 *        node --import tsx/esm src/bin.ts --out ../../apps/web/dist/preview/vfs-image.tar.gz
 * @module @worldapptechnologies/nulu-experimental-webworker-packer/src/bin
 */
...
function flag(name: string, fallback?: string): string {
  ...
  if (index === -1) {
    ...
    throw new Error(`nulu-pack-vfs-image: --${name} is required`)
  }
  const value = process.argv[index + 1]
  if (value === undefined || value.startsWith('--')) {
    throw new Error(`nulu-pack-vfs-image: --${name} needs a value`)
  }
  return value
}
