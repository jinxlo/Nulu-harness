...
import SubagentService, { snapshotSubagentDescriptor } from '@worldapptechnologies/nulu-subagent'
...
for (const backend of backends) {
  describe(`${backend.name} Agent Teams recovery`, () => {
    it('reconciles a persisted child to active and a missing child to durable failed', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nulu-team-${backend.name.toLowerCase()}-`))
    ...
    it('reconciles a provisioning child whose initial prompt is durably pending', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nulu-team-pending-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('retries queued mail through cold-resume Steer after restart', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nulu-team-mail-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges target-recorded mail after restart without delivering it twice', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nulu-team-dedup-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
    ...
    it('acknowledges durably pending target mail without cold-resume duplication', {
      ...
      const storageRoot = mkdtempSync(join(tmpdir(), `nulu-team-inbox-${backend.name.toLowerCase()}-`))
      roots.push(storageRoot)
