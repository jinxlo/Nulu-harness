...
describe.skipIf(!requiredArtifacts)('Agent Teams built LIB service', () => {
  it('loads the Host service and its generated browser contribution under plain Node', async () => {
    ...
    expect(output).toEqual({
      ...
      methods: [
        '@worldapptechnologies/nulu-experimental-agent-team#agentTeams/createTask',
        '@worldapptechnologies/nulu-experimental-agent-team#agentTeams/updateTask',
        '@worldapptechnologies/nulu-experimental-agent-team#agentTeams/view',
      ],
    })
  })
})
