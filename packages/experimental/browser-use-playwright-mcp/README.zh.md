...
- name: '@worldapptechnologies/nulu-experimental-browser-use-playwright-mcp'
  config:
    mode: launch
    headless: true
```
...
[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-experimental-browser-use-playwright-mcp)列出接受的字段。浏览器模式由 profile 或 preset 选择。子进程会清空继承的 `PLAYWRIGHT_MCP_*` 选项，避免其替换该配置。
