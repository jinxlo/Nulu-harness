...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@worldapptechnologies/nulu-fs'
...
describe('nulu-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.nulu/skills'), 'same', 'user nulu skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.nulu/skills'), 'same', 'project nulu skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.nulu/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project nulu skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-nulu')
    ...
    await writeSkill(join(noGit, '.nulu/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.nulu/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.nulu/skills'), 'runtime-name', 'User loses')
  ...
  it.each([false, true])('publishes regular-file paths for linked skills while retaining their resource roots (filesystem service: %s)', async (withFileSystem) => {
    ...
    await mkdir(join(home, '.nulu/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.nulu/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.nulu/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.nulu/skills/broken-link'))
    await symlink('/dev/null', join(home, '.nulu/skills/device-link'))
    ...
    const fiber = ctx.plugin(SkillFileSystem, { nuluHome: join(home, '.nulu'), agentsHome: join(home, '.agents'), watch: false })
    await fiber
    ...
    try {
      ...
      for (const name of ['linked-dir', 'linked-flat']) {
        ...
        expect(loaded?.resourceBase).toEqual({ kind: 'directory', path: name === 'linked-dir' ? join(home, '.nulu/skills', name) : join(home, '.nulu/skills') })
        expect((await lstat(path)).isFile()).toBe(true)
      }
      await writeFile(join(external, 'replacement.md'), '---\nname: linked-flat\ndescription: Replacement\n---\n\nReplacement body.\n')
      await rm(join(home, '.nulu/skills/linked-flat.md'))
      await symlink(join(external, 'replacement.md'), join(home, '.nulu/skills/linked-flat.md'))
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.nulu/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.nulu/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousNuluHome = process.env.NULU_HOME
    const previousAgentsHome = process.env.NULU_AGENTS_HOME
    const previousBundledSkillDir = process.env.NULU_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.NULU_HOME = join(envHome, '.nulu')
      process.env.NULU_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.NULU_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.nulu/skills'), 'env-skill', 'Env skill')
      ...
      process.env.NULU_HOME = join(envHome, 'empty-nulu')
      delete process.env.NULU_BUNDLED_SKILL_DIR
      process.env.NULU_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.NULU_AGENTS_HOME
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        signal: new AbortController().signal,
        invalidate() {},
      }, { nuluHome: join(envHome, 'empty-nulu') }).name).toBe('filesystem')
    } finally {
      if (previousNuluHome === undefined) {
        delete process.env.NULU_HOME
      } else {
        process.env.NULU_HOME = previousNuluHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.NULU_AGENTS_HOME
      } else {
        process.env.NULU_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.NULU_BUNDLED_SKILL_DIR
      } else {
        process.env.NULU_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
