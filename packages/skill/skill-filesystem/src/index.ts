 ...
 *
 * @module @worldapptechnologies/nulu-skill-filesystem
 */
...
import type Schema from '@worldapptechnologies/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@worldapptechnologies/nulu-fs'
...
import {
  ...
  type SkillSource,
} from '@worldapptechnologies/nulu-skill'

const PROJECT_NULU_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_NULU_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.nuluHome = resolveDshHome(config.nuluHome)
    this.agentsHome = resolve(config.agentsHome ?? process.env.NULU_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.NULU_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.nulu/skills'), source: 'project-nulu', rank: PROJECT_NULU_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.nuluHome, 'skills'), source: 'user-nulu', rank: USER_NULU_RANK, skipSystem: true },
