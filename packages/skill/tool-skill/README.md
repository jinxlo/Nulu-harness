...
- name: '@worldapptechnologies/nulu-tool-skill'
```
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-tool-skill) is the exhaustive source for every accepted field.
...
The package is built on two ideas. First, the catalog is a durable projection, diffed by a digest over the published entries rather than the rendered prose, so the `<system-reminder>` framing can never force a republish and consumers never re-parse the `<available_skills>` block. Second, one canonical rendering serves both load paths — the tool result and the user-explicit injection — through `renderSkillContent` shared from `nulu-skill`, so the model sees the same `<skill_content>` shape regardless of who initiated the load.
...
- [Generated tool catalog](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-skill) — the exact `skill` schema the model receives.
...
The model sees the generated [`skill` schema](../../../docs/tool-catalog.md#worldapptechnologiesnulu-tool-skill).
