...
生成的[配置目录](../../../docs/config-catalog.zh.md#worldapptechnologiesnulu-tool-skill)是每个受支持字段的穷尽式真源。
...
本包建立在两个想法之上。第一，目录是一种持久投影，按已发布条目的 digest 而非渲染后的正文做差异比较，因此 `<system-reminder>` 包装永远不会强制重新发布，消费方也不需要重新解析 `<available_skills>` 块。第二，一条规范渲染服务两条加载路径——工具结果与用户显式注入——经由共享自 `nulu-skill` 的 `renderSkillContent`，因此无论加载由谁发起，模型看到的都是同一种 `<skill_content>` 形态。
...
- [生成工具目录](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-skill)——模型接收的精确 `skill` schema。
...
模型会看到生成的 [`skill` schema](../../../docs/tool-catalog.zh.md#worldapptechnologiesnulu-tool-skill)。
