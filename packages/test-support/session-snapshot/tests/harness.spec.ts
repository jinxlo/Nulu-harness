...
describe('runScenario', () => {
  ...
  it('builds nulu profile argv and rebases relative modules in live and replay patches', async () => {
    ...
    expect(await realpath(join(dir, '.nulu', 'profiles', 'acp', 'node_modules', 'example-package')))
      .toBe(await realpath(packageDir))
    expect(await realpath(join(dir, '.nulu', 'profiles', 'acp', 'node_modules', '@fixture', 'example-package')))
    ...
    const conflictLink = join(dir, '.nulu', 'profiles', 'acp', 'node_modules', 'conflict-package')
    await Promise.all([
