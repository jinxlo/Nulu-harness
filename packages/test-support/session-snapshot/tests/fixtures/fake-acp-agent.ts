/**
 * Scripted fake ACP agent bin for `nulu-session-snapshot`'s ACP adapter specs. Speaks
 * newline-delimited JSON-RPC on stdio like the real `nulu-acp-agent` bin, but
 ...
 * `behavior.json` sitting NEXT to the `$NULU_SNAPSHOT_FILE` fixture, so a spec
...
interface ScriptedLog {
  /** Path relative to `$NULU_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `NULU_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.NULU_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.NULU_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.NULU_SNAPSHOT,
      override: process.env.NULU_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.NULU_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.NULU_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.NULU_PERMISSION_MODE ?? null,
    })}`)
  }
