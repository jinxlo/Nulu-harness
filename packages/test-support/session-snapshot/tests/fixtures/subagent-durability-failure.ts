...
import type { SubagentPromptRequestId } from '@worldapptechnologies/nulu-subagent'
 ...
 *  - Under `NULU_SNAPSHOT_HUMAN_STEER`, a browser-authored prompt steers the
 ...
 *    last flush.
 *  - Under `NULU_SUBAGENT_PUBLISHED_FAILURE`, a one-shot child's first
...
export function apply(ctx: Context): void {
  ...
  const publishedFailure = process.env.NULU_SUBAGENT_PUBLISHED_FAILURE === '1'
  const humanSteer = process.env.NULU_SNAPSHOT_HUMAN_STEER === '1'
