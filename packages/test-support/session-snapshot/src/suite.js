...
import { parseSnapshotManifest, writesCurrentSessionFixtures, } from "./manifest.js";
import { redactSessionSnapshotIds } from "./identity.js";
import { captureExpectedWorkspaceSnapshot } from "./workspace.js";
import { assertSessionFixtureVersion, sessionFixtureName, sessionFixtureNames, sessionHeaderVersion, } from "./session-files.js";
import { extractSnapshotSpillPaths, normalizeSessionLog, normalizeSessionSnapshots, normalizeStdout, scrubSessionSnapshot, scrubSystemPrompts, scrubToolSchemas, tokenizeSessionFixtureCwd, } from "./normalize.js";
...
export function scenarioSkipped(scenario, recording, platform = process.platform, hasPwsh) {
    if (recording && (!scenario.recorded || scenario.sessionFormat !== undefined))
        return true;
 ...
 */
export function systemPromptPrecedesRequests(rawLog) {
 ...
 */
export function parseSystemPromptSnapshot(snapshot) {
    const parts = snapshot.split(/\n<!-- system\/message change [1-9]\d* -->\n\n/);
    return { initial: parts[0], changes: parts.slice(1) };
}
/**
 ...
 */
export async function assertSessionFixtureStorage(dir, scenarioName) {
...
export function defineAcpSnapshotSuite(options) {
    ...
    for (const scenario of scenarios) {
        ...
        for (const field of ['systemPromptSource', 'toolSchemasSource', 'expectedHeaderChanges', 'expectedPromptChanges']) {
    ...
    const sourceFor = (pinningScenario, field, label) => {
        ...
        if ((source[countField] ?? 0) !== (pinningScenario[countField] ?? 0)) {
            throw new Error(`acp-snapshot: ${pinningScenario.name} and ${sourceName} declare different ${countField} counts for shared ${label}`);
        }
        return source;
    };
            ...
            it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
                ...
                for (const [logIndex, log] of result.sessionLogs.entries()) {
                    ...
                    if (pinsPrimary) {
                        ...
                        expect(formatToolSchemasSnapshot(pinningSchemas[0], pinningSchemas.slice(1)), `session ${log.id}: changed tool schemas diverged from ${schemaSource.name}/${TOOL_SCHEMAS_SNAPSHOT}`)
