 ...
 */
import { decodeSeqRanges, } from '@worldapptechnologies/nulu-session';
...
const SNAPSHOT_SPILL_PATH_RE = new RegExp(String.raw `(?:[A-Za-z]:)?[\\/]+(?:tmp|t)[\\/]+(?:nulu-acp-snap-[0-9a-f]{9}|nulu-acp-snapshot-spill)[\\/]+session-[0-9a-f]{12}[\\/]+[0-9a-f]{12}-([A-Za-z0-9._~-]+?)`
    + String.raw `(?=\. Use read with offset/limit|[\s)"]|\\+"|$)`, 'g');
/**
