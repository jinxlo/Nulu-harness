 ...
 *
 * @module @worldapptechnologies/nulu-session-snapshot/launcher
 */
...
import { entryListSchema, type PatchOptions } from '@worldapptechnologies/cordis-plugin-include'
...
export interface AgentUnderTest {
  ...
  configPath: string
  /** Named nulu profile; omitted only for test-only fake bins with their own config grammar. */
  profile?: string
...
export function launchAcpTestAgent(options: AcpTestLaunchOptions): LaunchedAcpTestAgent {
  ...
  const launch = resolveExampleLaunch({
    ...
    configArgs: agent.profile === undefined
      ...
      : profileArgs(agent.profile, agent.configPath, selectedConfig, options.env?.NULU_SNAPSHOT, cwd),
  ...
  const clientApp = createAcpClientApp({ name: 'nulu-harness-acp-test-client' })
...
/** Build one nulu profile invocation from the base and optional scenario patches. */
function profileArgs(
  ...
  const materializedRoot = join(cwd, '.nulu-profile-patches')
 ...
 * profile fallback. This mirrors `nulu plugin` while retaining the bare entry
...
function linkProfilePackage(source: string, cwd: string, packageName: string): void {
  ...
  const link = join(cwd, '.nulu', 'profiles', 'node_modules', packageName)
