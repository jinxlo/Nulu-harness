...
import { RemoteMock, frames, ok, openStream } from '@worldapptechnologies/nulu-remote-mock'
...
const API_ROSTER = webApp.closure(['@worldapptechnologies/nulu-api-remotes'])
...
describe('remote proxies over a booted client', () => {
  ...
  it('drops the api-remotes row and answers ctx.remote.<ns>.<method> with the mock value, logging positional args', async () => {
    ...
    expect([...client.ctx.loader.entries()].map(entry => entry.options.name)).not.toContain('@worldapptechnologies/nulu-api-remotes')
