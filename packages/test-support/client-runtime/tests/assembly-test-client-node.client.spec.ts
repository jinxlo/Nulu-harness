...
import type { ClientTransportHooks } from '@worldapptechnologies/nulu-client-connection/client'
...
const TYPERT_ONLY = webApp.pick(['@worldapptechnologies/nulu-typert-registry'])
const globals = globalThis as { __NULU_TRANSPORT__?: ClientTransportHooks }
...
describe('TestClient (node environment)', () => {
  ...
  it('rethrows a row that fails to apply, releases the globals, and lets the next boot take its turn', async () => {
    ...
    await expect(TestClient.start({ roster: TYPERT_ONLY, provide: { '@worldapptechnologies/nulu-typert-registry': failing } }, RemoteMock.create()))
  ...
  it('skips readiness on request and restores a pre-existing transport on dispose', async () => {
    ...
    expect(globals.__NULU_TRANSPORT__).not.toBe(previous)
