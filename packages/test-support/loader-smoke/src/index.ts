 ...
 * zero-build dev path, resolving `@worldapptechnologies/nulu-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @worldapptechnologies/nulu-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'NULU_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.NULU_EXAMPLE_MODE`.
...
export function resolveExampleLaunch(options: ExampleLaunchOptions): ExampleLaunch {
  ...
  // A smoke launches a real `nulu` against local fixtures, so it must not inherit the machine's
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated NULU homes. */
