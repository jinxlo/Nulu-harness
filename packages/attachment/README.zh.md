...
`attachment/` 组提供持久图片附件：把图片附加到提示词和命令，harness 会把它保存到你的机器上，重新显示在对话历史中，并在后续轮次发送给模型。随附的 `nulu` 组合无需任何设置即可支持这一点。该能力与它的存储拆分为两个包，见下文。已存储的图片在重启后依然存在且永远不会被自动删除，并且只支持光栅图片格式。
...
| [`attachment-local/`](attachment-local/README.zh.md) | 把附加图片存储在本机 `NULU_HOME` 下 | 注册到 `ctx.attachments` |
...
- [生成配置目录](../../docs/config-catalog.zh.md#worldapptechnologiesnulu-attachment-local)——本地后端的每个受支持字段。
