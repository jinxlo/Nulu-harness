...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nuluHome })
      ...
      await rm(nuluHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nuluHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nuluHome })
  ...
  it('prepares every batch member before any write', async () => {
    const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nuluHome })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { nuluHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { nuluHome, maxImageBytes: 1 })
