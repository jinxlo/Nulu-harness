...
async function home(): Promise<string> {
  const nuluHome = await mkdtemp(join(tmpdir(), 'nulu-request-image-'))
  homes.push(nuluHome)
  return nuluHome
}

async function store(): Promise<LocalAttachmentStore> {
  return new LocalAttachmentStore(new Context(), { nuluHome: await home() })
}
...
describe('local request-image cache', () => {
  it('rebuilds a cleared cache without moving or losing durable attachments', async () => {
    ...
    vi.stubEnv('NULU_HOME', fallbackHome)
    try {
      const nuluHome = await home()
      ...
      const cacheRoot = join(nuluHome, 'cache')
      const path = join(cacheRoot, 'attachments', 'request-images', hash.slice(0, 2), hash)

      expect(attachments.root).toBe(join(nuluHome, 'attachments', 'v1'))
      ...
      const reopened = new LocalAttachmentStore(new Context(), { nuluHome })
  ...
  it('regenerates invalid, oversized, incompatible, or mismatched cached variants', async () => {
    ...
    const initial = await attachments.readImageRequest(attachment, policy)
    const hash = String(initial.variantId).slice('sha256:'.length)
    const path = join(nuluHome, 'cache', 'attachments', 'request-images', hash.slice(0, 2), hash)
