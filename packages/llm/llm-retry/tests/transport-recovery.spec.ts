import { createUserMessage, expandAssistantStream } from '@worldapptechnologies/nulu-llm'
...
import type { MockLlmBehavior, MockLlmServer } from '@worldapptechnologies/nulu-llm-mock-server'
...
describe('bounded retry through the real Nulu HTTP/SSE adapter', () => {
  ...
  it('stops after the configured transport retry budget is exhausted', async () => {
    ...
    if (end?.type === 'turn/end' && end.data.reason.kind === 'error') {
      expect(end.data.reason.error.message).toContain('Nulu API request to')
    }
  })
})
