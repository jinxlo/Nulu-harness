...
import NuluLlmApiExtensionRegistry from '../src/index.ts'
...
describe('NuluLlmApiExtensionRegistry', () => {
  it('prepares detached fields and accepts every provider exactly once', async () => {
    const ctx = await harness()
    const first = vi.fn()
    const second = vi.fn()
    const mutable = { value: 'original' }
    ctx.nuluLlmApiExtensions.register('test_alpha', {
      prepare: () => ({
        value: mutable,
        accept: first,
      }),
    })
    ctx.nuluLlmApiExtensions.register('test_beta', {
    ...
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: 's' })
  ...
  it('preserves the prepared result as an acceptance method receiver', async () => {
    ...
    ctx.nuluLlmApiExtensions.register('test_alpha', { prepare: () => result })

    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })
  ...
  it('rejects duplicate fields and releases ownership with the registering fiber', async () => {
    ...
    const dispose = owner.nuluLlmApiExtensions.register('test_alpha', {
      prepare: () => ({ value: { value: 'one' } }),
    })
    expect(() => ctx.nuluLlmApiExtensions.register('test_alpha', {
    ...
    await expect(ctx.nuluLlmApiExtensions.prepare({ body: {}, signal: SIGNAL }))
  ...
  it('reports a single acceptance failure verbatim and omits an undefined contribution', async () => {
    ...
    ctx.nuluLlmApiExtensions.register('test_beta', { prepare: () => undefined })
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })
  ...
  it('rejects invalid field names and preparation failures before returning fields', async () => {
    const ctx = await harness()
    expect(() => ctx.nuluLlmApiExtensions.register('' as 'test_alpha', {
      prepare: () => ({ value: { value: 'x' } }),
    })).toThrow(/non-blank trimmed/)
    ctx.nuluLlmApiExtensions.register('test_alpha', {
      prepare: () => { throw new Error('prepare failed') },
    })
    await expect(ctx.nuluLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })).rejects.toThrow('prepare failed')
  })
  ...
  it('stops waiting for a provider that ignores request cancellation', async () => {
    ...
    const pending = ctx.nuluLlmApiExtensions.prepare({
      body: {},
