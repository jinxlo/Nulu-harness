...
import type {
  NuluLlmApiExtensionMap,
  NuluLlmApiExtensionProvider,
...
interface ErasedProvider {
  prepare(request: NuluLlmApiExtensionRequest):
    | { readonly value: NuluLlmApiJson; accept?(): void | Promise<void> }
    | undefined
    | Promise<{ readonly value: NuluLlmApiJson; accept?(): void | Promise<void> } | undefined>
}

/** Recursively freeze a fresh structured clone. */
function freezeJson<T extends NuluLlmApiJson>(value: T): T {
...
async function acceptAll(callbacks: readonly (() => void | Promise<void>)[]): Promise<void> {
  ...
  if (failures.length > 1) throw new AggregateError(failures, 'Nulu LLM API extension acceptance failed')
}
...
export class NuluLlmApiExtensionRegistry extends Service {
  private readonly providers = new Map<string, ErasedProvider>()

  constructor(ctx: Context) {
    super(ctx, 'nuluLlmApiExtensions')
  }
   ...
   */
  register<K extends keyof NuluLlmApiExtensionMap>(
    field: K,
    provider: NuluLlmApiExtensionProvider<NuluLlmApiExtensionMap[K]>,
  ): () => Promise<void> {
    ...
    }, `nuluLlmApiExtensions.register(${JSON.stringify(fieldName)})`)
    return dispose
  }
  ...
  async prepare(request: NuluLlmApiExtensionRequest): Promise<PreparedNuluLlmApiExtensions> {
    ...
    const fields: Record<string, NuluLlmApiJson> = Object.create(null) as Record<string, NuluLlmApiJson>
