/** OpenAI-compatible Nulu Files API transport. @module nulu-llm-gateway/files-api */
...
export interface NuluFilePage {
  data: NuluFileObject[]
  firstId?: NuluFileIdType
  lastId?: NuluFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class NuluFilesError extends LlmError {
 ...
 */
export function isFilesQuotaError(error: unknown): error is NuluFilesError {
  return error instanceof NuluFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`Nulu Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): NuluFileObject {
...
export class NuluFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`Nulu Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new NuluFilesError(
      message ?? `Nulu Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<NuluFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('Nulu Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('Nulu file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
  ...
  async list(options: {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: NuluFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: NuluFileId(wire.last_id) } : {},
