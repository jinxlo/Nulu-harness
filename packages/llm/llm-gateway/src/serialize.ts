/**
 * Serialize harness messages into Nulu chat completions. Text-only
...
import { contentHasImage, LlmError, offloadedImageText, offloadRequestImagesWithPolicy, requestImageHandleText } from '@worldapptechnologies/nulu-llm'
...
export type ImageRequestRepresentation =
  | {
    kind: 'file'
    /** Resolve a retained request version to a reusable Nulu file id. */
    resolveFileId: (
...
/** Validate the adapter-owned effort before resolving its Nulu wire fields. */
function reasoningEffort(effort: NonNullable<GenerateOptions['reasoningEffort']>): 'off' | 'low' | 'high' | 'max' {
  ...
  throw new LlmError(
    `Nulu does not support reasoning effort "${effort}"`,
...
function resolveThinking(options: GenerateOptions, defaults: RequestDefaults): ResolvedThinking {
  ...
  if (defaults.thinking === 'disabled' && effort !== undefined && effort !== 'off') {
    throw new LlmError(
      `Nulu deployment does not support reasoning effort "${effort}"`,
      'UNSUPPORTED_REASONING_EFFORT',
    )
  }
...
function assertTextOnly(blocks: readonly ContentBlock[]): void {
  if (contentHasImage(blocks)) {
    throw new LlmError('The Nulu chat-completions adapter does not support image content.', 'UNSUPPORTED_CONTENT')
  }
}

/** Reject roles whose Nulu history format cannot carry image input. */
function assertSupportedImageRoles(messages: readonly Message[]): void {
  for (const message of messages) {
    if (message.role !== 'user' && contentHasImage(message.content)) {
      throw new LlmError(
        `The Nulu chat-completions adapter cannot represent image content in a ${message.role} message.`,
        'UNSUPPORTED_CONTENT',
      )
    }
  }
...
/** Resolve one durable image into its descriptor and transient Nulu image part. */
async function imageParts(
  ...
  if (version === undefined) {
    throw new LlmError(
      `Nulu request image ${block.attachment.attachmentId} was not prepared.`,
      'INVALID_REQUEST',
    )
  }
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'tool-result':
        ...
        break
      default:
        // Other merge-extensible blocks are not Nulu user-input vocabulary.
        break
    }
  }
  return parts
...
export function serializeMessages(messages: Message[]): WireMessage[] {
  ...
  for (const message of messages) {
    ...
    // vocabulary, but Nulu wants them as role:'tool' messages.
 ...
 * @returns ordered Nulu wire messages.
 */
 ...
 * @returns the fully materialized Nulu request body.
 */
export async function serializeRequestWithImages(
  ...
  const requestMessages = offloadRequestImagesWithPolicy(options.messages, {
    ...
    byteLength: (ref) => {
      ...
      if (version === undefined) {
        throw new LlmError(`Nulu request image ${ref.attachmentId} was not prepared.`, 'INVALID_REQUEST')
      }
