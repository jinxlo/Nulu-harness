...
import type {
  NuluLlmApiExtensionRequest,
  NuluLlmApiJson,
  PreparedNuluLlmApiExtensions,
...
import { nuluImageRequestPricing, resolveRequestImagePolicy } from './request-pricing.ts'
import { NuluFileStore } from './file-store.ts'
import type { NuluFilePolicy } from './file-store.ts'
 ...
 */
export interface NuluConnectionOptions {
  ...
  models: readonly NuluCatalogModel[]
  ...
  filePolicy: NuluFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link NuluAdapter}: the operation-local resolution hooks the plugin owns. */
export interface NuluAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => NuluConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: NuluConnectionOptions) => Promise<string>
  ...
  resolveFiles?: () => NuluFileStore
  /** Prepare the official API's plugin-contributed top-level fields for one exact wire request. */
  prepareExtensions: (request: NuluLlmApiExtensionRequest) => Promise<PreparedNuluLlmApiExtensions>
}
...
const FILES_API_TIMEOUT_CODE = 'GATEWAY_FILES_API_TIMEOUT'
...
function modelInfo(provider: string, model: NuluCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
export class NuluAdapter extends LlmAdapter {
  ...
  constructor(private readonly config: NuluAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new NuluFileStore()
  }
    ...
    while (true) {
      ...
      try {
        extensions = await this.config.prepareExtensions({
          body: body as unknown as Readonly<Record<string, NuluLlmApiJson>>,
          signal,
