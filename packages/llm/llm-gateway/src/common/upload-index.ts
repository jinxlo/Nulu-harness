/** Durable Nulu attachment-to-file-id index. @module nulu-llm-gateway/upload-index */
...
import { NuluFileId, NuluFileScope } from './file-id.ts'
import type { NuluFileId as NuluFileIdType, NuluFileScope as NuluFileScopeType } from './file-id.ts'

/** One durable remote upload mapping. Unix times are milliseconds. */
export interface NuluUploadRecord {
  scope: NuluFileScopeType
 ...
 */
export function nuluFileScope(baseURL: string, apiKey: string): NuluFileScopeType {
  const digest = createHash('sha256')
    .update(baseURL.replace(/\/+$/u, ''))
    .update('\0')
    .update(apiKey)
    .digest('hex')
  return NuluFileScope(digest)
}
...
function parseRecord(value: unknown): NuluUploadRecord {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-gateway: upload index contains a non-object record')
  }
  ...
  if (typeof record.scope !== 'string' || !/^[0-9a-f]{64}$/u.test(record.scope)
    ...
    throw new InvalidUploadIndexError('llm-gateway: upload index contains an invalid record')
  }
  return {
    scope: NuluFileScope(record.scope),
    attachmentId: record.attachmentId as AttachmentId,
    variantId: ImageVariantId(record.variantId),
    fileId: NuluFileId(record.fileId),
...
function parseIndex(text: string): StoredIndex {
  ...
  try {
    ...
    throw new InvalidUploadIndexError('llm-gateway: upload index is not valid JSON', { cause: error })
  }
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-gateway: upload index is not an object')
  }
  const index = value as { formatVersion?: unknown; records?: unknown }
  if (index.formatVersion !== 3 || !Array.isArray(index.records)) {
    throw new InvalidUploadIndexError('llm-gateway: unsupported upload index format')
  }
  const records = index.records.map(parseRecord)
  const keys = new Set<string>()
  for (const record of records) {
    const key = `${record.scope}\0${record.variantId}`
    if (keys.has(key)) throw new InvalidUploadIndexError('llm-gateway: upload index contains duplicate mappings')
    keys.add(key)
  }
  return { formatVersion: 3, records }
}

function reusable(record: NuluUploadRecord, now: number, refreshMarginMs: number): boolean {
  return record.expiresAt - now > refreshMarginMs
}

/** Atomic local index shared by every Nulu session in this NULU home. */
export class NuluUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `NULU_HOME/llm-gateway/files-v3.json`.
   */
  constructor(path = join(resolveNuluHome(), 'llm-gateway', 'files-v3.json')) {
    this.path = path
  }
  ...
  ): Promise<NuluUploadRecord | undefined> {
   ...
   * @param scope - endpoint/API-key namespace.
   */
  async clear(scope: NuluFileScopeType): Promise<void> {
    await mkdir(dirname(this.path), { recursive: true, mode: 0o700 })
    await withFileLock(this.path, async () => {
      const index = await this.load()
