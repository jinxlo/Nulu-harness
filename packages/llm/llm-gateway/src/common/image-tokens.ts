/**
 * Nulu vision-token accounting: the provider's published image-token
 * calculator (api-docs.nulu.com, Token & Token Usage) ported verbatim in
...
function safeResize(height: number, width: number, paddedHeight: number, paddedWidth: number): GridResize {
  ...
  if (solved.numTokens > MAX_IMAGE_TOKENS) {
    throw new Error(`nulu image tokens: no grid fits the token budget for ${width}x${height}`)
  }
  return solved
}
 ...
 */
export function nuluImageTokens(width: number, height: number): number {
  ...
  throw new Error(`nulu image tokens: resize did not converge for ${width}x${height}`)
}
