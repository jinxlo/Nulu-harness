/**
 * Provider-side request-image pricing for Nulu routes: reproduces the
...
import { offloadedImageText, offloadedImagePrefixCount, requestImageHandleText, textOnlyImageText } from '@worldapptechnologies/nulu-llm'
import type { ImageAttachmentAccessResolver, LlmImageRequestPrice, LlmImageRequestPricing } from '@worldapptechnologies/nulu-llm'
import { requestImageDimensions } from '@worldapptechnologies/nulu-attachment'
import type { ImageAttachmentRef, ImageRequestPolicy } from '@worldapptechnologies/nulu-attachment'
import { nuluImageTokens } from './image-tokens.ts'
...
/**
 * Resolve the request-image budgets owned by one Nulu model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: NuluCatalogModel): ImageRequestPolicy {
  ...
  return {
    priceImages: (images) => {
      ...
      return images.map((ref, index) => {
        ...
        return {
          visualTokens: nuluImageTokens(dimensions.width, dimensions.height),
