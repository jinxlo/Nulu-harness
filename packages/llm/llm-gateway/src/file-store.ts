...
/** Nulu chat accepts at most 32 MiB per image even when it is referenced by file id. */
...
export interface NuluFileConnection {
  baseURL: string
  apiKey: string
}

/** Result of one file-id resolution. */
export interface NuluFileReference {
  record: NuluUploadRecord
  uploaded: boolean
}

interface FileStoreOptions {
  index?: NuluUploadIndex
  now?: () => number
  fetch?: typeof fetch
}
...
interface SharedUpload {
  ...
  promise: Promise<NuluFileReference>
  settled: boolean
  waiters: number
}
...
function abortReason(signal: AbortSignal): Error {
  ...
  return reason instanceof Error
    ? reason
    : new Error('Nulu file upload cancelled with a non-Error reason.', { cause: reason })
}

function uploadFailure(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('Nulu file upload failed with a non-Error reason.', { cause: error })
}

function waitForUpload(operation: SharedUpload, signal: AbortSignal | undefined): Promise<NuluFileReference> {
  ...
  return new Promise<NuluFileReference>((resolve, reject) => {
    const abort = (): void => {
...
/** User-scoped durable file-id reuse for the Nulu route. */
export class NuluFileStore {
  private readonly index: NuluUploadIndex
  ...
  constructor(options: FileStoreOptions = {}) {
    this.index = options.index ?? new NuluUploadIndex()
    this.now = options.now ?? Date.now
    this.fetchImpl = options.fetch
  }

  private client(connection: NuluFileConnection): NuluFilesClient {
    return new NuluFilesClient({
  ...
  ensureUploaded(
    ...
    signal?.throwIfAborted()
    const scope = nuluFileScope(connection.baseURL, connection.apiKey)
  ...
  private async ensureUploadedOnce(
    ...
    if (version.bytes > MAX_CHAT_IMAGE_BYTES) {
      throw new LlmError('Nulu chat image exceeds the 32 MiB per-image limit.', 'INVALID_REQUEST')
    }
    const scope = nuluFileScope(connection.baseURL, connection.apiKey)
    const now = this.now()
    ...
    const upload = async (): Promise<NuluUploadRecord> => {
      ...
      if (remote.bytes !== version.data.byteLength) {
        throw new LlmError('Nulu Files API upload response does not match the submitted image.', 'INVALID_RESPONSE')
      }
      return {
        scope,
    ...
    let candidate: NuluUploadRecord
    try {
  ...
  async invalidate(
    ...
    await this.index.remove(
      nuluFileScope(connection.baseURL, connection.apiKey),
      version.variantId,
      fileId,
    )
  }
   ...
   */
  async releaseAll(connection: NuluFileConnection, signal?: AbortSignal): Promise<number> {
    let total = 0
    for (;;) {
    ...
    await this.index.clear(nuluFileScope(connection.baseURL, connection.apiKey))
    return total
  }
}
