...
describe('NuluFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    const fetchImpl = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      expect(requestUrl(url)).toBe('https://platform.worldapptechnologies.com/api/v1/files')
    ...
    const client = new NuluFilesClient({ baseURL: 'https://platform.worldapptechnologies.com/api/v1/', apiKey: 'key', fetch: fetchImpl })
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.retrieve(NuluFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
  ...
  it.each([
    ...
    await expect(client.retrieve(NuluFileId('missing'))).rejects.toMatchObject({
      name: 'NuluFilesError',
      code,
      detail: '',
    })
  ...
  it.each([
    ...
    const error = await client.retrieve(NuluFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(NuluFilesError)
    expect(error).toMatchObject({ message: 'Nulu Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(NuluFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(NuluFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(NuluFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new NuluFilesClient({ baseURL: 'https://platform.worldapptechnologies.com/api/v1///', apiKey: 'key' })
