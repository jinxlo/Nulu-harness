...
import { NuluFileStore, MAX_CHAT_IMAGE_BYTES } from '../src/file-store.ts'
...
const CONNECTION = { baseURL: 'https://platform.worldapptechnologies.com/api/v1', apiKey: 'key' }
...
describe('NuluFileStore', () => {
  it('singleflights the first upload and reuses the durable mapping across store instances', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nulu-file-store-'))
    roots.push(dir)
    const index = new NuluUploadIndex(join(dir, 'index.json'))
    const remote = uploadFetch()
    const first = new NuluFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
    ...
    const resumed = new NuluFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
  ...
  it('keeps a shared upload alive while another waiter remains', async () => {
    ...
    const store = new NuluFileStore({ index, now: () => NOW, fetch: fetchImpl })
  ...
  it('normalizes a non-Error cancellation reason', async () => {
    ...
    const store = new NuluFileStore({
      index: new NuluUploadIndex(join(dir, 'index.json')),
      now: () => NOW,
      fetch: fetchImpl,
    })
    const controller = new AbortController()
    ...
    await expect(upload).rejects.toMatchObject({
      message: 'Nulu file upload cancelled with a non-Error reason.',
      cause: 'cancelled',
    })
  })
  ...
  it('starts a fresh upload while the cancelled transport is settling', async () => {
    ...
    const fetchImpl = vi.fn((_url: string | URL | Request, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        filename: 'nulu-retry.png', purpose: 'user_data',
        expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
      }), { status: 200 }))
    }) as typeof fetch
    const store = new NuluFileStore({
  ...
  it('rejects an upload response whose byte count differs from the request version', async () => {
    ...
    const fetchImpl = vi.fn(() => Promise.resolve(new Response(JSON.stringify({
      ...
      filename: 'nulu-wrong.png', purpose: 'user_data',
      expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
  ...
  it.each([
    ...
    const remote = uploadFetch()
    const store = new NuluFileStore({
      index: new NuluUploadIndex(join(dir, `${extension}.json`)),
      now: () => NOW,
  ...
  it('normalizes a non-Error failure from the durable upload index', async () => {
    ...
    const store = new NuluFileStore({ index, now: () => NOW, fetch: vi.fn() as typeof fetch })

    await expect(store.ensureUploaded(VERSION, CONNECTION, POLICY)).rejects.toMatchObject({
      message: 'Nulu file upload failed with a non-Error reason.',
  ...
  it('reuses local expires_at above the refresh margin and uploads again at the margin', async () => {
    ...
    const store = new NuluFileStore({ index, now: () => now, fetch: remote.fetchImpl })
  ...
  it('removes a losing upload and keeps the winning durable mapping when duplicate cleanup fails', async () => {
    ...
    vi.spyOn(index, 'commit').mockResolvedValue({
      accepted: false,
      record: {
        scope: nuluFileScope(CONNECTION.baseURL, CONNECTION.apiKey),
        attachmentId: VERSION.attachment.attachmentId,
        variantId: VERSION.variantId,
        fileId: NuluFileId('file-api-winner'),
        bytes: 3,
        createdAt: NOW,
    ...
    const fetchImpl = vi.fn((input: string | URL | Request, init?: RequestInit) => {
      if (init?.method === 'POST') {
        ...
        return Promise.resolve(new Response(JSON.stringify({
          ...
          filename: 'nulu-recovered.png', purpose: 'user_data',
          expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
        }), { status: 200 }))
      }
      if (init?.method === 'DELETE') {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        data: [{
          ...
          filename: 'nulu-old.png', purpose: 'user_data',
        }],
  ...
  it('stops pagination when a page omits or repeats its cursor', async () => {
    ...
    for (const mode of ['missing', 'repeated'] as const) {
      ...
      const store = new NuluFileStore({
        index: new NuluUploadIndex(join(dir, `${mode}.json`)),
        now: () => NOW,
        fetch: fetchImpl,
      })
