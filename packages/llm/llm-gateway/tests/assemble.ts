...
import { BlockAssembler } from '@worldapptechnologies/nulu-llm'
import type { Context } from '@worldapptechnologies/cordis'
import type { FinishReason, GenerateOptions, Message, TokenUsage } from '@worldapptechnologies/nulu-llm'
...
export async function assemble(ctx: Context, options: Omit<GenerateOptions, 'provider'> & { provider?: string }): Promise<AssembledResult> {
  ...
  const request = { provider: 'worldapp-gateway', ...options }
