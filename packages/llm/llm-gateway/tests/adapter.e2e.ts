...
import LlmRuntime, { createUserMessage, ToolCallId, ReasoningEffortId, createMessage, createSystemMessage } from '@worldapptechnologies/nulu-llm'
import type { Message, ToolSchema } from '@worldapptechnologies/nulu-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@worldapptechnologies/nulu-attachment'
 ...
 * entirely without $WORLD_APP_TECHNOLOGIES_API_KEY; the pre-release vision smoke additionally
 * requires $GATEWAY_VISION_E2E=1, and the Flash image/system-update smoke
 * requires $GATEWAY_FLASH_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'nulu-5'
const VISION = 'nulu-5-vision'
const VISION_E2E_ENABLED = process.env.GATEWAY_VISION_E2E === '1'
/** A model whose endpoint reads the latest `system` message at any position; unset skips the in-history smoke. */
const IN_HISTORY_MODEL = process.env.GATEWAY_IN_HISTORY_MODEL
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'nulu-e2e-user-id-'))
  vi.stubEnv('NULU_HOME', identityHome)
})
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('llm-gateway e2e (real API)', () => {
  it.skipIf(process.env.GATEWAY_FLASH_E2E !== '1')('nulu-5 accepts images and retains system updates', async () => {
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(LlmRuntime)
  ...
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    ...
    let uploadedFile: LlmGateway.NuluFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmGateway.NuluFileId(value.id)
      }
      return response
    }
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `Nulu vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
  ...
  it(
    ...
    async () => {
      ...
      expect(
        first.finish.kind,
        `Nulu Flash tool-call turn finished as ${JSON.stringify(first.finish)}`,
      ).toBe('tool-calls')
      ...
      expect(
        second.finish.kind,
        `Nulu Flash tool-result turn finished as ${JSON.stringify(second.finish)}`,
      ).toBe('stop')
  ...
  it.skipIf(IN_HISTORY_MODEL === undefined)(
    ...
    async () => {
      ...
      await expect(ctx.llm.resolveModelInfo('worldapp-gateway', model))
