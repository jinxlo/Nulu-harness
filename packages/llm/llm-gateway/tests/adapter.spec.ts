...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'nulu-llm-gateway-'))
...
function noExtensions(): Promise<PreparedNuluLlmApiExtensions> {
  return Promise.resolve({ fields: {}, accept: () => Promise.resolve() })
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(
  config: Partial<LlmGateway.Config> & { apiKey?: string } = {},
...
function fileStoreOf(
  implementation: (...args: Parameters<LlmGateway.NuluFileStore['ensureUploaded']>) => ReturnType<LlmGateway.NuluFileStore['ensureUploaded']>,
) {
  const ensureUploaded = vi.fn(implementation)
  const invalidate = vi.fn(() => Promise.resolve())
  return {
    store: { ensureUploaded, invalidate } as unknown as LlmGateway.NuluFileStore,
    ensureUploaded,
    invalidate,
  }
}

function fileReference(fileId: string): Awaited<ReturnType<LlmGateway.NuluFileStore['ensureUploaded']>> {
  return {
    record: { fileId: LlmGateway.NuluFileId(fileId) },
    uploaded: true,
  } as Awaited<ReturnType<LlmGateway.NuluFileStore['ensureUploaded']>>
}
...
describe('NuluAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    await drain(adapter.stream({ provider: 'worldapp-gateway', model: 'm', messages: [], sessionId: SessionId('s') }))
  ...
  it('fails before fetch on extension preparation or base-field collision', async () => {
    ...
    const failed = new NuluAdapter({
      ...base,
      prepareExtensions: () => Promise.reject(new Error('metadata unavailable')),
    })
    await expect(drain(failed.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })

    const collision = new NuluAdapter({
      ...base,
      prepareExtensions: (() => Promise.resolve({ fields: { model: 'replacement' }, accept: () => Promise.resolve() })) as never,
    })
    await expect(drain(collision.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const request = { provider: 'worldapp-gateway', model: 'm', messages: [] }
  ...
  it('reports a post-2xx extension acceptance failure without relabelling it as transport', async () => {
    ...
    await expect(drain(adapter.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })))
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and Nulu request identity are independent wire facts.
  ...
  it('does not prepare a surface-offloaded image and sends its placeholder instead', async () => {
    ...
    const adapter = adapterOf({
      baseURL: server.url,
      models: [{ id: 'nulu-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
  ...
  it('projects nested tool-result images with route-owned request budgets', async () => {
    ...
    await drain(adapter.stream({ provider: 'worldapp-gateway', model: 'vision-low', messages: [nested] }))
    await drain(adapter.stream({ provider: 'worldapp-gateway', model: 'vision-custom', messages: [nested] }))
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    await drain(adapter.stream({ provider: 'worldapp-gateway', model: 'nulu-5-vision', messages }))
    await drain(adapter.stream({
      provider: 'worldapp-gateway',
      model: 'nulu-5-vision',
      messages,
  ...
  it.each(['nulu-5', 'unlisted-pass-through'])(
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'nulu-5', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'nulu-5'))
  ...
  it('parses a future Retry-After HTTP date and the Nulu request-id fallback', async () => {
    ...
    try {
      ...
      expect(result.finish).toEqual({
        ...
        failure: {
          ...
          requestId: ProviderRequestId('nulu-503'),
        },
      })
    } finally {
      dateNow.mockRestore()
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'Nulu API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^Nulu API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'Nulu API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('defaults to Messages and resolves the selected protocol endpoint without rewriting overrides', () => {
    expect(resolveAdapterOptions({})).toMatchObject({ protocol: 'messages', baseURL: 'https://api.nulu.com/anthropic' })
    expect(resolveAdapterOptions({ protocol: 'chat-completions' })).toMatchObject({ protocol: 'chat-completions', baseURL: 'https://api.nulu.com' })
    expect(resolveAdapterOptions({ protocol: 'messages' })).toMatchObject({ protocol: 'messages', baseURL: 'https://api.nulu.com/anthropic' })
  ...
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmGateway).not.toHaveProperty(helper)
  })

  it('registers the nulu provider and unregisters on dispose (HMR safety)', async () => {
    const server = await mockServer([])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('worldapp-gateway')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the nulu provider and advertises the default models', async () => {
    ...
    await expect(ctx.llm.listModels('nulu-official')).resolves.toEqual([
      ...
      { provider: 'worldapp-gateway', id: 'nulu-5-vision', name: 'Nulu 5 Vision', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'nulu-5'))
      .resolves.toMatchObject({
        provider: 'worldapp-gateway',
  ...
  it.each([
    { model: 'nulu-5', inputModalities: ['text'] },
    { model: 'nulu-5-ultra', inputModalities: ['text'] },
    { model: 'nulu-5-vision', inputModalities: ['text', 'image'] },
  ...
  it.each(['off', 'low', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off', description: 'Use for simple tasks that do not need reasoning.' },
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('worldapp-gateway', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{
          id: ReasoningEffortId('off'),
          name: 'Off',
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmGateway.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
  ...
  it('defaults an adapter-supplied catalog entry to text input', async () => {
    ...
    await expect(adapter.listModels('worldapp-gateway')).resolves.toEqual([{
      provider: 'worldapp-gateway',
      id: 'adapter-model',
      name: 'adapter-model',
      inputModalities: ['text'],
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('worldapp-gateway')).resolves.toEqual([
      { provider: 'worldapp-gateway', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'worldapp-gateway', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
        inputModalities: ['text', 'image'],
      })
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('worldapp-gateway', 'exact-override'))
  ...
  const invalidModels: Array<[LlmGateway.NuluCatalogModel[], RegExp]> = [
    ...
    [[{
      id: 'm',
      inputModalities: ['audio'] as unknown as NonNullable<LlmGateway.NuluCatalogModel['inputModalities']>,
    }], /expected "text" \| "image"/],
  ...
  const invalidProgrammaticModalities: Array<[LlmGateway.NuluCatalogModel[], RegExp]> = [
  ...
  it('surfaces a catalog model\'s in-history system prompt update mode and rejects any other mode', async () => {
    ...
    await expect(adapter.resolveModel('worldapp-gateway', 'capable'))
      .resolves.toMatchObject({ systemPromptUpdate: 'in-history' })
    await expect(adapter.resolveModel('worldapp-gateway', 'plain'))
      .resolves.not.toHaveProperty('systemPromptUpdate')
    await expect(adapter.resolveModel('worldapp-gateway', 'not-in-catalog'))
      .resolves.not.toHaveProperty('systemPromptUpdate')
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('worldapp-gateway', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('worldapp-gateway', 'uncapped'))
  ...
  it('falls back to WORLD_APP_TECHNOLOGIES_API_KEY and WORLD_APP_TECHNOLOGIES_BASE_URL env vars', async () => {
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'worldapp-gateway', name: 'Nulu' }])
    await expect(ctx.llm.listModels('worldapp-gateway')).resolves.toHaveLength(4)
    const first = await assemble(ctx, { model: 'nulu-5', messages: [] })
    expect(first.finish).toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    // The guidance leads with the managed credential store.
    const second = await assemble(ctx, { model: 'nulu-5', messages: [] })
    ...
    expect(second.finish.failure.message)
      .toMatch(/store WORLD_APP_TECHNOLOGIES_API_KEY through the credentials service.*export WORLD_APP_TECHNOLOGIES_API_KEY/s)
  })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'env-key')
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses WORLD_APP_TECHNOLOGIES_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_BASE_URL', server.url)
    vi.stubEnv('WORLD_APP_TECHNOLOGIES_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmNulu, { protocol: 'chat-completions' })
  ...
  it('takes WORLD_APP_TECHNOLOGIES_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.nulu/.env', values: { WORLD_APP_TECHNOLOGIES_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { WORLD_APP_TECHNOLOGIES_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { WORLD_APP_TECHNOLOGIES_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    const adapter = adapterOf()
    expect(adapter).toBeInstanceOf(NuluAdapter)
    ...
    await expect(adapter.listModels('worldapp-gateway')).resolves.toHaveLength(4)
  })
