...
import type { ImageAttachmentRef, ImageMediaType, RequestImageAttachment } from '@worldapptechnologies/nulu-attachment'
import { createUserMessage, ToolCallId, ReasoningEffortId, createMessage } from '@worldapptechnologies/nulu-llm'
import type { ContentBlock, GenerateOptions, Message } from '@worldapptechnologies/nulu-llm'
import {
  serializeMessages,
...
function request(overrides: Partial<GenerateOptions> = {}): GenerateOptions {
  return { provider: 'worldapp-gateway', model: 'nulu-5', messages: [], ...overrides }
}
...
describe('serializeRequest', () => {
  ...
  it('rejects an effort outside the Nulu capability', () => {
...
describe('image serialization', () => {
  ...
  it('includes provider-resolved normalized access in a retained image handle', async () => {
    ...
    version.width = 1130
    version.height = 565
    images.resolveImageAccess = () => ({ readonlyPath: '/tmp/nulu/objects/aa/object' })
    const wire = await serializeRequestWithImages(request({
      model: 'nulu-5-vision',
      messages: [createUserMessage({
        content: [{ type: 'image', attachment: ref }],
    ...
    expect(JSON.stringify(wire.messages[0])).toContain('/tmp/nulu/objects/aa/object')
  ...
  it('projects surface-offloaded occurrences to placeholders without resolving them', async () => {
    ...
    images.resolveImageAccess = ref => ref.mediaType === 'image/png'
      ? { readonlyPath: '/tmp/nulu/objects/png' }
      : undefined
    const wire = await serializeRequestWithImages(request({
      model: 'nulu-5-vision',
      messages: [createUserMessage({
    ...
    expect(wire.messages[0]).toMatchObject({
      ...
      content: [
        {
          type: 'text',
          text: expect.stringContaining(`image omitted to fit request image limits; ${png.attachmentId}. Normalized copy (read-only; may be resized or re-encoded): "/tmp/nulu/objects/png"`) as string,
        },
