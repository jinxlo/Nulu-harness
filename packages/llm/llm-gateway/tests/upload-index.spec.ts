...
import { NuluFileId } from '../src/file-id.ts'
import { nuluFileScope, NuluUploadIndex } from '../src/upload-index.ts'
...
describe('NuluUploadIndex', () => {
  it('normalizes trailing endpoint slashes in the credential scope', () => {
    expect(nuluFileScope('https://platform.worldapptechnologies.com/api/v1///', 'key'))
      .toBe(nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'key'))
  })

  it('isolates API-key namespaces and reuses only records above the refresh margin', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nulu-upload-index-'))
    roots.push(dir)
    const index = new NuluUploadIndex(join(dir, 'index.json'))
    const first = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'first-key')
    const second = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'second-key')
    const record = {
      scope: first,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: NuluFileId('file-api-one'),
      bytes: 3,
      createdAt: 1_000,
      expiresAt: 10_000,
    }
  ...
  it('keeps a reusable cross-process winner and removes only an exact generation', async () => {
    ...
    roots.push(dir)
    const index = new NuluUploadIndex(join(dir, 'index.json'))
    const scope = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'key')
    const first = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: NuluFileId('file-api-first'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
    const duplicate = { ...first, fileId: NuluFileId('file-api-duplicate') }
  ...
  it('treats a corrupt upload cache as empty and repairs it on the next commit', async () => {
    ...
    const index = new NuluUploadIndex(path)
    const scope = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'key')
    const record = {
      scope,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: NuluFileId('file-api-repaired'),
      bytes: 3,
      createdAt: 1,
      expiresAt: 10_000,
    }
  ...
  it.each([
    ...
    await expect(index.get(
      nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'key'), VARIANT, 1, 1,
  ...
  it('rejects duplicate persisted mappings as a corrupt cache', async () => {
    ...
    const record = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: NuluFileId('file-api-one'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
  ...
  it('drops expired records on commit and clears only the selected namespace', async () => {
    ...
    roots.push(dir)
    const index = new NuluUploadIndex(join(dir, 'index.json'))
    const first = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'first')
    const second = nuluFileScope('https://platform.worldapptechnologies.com/api/v1', 'second')
    const expired = {
      scope: first, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: NuluFileId('file-api-expired'), bytes: 3, createdAt: 1, expiresAt: 2,
    }
    const live = {
      ...expired, scope: second, fileId: NuluFileId('file-api-live'), expiresAt: 10_000,
    }
