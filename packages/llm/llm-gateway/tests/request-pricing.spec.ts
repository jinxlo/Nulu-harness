...
import { nuluImageRequestPricing } from '../src/request-pricing.ts'
...
describe('Nulu request-image pricing', () => {
  it('prices an uncatalogued model as its text-only substitution', () => {
    ...
    const prices = nuluImageRequestPricing(connection(), 'unlisted').priceImages([image])
  ...
  it('prices a catalogued text-only model as its text-only substitution', () => {
    ...
    const prices = nuluImageRequestPricing(options, 'text-only').priceImages([image])
    expect(prices).toEqual([{ visualTokens: 0, text: textOnlyImageText(image) }])
  })

  it('prices a retained image by its projected request dimensions plus its handle text', () => {
    const image = ref('photo', 1920, 1080)
    const prices = nuluImageRequestPricing(connection(), 'vision').priceImages([image])
  ...
  it('honors the low-detail pixel budget preset', () => {
    ...
    const prices = nuluImageRequestPricing(options, 'vision').priceImages([image])
