...
async function streamOnce(): Promise<void> {
  ...
  await ctx.plugin(LlmGateway, { baseURL: 'http://nulu-probe.invalid/v1', models: [{ id: 'm' }] })
  for await (const _chunk of ctx.llm.stream({ provider: 'worldapp-gateway', model: 'm', messages: [] })) {
