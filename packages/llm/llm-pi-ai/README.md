...
Choose this adapter when the same composition serves several providers, when a route needs pi-ai's catalog defaults with a few fields corrected, or when a hand-declared gateway must be reached through its own endpoint and protocol. Choose `nulu-llm-gateway` for the direct Nulu route when the deployment needs no other provider. Both adapters can be mounted together because their route names do not collide; registering a route another adapter already owns fails plugin loading.
...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-llm-pi-ai) is the exhaustive source for every accepted field and its JSDoc.
...
- [llm-gateway adapter](../llm-gateway/README.md) — the direct Nulu twin for the `worldapp-gateway` route.
...
- [Generated configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-llm-pi-ai) — every accepted config field and its source declaration.
