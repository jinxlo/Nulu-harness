...
import type { CredentialKey, CredentialProvider, CredentialRecord } from '@worldapptechnologies/nulu-credentials'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      ...
      + ' credential a sign-in produces; mount one (nulu-credentials-local) to sign in',
      'NO_CREDENTIAL_STORE',
    )
  }
  return credentials
