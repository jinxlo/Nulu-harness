...
import LlmRuntime, { createMessage, createUserMessage, userAgent } from '@worldapptechnologies/nulu-llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'nulu-pi-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@worldapptechnologies/nulu-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
