...
import LlmRuntime, { createUserMessage, ToolCallId } from '@worldapptechnologies/nulu-llm'
...
const openAIBaseURL = process.env.NULU_PI_AI_OPENAI_BASE_URL
const azureOpenAIKey = process.env.AZURE_OPENAI_API_KEY
// Strictly ANTHROPIC_*: the Nulu endpoint does not serve the anthropic-messages
// protocol, so falling back to WORLD_APP_TECHNOLOGIES_API_KEY turns the keyless skip into a 404.
const anthropicApiKey = process.env.ANTHROPIC_API_KEY
const anthropicBaseURL = process.env.NULU_PI_AI_ANTHROPIC_BASE_URL

const providerCases: ProviderCase[] = [
  {
    provider: 'openai',
    api: 'openai-responses',
    model: process.env.NULU_PI_AI_OPENAI_MODEL ?? 'gpt-5.5',
    ...azureOpenAIKey
  ...
  },
  {
    ...
    model: process.env.NULU_PI_AI_ANTHROPIC_MODEL ?? 'claude-opus-4-8',
