...
import { ToolCallId, createMessage, createUserMessage, offloadedImageText } from '@worldapptechnologies/nulu-llm'
...
describe('pi-ai request context conversion', () => {
  ...
  it('uses the shared normalized-path description for retained images', async () => {
    ...
    const context = await toPiContext(request([user([{ type: 'image', attachment: named }])]), imageContext(store, {
      resolveImageAccess: () => ({ readonlyPath: '/tmp/nulu/objects/aa/object' }),
    }))
    ...
    expect(JSON.stringify(context.messages[0])).toContain('/tmp/nulu/objects/aa/object')
