...
/** A context whose credential records live in a throwaway `$NULU_HOME`. */
async function stored(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'nulu-pi-auth-'))
  dirs.push(dir)
...
describe('pi-ai ambient auth context', () => {
  ...
  it('answers about the host filesystem, expanding a leading ~', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'nulu-pi-home-'))
    dirs.push(dir)
