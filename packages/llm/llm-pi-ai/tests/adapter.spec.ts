...
import { AttachmentId, AttachmentStore, ImageVariantId } from '@worldapptechnologies/nulu-attachment'
import type {
...
import LlmRuntime, { createUserMessage, CONTEXT_WINDOW_EXCEEDED_CODE, LlmError, ReasoningEffortId, userAgent } from '@worldapptechnologies/nulu-llm'
...
const HOST_IMAGE_PATH = '/host/.nulu/attachments/objects/aa/object'
const MODEL_IMAGE_PATH = '/model/.nulu/attachments/objects/aa/object'
...
async function harness(baseURL: string, overrides: Record<string, unknown> = {}): Promise<Context> {
  ...
  await ctx.plugin(LlmPiAi, {
    providers: { nulu: { apiKeyEnv: 'PI_TEST_KEY', baseURL, ...overrides } },
  })
  return ctx
}
...
describe('PiAiAdapter provider routing', () => {
  ...
  it('keeps prepared model metadata and dispatch on one profile snapshot', async () => {
    ...
    let providers: Record<string, LlmPiAi.PiAiProviderProfile> = {
      nulu: { apiKeyEnv: 'PI_TEST_KEY', baseURL: first.url },
    }
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['nulu'], new PiAiAdapter({
      profiles: () => resolveProfiles(providers),
      resolveApiKey: () => Promise.resolve('test-key'),
      auth: memoryAuth(),
    }))

    const prepared = await ctx.llm.prepareCall({ provider: 'nulu', model: 'nulu-v4-flash' })
    providers = { nulu: { apiKeyEnv: 'PI_TEST_KEY', baseURL: second.url } }
  ...
  it('forwards common stream options and profile reasoning', async () => {
    ...
    expect(server.requests[0]).not.toHaveProperty('nulu_session_log')
    expect(server.requests[0]).not.toHaveProperty('nulu_plugin_packages')
  })
  ...
  it('preserves omitted profile options when constructing the adapter directly', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['nulu'], adapterOf({
      nulu: { apiKeyEnv: 'PI_TEST_KEY', baseURL: server.url },
    }))
  ...
  it('reports unsupported stop sequences rather than silently ignoring them', async () => {
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx, { model: 'nulu-v4-flash', messages: [], stop: ['END'] })
...
describe('provider profile lifecycle', () => {
  ...
  it('exposes pi-ai model thinking levels verbatim without inventing a provider default', async () => {
    ...
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmPiAi, {
      providers: { nulu: {}, openai: {} },
    })

    await expect(ctx.llm.resolveModelInfo('nulu', 'nulu-v4-flash'))
  ...
  it('uses a supported profile reasoning value as the model default and rejects an unsupported one', async () => {
    ...
    await supported.plugin(LlmPiAi, {
      providers: { nulu: { reasoning: 'max' } },
    })
    await expect(supported.llm.resolveModelInfo('nulu', 'nulu-v4-flash'))
    ...
    await unsupported.plugin(LlmPiAi, {
      providers: { nulu: { reasoning: 'medium' } },
    })
    const described = await unsupported.llm.resolveModelInfo('nulu', 'nulu-v4-flash')
    ...
    await disabled.plugin(LlmPiAi, {
      providers: { nulu: { reasoning: 'off' } },
    })
    await expect(disabled.llm.resolveModelInfo('nulu', 'nulu-v4-flash'))
  ...
  it('fails a named-but-missing apiKeyEnv instead of using another ambient key', async () => {
    ...
    const second = await assemble(ctx, { model: 'nulu-v4-flash', messages: [] })
    expect(second.finish.kind).toBe('error')
    if (second.finish.kind !== 'error') throw new Error('expected an error finish')
    expect(second.finish.failure.message).toMatch(/provider route "nulu".*PI_CUSTOM_REF_KEY/s)
  ...
  it('rejects unsupported or unresolved image input before provider I/O', async () => {
    const adapter = adapterOf({ openai: {}, nulu: {} })
...
describe('abort wiring', () => {
  ...
  it('honors a pre-aborted caller signal', async () => {
    ...
    const controller = new AbortController()
    controller.abort('already stopped')
    const result = await assemble(ctx, { model: 'nulu-v4-flash', messages: [], signal: controller.signal })
  ...
  it('forwards an abort that arrives while provider streaming is active', async () => {
    ...
    const resultPromise = assemble(ctx, {
      model: 'nulu-v4-flash', messages: [], signal: controller.signal,
    })
  ...
  it('aborts upstream when a consumer stops early', async () => {
    const server = await mockServer([{ events: textEvents, delayMs: 30 }])
    const ctx = await harness(server.url)
    for await (const chunk of ctx.llm.stream({ provider: 'nulu', model: 'nulu-v4-flash', messages: [] })) {
