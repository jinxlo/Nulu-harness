...
import { assertUsableApiKey, INVALID_CREDENTIAL_CODE, normalizeApiKey } from '@worldapptechnologies/nulu-llm'
...
describe('assertUsableApiKey', () => {
  it('returns the trimmed key when it is usable', () => {
    expect(assertUsableApiKey('  sk-abc  ', 'llm-gateway', 'WORLD_APP_TECHNOLOGIES_API_KEY')).toBe('sk-abc')
  })

  it('refuses a blank stored credential, naming the reference', () => {
    expect(() => assertUsableApiKey('   ', 'llm-gateway', 'WORLD_APP_TECHNOLOGIES_API_KEY'))
      .toThrow(/llm-gateway: the API key resolved from WORLD_APP_TECHNOLOGIES_API_KEY is blank/)
  })
  ...
  it('never echoes the key it refuses', () => {
    try {
      assertUsableApiKey('sk-\u{1F600}supersecret', 'llm-gateway', 'WORLD_APP_TECHNOLOGIES_API_KEY')
      expect.fail('an illegal key must throw')
    } catch (error) {
