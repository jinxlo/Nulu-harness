...
import type { AttachmentStore, ImageMediaType } from '@worldapptechnologies/nulu-attachment'
import {
  ToolCallId,
  contentHasFile,
  createUserMessage,
...
describe('model-facing image access', () => {
  it('describes the request preview, immutable normalized path, and source uncertainty', () => {
    ...
    const access = { readonlyPath: '/tmp/.nulu/attachments/v1/objects/bb/object' }
    const version = {
    ...
    expect(requestImageHandleText(attachment, version, access)).toBe(
      ...
      + ' Normalized copy (read-only; may be resized or re-encoded): "/tmp/.nulu/attachments/v1/objects/bb/object" (2048x1536px, image/png).'
  ...
  it('bridges a provider host object only through the mounted filesystem mapping', () => {
    ...
    const attachments = {
      imageHostPath: () => '/host/.nulu/attachments/object',
    } as unknown as AttachmentStore
    const mapped = (hostPath: string): string | undefined => hostPath === '/host/.nulu/attachments/object'
...
describe('file projection', () => {
  ...
  it('renders the handle with the read path or the explicit no-path fallback', () => {
    const withPath = fileHandleText(fileBlock('notes.pdf').attachment, '/home/.nulu/attachments/v1/files/ab/x/notes.pdf')
    expect(withPath).toContain('"notes.pdf"')
    expect(withPath).toContain('42 bytes')
    expect(withPath).toContain('sha256:abababab')
    expect(withPath).toContain('"/home/.nulu/attachments/v1/files/ab/x/notes.pdf"')
