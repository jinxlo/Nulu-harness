...
Choose this package whenever a plugin or composition needs to call a model: it is the only supported path into provider adapters, and it keeps one vocabulary across the loop, the session log, and every consumer. Do not reach for it when you need provider-specific wire behavior (that belongs in an adapter such as `nulu-llm-deepseek` or `nulu-llm-pi-ai`) or retry execution (that belongs in `nulu-llm-retry`).
