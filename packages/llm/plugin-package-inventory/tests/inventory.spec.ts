...
describe('Nulu plugin package inventory', () => {
  it('contributes by default and can be explicitly disabled', async () => {
    const defaultHarness = await harness()
    const defaultFields = await defaultHarness.ctx.nuluLlmApiExtensions.prepare({
    ...
    const disabledFields = await disabledHarness.ctx.nuluLlmApiExtensions.prepare({
      body: { messages: [] }, signal: SIGNAL,
    })
  ...
  it('reports active package versions once, retains parallel versions, and excludes inactive or loose entries', async () => {
    ...
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })
  ...
  it('uses the host inventory when a request has no matching or joined live agent', async () => {
    ...
    const missing = await ctx.nuluLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: 'missing' })
  ...
  it('mirrors the standing preset bare-package override instead of its local node_modules', async () => {
    ...
    const prepared = await ctx.nuluLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: id })
  ...
  it('withdraws the inventory field when the contributing plugin reloads', async () => {
    ...
    expect((await ctx.nuluLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })).fields)
