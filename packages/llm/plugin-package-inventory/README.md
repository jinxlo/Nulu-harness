---
description: "Active Loader package inventory metadata for deployments sending official Nulu requests."
...
Complete active Loader-backed plugin package inventory for official Nulu LLM API requests. This function plugin injects the Loader, live Agent registry, and `ctx.nuluLlmApiExtensions`, then owns the `nulu_plugin_packages` field. Enable it when the official API needs the active package list for request diagnostics.
...
| Key | Default | Meaning |
|---|---:|---|
| `enabled` | `true` | Register the `nulu_plugin_packages` contribution. Set it to `false` to omit package metadata. |

Shipped profiles use the default, so every official Nulu request carries the package inventory when preparation succeeds.
...
The version-1 `nulu_plugin_packages` field contains only `{ name, version }` pairs. Disabled, pending, failed, disposed, unloading, structural `cordis:` rows, ordinary dependencies, loose files without an owning package identity, programmatically mounted child fibers, and in-memory dynamic plugins are excluded.
...
Nothing. `nulu_plugin_packages` is provider metadata outside the model's messages, system prompt, and tool schemas.
