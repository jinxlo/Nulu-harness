...
export const inject = ['agents', 'nuluLlmApiExtensions', 'loader']
...
function identityFromManifest(path: string, allowAnonymous: boolean): NuluPluginPackageIdentity | undefined {
...
async function collectActivePluginPackages(
  ...
  hostBaseUrl: string,
  sessionId?: string,
): Promise<NuluPluginPackageIdentity[]> {
  ...
  const unique = new Map<string, NuluPluginPackageIdentity>()
