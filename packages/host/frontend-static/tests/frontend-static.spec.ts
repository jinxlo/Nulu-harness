...
import * as Connection from '@worldapptechnologies/nulu-client-connection'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'nulu-frontend-static-'))
  ...
  await writeFile(configPath, [
    ...
    "    host: '127.0.0.1'",
    '    port: 0',
    "- name: '@worldapptechnologies/nulu-client-connection'",
    '- id: frontend',
  ...
  const modules = new Map<string, unknown>([
    ['@worldapptechnologies/nulu-credentials-local', LocalCredentials],
    ['@worldapptechnologies/nulu-host-webserver', HttpServer],
    ['@worldapptechnologies/nulu-client-connection', Connection],
    ['@worldapptechnologies/nulu-host-frontend-static', FrontendStatic],
  ])
...
describe('real Loader composition', () => {
  it('serves explicit index entries and files while preserving HTTP error semantics', { timeout: 60_000 }, async () => {
    ...
    expect(await request(port, '/')).toMatchObject({
      ...
      body: 'nulu web authentication required; reopen the URL printed by nulu web.\n',
    })
