 ...
 * technique as nulu-session-persistence-jsonl's win32 suite): a small in-memory
...
describe('the worker entry over a mocked process boundary', () => {
  ...
  afterEach(() => {
    ...
    if (originalTitle === undefined) delete process.env.NULU_DIALOG_TITLE
    else process.env.NULU_DIALOG_TITLE = originalTitle
    vi.doUnmock('../src/win32-dialog-bindings.ts')
    vi.resetModules()
  })
  ...
  it('refuses to run without the dialog title', async () => {
    delete process.env.NULU_DIALOG_TITLE
    ;(process as { send?: unknown }).send = () => true
    await expect(import('../src/win32-dialog-worker.ts')).rejects.toThrow('NULU_DIALOG_TITLE is required')
  })
