...
# @worldapptechnologies/nulu-host-plugin-inventory
...
- **Presets appear only with a roster** — a deployment without `nulu-agent-presets` serves Loader entries alone; the `agentPresets` field is absent rather than empty.
