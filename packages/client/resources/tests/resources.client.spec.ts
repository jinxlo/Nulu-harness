...
describe('protocolOf', () => {
  it('reads the nulu-resource host, lower-cased, and reports none for any other address', () => {
    expect(protocolOf('nulu-resource://file/session/s1/home/ys/b.txt')).toBe('file')
    expect(protocolOf('NULU-RESOURCE://File/session/s1/a')).toBe('file')
    expect(protocolOf('nulu-resource://chat/node/1')).toBe('chat')
    ...
    expect(protocolOf('nulu-resource:///no-host')).toBeUndefined()
...
describe('ResourceRegistry stream generations', () => {
  ...
  it('streams another protocol as plain numbers', async () => {
    ...
    const source = b.registry.source('nulu-resource://counter/one')
    const unsubscribe = source.subscribe(() => {})
