...
import { createSnapshotStore, type ObservableSnapshot, type SnapshotStore } from '@worldapptechnologies/nulu-client-store'
import type {
  ResourceOpenContext,
  ResourceProtocol,
  ResourceProvider,
...
interface ResourceRecord {
  ...
  /** The address's protocol key (`nulu-resource://` host); absent when the address is not a resource address. */
...
/**
 * The one URL scheme resource addresses use: `nulu-resource://<type>/…`, where
 * the host names the protocol. Other schemes (`sidebar://…`) are navigation
 * addresses and name no resource.
 */
export const RESOURCE_SCHEME = 'nulu-resource'

/**
 * The protocol key of one address: the host of a `nulu-resource://` URL, as the
