...
import { inspectRequestPrompt } from '@worldapptechnologies/nulu-client-ui-conversation/client'
  ...
  it('shows the system node text as the request prompt card before the request messages', () => {
    const value = assembler([
      ...
      at(5, 'user/message', {
        ...
        source: { kind: 'plugin', plugin: '@worldapptechnologies/nulu-system-prompt', form: 'snapshot' },
      }, { surfaceOp: 'append' }),
      at(6, 'request/header', {
        reason: 'initial',
        header: { config: { provider: 'fake', model: 'fake' } },
