...
import type { RpcResponse } from '@worldapptechnologies/nulu-api-remotes/client'
...
const NAMESPACES = [
  {
    ns: 'llm-gateway',
    schema: {},
    value: { apiKeyEnv: 'WORLD_APP_TECHNOLOGIES_API_KEY', baseURL: 'https://base' },
...
describe('ModelsSettingsStore', () => {
  it('joins rows with configured, removable, and credential state', async () => {
    ...
    expect(seenRefs).toEqual([['WORLD_APP_TECHNOLOGIES_API_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GHOST_API_KEY']])
    const byProvider = new Map(state.rows.map(row => [row.entry.provider, row]))
    expect(byProvider.get('worldapp-gateway')).toMatchObject({
      configured: true,
      removable: false,
