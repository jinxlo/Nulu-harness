...
The Nulu card edits the shared `llm-gateway` endpoint, credentials, and model catalog without a protocol selector. When Cordis YAML selects Messages, the public endpoint placeholder is `https://api.nulu.com/anthropic`. Saving the card preserves protocol configuration.
