/**
 * Curated editor for the direct Nulu adapter's advisory model catalog.
...
export type NuluModelDraft = Record<string, unknown>
 ...
 * (rejected by {@link validateNuluModels} before any write).
 */
...
export interface NuluModelsValidationFailure {
...
export function modelDrafts(value: unknown): NuluModelDraft[] {
  if (!Array.isArray(value)) return []
  return value.map(entry =>
    typeof entry === 'object' && entry !== null && !Array.isArray(entry)
      ? entry as NuluModelDraft
      : {})
}
 ...
 */
export function validateNuluModels(value: unknown): NuluModelsValidationFailure | undefined {
...
/** Props of {@link NuluModelsEditor}. */
export interface NuluModelsEditorProps {
  /** Effective rows: inherited until the parent materializes an override. */
  models: readonly NuluModelDraft[]
  ...
  onChange: (models: NuluModelDraft[]) => void
  /** Remove the user-owned array and return to inheritance. */
  onReset: () => void
}

/**
 * Render the direct Nulu adapter's model catalog: id and display name on
 * each row, capacities behind the row's own disclosure.
 * @param props - effective rows plus the array-level override actions.
 * @returns the catalog editor.
 */
export function NuluModelsEditor(props: NuluModelsEditorProps): ReactNode {
  ...
  const capacityText = (model: NuluModelDraft, index: number, field: CapacityField): string => {
