...
This package is infrastructure: the web shell and the boot kernel are its only direct consumers. A composition needs it whenever it wants a React-rendered GUI — `nulu-client-web` loads the roster, waits for every entry to activate, then calls `ctx.uiRenderer.mount(container)`.
