...
import { NuluBrandMark, NuluBrandName } from '../src/client/Brand.tsx'
...
describe('Nulu browser-brand plugin', () => {
  ...
  it('leaves every slot empty outside the official build profile', async () => {
    vi.stubEnv('NULU_CLIENT_BUILD_PROFILE', 'local')
  ...
  it('fills declarations before or after apply and removes every occupant on teardown', async () => {
    vi.stubEnv('NULU_CLIENT_BUILD_PROFILE', 'official')
  ...
  it('renders the Nulu name independently from both requested mark sizes', () => {
    const name = render(<NuluBrandName />)
    expect(name.container.querySelector('svg')?.getAttribute('viewBox')).toBe('32 0 136 24')
    name.unmount()

    const mark = render(<NuluBrandMark size={34} />)
    expect(mark.container.querySelector('svg')?.getAttribute('width')).toBe('34')
    mark.rerender(<NuluBrandMark size={24} />)
