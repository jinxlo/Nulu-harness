---
description: "Nulu Harness brand occupants for the sidebar brand slots; for users and maintainers choosing or replacing brand presentation."
kind: "package-reference"
---

# @worldapptechnologies/nulu-client-ui-brand


## Summary

This package gives the Web client the Nulu Harness mark and name in the sidebar. The shell declares the slots and keeps its own fallbacks, so leaving this package out yields a coherent local-build presentation instead of an empty brand row. Choose it for deployments branded as Nulu Harness; deployments with another identity should provide a replacement brand package. It has no runtime state and does not affect model requests.
...
Mount this plugin in the browser roster of the deployment. The occupants register as soon as the sidebar declares its brand slots, in every build profile — the Nulu identity is the product identity, not a build-time variant.

### Replacing the brand

A deployment with its own identity leaves this package out and composes another package that occupies the sidebar slots. The conversation hero is declared and filled by `nulu-client-ui-conversation` and is not part of this package. Occupying a slot is the only composition route; there is no brand configuration surface here.
...
- **The browser title is independent** — `NULU_CLIENT_TITLE` selects title text at build time rather than through a UI slot.
