...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"nulu-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"nulu-manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: 'Skillnulu-manage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"nulu-manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('nulu-manage-issues')
