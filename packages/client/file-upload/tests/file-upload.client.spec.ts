...
afterEach(() => {
  delete (globalThis as UploadGlobal).__NULU_FILE_UPLOAD__
  vi.restoreAllMocks()
  vi.unstubAllGlobals()
})
...
describe('file upload service', () => {
  it('uses a page-owned Host fetch for Blob and ReadableStream bodies', async () => {
    ...
    ;(globalThis as UploadGlobal).__NULU_FILE_UPLOAD__ = { fetch }
  ...
  it('mounts through the plugin entry and resolves non-browser URLs', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(new URL('http://nulu.internal/fallback'), {
      method: 'POST', body,
    })
    await fiber.dispose()
  })
...
describe('Session-addressed file upload', () => {
  ...
  it('rejects malformed background results', async () => {
    vi.stubGlobal('location', { origin: 'https://preview.test' })
    ;(globalThis as UploadGlobal).__NULU_FILE_UPLOAD__ = {
