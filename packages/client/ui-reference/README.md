...
File picks preserve the natural text defined by the shared `@path` grammar as the hidden serialized and clipboard form. Session picks use the canonical `@[label](nulu-session:…)` mention; serialization never reconstructs identity from the visible title.
