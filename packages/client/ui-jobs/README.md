...
# @worldapptechnologies/nulu-client-ui-jobs


## Summary

This package renders the background-job surface of the Web GUI: a session-header action that opens a popover listing the jobs this session can see. It reads host-computed registry state through the runtime's `jobsBySession` mirror and issues no RPC of its own. The trigger appears only when the session has at least one job, with a badge counting running and stopping jobs; settled rows stay visible and de-emphasized until the registry drops them. The model's own view of the same jobs belongs to `nulu-tool-jobs`; this package is a read-only projection for the human.
...
- [nulu-tool-jobs](../../jobs/tool-jobs/README.md) — the model-facing jobs tool over the same registry.
