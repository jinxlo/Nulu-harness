...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--nulu-session-list-edge-inset')).toBe('var(--nulu-sidebar-inline-padding)')
    expect(root?.get('--nulu-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--nulu-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--nulu-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--nulu-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--nulu-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--nulu-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--nulu-session-list-edge-inset)',
      '- var(--nulu-session-list-scrollbar-width)',
      '- var(--nulu-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
