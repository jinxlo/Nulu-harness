...
describe('basenameOf', () => {
  it('decodes the last segment, so an escaped name reads as itself', () => {
    expect(basenameOf('nulu-resource://file/session/s-1/work/notes/a%20b%23c.md')).toBe('a b#c.md')
  })

  it('falls back to the whole address when there is no last segment', () => {
    expect(basenameOf('nulu-resource://file/session/s-1/')).toBe('nulu-resource://file/session/s-1/')
  })

  it('keeps a malformed percent escape as it is rather than refusing the address', () => {
    expect(basenameOf('nulu-resource://file/session/s-1/work/%E0%A4%A')).toBe('%E0%A4%A')
  })
})
...
describe('textDefinition', () => {
  it('is the fallback claimant of every Session file address, titled by basename', () => {
    ...
    expect(definition.patterns).toEqual(['nulu-resource://file/**'])
    expect(definition.priority).toBe('fallback')
    expect(definition.title('nulu-resource://file/session/s-1/work/README.md')).toBe('README.md')
    // Reads require an addressed Session, even when the file path is absolute.
    expect(definition.canOpen?.('nulu-resource://file/session/s-1/work/README.md')).toBe(true)
    expect(definition.canOpen?.(sessionFileAddress('s-1', '/home/me/README.md'))).toBe(true)
    expect(definition.canOpen?.('nulu-resource://file/absolute/home/me/README.md')).toBe(false)
    expect(definition.canOpen?.('nulu-resource://file/shared/team/README.md')).toBe(false)
    expect(definition.canOpen?.('nulu-resource://file/session')).toBe(false)
  })
})
...
describe('text type in the registry', () => {
  ...
  it('claims files of any extension, depth, and dot-prefix with relative or absolute Session paths', () => {
    ...
    for (const address of [
      'nulu-resource://file/session/s-1/a.md',
      'nulu-resource://file/session/s-1/deep/er/path/x.py',
      'nulu-resource://file/session/s-1/w/.env',
      'nulu-resource://file/session/s-1/w/Makefile',
  ...
  it.each([
    'nulu-resource://file/shared/team/notes.md',
    'nulu-resource://file/absolute/home/me/notes.md',
    'nulu-resource://file/absolute/C:/w/notes.md',
  ...
  it('yields an address to a narrower type at the extension band, and keeps the rest', () => {
    ...
    expect(tabs.claim('nulu-resource://file/session/s-1/w/logo.png').kind).toBe('image')
    expect(tabs.claim('nulu-resource://file/session/s-1/w/logo.md').kind).toBe(TEXTPREVIEW_KIND)
    // Still listed for the picture: a caller naming the kind may open it as text.
    expect(tabs.candidates('nulu-resource://file/session/s-1/w/logo.png').map(type => type.kind)).toEqual(['image', TEXTPREVIEW_KIND])
  })
