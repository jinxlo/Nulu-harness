...
describe('createFixtureApi', () => {
  ...
  it('serves raw history entries with replayable tool-result metadata', async () => {
    ...
    expect(results.find(event => event.data.turn === 71)).toMatchObject({
      data: { meta: { url: 'https://worldapptechnologies.com', statusCode: 200 } },
    })
  ...
  it('serves grouped models and keeps a selection for later history and fixture requests', async () => {
    ...
    expect(catalog.value.groups.map(group => group.name)).toEqual(['Nulu', 'OpenAI'])
  ...
  it('serves configured Nulu readiness and keeps credential values write-only', async () => {
    const api = createFixtureApi()
    ...
    for (const result of [
      await api.settingsRemote.update('llm-gateway', {}, undefined),
      await api.settingsRemote.replace('llm-gateway', {}, undefined),
    ]) {
    ...
    expect(await describe(['WORLD_APP_TECHNOLOGIES_API_KEY', 'TEST_API_KEY'])).toEqual({
      WORLD_APP_TECHNOLOGIES_API_KEY: { configured: true, source: 'file', writable: true },
