...
describe.each(SHEETS)('%s theme styles', (name) => {
  ...
  it('names only theme variables the token sheet defines', () => {
    ...
    // not just `--dsw-`: a `--nulu-` name reads as a plausible sibling and would
    // otherwise slip past into an invalid declaration.
    const named = [...sheet.matchAll(/var\((--(?:dsw|nulu|ds)-[a-z0-9-]+)/g)].map(match => match[1])
...
describe('MessageFeedbackActions row styles', () => {
  it('slot-injected actions ride the content font-size axis like their host row', () => {
    ...
    expect(block('MessageFeedbackActions', '.action')).toMatch(/width:\s*calc\(28px \+ var\(--nulu-content-font-delta, 0px\)\)/)
    expect(block('MessageFeedbackActions', '.action')).toMatch(/height:\s*calc\(28px \+ var\(--nulu-content-font-delta, 0px\)\)/)
    expect(block('MessageFeedbackActions', '.action svg')).toMatch(/width:\s*calc\(15px \+ var\(--nulu-content-font-delta, 0px\)\)/)
  })
