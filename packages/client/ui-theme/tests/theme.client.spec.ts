...
import type {
  ...
  ThemeSnapshot,
  ThemeTokenOverrides,
} from '@worldapptechnologies/nulu-client-ui-theme/client'
import { ThemeRuntime } from '@worldapptechnologies/nulu-client-ui-theme/client'
...
describe('ThemeRuntime', () => {
  ...
  it('seeds the initial font size from the boot-script body variable, ignoring junk', () => {
    ...
    document.body.style.setProperty('--nulu-content-font-size', '16px')
    try {
      expect(make().theme.getTheme().fontSize).toBe(16)
      document.body.style.setProperty('--nulu-content-font-size', '99px')
      expect(make().theme.getTheme().fontSize).toBe(14)
    } finally {
      document.body.style.removeProperty('--nulu-content-font-size')
    }
  })
