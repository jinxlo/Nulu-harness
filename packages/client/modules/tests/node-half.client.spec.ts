...
const UI_RENDERER_ID = '@worldapptechnologies/nulu-client-ui-renderer'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { nulu: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'nulu-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { nulu: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__NULU_BOOT__"] = ')
...
describe('client bundle activation', () => {
  ...
  it('serves the source map beside a registered client bundle', async () => {
    ...
    const shellResponse = service.fetchBundle(new Request(`nulu-app://app${batch.url}`))
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} nulu.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@worldapptechnologies/cordis'] }),
      entry('other'),
