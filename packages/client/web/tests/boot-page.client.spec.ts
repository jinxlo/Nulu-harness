...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    const { el } = mount()
    expect(el.firstElementChild?.getAttribute('data-nulu-boot')).toBe('')
  ...
  it('keeps loading while entries are active or loading', () => {
    const { el, page } = mount()
    page.setTotal(2)
    const spinner = el.querySelector<HTMLElement>('[data-nulu-boot-spinner]')
    expect(spinner?.style.getPropertyValue('--nulu-boot-arc')).toBe('72deg')
    page.setState('a', 'active')
    expect(spinner?.style.getPropertyValue('--nulu-boot-arc')).toBe('180deg')
    page.setState('b', 'loading')
    expect(el.querySelector('[data-nulu-boot-spinner]')).toBe(spinner)
    page.setState('b', 'active')
    expect(spinner?.style.getPropertyValue('--nulu-boot-arc')).toBe('288deg')
    expect(el.textContent).toContain('Loading plugins…')
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@worldapptechnologies/nulu-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@worldapptechnologies/nulu-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@worldapptechnologies/nulu-client-ui-layout')
    expect(el.textContent).toContain('@worldapptechnologies/nulu-client-ui-tool')
