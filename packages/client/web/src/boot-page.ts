 ...
 * @module @worldapptechnologies/nulu-client-web/src/boot-page
 */
...
export class BootPage {
  ...
  constructor(container: HTMLElement) {
    ...
    this.root.dataset.nuluBoot = ''
    this.card = div(css.card)
    this.wordmark = div(css.wordmark, 'NULU HARNESS')
    this.spinner = div(css.spinner)
    this.spinner.dataset.nuluBootSpinner = ''
  ...
  private updateProgress(): void {
    ...
    this.spinner.style.setProperty('--nulu-boot-arc', `${String(Math.round(72 + ratio * 216))}deg`)
  }
}
