...
describe('splitting', () => {
  ...
  it('names the pane a new tab lands in: the active pane, or the first docked one while a panel floats', () => {
    ...
    const tabId = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'text-preview' })
...
describe('tab identity', () => {
  it('focuses the existing tab when the same content is opened twice', () => {
    const { controller, paneId } = expanded()
    const first = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'text-preview' })
    controller.splitPane()
    const again = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'text-preview' })
    expect(again).toBe(first)
    const state = controller.getSnapshot().state
    expect(getPane(state, paneId).activeTabId).toBe(first)
    expect(state.activePaneId).toBe(paneId)
    expect(Object.values(state.tabs).filter(tab => tab.contentId === 'nulu-resource://file/session/s/a.txt')).toHaveLength(1)
  })
  ...
  it('opens an explicit second copy beside the original', () => {
    ...
    const state = controller.getSnapshot().state
    expect(state.tabs[copy]?.contentId).toBe('nulu-resource://file/session/s/a.txt')
...
describe('explicit tab placement', () => {
  ...
  it('refuses a floating pane as the destination', () => {
    const { controller } = expanded()
    const first = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'text-preview' })
    const second = controller.openContent({ contentId: 'nulu-resource://file/session/s/b.txt', title: 'b.txt', kind: 'text-preview' })
