...
describe('planner determinism', () => {
  ...
  it('leaves the state it was handed untouched', () => {
    ...
    planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
...
describe('planPlaceTab', () => {
  ...
  function threeTabs(): { state: LayoutState; paneId: PaneId; tabs: readonly [TabId, TabId, TabId] } {
    ...
    const b = planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/b.txt', title: 'b', kind: 'file' })
    const withB = applyAll(state, b.ops)
    const c = planOpenContent(withB, mint, { contentId: 'nulu-resource://file/session/s/c.txt', title: 'c', kind: 'file' })
...
describe('planDropTab on the tab\'s own pane', () => {
  ...
  it('splits without a backfill when the pane keeps another tab, factory or not', () => {
    const { state, minter } = seededState()
    const mint = minter.next
    const opened = planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
...
describe('planOpenContent placement and identity', () => {
  it('identifies content by (kind, contentId): the same address under another kind opens another tab', () => {
    const { state, minter } = seededState()
    const mint = minter.next
    const first = planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    const opened = applyAll(state, first.ops)
    expect(findContentTab(opened, 'nulu-resource://file/session/s/a.txt')).toBe(first.tabId)
    expect(findContentTab(opened, 'nulu-resource://file/session/s/a.txt', 'file')).toBe(first.tabId)
    expect(findContentTab(opened, 'nulu-resource://file/session/s/a.txt', 'hex')).toBeUndefined()
    ...
    expect(findPaneContentTab(opened, pane, 'nulu-resource://file/session/s/a.txt')).toBe(first.tabId)
    expect(findPaneContentTab(opened, pane, 'nulu-resource://file/session/s/a.txt', 'hex')).toBeUndefined()
    expect(findPaneContentTab(opened, pane, 'nulu-resource://file/session/s/nowhere.txt')).toBeUndefined()
    const again = planOpenContent(opened, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    expect(again.ops.map(op => op.type)).toEqual(['focusTab'])
    const other = planOpenContent(opened, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'hex' })
  ...
  it('opens another tab when told not to reveal the existing one', () => {
    const { state, minter } = seededState()
    const mint = minter.next
    const opened = applyAll(state, planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' }).ops)
    const copy = planOpenContent(opened, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file', revealIfOpened: false })
    expect(copy.ops.map(op => op.type)).toEqual(['openTab'])
    const both = applyAll(opened, copy.ops)
    expect(Object.values(both.tabs).filter(tab => tab.contentId === 'nulu-resource://file/session/s/a.txt')).toHaveLength(2)
  })
  ...
  it('seats a new tab at an explicit strip slot', () => {
    ...
    const mint = minter.next
    const paneId = getPane(state, state.rootId).id
    const planned = planOpenContent(state, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file', paneId, index: 0 })
...
describe('one undo behaviour across both embeddings', () => {
  ...
  function driveByController(): DockController {
    ...
    const opened = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
...
describe('planned intents survive replay', () => {
  it('rebuilds the same tree from the operations a planner produced', () => {
    ...
    const opened = planOpenContent(current, mint, { contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    push(opened.ops)
    ...
    expect(fileTab(asTab('t'), 'nulu-resource://file/session/s/a.txt', 'a.txt').kind).toBe('file')
  })
})
