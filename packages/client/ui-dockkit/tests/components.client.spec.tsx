...
function twoPanes(
  ...
  const fileTabId = controller.openContent({
    contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file', paneId: first,
  })
...
describe('DockSurface', () => {
  it('renders each pane with its tabs and the active tab body', () => {
    const controller = seededController()
    controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    ...
    expect(screen.getByTestId('body').textContent).toBe('nulu-resource://file/session/s/a.txt')
  ...
  it('scrolls the chip box to the active chip when it lies past either edge, clearing the fade band', () => {
    ...
    try {
      ...
      openedTab = controller.openContent({ contentId: 'nulu-resource://file/session/s/b.txt', title: 'b.txt', kind: 'file' })
...
describe('keyboard tabs', () => {
  ...
  function threeChips(): { intents: ReturnType<typeof spyIntents>; chip: (title: string) => HTMLElement; ids: Record<string, TabId> } {
    const controller = seededController()
    const a = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    const b = controller.openContent({ contentId: 'nulu-resource://file/session/s/b.txt', title: 'b.txt', kind: 'file' })
...
describe('divider drags', () => {
  ...
  function withColumn(columnBox: DOMRect): { intents: ReturnType<typeof spyIntents>; columnId: string } {
    ...
    const [first, second] = dockPaneIds(controller.getSnapshot().state)
    if (first === undefined || second === undefined) throw new Error('expected two docked panes')
    const opened = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file', paneId: second })
...
describe('FloatLayer', () => {
  ...
  function twoFloats(): {
    ...
    const controller = seededController()
    const first = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
    const second = controller.openContent({ contentId: 'nulu-resource://file/session/s/b.txt', title: 'b.txt', kind: 'file' })
  ...
  function floating(canCloseTab?: FloatLayerProps['canCloseTab']): {
    ...
    const controller = seededController()
    const tabId = controller.openContent({ contentId: 'nulu-resource://file/session/s/a.txt', title: 'a.txt', kind: 'file' })
...
describe('surface chrome', () => {
  it('renders the embedder\'s controls once, at the end of the top-right pane\'s strip', () => {
    ...
    const opened = controller.openContent({ contentId: 'nulu-resource://file/session/s/b.txt', title: 'b.txt', kind: 'file', paneId: seeded.id })
...
describe('fileTab', () => {
  it('builds a content tab the kit treats as opaque', () => {
    expect(fileTab(asTab('t1'), 'nulu-resource://file/session/s/x', 'x').kind).toBe('file')
  })
})
