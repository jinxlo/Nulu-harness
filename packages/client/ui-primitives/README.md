---
description: "Shared React UI atoms for the nulu web client: controls, icons, markdown and math rendering, and the terminal/read/diff/search/web output cards (zero Cordis)."
...
| `icons/*`, `NuluMark`, `BrandWordmark`, `ReferenceIcon`, `LinkIcon` | Glyphs and brand marks. Use `LinkIcon` for 14px clickable-link categories. |
...
- **Mark and wordmark are hand-authored** — `NuluMark` and `BrandWordmark` are original vector geometry defined by this package, not exports from a design file; update both together when the brand system gains an exact source.
