...
import { en as conversationEn, NS as CONVERSATION_NS, zh as conversationZh } from '@worldapptechnologies/nulu-client-ui-conversation/src/client/locales.ts'
import { apply as applyChat, inject as injectChat } from '@worldapptechnologies/nulu-client-ui-chat/client'
...
describe('run_code sub-calls through the real chat machinery', () => {
  ...
  it('a file sub-row click opens the file in the Sidebar; bash sub-rows open nothing', async () => {
    ...
    await vi.waitFor(() => {
      expect(b.sidebarRight.openResource).toHaveBeenCalledWith('nulu-resource://file/session/s1/notes/demo.txt')
    })
