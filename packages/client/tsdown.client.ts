 ...
 */
const CSS_VIRTUAL_PREFIX = '\0nulu-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0nulu-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0nulu-inline-css:'
 ...
 * Everything else under @worldapptechnologies/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@worldapptechnologies\/nulu-(?:file-reference|session|llm|tools|brand|deque|output-retention|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@worldapptechnologies\/nulu-token-meter\/client$|@worldapptechnologies\/nulu-host-open-in-app\/shared$|@worldapptechnologies\/nulu-agent-presets\/display$|@worldapptechnologies\/nulu-spill-policy\/notice$)/
...
const GENERATED_REMOTE = /^@worldapptechnologies\/nulu-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.NULU_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.NULU_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.NULU_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly nulu?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `nulu.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `nulu.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'nulu.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `nulu.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).nulu?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @worldapptechnologies value import is a build error — a
      ...
      name: 'nulu-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@worldapptechnologies/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s nulu.client.external, an inline-safe wire layer, or a generated /remote contribution — `
...
const STATIC_LINKED_PLUGIN = 'nulu-static-linked-external'
