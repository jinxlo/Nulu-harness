 ...
 * not built (`pnpm --filter @worldapptechnologies/nulu-client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@worldapptechnologies/cordis'
import { stubSettingsScope } from '@worldapptechnologies/nulu-client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@worldapptechnologies/nulu-client-ui-conversation/client'
import { SlotRegistry } from '@worldapptechnologies/nulu-client-ui-renderer/client'

const PLUGIN_ID = '@worldapptechnologies/nulu-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@worldapptechnologies/nulu-client-store', await import('@worldapptechnologies/nulu-client-store')],
      ['@worldapptechnologies/nulu-client-ui-conversation/client', await import('@worldapptechnologies/nulu-client-ui-conversation/client')],
      ['@worldapptechnologies/nulu-client-ui-primitives', await import('@worldapptechnologies/nulu-client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@worldapptechnologies/nulu-client-locale/client')
