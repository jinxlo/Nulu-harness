...
随附 Web graph 已通过 disabled 的 `ui-schedule` row 解析 `@worldapptechnologies/nulu-client-ui-schedule`；overlay 会把该 row 与 `@worldapptechnologies/nulu-schedule` 一起启用。只有会话已成功打开且 projection 至少包含一条活动记录时，触发器才会出现。打开目录后，逾期行在前，未来行再按目标时间排序；完全并列时保留 projection 的创建顺序。
