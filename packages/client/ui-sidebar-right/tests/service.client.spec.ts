...
import type { LayoutState, PaneId, TabId } from '@worldapptechnologies/nulu-client-ui-dockkit'
import { dockPaneIds, findTabPane, getPane } from '@worldapptechnologies/nulu-client-ui-dockkit'
...
describe('SidebarRightController — opening', () => {
  ...
  it('refuses every write while no seat is mounted', () => {
    ...
    expect(() => { controller.openResource('nulu-resource://file/session/s-test/a.txt') }).toThrow('no session surface is mounted')
  ...
  it('opens claimed content and reveals the column in one history entry', () => {
    ...
    publish()
    controller.openResource('nulu-resource://file/session/s-test/notes/readme.txt')
  ...
  it('opens another tab for the same address when told not to reveal, and when another kind is named', () => {
    ...
    controller.openResource('nulu-resource://file/session/s-test/notes/readme.txt')
    publish()
    controller.openResource('nulu-resource://file/session/s-test/notes/readme.txt', { revealIfOpened: false })
    publish()
    controller.openResource('nulu-resource://file/session/s-test/notes/readme.txt', { kind: 'hex' })
  ...
  it('lands a new tab in the pane the caller names', () => {
    ...
    controller.openResource('nulu-resource://file/session/s-test/a.txt', { paneId: left })
  ...
  it('takes the replaced tab\'s pane and slot, closes it, and records one entry', () => {
    ...
    publish()
    controller.openResource('nulu-resource://file/session/s-test/a.txt')
    publish()
    controller.openResource('nulu-resource://file/session/s-test/b.txt')
    publish()
    const a = tabOf('a.txt')
    const before = entries()
    controller.openResource('nulu-resource://file/session/s-test/c.txt', { replaceTab: a })
  ...
  it('still closes the replaced tab when the address is already open elsewhere', () => {
    ...
    controller.openResource('nulu-resource://file/session/s-test/a.txt', { replaceTab: spare })
  ...
  it('records the navigation in the Tab domain: params delivered, revision counting every open', () => {
    ...
    publish()
    controller.openResource('nulu-resource://file/session/s-test/a.txt', { params: { line: 3 } })
    const occurrence = controller.tabDomain.occurrence(SESSION, { id: tabOf('a.txt') })
    expect(occurrence.navigation.getSnapshot()).toEqual({ address: 'nulu-resource://file/session/s-test/a.txt', params: { line: 3 }, revision: 1 })
    publish()
    controller.openResource('nulu-resource://file/session/s-test/a.txt')
    expect(occurrence.navigation.getSnapshot()).toEqual({ address: 'nulu-resource://file/session/s-test/a.txt', params: undefined, revision: 2 })
  })
...
describe('SidebarRightController — the two opens', () => {
  it('refuses an address outside nulu-resource:// on the same path as an unclaimed one', () => {
    ...
    expect(() => { controller.openResource('nulu-resource://unknown/x') }).toThrow('no registered tab type claims')
    expect(entries()).toBe(before)
  })
...
describe('SidebarRightController — a tab\'s own actions', () => {
  ...
  const A_TXT = 'nulu-resource://file/session/s-test/a.txt'
  const B_TXT = 'nulu-resource://file/session/s-test/b.txt'
  ...
  it('adoption syncs the Tab domain on each commit of that store: the seeded guide is pinned, a closed tab aborted', () => {
    ...
    instance.actions.openContent(SESSION, { kind: 'text', contentId: 'nulu-resource://file/session/s-test/c.txt', title: 'c' }, () => {})
...
describe('SidebarRightController — the readable slice', () => {
  ...
  it('reports the last committed surface', () => {
    ...
    controller.openResource('nulu-resource://file/session/s-test/a.txt')
    publish()
    expect(Object.values(layout().tabs).filter(tab => tab.contentId === 'nulu-resource://file/session/s-test/a.txt')).toHaveLength(1)
    expect(controller.active()?.contentId).toBe('nulu-resource://file/session/s-test/a.txt')
  })
})
...
describe('explicit tab resource cleanup', () => {
  it.each(['close', 'replace'] as const)('immediately removes the tab after a synchronous cleanup handler during %s', (operation) => {
    ...
    h.controller.openResource('nulu-resource://file/session/s-test/terminal')
  ...
  it('does not invoke cleanup on collapse or when revealing the same tab', () => {
    ...
    h.publish()
    const address = 'nulu-resource://file/session/s-test/terminal'
...
it('releases close handlers without a stale disposer removing a replacement', () => {
  ...
  release()
  h.controller.openResource('nulu-resource://file/session/s-test/one')
  h.publish()
  h.controller.closeIn(SESSION, h.tabOf('one'))
  expect(first).not.toHaveBeenCalled()
  expect(second).toHaveBeenCalledOnce()
  releaseSecond()
  h.controller.openResource('nulu-resource://file/session/s-test/two')
  h.publish()
