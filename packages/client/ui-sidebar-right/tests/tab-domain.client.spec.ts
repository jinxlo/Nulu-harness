...
import type { LayoutState, TabId } from '@worldapptechnologies/nulu-client-ui-dockkit'
import { DockController, getPane } from '@worldapptechnologies/nulu-client-ui-dockkit'
...
describe('TabDomain — occurrences follow records', () => {
  it('pins a record\'s address when it first appears, with the occurrence\'s signal', () => {
    ...
    const tabId = controller.openContent({ kind: 'text', contentId: 'nulu-resource://file/session/s-one/a.txt', title: 'a' })
    domain.sync(SESSION, current())
    const occurrence = domain.occurrence(SESSION, recordOf(current(), tabId))
    expect(pin).toHaveBeenCalledWith('nulu-resource://file/session/s-one/a.txt', occurrence.signal)
  ...
  it('pins once per occurrence, however many times the layout commits', () => {
    const { domain, pin, controller, current } = harness()
    controller.openContent({ kind: 'text', contentId: 'nulu-resource://file/session/s-one/a.txt', title: 'a' })
    domain.sync(SESSION, current())
  ...
  it('aborts the occurrence when its record vanishes, and builds a new one when undo restores it', () => {
    ...
    expect(second.navigation.getSnapshot()).toEqual({ address: 'nulu-resource://file/session/s-one/a.txt', params: undefined, revision: 0 })
    expect(pin.mock.calls.filter(([address]) => address === 'nulu-resource://file/session/s-one/a.txt')).toHaveLength(2)
  })
...
describe('TabDomain — navigation', () => {
  it('creates the occurrence for a tab the seat has not shown yet, at revision 1, and pins it on the next sync', () => {
    const { domain, pin, controller, current } = harness()
    const tabId = controller.openContent({ kind: 'text', contentId: 'nulu-resource://file/session/s-one/a.txt', title: 'a' })
    domain.navigate(SESSION, tabId, { address: 'nulu-resource://file/session/s-one/a.txt', params: { line: 7 } })
    expect(pin).not.toHaveBeenCalled()
    const occurrence = domain.occurrence(SESSION, recordOf(current(), tabId))
    expect(occurrence.navigation.getSnapshot()).toEqual({ address: 'nulu-resource://file/session/s-one/a.txt', params: { line: 7 }, revision: 1 })
    domain.sync(SESSION, current())
  ...
  it('steps the revision on every navigation, params changed or not, and notifies', () => {
    ...
    domain.navigate(SESSION, tabId, { address: 'nulu-resource://file/session/s-one/a.txt', params: { line: 3 } })
    domain.navigate(SESSION, tabId, { address: 'nulu-resource://file/session/s-one/a.txt', params: { line: 3 } })
    expect(occurrence.navigation.getSnapshot()).toEqual({ address: 'nulu-resource://file/session/s-one/a.txt', params: { line: 3 }, revision: 2 })
...
describe('TabDomain — a tab\'s own actions', () => {
  it('opens into the pane holding the tab at call time, including after it was moved', () => {
    ...
    occurrence.tabActions.openResource('nulu-resource://file/session/s-one/b.txt')
    expect(navigator.openResourceIn).toHaveBeenLastCalledWith(SESSION, 'nulu-resource://file/session/s-one/b.txt', { paneId: leftPane })
    controller.placeTab(tabId, rightPane, 0)
    domain.sync(SESSION, current())
    occurrence.tabActions.openResource('nulu-resource://file/session/s-one/b.txt', { params: { line: 1 } })
    expect(navigator.openResourceIn).toHaveBeenLastCalledWith(SESSION, 'nulu-resource://file/session/s-one/b.txt', { paneId: rightPane, params: { line: 1 } })
    // The caller's pane wins over the default.
    occurrence.tabActions.openResource('nulu-resource://file/session/s-one/b.txt', { paneId: leftPane })
    expect(navigator.openResourceIn).toHaveBeenLastCalledWith(SESSION, 'nulu-resource://file/session/s-one/b.txt', { paneId: leftPane })
  })
  ...
  it('opens unplaced from a floating tab, since a floating pane holds one tab', () => {
    ...
    domain.sync(SESSION, current())
    domain.occurrence(SESSION, recordOf(current(), tabId)).tabActions.openResource('nulu-resource://file/session/s-one/b.txt')
    expect(navigator.openResourceIn).toHaveBeenLastCalledWith(SESSION, 'nulu-resource://file/session/s-one/b.txt', {})
  })
  ...
  it('passes revealIfOpened through, so a tab may open a second copy beside itself', () => {
    ...
    domain.sync(SESSION, current())
    const { tabActions } = domain.occurrence(SESSION, recordOf(current(), tabId))
    tabActions.openResource('nulu-resource://file/session/s-one/a.txt', { revealIfOpened: false })
    expect(navigator.openResourceIn).toHaveBeenLastCalledWith(
      SESSION,
      'nulu-resource://file/session/s-one/a.txt',
