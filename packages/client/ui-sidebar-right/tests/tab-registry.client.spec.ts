...
describe('SidebarRightTabRegistry — recognition', () => {
  ...
  it('matches a pattern containing ":" against the whole address', () => {
    ...
    registry.register(typeFor('text', ['nulu-resource://file/**']))
    expect(registry.claim('sidebar://guide').kind).toBe('guide')
    expect(registry.claim('nulu-resource://file/session/s/notes/a.txt').kind).toBe('text')
  ...
  it('matches a pattern without ":" against the URI path at any depth, ignoring case', () => {
    ...
    expect(registry.claim('nulu-resource://file/session/s/deep/er/shot.PNG').kind).toBe('image')
    expect(ranked(registry, 'nulu-resource://file/session/s/shot.png.txt')).toEqual([])
  })
  ...
  it('does not hide dotfiles from a path pattern', () => {
    ...
    expect(registry.claim('nulu-resource://file/session/s/proj/.env.local').kind).toBe('env')
  })
})
...
describe('SidebarRightTabRegistry — ranking', () => {
  it('ranks by band: extension over builtin over fallback, whatever the registration order', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**'], { priority: 'fallback' }))
    registry.register(typeFor('markdown', ['*.md'], { priority: 'builtin' }))
    registry.register(typeFor('third', ['*.md']))
    expect(ranked(registry, 'nulu-resource://file/session/s/a.md')).toEqual(['third', 'markdown', 'text'])
    expect(registry.claim('nulu-resource://file/session/s/a.md').kind).toBe('third')
  })
  ...
  it('lets a more specific builtin beat the fallback viewer despite the viewer\'s longer pattern', () => {
    ...
    expect(registry.claim('nulu-resource://file/session/s/shot.png').kind).toBe('image')
    expect(registry.claim('nulu-resource://file/session/s/notes.txt').kind).toBe('text')
  })

  it('lets an extension take over a builtin kind, and hands it back when the extension leaves', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**'], { priority: 'builtin', title: () => 'builtin text' }))
    ...
    expect(registry.claim('nulu-resource://file/session/s/a.txt').title).toBe('extension text')
    ...
    expect(ranked(registry, 'nulu-resource://file/session/s/a.bin')).toEqual([])
    release()
    expect(registry.get('text')?.title('x')).toBe('builtin text')
    expect(registry.claim('nulu-resource://file/session/s/a.bin').title).toBe('builtin text')
  })

  it('lets a builtin register under an extension already holding its kind, shadowed until the extension leaves', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'third-party/text' }))
    const releaseBuiltin = registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'shipped/text', priority: 'builtin' }))
    expect(registry.get('text')?.id).toBe('third-party/text')
    expect(ranked(registry, 'nulu-resource://file/session/s/a.txt')).toEqual(['text'])
  ...
  it('refuses a second registration in the same band, and any meeting a fallback of the kind', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'a/text', priority: 'builtin' }))
    expect(() => registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'b/text', priority: 'builtin' }))).toThrow('already registered')
    registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'c/text' }))
    expect(() => registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'd/text' }))).toThrow('already registered')
    expect(() => registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'e/text', priority: 'fallback' }))).toThrow('already registered')
    registry.register(typeFor('hex', ['nulu-resource://file/**'], { id: 'a/hex', priority: 'fallback' }))
    expect(() => registry.register(typeFor('hex', ['nulu-resource://file/**'], { id: 'b/hex', priority: 'builtin' }))).toThrow('already registered')
    expect(() => registry.register(typeFor('hex', ['nulu-resource://file/**'], { id: 'c/hex' }))).toThrow('already registered')
  })

  it('refuses a second registration of an id, whatever its kind', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    const dispose = registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'pkg/viewer', priority: 'builtin' }))
  ...
  it('within a band, the longer matched pattern wins', () => {
    ...
    expect(ranked(registry, 'nulu-resource://file/session/s/proj/README.md')).toEqual(['readme', 'markdown'])
    expect(ranked(registry, 'nulu-resource://file/session/s/proj/notes.md')).toEqual(['markdown'])
  })

  it('then registration order', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('first', ['nulu-resource://file/**']))
    registry.register(typeFor('second', ['nulu-resource://file/**']))
    expect(ranked(registry, 'nulu-resource://file/session/s/a.txt')).toEqual(['first', 'second'])
  })
  ...
  it('measures specificity by the longest pattern that matched, not the longest declared', () => {
    ...
    expect(ranked(registry, 'nulu-resource://file/session/s/notes.txt')).toEqual(['narrow', 'wide'])
  })
})
...
describe('SidebarRightTabRegistry — claiming', () => {
  it('lets canOpen veto an address its globs matched', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('image', ['*.png'], { canOpen: address => address.startsWith('nulu-resource://file/') }))
    expect(registry.claim('nulu-resource://file/session/s/shot.png').kind).toBe('image')
  ...
  it('opens with a named type, skipping its globs but honouring its canOpen', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**'], { canOpen: address => !address.endsWith('.bin') }))
    expect(registry.claim('sidebar://guide', 'text').kind).toBe('text')
    expect(() => registry.claim('nulu-resource://file/session/s/a.bin', 'text')).toThrow('tab type "text" refuses')
    expect(() => registry.claim('nulu-resource://file/session/s/a.txt', 'nope')).toThrow('no tab type is registered as "nope"')
  })

  it('answers with the address as contentId and the type\'s title', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    registry.register(typeFor('text', ['nulu-resource://file/**']))
    expect(registry.claim('nulu-resource://file/session/s/a.txt')).toEqual({
      kind: 'text',
      contentId: 'nulu-resource://file/session/s/a.txt',
      title: 'text:nulu-resource://file/session/s/a.txt',
    })
  })
...
describe('SidebarRightTabRegistry — ids and page types', () => {
  it('answers by kind with the definition in force, whose id is where its body lives', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    const disposeBuiltin = registry.register(typeFor('text', ['nulu-resource://file/**'], { id: 'shipped/text', priority: 'builtin' }))
  ...
  it('lets a page type omit patterns: it claims no address but is found by kind, and its guide entries carry its kind', () => {
    ...
    expect(ranked(registry, 'nulu-resource://file/session/s/a.txt')).toEqual([])
...
describe('SidebarRightTabRegistry — lifetime', () => {
  ...
  it('drops a type when its owner disposes, and frees the kind again', () => {
    const registry = new SidebarRightTabRegistry(new Context())
    const dispose = registry.register(typeFor('text', ['nulu-resource://file/**']))
    dispose()
  ...
  it('drops a type registered inside another plugin\'s effect when that plugin is disposed', async () => {
    ...
    const fiber = ctx.plugin({
      apply(inner: Context) {
        inner.effect(() => registry.register(typeFor('text', ['nulu-resource://file/**'])), 'test: text type')
      },
    })
    await fiber.await()
