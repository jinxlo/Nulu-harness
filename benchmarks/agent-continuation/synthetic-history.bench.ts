...
import { createSystemMessage } from '@worldapptechnologies/nulu-llm'
...
for (const [name, generate] of [
  ...
  it(name + ' seed preserves a protected head when the next prompt replaces it', () => {
    ...
    const replacement = session.append('system/message', {
      turn, step: 1, message: createSystemMessage('Next synthetic prompt', '@worldapptechnologies/nulu-system-prompt'),
