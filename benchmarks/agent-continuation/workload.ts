...
import { MessageId, ToolCallId } from '@worldapptechnologies/nulu-llm'
...
export function syntheticHistory(turns: number): SessionEvent[] {
  ...
  for (let turn = 1; turn <= turns; turn++) {
    ...
    if (turn === 1) session.append('system/message', {
      turn, step: 1, message: { id: MessageId('system-head'), role: 'system', content: [], source: { kind: 'plugin', plugin: '@worldapptechnologies/nulu-system-prompt' } },
    }, { surfaceOp: 'append' })
