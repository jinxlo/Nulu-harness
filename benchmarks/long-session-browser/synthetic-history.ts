...
import { createAssistantMessage, createSystemMessage, createUserMessage, createToolResultMessage, ToolCallId } from '@worldapptechnologies/nulu-llm'
...
export function syntheticHistory(): string {
  ...
  for (let turn = 1; turn <= HISTORY_TURNS; turn++) {
    ...
    if (turn === 1) session.append('system/message', {
      turn, step: 1, message: createSystemMessage('', '@worldapptechnologies/nulu-system-prompt'),
    }, { surfaceOp: 'append' })
