 ...
 * the guide sidebar, giving every module label and collection one home shared
...
} as const satisfies { guide: DocsSidebar; develop: GuideModuleLink; reference: GuideModuleLink }
...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/worldapptechnologies/nulu-harness' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/worldapptechnologies/nulu-harness/edit/master/${editSource}`
    },
...
/**
 * The Nulu wordmark, inlined so its `currentColor` fills follow the active
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="nulu-wordmark" ')
 ...
 */
const siteStyle = `
.nulu-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.nulu-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.nulu-tag {
  display: inline-flex;
  align-items: center;
...
/**
 * Navigation-bar title: the Nulu wordmark and the release-stage tag.
...
function siteTitle(previewTag: string): string {
  return `<span class="nulu-lockup">${wordmark}<span class="nulu-tag">${previewTag}</span></span>`
}
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'nulu-harness-doc-projector',
