...
3. Fix owner-first per surface: generated catalogs → trace every consumer, fix the source JSDoc or generator template, then regenerate all derivatives; type-equivalence fences → fix the source JSDoc, then re-paste the fenced copies (`verify-type-equiv` pins them); verbatim fences → copy the corrected block byte-for-byte into every committed copy per [nulu-doc](../nulu-doc/SKILL.md); model- or user-visible strings → route through [nulu-prose-standard](../nulu-prose-standard/SKILL.md) and change only with owning behavior evidence, otherwise leave unchanged and report the deferral.
