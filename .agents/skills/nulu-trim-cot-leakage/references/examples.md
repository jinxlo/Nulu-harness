...
### Authoring-language slip inside a code fence

**Leaked:** `// 更新这里 before returning` inside a verbatim code block.

**Fixed:** `// Update this before returning.`

Correct the block at its owner, then copy that byte-exact fence into every committed copy; a regenerated copy that keeps the Chinese comment reintroduces the leak.
