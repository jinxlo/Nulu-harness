---
name: nulu-speed-up-perf
description: 'Use when investigating or optimizing Nulu Harness performance, designing realistic synthetic benchmarks or CI performance gates, profiling long Sessions or Web responsiveness, or turning performance PR evidence into measured behavior-preserving fixes.'
---

# Speed Up Nulu Harness
...
Read [AGENTS.md](../../../AGENTS.md), [architecture](../../../docs/architecture.md), [testing policy](../../../docs/testing.md), [defensive patterns](../../../docs/defensive-patterns.md), and the affected packages’ instructions and Agent Notes. Use [CI test reliability](../nulu-ci-test-reliability/SKILL.md) for processes, clocks, browser tests, and asynchronous cleanup.
...
Measure built JavaScript under plain Node for CPU workers; source-loader overhead and module resolution are not the shipped path. Browser cases use built product assets and the supported `nulu` profile through the existing test harness. Do not add a production export solely for measurement or copy the algorithm into a “benchmark implementation.”
...
Apply [pre-push checks](../nulu-pre-push-checks/SKILL.md), report only executed evidence, and inspect CI rather than assuming local timing proves runner stability. After marking ready, evaluate review findings against code and executable evidence; reply with the reason or fix and resolve addressed threads. Do not dismiss a report merely because it came from a bot.
