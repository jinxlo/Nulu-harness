...
The generated [configuration catalog](../../../docs/config-catalog.md#worldapptechnologiesnulu-<name>) is the exhaustive source for every accepted field.
...
- **Installation guidance.** A plugin package mounts through `cordis.yml` rows. Only a package declaring `nulu.bundle.patch` installs as a profile layer via `nulu plugin --profile <name> add <package>` — if this package lacks that declaration, say how it mounts in a composition, never `nulu plugin add`.
