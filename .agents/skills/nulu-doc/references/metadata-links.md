...
| `package-bundle` | `packages/<group>/<package>/README.md` declaring `nulu.bundle.patch` | [package-bundle.md](../templates/package-bundle.md) | Installable layer: the verified `nulu plugin` install path, layer semantics, and patch document. |
...
| `persistence-release` | `docs/persistence-changes/releases/nulu-v*.md` and its Chinese sibling | [persistence-release.md](../templates/persistence-release.md) | Retrospective observation: pinned tag, adjacent before/after digests, actual version constants, and changed after schemas; no compatibility acknowledgement. |
...
Before assigning `package-library` or `package-bundle`, inspect the facts: read `package.json` for `nulu.bundle.patch` and `src/index.ts` for the entry shape (`apply` export or a default service export is a plugin; a plain module API is a library). `nulu plugin --profile <name> add <package>` installs any npm dependency, but the profile reconcile activates a layer only for a package that declares `nulu.bundle`; never present that command as an install path for a library or a plain plugin. The documentation check derives the expected kind from these same facts, rejects another value, and rejects `name`, `audience`, and ungoverned `tags`. Add a new kind only with a distinct template, an unambiguous repository position or declared owner, and a focused check that maps documents to it.
