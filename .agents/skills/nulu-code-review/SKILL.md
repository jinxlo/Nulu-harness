---
name: nulu-code-review
description: Use when reviewing a pull request in the nulu-harness repo — orients the reviewer to this codebase's standards (AGENTS.md conventions, defensive patterns, ADRs, quality gates) and the review-specific checks that code alone can't show
---

# Reviewing a Nulu-Harness PR
...
- [nulu-prose-standard](../nulu-prose-standard/SKILL.md): required coverage and editorial judgment for comments, docs, prompts, and visible strings.
- [nulu-ci-test-reliability](../nulu-ci-test-reliability/SKILL.md): isolation and regression-proof rules for resource-owning, asynchronous, or flaky tests and fixtures.
...
1. **New prose receives semantic review.** Use [nulu-prose-standard](../nulu-prose-standard/SKILL.md) to critically review every added or changed Markdown passage, JSDoc, comment, prompt, description, diagnostic, and visible string. Verify required coverage, accuracy, placement, and editorial quality against the owning code or behavior; automated checks do not establish those properties.
...
- **Test reliability:** for a resource-owning, asynchronous, platform-sensitive, or flaky test, apply [nulu-ci-test-reliability](../nulu-ci-test-reliability/SKILL.md) to the real worker/job topology, resource allocation, global-state restoration, synchronization, timeout budget, and quiescent teardown.
