# Agent Note: Real-API e2e in CI against the external Nulu API
...
The repo secret is named `WORLD_APP_TECHNOLOGIES_API_KEY_EXTERNAL`; it is mapped to the `WORLD_APP_TECHNOLOGIES_API_KEY` env var the adapters and tests read (`process.env.WORLD_APP_TECHNOLOGIES_API_KEY`). The distinct secret name documents intent (this is the *external* public-API key, not an internal-endpoint key) and lets an internal-endpoint key coexist later without collision. Hygiene choices, each defensive:

- **Step-scoped secret.** `WORLD_APP_TECHNOLOGIES_API_KEY` is set in the `env:` of only the preflight and e2e steps, never job-level — so checkout/setup-node/install never see it. A compromised install-time lifecycle script in a dependency cannot read a secret that isn't in its environment.
...
- **`WORLD_APP_TECHNOLOGIES_BASE_URL` pinned** to `https://platform.worldapptechnologies.com/api/v1` on the e2e step. The adapter would default to this when unset ([packages/llm/llm-gateway/src/index.ts](../../../../packages/llm/llm-gateway/src/index.ts) `PUBLIC_BASE_URL`), but pinning is self-documenting and hermetic — a stray repo-root `.env` (which `vitest.e2e.config.ts` loads if present) cannot silently redirect the run to another endpoint.
- **No secret echoed.** The preflight prints only `WORLD_APP_TECHNOLOGIES_API_KEY present.` — not the value or its length.
...
- **No write access (fork PRs): cannot.** Two independent facts block it. First, the workflow uses `pull_request`, **not** `pull_request_target` — GitHub does not pass repo secrets to fork-PR runs of `pull_request`, so `secrets.WORLD_APP_TECHNOLOGIES_API_KEY_EXTERNAL` resolves to empty on a fork runner. Second, the `if:` gate skips fork PRs entirely. The withholding is the real boundary; the gate is defense-in-depth and UX.
...
- **Rotate on flip.** The key lived in a private repo's CI; treat going-public as "assume exposed" and rotate `WORLD_APP_TECHNOLOGIES_API_KEY_EXTERNAL` at that moment.
...
The scheduled trigger auto-disables after 60 days of repo inactivity (a GitHub behavior); push/PR/dispatch are backstops, and an active monorepo will not hit it. Runner egress to `https://platform.worldapptechnologies.com/api/v1` is assumed — GitHub-hosted `ubuntu-latest` has it; an egress-restricted self-hosted runner would need connectivity confirmed before relying on the nightly.
