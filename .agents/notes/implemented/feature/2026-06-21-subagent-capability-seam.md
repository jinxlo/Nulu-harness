...
> The full seam is shipped: the `nulu-subagent` interface and `nulu-tool-subagent` consumer; the two in-process backends (`nulu-subagent-spawn-in-process`, `nulu-subagent-fork-in-process`); the nested-agent snapshot infrastructure ([per-session snapshot replay](../../archived/testing/2026-06-22-subagent-snapshot-replay.md)); and the out-of-process ACP, Codex, and Claude Code backends ([ACP Agent Note](../../archived/feature/2026-06-22-acp-subagent-backend.md), [product-provider Agent Note](2026-08-04-claude-code-and-codex-subagent-backends.md)).
...
| Package | Role |
|---|---|
| `@worldapptechnologies/nulu-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@worldapptechnologies/nulu-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@worldapptechnologies/nulu-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@worldapptechnologies/nulu-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@worldapptechnologies/nulu-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@worldapptechnologies/nulu-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@worldapptechnologies/nulu-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
