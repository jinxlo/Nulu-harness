...
- 每次委派最多增加两条仅日志事件。两个策略服务的可选 peer 类型由 `nulu-subagent` 拥有——其共享辅助函数持有 `ctx.get` 消费；未组合任一服务的组合保持原有行为。进程外子 agent 仍采用自身的部署策略，正在运行的子 agent 不跟随父级后续切换。
