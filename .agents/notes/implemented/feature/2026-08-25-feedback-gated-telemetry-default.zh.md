...
诊断一条 `/feedback` 报告需要报告所描述的会话数据。共享基础配置把未设置的 `NULU_TELEMETRY_MODE` 解析为 `DISABLED`，因此默认安装发出的反馈到达接收方时不带任何会话数据，报告者在求助的那一刻也没有授权共享的途径；只有事先导出了 `NULU_TELEMETRY_MODE` 的部署才能交付可诊断的报告。
...
共享基础配置把未设置或为空的 `NULU_TELEMETRY_MODE` 解析为 `FEEDBACK_ONLY`。插件自身省略 `mode` 的默认值是 `DISABLED`；`FULL` 被拒绝，非空 `NULU_TELEMETRY_DISABLED` 是加载前的强制关闭开关。新的自身文本反馈、消息评分编辑和撤回释放尚未交接的权威前缀，截止该事件，包含存储的上下文。继承的父会话反馈不授权子会话导出。
