...
| Diagnostic type and byte limit | `nulu-subagent` | Consumers receive a bounded optional field separate from assistant output. |
| Foreground and Job presentation | `nulu-tool-subagent` and the generic Job runtime | Scheduling choice does not change the underlying failure fact. |
| Process cancellation and quiescence | Product Provider and `nulu-subprocess` | Result settlement still precedes idempotent whole-tree disposal. |
