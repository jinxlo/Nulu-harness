...
`--unsigned` package invocations support macOS and Windows. A macOS unsigned package strips the Developer ID identity, notarization, and release completion record through the same scrubbed environment as Windows, isolates outputs in the target's `unsigned-artifacts` directory, and ad-hoc signs every Mach-O runtime file (`codesign --force --sign - --identifier <appId>.runtime.<sha256>`) so the runtime executes on Apple silicon without Developer ID. `desktopTargetEnvironment` in `package-target.ts` passes the target platform, architecture, and unsigned selector to both runtime preparation and electron-builder. `package-target.ts` rejects `--unsigned` for Linux because Linux packaging has no signing input to remove, and its error names the accepted platforms. The regular package command selects signed mode explicitly even when its parent requests unsigned mode, so unsigned output cannot qualify for release upload.
...
**Leave the unsigned macOS runtime unsigned.** The Apple silicon kernel refuses to execute a Mach-O binary without a valid signature, and the prebuilt native payloads (Node, pnpm, koffi, node-pty) are not signed by a common authority. An ad-hoc signature is the minimal signature that makes the runtime executable without introducing Developer ID or notarization.
...
- Unsigned artifacts remain isolated under `unsigned-artifacts`, their macOS runtime binaries carry ad-hoc signatures instead of Developer ID, and they are omitted from update configuration and release completion records.
