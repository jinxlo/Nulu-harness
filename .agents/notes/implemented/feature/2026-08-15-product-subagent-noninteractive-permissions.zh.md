...
| 诊断类型与字节上限 | `nulu-subagent` | 消费方收到与 assistant 输出分离的有界可选字段。 |
| 前台与 Job 呈现 | `nulu-tool-subagent` 和通用 Job 运行时 | 调度选择不会改变底层失败事实。 |
| 进程取消与完全停稳 | 产品提供方和 `nulu-subprocess` | 结果结算后仍执行幂等的完整进程树资源释放。 |
