...
### The engine (nulu-workflow-worker-thread): one worker thread per run
...
- **Saved/bundled workflows** (a `.nulu/workflows/` registry, slash-command API) and **script persistence to a run directory** (the tool-call event already records the script durably).
...
- **Background execution as the default** (CC's shape): deferred; foreground-synchronous matches `nulu-tool-subagent`'s cut, and background semantics should be designed ONCE across shell/subagent/workflow rather than per-tool.
