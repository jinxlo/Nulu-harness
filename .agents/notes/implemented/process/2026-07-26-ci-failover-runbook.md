...
`NULU_CI_FAILOVER_LINUX=selfhosted` also routes the credential-free dependency-layout job and both nulu/vendor pack jobs onto `vm-backup` for eligible same-repository PRs and master pushes. Their [release rehearsal decision](2026-09-06-release-rehearsal-selfhosted.md) owns the stricter event eligibility and hosted manual dispatch. This coupling is intentional: keeping the variable set to save release minutes also keeps the eligible main-CI Linux jobs self-hosted. Clearing it returns both workloads to their hosted targets for subsequent runs; publication stays hosted regardless.
...
1. Repository **Settings → Secrets and variables → Actions → Variables → New repository variable**: name `NULU_CI_FAILOVER_LINUX` (Linux pool outage) or `NULU_CI_FAILOVER_WINDOWS` (Windows pool outage), value `selfhosted`.
...
Delete the `NULU_CI_FAILOVER_LINUX` or `NULU_CI_FAILOVER_WINDOWS` variable (or set it to any value other than `selfhosted` or `blacksmith`). New runs resolve back to their hosted pools. Setting it to `blacksmith` keeps the jobs on Blacksmith until the value changes. Remove any extra instances that were registered during the incident.
