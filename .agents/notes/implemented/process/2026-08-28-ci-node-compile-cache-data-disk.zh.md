...
- CI lane：三个必需的 Linux job（默认 hosted，`NULU_CI_FAILOVER_LINUX` 时自托管 `vm-backup`）会在新 env 下跑完整套件；缓存处理回归会表现为 lane 失败。
...
### 为什么不把 `node-compile-cache` 纳入 `nulu-*` 清理？
