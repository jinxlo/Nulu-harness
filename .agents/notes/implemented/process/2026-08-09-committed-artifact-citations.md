...
- Recall probes use lexical boundaries and are calibrated against a known positive and a near-miss negative. Authoring-language probes flag non-English residue in English prose instead of exempting translated documents.
- Owner-first edits trace every generated consumer. Verbatim code fences are copied byte-for-byte between the two README siblings; model- or user-visible wording changes only with its owning behavior evidence, otherwise the audit leaves it unchanged and reports the deferral ([English-only documentation](2026-09-15-english-only-docs-and-pairing-retirement.md)).

One repo-wide purge applied these rules across the prose surfaces, including the generator-owned templates (`scripts/gen-doc-graphs.ts`, `scripts/gen-tool-catalog.ts`, the typert generator's page notice) with regeneration, and the type-equiv source JSDoc with page re-pastes. The [nulu-trim-cot-leakage skill](../../../skills/nulu-trim-cot-leakage/SKILL.md) operationalizes these rules: the audit taxonomy, the committed recall batteries, and few-shot examples for deciding what to keep or delete.
...
The audit's grep batteries (English, comments and prose, `--hidden` for `.agents/`) return no design-ordinal citations outside recorded fixtures, archived notes, the trim skill's own files, and this note's quoted evidence; `verify-type-equiv` and the `gen-*` freshness checks pin the regenerated surfaces. Coverage gap: no gate rejects a new ordinal citation — review owns the rule.
