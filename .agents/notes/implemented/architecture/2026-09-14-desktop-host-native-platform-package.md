...
English | [中文](2026-09-14-desktop-host-native-platform-package.zh.md)
...
**Resolve the platform package from the npm registry at payload preparation.** The registry publishes the family under the upstream `@deepseek-ai` scope, not `@worldapptechnologies`, so payload preparation would fail on every platform. Depending on the registry would also break offline and pinned builds.
