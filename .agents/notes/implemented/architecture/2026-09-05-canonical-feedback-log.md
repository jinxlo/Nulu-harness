...
The existing opt-in [session-log-gateway contribution](../../../../packages/session/session-log-gateway/README.md) includes feedback in the ordinary `nulu_session_log` suffix on a subsequent eligible request. It uses the existing Nulu destination selection and acceptance watermark. There is no separate `nulu_feedback` uploader, feedback-triggered LLM request, or model-input field. The [explicit-feedback OTel decision](2026-09-05-nonofficial-feedback-otel.md) owns the independent feedback-triggered upload for all users and providers.
