...
| Direct DeepSeek endpoint | `User-Agent` for app attribution; `x-nulu-harness-user-id` and conditional `x-nulu-harness-session-id` are separate request identity under the DeepSeek-specific decision. Do not send OpenRouter-only headers unless DeepSeek documents an equivalent contract. |
...
- `nulu-llm-deepseek` sends the shared `User-Agent` on every request and its mock-server suite asserts the exact value.
