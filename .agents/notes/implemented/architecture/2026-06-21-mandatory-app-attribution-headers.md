...
LLM provider requests should identify the product making them. That is useful for provider-side support, abuse investigation, compatibility debugging, and traffic analytics. Before this Agent Note the harness only partially did this: the hand-rolled Nulu adapter sent a hand-copied `User-Agent` constant (`packages/llm/llm-gateway/src/adapter.ts`), while the pi-ai-backed twin sent no harness-owned headers at all (`packages/llm/llm-pi-ai/src/adapter.ts`). New adapters could therefore omit attribution silently, and a library-backed adapter could drift from the hand-rolled adapter even though [the twin-adapter Agent Note](2026-06-13-twin-llm-adapters.md) exists to keep the provider contract honest across both implementations.

The immediate prompt came from OpenRouter's [App Attribution](https://openrouter.ai/docs/app-attribution) docs. OpenRouter creates app pages and rankings from `HTTP-Referer` plus display/category headers. That is valuable, but it is not the HTTP standard for application identity. The risk is adopting OpenRouter's exact header set as if it were universal, then leaking provider-specific headers to direct Nulu requests, future OpenAI/Anthropic/Vertex adapters, test servers, or proxies that log unknown fields indefinitely.
...
The provider-neutral identity is owned by `nulu-llm` (`packages/llm/llm/src/attribution.ts`), not by individual adapters. `AppIdentity` contains only public product facts needed to build `User-Agent`, and the default `APP_IDENTITY` values:

- product token for `User-Agent`: `nulu-harness` (continuity with the pre-Agent Note wire value and the repo/org identity)
- version: read from the owning package's manifest via `createRequire`, never a hand-copied constant
- app URL: `https://github.com/worldapptechnologies/nulu-harness` - the repository home
...
| Direct Nulu endpoint | `User-Agent` for app attribution; `x-nulu-harness-user-id` and conditional `x-nulu-harness-session-id` are separate request identity under the Nulu-specific decision. Do not send OpenRouter-only headers unless Nulu documents an equivalent contract. |
...
- `nulu-llm` documents the mandatory `User-Agent` attribution contract for `LlmAdapter` authors (`LlmAdapter` JSDoc, package README, and the adapter-contract section of `docs/subsystems/llm-streaming.md`).
...
- `nulu-llm-gateway` sends the shared `User-Agent` on every request and its mock-server suite asserts the exact value.
- `nulu-llm-pi-ai` sends the same `User-Agent` through pi-ai's `StreamOptions.headers` hook and its mock-server suite asserts the exact value.
...
**SDK-named token (`nulu-harness-sdk`).** Considered for the `User-Agent` token because the supported runtime client stack uses the SDK name. `nulu-harness` won because it names the Nulu Harness product, matches the org/repo identity and package scope, and keeps wire attribution stable without calling the complete product an SDK.
