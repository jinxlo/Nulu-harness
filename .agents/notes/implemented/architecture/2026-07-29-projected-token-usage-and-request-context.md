...
Both values are ordinary durable session-projection state. `@worldapptechnologies/nulu-token-meter` registers two units when `ctx.sessionProjections` is present.
