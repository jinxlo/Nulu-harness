...
`nulu-llm` registered adapters by exact model name. A plugin supplied a model list at Cordis startup, `LlmRuntime` stored one adapter per listed string, and `GenerateOptions.model` selected the adapter and the provider model at once. This worked while both shipping adapters targeted the same two Nulu models, but it conflated two independent decisions: which upstream provider owns a request, and which model that provider should run.
...
`nulu-llm-pi-ai` exposed none of pi-ai's provider abstraction. It constructed an inline Nulu `openai-completions` model, applied Nulu-specific payload patches, and stamped every replayed assistant message as Nulu. pi-ai itself has a provider/model catalog, selects APIs such as `openai-responses`, `anthropic-messages`, and `google-generative-ai`, and preserves provider-specific response ids and reasoning/tool signatures for later turns. The Harness conversion dropped the provider/model route and provider response fields, so simply replacing the inline model with a catalog lookup would have made same-model replay and cross-provider handoff incomplete.

The adapter configuration also assumes one Nulu API key and endpoint. A generic backend needs independent credentials and endpoint overrides per provider while leaving AWS, Google ADC, OAuth, and other ambient authentication mechanisms to pi-ai.
...
A provider has exactly one adapter owner in a Cordis context. `nulu-llm-gateway` registers `nulu`; `nulu-llm-pi-ai` may also register `nulu`, but loading both owners is a configuration error rather than an ordering rule or fallback. A deployment that wants the hand-rolled Nulu implementation excludes `nulu` from the pi-ai profiles. A deployment that wants pi-ai's Nulu implementation does not mount `nulu-llm-gateway`.

`nulu-llm-gateway` removes its model registration list and accepts any model string routed through provider `nulu`. Its request serialization, `/chat/completions` endpoint, thinking options, SSE parsing, and error behavior remain unchanged; `options.model` is still sent verbatim.

### Explicit pi-ai provider profiles

`nulu-llm-pi-ai` takes one non-empty list of provider profiles. Provider names must be unique within the list and present in pi-ai's `getProviders()` result. Each profile contains the provider name plus optional `apiKey`, `baseURL`, headers, reasoning level and budgets, cache retention, transport, SDK timeouts, a Harness stream-idle timeout, and a provider-owned `retryPolicy`. The adapter forces pi-ai's `maxRetries` to zero so one `stream()` call makes one visible provider attempt, while `nulu-llm-retry` executes the resolved policy at the agent failed-step extension point. Credentials are never global: an explicit key applies only to its profile, while an absent key lets pi-ai resolve its standard environment variable, OAuth token, AWS credential chain, Google ADC, or other provider-native ambient authentication. An explicitly empty key is invalid configuration rather than an environment fallback.
...
The JSON-RPC runtime receives provider and model explicitly. Its convenience fallback mounts `nulu-llm-gateway` only for provider `nulu` when that provider has no registered owner; other missing providers fail without guessing an adapter.
...
**Add `backend + provider + model`.** A backend key would allow `nulu-llm-gateway` and pi-ai's Nulu implementation to coexist and switch per request. The accepted deployment rule is instead one adapter owner per provider: implementations of the same upstream are alternatives selected by plugin composition. A third routing dimension would burden every request and configuration for a capability with no current consumer.

**Let `nulu-llm-pi-ai` automatically register every pi-ai provider.** This would claim ambient credentials and provider names the deployment never intended to expose, and would conflict with native adapters such as `nulu-llm-gateway`. Explicit profiles make capability and credential scope reviewable.
...
- Model selection no longer changes the Cordis plugin graph. Catalog-backed adapters can accept any installed catalog model selected after startup, while the native Nulu adapter forwards arbitrary Nulu model ids.
...
- `nulu-llm-pi-ai` rejects stop sequences because pi-ai's common stream API cannot express them; the native Nulu adapter retains its stop support.
...
- Keyless loop/session tests and ACP snapshots exercise durable provider/model metadata, resume and fork propagation, workflow/subagent overrides, and unchanged user-visible transcripts; the key-gated Nulu e2e retains real provider streaming and tool follow-up coverage.
