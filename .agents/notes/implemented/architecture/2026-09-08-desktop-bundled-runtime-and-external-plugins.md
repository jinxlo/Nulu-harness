...
[Runtime preparation](../../../../apps/desktop/scripts/prepare-nulu.ts) materializes the production graph once at build time and ships it through `extraResources/nulu`. The Electron shell stays in ASAR. A bundled upstream Node process runs the private Desktop Host from resources and loads enabled plugins from `$NULU_HOME/profiles/desktop`.
