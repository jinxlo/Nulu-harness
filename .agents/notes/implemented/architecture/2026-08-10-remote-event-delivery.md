...
### Consumer contract (`nulu-typert-protocol`)
...
JSON safety remains a runtime concern. Before queueing, the API Remotes Host source checks every argument with `nulu-session`'s `isJsonValue` and fails loudly when one is invalid, because this is an allowlist composition error rather than untrusted input.
...
| Root `tsconfig.base.json` | Adds source-plane `paths` entries for `nulu-settings/types`, `nulu-credentials/types`, and `nulu-api-remotes/types` |
| `nulu-commands` / `nulu-settings` / `nulu-credentials` | Moves each `interface Events` member to the owner's Client-safe `./types`; settings and credentials add that export, move brands and pure types with it, retain constructors in index, and include `lib/types/**/*.js` in published files |
| `nulu-session` | Exposes `isJsonValue` for validation of every event argument by the API Remotes Host source |
