...
**Rotate the signing secret on every restart.** This prevents an existing browser from reconnecting after an ordinary NULU restart. Persisting only the signing secret keeps that workflow while process-token rotation limits the startup URL to one process lifetime.
