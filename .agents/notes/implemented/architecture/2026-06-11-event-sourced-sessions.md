...
- Persistence stays a plugin concern; the in-memory store ships in nulu-session.
...
- Derivation cost grows with log length — compaction (nulu-compaction) is the intended mitigation, not log mutation.
