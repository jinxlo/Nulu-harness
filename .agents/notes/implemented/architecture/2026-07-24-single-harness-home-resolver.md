...
- `@worldapptechnologies/nulu-home-paths` shipped a **second** `resolveNuluHome` with the same precedence plus tilde expansion — a near-duplicate of `nulu-home` that no gate flagged because the two lived in different packages and had already drifted (only one expanded tildes).
...
`@worldapptechnologies/nulu-home` is deleted. Home-owning providers and boot packages import `resolveNuluHome` from `nulu-home-paths`; composition bundles contain only the resolved configuration rows.
...
**Leave the two `resolveNuluHome` copies in place.** They had already drifted (one expands tildes, one didn't) and encode the same cross-cutting fact twice. Consolidation is the point of the `util/` layer; a duplicate resolver is a latent divergence bug.
