...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `nulu-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `nulu-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `nulu-timeout` (`idleWatchdog`) |
| Clear the timer | `nulu-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `nulu-timeout` (`timeoutOf`) |
