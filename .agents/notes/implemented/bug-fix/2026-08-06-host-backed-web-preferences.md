...
The Web Appearance, Language, and busy-Enter preferences lived in browser `localStorage`. Browser storage is scoped to an origin, so reopening `nulu web` on another port selected a different partition and lost choices even though both processes used the same NULU home. These are user-level product preferences; session selection, drafts, disclosure state, and other transient browser state remain page-local.
...
Appearance, Language, and busy-Enter choices follow the NULU user home across reloads, ports, and loopback origins. Direct edits to `settings.yaml` converge through the existing invalidation stream, while legacy `nulu.theme`, `nulu.locale`, and `nulu.conversation.busyEnter` entries are neither read nor written.
