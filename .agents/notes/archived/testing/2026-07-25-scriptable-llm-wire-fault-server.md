...
The server reports wire facts only and does not classify retryability. Real-composition tests route it through `nulu-llm-gateway`, `nulu-agent-loop`, and `nulu-llm-retry`: connection refusal, hard disconnect, partial reset, idle timeout, and a valid content-less completion recover under the existing default policy; clean partial EOF remains `STREAM_CLOSED` and is not retried by default. The package does not change those policies.
