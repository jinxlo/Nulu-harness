# Agent Note: SessionTelemetryBackend anonymous user id ($NULU_HOME/.anonymous-user-id) and the OTel Resource user.id
...
`getOrCreateAnonymousUserId()` returns the bare UUID line in `$NULU_HOME/.anonymous-user-id` (resolved by `resolveNuluHome`, `$NULU_HOME` > `~/.nulu`), minting and persisting a random UUID v4 on first use; the backend constructor carries it as the Resource's `user.id` (the OTel semconv user attribute), once per export batch. The original implementation lived inside `session-telemetry-otel` because no second real consumer existed. `/feedback` later became that consumer, so [the shared-id decision](../architecture/2026-08-07-shared-feedback-telemetry-user-id.md) moves ownership to `@worldapptechnologies/nulu-anonymous-user-id` without changing the storage, anonymity, concurrency, or loss semantics recorded here. [Direct Nulu request identity](2026-08-11-nulu-request-user-id-header.md) is a third consumer of the same id.
...
| Home | `@worldapptechnologies/nulu-anonymous-user-id`, shared by the OTel backend, `/feedback`, and direct Nulu requests | Consumers share one storage contract without depending on an exporter backend |
| Separate switch | None | Any consumer can create the identity; `NULU_TELEMETRY_DISABLED` stops telemetry reporting but does not disable feedback acknowledgement or the Nulu request header |
...
| Housing it in `@worldapptechnologies/nulu-home-paths` | paths is pure path computation with zero IO; a persisting identity capability would pollute the package boundary |

## Consequences

- One `$NULU_HOME` is one stable user in the OTel feed; separate homes are separate users by construction, with no cross-home linking mechanism.
- The OTel feed, `/feedback`, and direct Nulu requests share `.anonymous-user-id`.
