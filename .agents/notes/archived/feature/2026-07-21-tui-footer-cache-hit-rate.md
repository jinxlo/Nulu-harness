...
**Drop `cacheWrite` from the denominator.** Rejected: cache writes are billed input the provider spent to populate the cache, so excluding them overstates the hit rate on a writing turn. Nulu reports no cache-write metric today, but the formula stays general and the write path is covered.
