...
Web e2e could not see any of this: its scaffold pins an isolated `NULU_HOME` holding two sessions.
...
The 1139 ms figure is a measured floor for the server-side I/O against a real store, not an instrumented end-to-end UI latency; the web e2e scaffold's isolated `NULU_HOME` cannot reproduce the corpus that produces it.
