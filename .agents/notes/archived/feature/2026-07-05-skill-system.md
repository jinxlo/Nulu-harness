...
Nulu Harness uses the same primitive so project-specific review, plugin-authoring, and tool-usage guidance lives next to the workspace or the user's agent configuration instead of being hard-coded into the loop.

## Decision

`@worldapptechnologies/nulu-skill` is the pure provider registry (`ctx.skills`), `@worldapptechnologies/nulu-skill-filesystem` is the shipped local filesystem provider, and `@worldapptechnologies/nulu-tool-skill` owns the durable session catalog and model-facing loader tool. `nulu-base` loads the registry, local provider, and consumer as separate rows so its profiles get the same behavior while embedded or remote providers contribute skills without changing the registry or consumer. Each row exposes only its owning package's configuration.

Dedicated packaged providers can contribute immutable skills without filesystem discovery. The shipped CLI declares `@worldapptechnologies/nulu-skill-badge` disabled by default; enabling its composition row contributes the official badge instructions through the same registry and consumer (see [the package contract](../../../../packages/skill/skill-badge/README.md)).
...
The local provider scans cwd-sensitive project roots, custom roots, and user roots in first-wins rank order: project `.nulu`, project `.agents`, `customSkillDirs`, user `.nulu`, then user `.agents`. The user `.nulu/skills` scan skips `.system` so a system-owned directory is not treated as normal user content. The local provider does not synthesize built-in system skills; configured bundled roots and dedicated providers supply additional skills.
...
**Materialize built-in NULU authoring skills under `~/.nulu/skills/.system`.** Rejected because bundled skills do not write user home on startup, and embedded or remote providers supply configured skills.
