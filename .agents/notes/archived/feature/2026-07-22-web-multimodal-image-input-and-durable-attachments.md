...
| Accepted user image | Immutable object below `NULU_HOME` plus `ImageAttachmentRef` | The host commits every image before `agent.send()` or `agent.steer()` can append the owning user event. |
| Structured model image output | Immutable object below `NULU_HOME` plus `ImageAttachmentRef` | The provider adapter commits the bytes before it emits a completed image block or assistant message event. Temporary URLs, paths, and base64 are forbidden in the event. |
...
| `packages/llm/llm-gateway` | Resolve official vision input to deterministic request versions and Files API ids. |
...
- Adapter and compaction tests cover deterministic Pi-AI request versions, Nulu Files upload and reuse, stale-id recovery, text-only projection, recursively nested tool-result images, shared summary request versions, and explicit image-output rejection.
...
- Credentialed real-API tests cover the configured Anthropic route and the built-in `worldapp-gateway` Files path. The Nulu test does not use a custom provider entry.
