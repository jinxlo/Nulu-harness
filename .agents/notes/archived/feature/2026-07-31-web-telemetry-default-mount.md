# Agent Note: Default session-telemetry mount (OTel reporting) in the nulu web composition
...
| Sharing mode | `NULU_TELEMETRY_MODE`, default `FEEDBACK_ONLY` ([feedback-gated default](2026-08-25-feedback-gated-telemetry-default.md)); explicit `FULL` or `DISABLED` overrides | A fresh profile uploads only when the user records `/feedback`, while internal deployments retain both explicit policies |
| Endpoint | `NULU_TELEMETRY_OTLP_URL`, default `https://telemetry.worldapptechnologies.com/v1/logs` | Internal collector; the env override serves local/dev runs |
| Hard opt-out | any non-empty `NULU_TELEMETRY_DISABLED` (including `0`/`false`) disables the row | The launcher patch takes effect before load-time transport validation and overrides every configured mode |
...
| Exit-drain bound | `exporter.timeoutMillis: 1000` + `maxExportBatchSize: 2048` (== maxQueueSize) + `exportTimeoutMillis: 1500` + `shutdownTimeoutMillis: 3000` | Ordinary unreachable-collector failure releases in ~1s: timeoutMillis is the per-attempt socket timeout and retry deadline, while one queue-sized batch avoids sequential drain multiplication. The NULU-owned 3s outer bound covers the SDK's preceding unbounded `forceFlush()` wait when the transport Promise never obtains a socket. |
| Compression | `compression: gzip` | Event bodies carry full content; cross-datacenter bandwidth |
| CI isolation | top-level `env: NULU_TELEMETRY_DISABLED: '1'` in GitHub workflows | Defense in depth keeps test sessions local even when a job explicitly selects an uploading mode |
...
- A developer running `nulu web` without telemetry configuration makes no telemetry network request until they record `/feedback`. An internal deployment sets `NULU_TELEMETRY_MODE` and may point `NULU_TELEMETRY_OTLP_URL` at another collector.
