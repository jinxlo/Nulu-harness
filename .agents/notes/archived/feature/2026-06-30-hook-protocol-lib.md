# Agent Note: nulu-hook-protocol — the shared Claude Code / Codex hook wire-protocol core
...
This Agent Note introduces `@worldapptechnologies/nulu-hook-protocol`, a **library** (not a plugin — it registers and injects nothing) holding the genuinely-identical primitives both bridges build on. The split between shared and per-dialect is the design's center of gravity.

## Decision

A new `packages/hooks/` group with `hook-protocol` as a pure library. It owns four primitive families and the `hook/*` session events; each bridge plugin (`nulu-hooks-claude-code`, `nulu-hooks-codex`) owns what genuinely differs.
