...
The existing subagent registry already owns unique provider names, reversible registration, lifecycle events, and holder-owned published runs. The existing `nulu-tool-subagent` configuration already binds one provider name to one model-visible tool name. Product providers need to expose the missing Profile-owned identity without adding another registry or selection protocol.
...
| Model-visible tool name and binding | `nulu-tool-subagent` Config | One static tool resolves one configured provider name |
