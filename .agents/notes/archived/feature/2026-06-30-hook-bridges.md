# Agent Note: nulu-hooks-claude-code + nulu-hooks-codex — the Claude Code / Codex hook bridges
...
Matcher semantics, exit-code handling, and merge precedence live in `nulu-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. Per-file coverage includes config branches plus end-to-end mappings through a real loop, `nulu-bash-local`, and shell scripts, while a real-Loader smoke guards the package export shape. Native plugins bypass the wire protocol and return typed decisions directly.
