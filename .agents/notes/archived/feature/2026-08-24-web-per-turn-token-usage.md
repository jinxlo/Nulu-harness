...
The shared `TokenUsage` value carries optional `totalTokens` for one model call. Adapters publish it only from an exact provider total or authoritative aggregate prompt and output counters. Nulu checks its prompt-plus-completion aggregate against any wire total, and pi-ai preserves its provided total.
