...
**Localize producer names in the client.** A dictionary keyed by plugin id would read better than `@worldapptechnologies/nulu-system-prompt`, but it drifts silently on every rename, needs a client change per new producer, and cannot name a producer from a foreign log at all. The producer name already recorded in the log is more reliable than a label the client invents.
...
- Producer names in the UI are package-shaped (`nulu-tool-skill`, `@worldapptechnologies/nulu-system-prompt`) wherever the source carries only a plugin id. That is the cost of refusing a client-side name table; a producer that wants a better label must record one in its source fields.
...
- The `recall` arm has no producer in a shipped Web leaf until a host mounts `nulu-session-reference`; it is reachable only through logs written elsewhere.
