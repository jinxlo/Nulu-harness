# Agent Note: Rescope vendored Cordis into @worldapptechnologies
...
The nine packages under `vendor/` kept their upstream npm names (`cordis`, `cosmokit`, `schemastery`, `@cordisjs/plugin-*`). That premise does not survive publication: every harness package declares `cordis` as a peer dependency, so a consumer installing `@worldapptechnologies/nulu-*` must resolve it from the registry, which means publishing the harness publishes this framework layer too. Publishing it under the upstream names squats them on the registry, and where that registry proxies npmjs, the same-name entries shadow the real upstream packages and install the wrong framework into unrelated projects.

## Decision

All nine packages move into the `@worldapptechnologies` scope. Directory names, upstream version numbers, and dependency ranges stay untouched, so the `vendor/README.md` manifest still reads as an upstream snapshot. [docs/rescope.md](../../../../docs/rescope.md) restates this mapping for consumers.

| Directory | npm name | Upstream name |
|---|---|---|
| `cordis/` | `@worldapptechnologies/cordis` | `cordis` |
| `cosmokit/` | `@worldapptechnologies/cosmokit` | `cosmokit` |
| `schemastery/` | `@worldapptechnologies/schemastery` | `schemastery` |
| `loader/` | `@worldapptechnologies/cordis-plugin-loader` | `@cordisjs/plugin-loader` |
| `include/` | `@worldapptechnologies/cordis-plugin-include` | `@cordisjs/plugin-include` |
| `group/` | `@worldapptechnologies/cordis-plugin-group` | `@cordisjs/plugin-group` |
| `timer/` | `@worldapptechnologies/cordis-plugin-timer` | `@cordisjs/plugin-timer` |
| `hmr/` | `@worldapptechnologies/cordis-plugin-hmr` | `@cordisjs/plugin-hmr` |
| `logger-console/` | `@worldapptechnologies/cordis-plugin-logger-console` | `@cordisjs/plugin-logger-console` |

The rewrite touches only **delimited, complete package-name tokens**: quoted or backticked specifiers (optionally with a `/subpath`), `package.json` names and dependency keys, `cordis.yml` `name:` values, and `tsconfig.base.json` `paths` keys. Identically spelled strings that are not package names therefore stayed as they were: the `cordis.yml` config-file family, the Loader's literal `cordis:` builtin prefix (`cordis:include`, `cordis:group` — see `vendor/loader/src/config/tree.ts`), kind strings like `cordis-config-entry`, `@worldapptechnologies/nulu-tool-cordis`, Schemastery's upstream `Symbol.for('schemastery')` and `vendor:` metadata field, the `packages/<group>/` directory names in `GROUP_ORDER` (`scripts/gen-module-graph.ts`, `scripts/gen-doc-graphs.ts`), and the upstream install instructions in `vendor/*/README.md`.
...
- No upstream name remains in the publication set. `publish-npm-baseline.ts` now requires every published package to be `@worldapptechnologies/*` with no vendored exemption, so regressing the rename fails before packing.
...
**Keep the upstream names and exclude `vendor/` from publication.** Rejected because every harness package declares `cordis` as a peer dependency, so an installed `@worldapptechnologies/nulu-*` would have no resolvable framework.
