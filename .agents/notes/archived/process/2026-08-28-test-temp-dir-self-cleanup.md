# Agent Note: Unit tests remove the nulu-* temp dirs they create
...
Test processes create `/tmp/nulu-*` directories with `mkdtemp(join(tmpdir(), 'nulu-*'))` and leave them behind. On the self-hosted Linux CI host (32 runner instances sharing one `/tmp`) the residue exhausted the root partition's inode capacity twice (issue #3134, 2026-08-13 and 2026-08-26). The machine-side `nulu-tmp-sweep` timer and the CI lane sweep (kept, unmerged, on branch `fix/ci-tmp-residue-cleanup`) remove residue after the fact but leave the producing defect in place. Human review of #3233 (2026-08-28) rejected the sweep: unit tests must clean up the directories they create instead.

## Decision

Retrofit removal of every `nulu-*` temp dir a spec file creates, at the owning test's teardown:
...
- Product cleanup is limited to the per-process spill directory of `nulu-subprocess-local/spawn` (`privateSpillDir`): it is removed at a JavaScript-observable process exit when it holds no completed spill file — completed spill files are retained as full-output recovery artifacts until an external cleanup, so only directories that never spilled (the dominant residue shape on the CI host: 92% of sampled `nulu-subprocess-*` dirs are empty) are removed. The removal is best-effort (ENOENT/ENOTEMPTY/EBUSY/EPERM must not change the exit code). `nulu-spill-local`'s default root is deliberately NOT exit-deleted: it is covered by the package's own 30-day startup sweep, and the [retention decision](../architecture/2026-07-17-local-spill-startup-cleanup.md) forbids deleting fresh spill artifacts that resumed or forked sessions may still reference.
...
- CI runs the changed specs on the Linux and Windows coverage lanes; after a full green run, the fixed files' residue templates (observed at up to ~5,000 dirs per two hours each, e.g. `nulu-profile-`, `nulu-app-boot-`, `nulu-presets-*`, `nulu-upload-index-`) should no longer appear in fresh `/tmp` residue on the CI host.
...
- Bought: on normal completion — including failed tests — a spec's `nulu-*` dirs are removed at teardown; a `nulu-subprocess-local` per-process spill directory holding no completed spill file is removed at a JavaScript-observable process exit.
...
- Cost: dirs created by a spawned child are covered only when the test knows their paths; product-owned spill dirs holding spill files, and `nulu-spill-local`'s default root, keep their files per the existing retention policy and are cleaned by that package's own sweep.
