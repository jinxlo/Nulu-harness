...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `nulu-tool-bash` (`bash`), `nulu-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `nulu-tool-subagent` (`subagent`). Example-only tools are excluded.
