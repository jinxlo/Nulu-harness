...
The [three release sequences](2026-08-10-npm-release-sequences.md) shipped with `publishConfig.access: restricted`, so every package published to the `@worldapptechnologies` scope was visible only inside the organization. Five rehearsal publications ran that way, through `nulu@0.0.1-rc.5`, `vendor *-rc.4`, and `landlock-run@0.0.1`.

A restricted dependency is what actually blocks a public consumer. Every harness package declares the vendored framework as a `peerDependency`, and `nulu-sandbox-local` declares the Landlock entry as a `dependency`. A public package that requires a restricted one cannot be installed by anyone outside the organization, so those two sequences have to be public before the nulu family can be — and while the nulu family is still restricted, they are the only two whose artifacts an outside consumer would need to resolve.
...
| nulu | `packages/*/*` + `apps/*` (221 members) | `restricted` |

`check-workspace-constraints.ts` holds every manifest to its own sequence's level, which is what stops the scope from drifting: a new `vendor/*` package left at `restricted`, or a nulu member flipped to `public`, fails the workspace constraints.
...
**Flip the whole scope public at once.** Rejected: it would make a nulu release public as a side effect of a manifest change rather than a deliberate release decision. Opening the two dependency sequences first keeps every published package installable at each step and is a precondition for opening nulu.
...
- **`@worldapptechnologies/nulu` is still not installable from outside the organization.** Its manifests stay `restricted`; what changed is that its published dependencies no longer would be, so opening it later is a version decision rather than a dependency problem.
