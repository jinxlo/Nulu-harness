...
**Import the controlled prompt into the tool.** Rejected because the prompt is one provider's protocol constant; a Consumer matching it would couple the tool to `nulu-terminal-bash` specifically, and any other mounted backend would break it again.
