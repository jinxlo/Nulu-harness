# Agent Note: Shared anonymous user id across telemetry, feedback, and Nulu requests
...
The OpenTelemetry backend already persisted one anonymous UUID in `$NULU_HOME/.anonymous-user-id`. `/feedback` now needs to report both the receiving session id and a user id so an operator can correlate the acknowledgement with exported records. Duplicating or independently generating that identity would make the reported user meaningless, while importing it from `session-telemetry-otel` would make a direct command depend on an exporter backend and create a dependency cycle when feedback export is mounted by telemetry.

The earlier [anonymous-user-id decision](../feature/2026-07-31-telemetry-anonymous-user-id.md) deliberately kept the helper inside the OTel backend until a second real consumer existed. Feedback became that second consumer. [Direct Nulu request identity](../feature/2026-08-11-nulu-request-user-id-header.md) is the third.

## Decision

`@worldapptechnologies/nulu-anonymous-user-id` owns `getOrCreateAnonymousUserId()` and the `$NULU_HOME/.anonymous-user-id` storage contract. `session-telemetry-otel` uses the returned id as OpenTelemetry Resource `user.id`; the `/feedback` success acknowledgement reports `Feedback recorded for session {sessionId}` followed by `Anonymous user: {userId}` on a second line; and direct Nulu requests carry it as `x-nulu-harness-user-id`. Invalid feedback is rejected before resolving the id, and the Nulu adapter resolves it only after credentials succeed, so neither an empty command nor a credential failure creates `.anonymous-user-id`.
...
- One harness home has one anonymous id shared by feedback acknowledgements, session telemetry exports, and direct Nulu requests.
