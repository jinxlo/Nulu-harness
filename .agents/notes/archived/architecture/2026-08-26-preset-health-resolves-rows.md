...
What the walk gives up: a package resolvable only through a loader hook — an import map, or a tree with no `node_modules` — is reported broken. No supported install produces one, because `nulu plugin install` puts every plugin beside the roster.
...
`@worldapptechnologies/cordis-plugin-group` is a devDependency of `nulu-agent-presets`: the mount fixtures now compose through `cordis:group` the way real presets do, and a preset outside the workspace cannot resolve that package by name, so the app registers it as a builtin and the fixture harness does the same.
