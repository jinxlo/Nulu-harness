...
**Replacing `nulu-subprocess-local` or the bash executor.** The first would let the proxy table replace a workspace package against its own classification and invert the layering; the second trips `nulu-permission-presets`' boot-time `sandboxMode` validation and drops tested timeout/output behavior.
