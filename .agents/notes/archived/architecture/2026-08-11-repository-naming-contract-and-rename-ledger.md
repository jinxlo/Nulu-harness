...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is Nulu Harness, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@worldapptechnologies/nulu-sdk-client`, `@worldapptechnologies/nulu-sdk-protocol`, and the wire identity `nulu-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. Nulu Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `nulu-subagent-nulu-sdk` names the Nulu Harness SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@worldapptechnologies/nulu-jsonrpc` | `@worldapptechnologies/nulu-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@worldapptechnologies/nulu-sdk-client`, `@worldapptechnologies/nulu-sdk-protocol`, and `nulu-harness-sdk-runtime`. Exclude `@worldapptechnologies/create-sdk`, `@worldapptechnologies/nulu-scripts`, `@worldapptechnologies/nulu-helper`, and `@worldapptechnologies/nulu-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@worldapptechnologies/nulu-bash`, `ctx.bash` | `@worldapptechnologies/nulu-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@worldapptechnologies/nulu-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@worldapptechnologies/nulu-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@worldapptechnologies/nulu-pty`, `ctx.pty`, `PtyService` | `@worldapptechnologies/nulu-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@worldapptechnologies/nulu-pty-local`, `LocalPtyBackend` | `@worldapptechnologies/nulu-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@worldapptechnologies/nulu-tool-pty` | `@worldapptechnologies/nulu-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@worldapptechnologies/nulu-lsp-local` | `@worldapptechnologies/nulu-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@worldapptechnologies/nulu-tasks`, `ctx.tasks`, `TaskService` | `@worldapptechnologies/nulu-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@worldapptechnologies/nulu-tasks-local`, `LocalTaskService` | `@worldapptechnologies/nulu-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@worldapptechnologies/nulu-tool-tasks` | `@worldapptechnologies/nulu-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@worldapptechnologies/nulu-client-ui-task`, `client/ui-task/` | `@worldapptechnologies/nulu-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@worldapptechnologies/nulu-client-ui-slash`, `ui-slash/` | `@worldapptechnologies/nulu-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@worldapptechnologies/nulu-agent-tool-mode`, plugin `tool-mode` | `@worldapptechnologies/nulu-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@worldapptechnologies/nulu-permission`, `ctx.permission`, `PermissionService` | `@worldapptechnologies/nulu-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@worldapptechnologies/nulu-client-ui-permission` | `@worldapptechnologies/nulu-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@worldapptechnologies/nulu-user-interaction`, `user-interaction/` | `@worldapptechnologies/nulu-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@worldapptechnologies/nulu-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@worldapptechnologies/nulu-type-meta` | `typert/protocol/`, `@worldapptechnologies/nulu-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@worldapptechnologies/nulu-tools` and `ctx.tools`. Keep `@worldapptechnologies/nulu-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@worldapptechnologies/nulu-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@worldapptechnologies/nulu-workspace-context`, `context/workspace-context/` | `@worldapptechnologies/nulu-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `nulu-session-telemetry` and `nulu-session-telemetry-otel`. |
...
| `session/user-id/`, `@worldapptechnologies/nulu-user-id` | `identity/anonymous-user-id/`, `@worldapptechnologies/nulu-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and Nulu requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@worldapptechnologies/nulu-environment` | `util/launch-environment/`, `@worldapptechnologies/nulu-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `NULU_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `NULU_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| Former | Current | Reason |
|---|---|---|
| `@worldapptechnologies/nulu-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@worldapptechnologies/nulu-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@worldapptechnologies/nulu-workflow` and workflow events and tools. |
| `@worldapptechnologies/nulu-workflow-workerthread`, `WorkerWorkflowEngine` | `@worldapptechnologies/nulu-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@worldapptechnologies/nulu-goal-session`, `goal/goal-session/` | `@worldapptechnologies/nulu-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@worldapptechnologies/nulu-compact`, `ctx.compact`, `CompactService` | `@worldapptechnologies/nulu-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@worldapptechnologies/nulu-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@worldapptechnologies/nulu-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@worldapptechnologies/nulu-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@worldapptechnologies/nulu-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@worldapptechnologies/nulu-settings-local`, `SettingsLocal` | `@worldapptechnologies/nulu-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@worldapptechnologies/nulu-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@worldapptechnologies/nulu-session-title-first-message-llm`, cadence `first-message` | `@worldapptechnologies/nulu-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@worldapptechnologies/nulu-session-title-all-messages-llm`, cadence `all-user-messages` | `@worldapptechnologies/nulu-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@worldapptechnologies/nulu-code-runtime-worker`, `WorkerCodeRuntime` | `@worldapptechnologies/nulu-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@worldapptechnologies/nulu-fs-policy` | `@worldapptechnologies/nulu-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@worldapptechnologies/nulu-skill-local`, `LocalSkillProvider`, provider id `local` | `@worldapptechnologies/nulu-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@worldapptechnologies/nulu-subagent-spawn`, `SpawnProvider` | `@worldapptechnologies/nulu-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@worldapptechnologies/nulu-subagent-fork`, `ForkProvider` | `@worldapptechnologies/nulu-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@worldapptechnologies/nulu-subagent-inprocess`, `subagent-inprocess/` | `@worldapptechnologies/nulu-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `nulu-subagent-nulu-sdk` | `SdkSubagentProvider` | The repeated package qualifier is intentional, and the class must say that it provides subagents through the SDK. |
...
| `@worldapptechnologies/nulu-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@worldapptechnologies/nulu-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@worldapptechnologies/nulu-subagent-nulu-sdk`, its provider id `nulu-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@worldapptechnologies/nulu-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@worldapptechnologies/nulu-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@worldapptechnologies/nulu-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@worldapptechnologies/nulu-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@worldapptechnologies/nulu-timeout-policy` | `@worldapptechnologies/nulu-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@worldapptechnologies/nulu-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@worldapptechnologies/nulu-paths` | `util/home-paths/`, `@worldapptechnologies/nulu-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@worldapptechnologies/nulu-retention` | `util/output-retention/`, `@worldapptechnologies/nulu-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@worldapptechnologies/nulu-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@worldapptechnologies/nulu-frontend-static` | `@worldapptechnologies/nulu-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@worldapptechnologies/nulu-jsonrpc-demo`, `@worldapptechnologies/nulu-sdk-jsonrpc-demo`, `@worldapptechnologies/nulu-sdk-python-runtime` | removed | The Python runtime packages the existing `@worldapptechnologies/nulu` CLI and its `sdk` profile; a private application package would recreate a second launcher. |
...
| `@worldapptechnologies/nulu-acp-demo` | `@worldapptechnologies/nulu-acp-app` | The package is the ACP profile's application bundle, not a standalone demo bin. |
| Deploy-root manifests `nulu-jsonrpc-agent-pkg`, `nulu-sdk-python-runtime-closure` | `nulu-python-runtime-closure` | The zero-code manifest defines the Python runtime wheel's complete `nulu` dependency closure without naming a separate SDK application. |
| `@worldapptechnologies/nulu-frontend` | `@worldapptechnologies/nulu-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `nulu-base`, `nulu-web-app`, `nulu-sdk-app`, `nulu-acp-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@worldapptechnologies/nulu-client-ui-model` | `@worldapptechnologies/nulu-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@worldapptechnologies/nulu-client-ui-models` | `@worldapptechnologies/nulu-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@worldapptechnologies/nulu-client-ui-plugin-config`, `client/ui-plugin-config/` | `@worldapptechnologies/nulu-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@worldapptechnologies/nulu-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@worldapptechnologies/nulu-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@worldapptechnologies/nulu-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@worldapptechnologies/nulu-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@worldapptechnologies/nulu-client-ui-question` | `@worldapptechnologies/nulu-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@worldapptechnologies/nulu-client-ui-command`, `ui-command/` | `@worldapptechnologies/nulu-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@worldapptechnologies/nulu-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@worldapptechnologies/nulu-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@worldapptechnologies/nulu-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@worldapptechnologies/nulu-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `nulu-subagent-nulu-sdk` and provider id `nulu-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@worldapptechnologies/nulu-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@worldapptechnologies/nulu-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `nulu-compact-basic` to `nulu-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product Nulu Harness rather than Nulu Harness SDK.
