...
The harness had no persistent shell on Windows. The persistent `bash` stack was POSIX-only by construction: `@worldapptechnologies/nulu-subprocess-local` threw at terminal allocation (`createProcessInspector()` rejected win32), `@worldapptechnologies/nulu-terminal-bash` was bash-shaped (`/bin/bash` defaults, `PS1`/`PROMPT_COMMAND` environment markers), `@worldapptechnologies/nulu-tool-bash-persistent` wrapped commands in bash syntax, and every pty test skipped on win32. The one-shot `pwsh` tool (`@worldapptechnologies/nulu-tool-pwsh` over `@worldapptechnologies/nulu-pwsh-local`) already ran on Windows, but each call started a fresh `pwsh -Command` process: cwd, `$env:` variables, functions, and interactive children ended with the call, and its README recorded "No persistent shell or PTY" as deferred work.
...
### Windows substrate in `@worldapptechnologies/nulu-subprocess-local`
