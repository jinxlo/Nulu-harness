...
Simple unary operations live on their natural business Remote owner. The business package owns the Remote signature and Host adaptation; `@worldapptechnologies/nulu-api-remotes/client` selects its generated contribution; the Client package owns presentation joins. Connection owns the transport envelope and exact Fetch route registry, and no API Proxy service remains.
...
The native path implementation lives in `@worldapptechnologies/nulu-native-command`. Settings controllers select Host-owned targets, while Session-aware Clients resolve workspace paths before calling `SessionController`; the utility only performs platform detection, WSL translation, browser preference, text-editor intent, and shell-free command execution.
