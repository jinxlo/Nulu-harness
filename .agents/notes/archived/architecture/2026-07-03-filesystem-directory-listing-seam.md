...
Add `FileSystem.listDir(target, signal?)` to `@worldapptechnologies/nulu-fs`.
...
**Keep directory enumeration in each consumer.** Rejected. That would bind product packages such as `nulu-skill` to Node/local filesystem behavior and bypass policy/remote/sandboxed backends.
