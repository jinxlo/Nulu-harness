...
Provider/model selection remains configuration and profile state. The boot composition registers the shipped Nulu, OpenAI, and Anthropic routes; the CLI does not add image-specific selection flags, inspect the yml provider roster, or dynamically mount an adapter.
