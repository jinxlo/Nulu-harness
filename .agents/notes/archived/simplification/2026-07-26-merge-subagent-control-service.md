...
`SubagentRuntime` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@worldapptechnologies/nulu-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@worldapptechnologies/nulu-tool-subagent-control` package injects `ctx.subagents` directly.
...
- The continuation manager is still Task- and persistence-aware inside the `nulu-subagent` package, so the package declares optional peer dependencies on those services even though ordinary `start` callers do not need them.
