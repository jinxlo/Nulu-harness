...
The `standard`, `code`, and `cordis` preset configurations mount `nulu-tool-fs` and `nulu-tool-fs-search`, but do not mount `nulu-tool-str-replace-editor`. PTC mode therefore omits `str_replace_editor` from both its registry and generated SDK. The `minimal` preset continues to mount `nulu-tool-str-replace-editor`, and deployments or user-authored presets may still mount the plugin explicitly.
