...
- `NULU_SESSION_JSONL` no longer exists; shell-env registers no persistence contributor. The variable was only honest with `compression: 'none'` — the default `.jsonl.zstd` artifact is unreadable from bash.
...
- [Session identity and log location](../feature/2026-07-10-agent-session-identity-and-log-location.md) — partially superseded: its `NULU_SESSION_ID` and shell-env registry decisions stand; its `locate`/`NULU_SESSION_JSONL`/`transcript_path` decisions are removed here.
