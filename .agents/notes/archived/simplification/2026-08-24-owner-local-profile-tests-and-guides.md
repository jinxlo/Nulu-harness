...
The `demo:acp` and `demo:cordis` scripts are absent. ACP starts through `nulu --profile acp`; the Cordis guide starts `nulu web` with its explicit overlay. `demo:ptc` remains as a thin wrapper over `nulu --profile headless` with `NULU_TOOLS_MODE=ptc`.
