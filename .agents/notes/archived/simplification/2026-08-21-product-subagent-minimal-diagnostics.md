...
| Exit code and signal | `nulu-subprocess` handle | Displayed independently when observed, without inferring missing values |
...
| Diagnostic delivery and byte limit | `nulu-subagent` and its foreground or Job consumers | One bounded optional string remains separate from assistant output |
