...
The SDK project toolchain that remained as the TUI package's final consumer is deleted by the [toolchain removal decision](2026-08-11-remove-sdk-project-toolchain.md). Host applications may still mount the provider-neutral `nulu-user-questions`, `nulu-commands`, and presentation services directly.

This decision supersedes the reusable-package retention in [the explicit-config `nulu` entrypoint decision](../../archived/simplification/2026-08-03-explicit-config-nulu-entrypoint.md) and the current applicability of the archived TUI implementation notes. Their historical records remain frozen, but they are not authority for the supported package or application inventory.
...
Nulu Harness has no terminal UI package. Existing imports and `cordis.yml` rows that depend on the package fail instead of being translated. Web remains the shipped interactive surface; ACP, JSON-RPC, and one-shot CLI remain the non-Web entry points.
