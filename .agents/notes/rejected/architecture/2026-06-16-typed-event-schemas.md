...
- **Six merge-extensible maps** (~370 LOC of core types): `ContentBlockMap`, `MessageSourceMap`, `FinishReasonMap` (in `nulu-llm`); `TurnTriggerMap`, `TurnEndReasonMap`, `SessionEventMap` (in `nulu-session`).
- **~10 `declare module` augmentation sites** across `nulu-agent`, `nulu-agent-loop`, `nulu-shell`, `nulu-llm`, `nulu-session`, `nulu-session-persistence`, `nulu-system-prompt`, `nulu-tools` — each would move from declaration merging to a runtime `register()` call.
- **The event producers** — 16 `session.append(...)` call sites in the loop — unchanged in shape but now validated at the boundary.
- **~7 switch-consumers** that branch on these unions: `deriveMessages` and the package-owned invariant companion (`nulu-session`), `BlockAssembler` (`nulu-llm`), both LLM adapters (`nulu-llm-deepseek`, `nulu-llm-pi-ai`), and the tool schema layer (`nulu-tools`). The `assertNever`-on-closed-unions vs fall-through-on-extensible-unions convention (a documented lint rule) would need rethinking — runtime variants are not statically exhaustive.
- **The `defineTool` `InferArgs` DSL** (`nulu-tools`), which derives zero-cast `execute` arg types from a compile-time schema spec — the showcase of the current approach.
