...
Compaction is split between `@worldapptechnologies/nulu-compaction`, which owns an abstract two-method service and shared types, and `@worldapptechnologies/nulu-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@worldapptechnologies/nulu-compaction` and remove `@worldapptechnologies/nulu-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@worldapptechnologies/nulu-compaction-basic` and its workspace/package metadata are removed.
- `@worldapptechnologies/nulu-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
