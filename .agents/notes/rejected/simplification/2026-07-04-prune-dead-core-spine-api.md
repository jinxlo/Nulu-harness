...
- `nulu-llm-gateway`: `httpErrorCode`, `serializeMessages`, `serializeRequest`, `DONE`, `parseSse`, `mapFinishReason`, `mapUsage`, and `translate`; `nulu-llm-pi-ai`: `buildModel`, `mapStopReason`, `mapUsage`, `toPiContext`, and `toStreamChunks`.
...
- `nulu-web-fetch-http`: `classifyContentType`, `decoderForCharset`, `isSameOrigin`, `parseCharset`, and `validateFetchUrl`; `nulu-web-search-exa`: `mapExaResponse` and `mapExaResult`; `nulu-web-search-gateway`: `citationSnippets` and `mapAnthropicResponse`; `nulu-web-search-perplexity`: `mapPerplexityResponse` and `mapPerplexityResult`.
