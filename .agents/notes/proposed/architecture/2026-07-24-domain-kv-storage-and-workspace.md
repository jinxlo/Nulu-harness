...
| --- | --- | --- | --- |
| `@worldapptechnologies/nulu-storage` | `packages/storage/storage/` | `ctx.storage` (the hub) | ✓ |
| `@worldapptechnologies/nulu-storage-json` | `packages/storage/storage-json/` | registers backend `json` | ✓ |
| `@worldapptechnologies/nulu-storage-sqlite` | `packages/storage/storage-sqlite/` | registers backend `sqlite` | ✓ |
| `@worldapptechnologies/nulu-storage-domain` | `packages/storage/storage-domain/` | mounts `ctx.storage.domain` | ✓ |
| `@worldapptechnologies/nulu-workspace` | `packages/workspace/workspace/` | `ctx.workspaceRegistry` | ✓ |
...
(workspace lives in its own group rather than `packages/host/`: the host group's naming rule requires the `nulu-host-*` prefix while this package is named `nulu-workspace`; and the workspace entity is a domain concept, not bound to the host assembly tier. Unrelated to the existing `agent-instructions` package — that is an AGENTS.md instruction loader.)

Dependency direction: `nulu-workspace` → `nulu-domain` → `nulu-storage` ← the two backends. `nulu-workspace` additionally depends on the read-only face of `ctx.sessionPersistence` (attach's cwd check reads the session header; when the service is absent, attach rejects outright — no verification, no bookkeeping). The `ctx.sessions` running-check for session deletion moves into future work together with the cascade.

### `nulu-storage`: the storage hub

A pure registration hub, no IO of its own, no Config. The `Storage` service mounts at `ctx.storage` with two faces: `backend` (a `BackendRegistry`: `register(name, backend)` returns the disposer, duplicate names throw; `get(name)` throws `backend-not-found` for unknown names) and data-form mounting (`mount(form, facility)` over the merge-extensible `StorageForms` map, into which `nulu-domain` merges the `domain` key; unmounted access throws `form-not-mounted`). The signature text lives in `packages/storage/storage/src/index.ts` and `src/registry.ts`.

**Multiple backends stay mounted side by side**; which backend serves a domain is `nulu-domain`'s configuration (below), never a global either-or. Disposer semantics = remove the name from the table; closing the backend itself belongs to the backend package's effect closure, unregister first then close.
...
### `nulu-domain`: the domain data form
...
export interface Workspace {
  ...
  readonly sessionIds: readonly SessionId[]      // single source of truth and ordered: array order is display order
...
export class WorkspaceRegistry extends Service {
  ...
  //          entity cache Map<WorkspaceId, WorkspaceEntity> rebuilt
  create(path: string, title?: string): Promise<Workspace>   // collision after realpath → reject
...
| --- | --- | --- |
| JSONL: temp write + fsync + link/unlink atomic publish, 0o700/0o600 permissions, Windows variant (win32.ts) | pure medium | copied by `nulu-storage-json` this phase (whole-file atomic rewrite is the same protocol); becomes the shared implementation at migration |
