...
export interface ClientDeclaration {
  ...
  /** Exact runtime specifiers used to validate `nulu.client.external` declarations. */
 ...
 * @returns Declarations and malformed nulu.client fields.
 */
...
/**
 * Repair malformed or redundant `nulu.client` declaration entries.
 * @param root - Absolute repository root.
...
export function fixClientPackageManifests(root: string, facts: ClientPackageFacts): string[] {
  ...
  for (const declaration of facts.declarations.filter(entry => entry.dynamic)) {
    ...
    const nulu = isRecord(target.manifest.nulu) ? target.manifest.nulu : undefined
    const client = isRecord(nulu?.client) ? nulu.client : undefined
...
function collectModeViolations(facts: ClientPackageFacts): string[] {
  ...
  for (const pkg of facts.packages) {
    if (pkg.dynamic && pkg.staticLinked) {
      violations.push(
        pkg.manifest + ': ' + pkg.name + ' declares nulu.client and uses the staticLinked preset;'
      ...
      violations.push(
        ...
        + ' declare nulu.client or use the staticLinked preset',
      )
    }
  }
  ...
  for (const specifier of facts.preloadedExternals) {
    if (rowPackageOf(specifier, rows) === undefined) {
      violations.push(
        ...
        + ' has no dynamic nulu.client row',
      )
    }
  ...
  for (const pkg of facts.declarations.filter(entry => entry.dynamic)) {
    for (const field of ['external', 'inject'] as const) {
      ...
      for (const value of pkg[field]) {
        if (value === '') violations.push(pkg.manifest + ': nulu.client.' + field + ' contains an empty value')
        else if (seen.has(value)) {
          violations.push(pkg.manifest + ': nulu.client.' + field + ' lists ' + JSON.stringify(value) + ' twice')
        }
        seen.add(value)
      }
    }
    ...
    for (const specifier of new Set(pkg.external)) {
      ...
      if (baseline.has(specifier)) {
        violations.push(
          pkg.manifest + ': nulu.client.external repeats baseline module ' + JSON.stringify(specifier)
      ...
      if (supplier === pkg.name) {
        violations.push(pkg.manifest + ': nulu.client.external names its own row ' + JSON.stringify(specifier))
        ...
        if (pkg.runtimeSourceSpecifiers[specifier] === undefined) {
          violations.push(
            pkg.manifest + ': nulu.client.external ' + JSON.stringify(specifier)
        ...
        violations.push(
          pkg.manifest + ': nulu.client.external ' + JSON.stringify(specifier) + ' has no supplier;'
          + (byName.has(owner)
            ? ' workspace package ' + owner
              + ' declares no dynamic nulu.client row and the shell does not seed this specifier'
...
function formatCycle(
  ...
  return manifest + ': synchronous nulu.client.external cycle: ' + chain + ' ' + (entry?.from ?? '')
}
...
function readDeclaration(
  ...
  const nulu = isRecord(manifest.nulu) ? manifest.nulu : undefined
  const rawClient = nulu?.client
  ...
  if (!isRecord(rawClient)) {
    malformed.push(manifestPath + ': ' + manifest.name + ' nulu.client must be an object')
    return {
      name: manifest.name, manifest: manifestPath, dynamic: false, external: [], inject: [],
      runtimeSourceUses: {}, runtimeSourceSpecifiers: {},
    }
...
function stringArray(
  ...
  if (!Array.isArray(value) || value.some(entry => typeof entry !== 'string')) {
    malformed.push(manifestPath + ': ' + packageName + ' nulu.client.' + field + ' must be a string array')
    return []
  }
