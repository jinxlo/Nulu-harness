...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'nulu-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@worldapptechnologies/nulu-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@worldapptechnologies/nulu-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@worldapptechnologies/nulu-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@worldapptechnologies/nulu-client-consumer' })
