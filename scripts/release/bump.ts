 ...
 *
 * The nulu family shares one version across its publishable members, private
...
/** Release types the nulu family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the nulu family's version. */
...
/** One private nulu workspace whose version follows the publishable family. */
interface PrivateNuluVersion {
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:nulu <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package and application manifests that share the nulu version
 * without joining its publish set.
 * @param root - repository root.
 * @returns Private workspace manifests sorted by path.
 */
function privateNuluVersions(root: string): PrivateNuluVersion[] {
...
/**
 * Plan the nulu family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the nulu family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateNuluVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <nulu|vendor> [version]')
  ...
  if (family.id === 'nulu') {
    ...
    if (request === undefined) throw new Error('usage: release:nulu <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:nulu takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
