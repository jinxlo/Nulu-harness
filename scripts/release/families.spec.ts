...
describe('release families', () => {
  it('publishes all current experimental packages', () => {
    ...
    expect(members
      ...
      '@worldapptechnologies/nulu-experimental-browser-use-runtime',
  ...
  it('publishes unlisted experimental packages while retaining private exclusions', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-release-experimental-'))
    roots.push(root)
    ...
    write(join(root, 'packages/experimental/inspector/package.json'), JSON.stringify({
      name: '@worldapptechnologies/nulu-experimental-inspector',
      version: '0.0.1',
      private: true,
    }))

    expect(releaseFamily('nulu').members(root).map(entry => entry.name)).toEqual([
      '@worldapptechnologies/nulu-experimental-prototype',
    ])
  })
