 ...
 * owns: `nulu` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@worldapptechnologies/nulu-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@worldapptechnologies/')) throw new Error(`${normalized} must name an @worldapptechnologies package`)
...
class NuluFamily extends ReleaseFamily {
  readonly id = 'nulu'
  readonly patterns = [
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`nulu release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@worldapptechnologies/nulu', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@worldapptechnologies/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new NuluFamily(), new VendorFamily()]
}
