...
const roots: string[] = []
const experimental = '@worldapptechnologies/nulu-experimental-example'

function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'nulu-installed-isolation-'))
  roots.push(root)
  return root
}
...
describe('installed default-product isolation', () => {
  it('ignores development dependencies and unrelated installed experimental packages', () => {
    const root = fixture()
    const entry = writePackage(root, '@worldapptechnologies/nulu', {
      dependencies: { core: '1.0.0' },
      devDependencies: { [experimental]: '1.0.0' },
    })
    writePackage(root, 'core', { peerDependencies: { '@worldapptechnologies/nulu': '1.0.0' } })
  ...
  it.each(['dependencies', 'optionalDependencies', 'peerDependencies'])(
    ...
    (section) => {
      const root = fixture()
      const entry = writePackage(root, '@worldapptechnologies/nulu', { dependencies: { thirdParty: '1.0.0' } })
      writePackage(root, 'thirdParty', { [section]: { [experimental]: '1.0.0' } })

      expect(() => verifyInstalledProductIsolation(entry)).toThrow(
        `@worldapptechnologies/nulu -> thirdParty -> ${experimental}`,
      )
    },
  )
  ...
  it('rejects experimental identities hidden behind an installed alias', () => {
    const root = fixture()
    const entry = writePackage(root, '@worldapptechnologies/nulu', { dependencies: { safeName: 'file:../prototype' } })
  ...
  it('follows nested installations rather than an unrelated hoisted package', () => {
    const root = fixture()
    const entry = writePackage(root, '@worldapptechnologies/nulu', { dependencies: { core: '1.0.0' } })
  ...
  it('permits missing optional packages and optional peers but rejects missing dependencies', () => {
    ...
    writePackage(root, '@worldapptechnologies/nulu', { dependencies: { missing: '1.0.0' } })
