/**
 * Generate nulu-scope's invariant resolver map from the repository TypeScript
 * Program.
 *
 ...
 * `Events` parameter tuple. Zero matches require `@nuluScopeScan unsupported`;
        ...
        if (ts.isInterfaceDeclaration(node) && node.name.text === 'Events' && isCordisModuleInterface(node)) {
          for (const member of node.members) {
            ...
            if (!scopedBase) {
              ...
              if (tag.present) {
                this.violations.push(`${where} has @nuluScopeScan metadata but is not a Scoped event`)
              }
              continue
            }
            if (!raw.includes(SCOPE_DOC_MARKER)) {
            ...
            if (candidates.length === 0) {
              if (!tag.unsupported) {
                ...
                this.violations.push(
                  ...
                  + 'add @nuluScopeScan unsupported only when the key is intentionally absent from the payload',
                )
              }
            ...
            if (tag.unsupported) {
              this.violations.push(
                `${where} has unnecessary @nuluScopeScan unsupported; ${candidates[0]?.path} exposes the routing key`,
              )
              continue
            }
...
/** Return whether an Events interface is inside declare module '@worldapptechnologies/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@worldapptechnologies/cordis'
}
...
/** Parse and validate the optional @nuluScopeScan unsupported tag. */
function parseScopeTag(raw: string, where: string, violations: string[]): ScopeTag {
  const tags = raw
    ...
    .filter(line => line.startsWith('@nuluScopeScan'))
  if (tags.length > 1) violations.push(`${where} has multiple @nuluScopeScan tags`)
  if (tags.length === 0) return { present: false, unsupported: false }
  const unsupported = tags[0] === '@nuluScopeScan unsupported'
  if (!unsupported) {
    violations.push(
      `${where} has invalid scoped-event scan metadata '${tags[0]}'; expected '@nuluScopeScan unsupported'`,
    )
  }
...
/** Generate or freshness-check the fixed nulu-scope source file. */
function main(): void {
