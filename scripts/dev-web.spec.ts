...
it('samples one local environment at startup without validating watcher outputs', async () => {
  const root = await mkdtemp(join(tmpdir(), 'nulu-dev-web-environment-'))
  try {
    ...
    writeClientBuildRecord(root, {
      ...
      NULU_CLIENT_COMMIT_HASH: 'fffffff',
      NULU_CLIENT_TITLE: 'Nulu Harness',
      NULU_CLIENT_VERSION: '1.2.2',
    })
    ...
    expect(devWebBuildEnvironment(root, {
      ...
      NULU_CLIENT_COMMIT_HASH: 'abc1234',
      NULU_CLIENT_EXTRA: 'launch-value',
    })).toEqual({
      PATH: '/bin',
      NULU_CLIENT_COMMIT_HASH: 'abc1234',
      NULU_CLIENT_EXTRA: 'launch-value',
...
it('discovers nulu.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'nulu-dev-web-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'nulu-dev-web-library-'))
  try {
    ...
    await write('packages/client/delivered', { nulu: { client: { platform: 'web' } } }, clientPreset)
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'nulu-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@nulu-test/dev-web-watch', private: true, type: 'module' }))
