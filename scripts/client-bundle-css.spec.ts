...
function cssPlugin(name: 'nulu-css-modules-inline' | 'nulu-css-global-inline' | 'nulu-css-text-inline'): CssPlugin {
  const configs = clientBundle(
    '@worldapptechnologies/nulu-client-test',
...
describe('client bundle global CSS', () => {
  it('compiles a side-effect stylesheet into a watched style injector', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-client-global-css-watch-'))
    try {
      const stylesheet = join(root, 'base.css')
      const importer = join(root, 'index.ts')
      await writeFile(stylesheet, 'body { color: red; }\n')
      const plugin = cssPlugin('nulu-css-global-inline')
  ...
  it('compiles inline stylesheets as watched text without a module side effect', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-client-inline-css-watch-'))
    try {
      const stylesheet = join(root, 'base.css')
      const importer = join(root, 'index.ts')
      await writeFile(stylesheet, 'body { color: red; }\n')
      const plugin = cssPlugin('nulu-css-text-inline')
