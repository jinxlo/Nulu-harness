...
const RELEASE_ENTRY_PACKAGE = '@worldapptechnologies/nulu'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"nulu web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed nulu web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed nulu web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'nulu-npm-baseline-'))
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@worldapptechnologies/')) {
        throw new Error(`${manifestPath} must name an @worldapptechnologies package`)
      }
      if (name === '@worldapptechnologies/nulu-root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped nulu entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'nulu-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'nulu-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@worldapptechnologies/nulu/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed nulu bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed nulu --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed nulu entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed nulu Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-nulu-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@worldapptechnologies/') || name === '@worldapptechnologies/nulu-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.NULU_HOME = resolve(consumerRoot, '.nulu')
  environment.NULU_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.NULU_TELEMETRY_DISABLED = '1'
  environment.WORLD_APP_TECHNOLOGIES_API_KEY = 'keyless-installed-web-no-call'
