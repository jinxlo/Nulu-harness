/**
 * Rescope the vendored Cordis packages into the `@worldapptechnologies` scope, and undo
 ...
 * `@worldapptechnologies/nulu-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@worldapptechnologies/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@worldapptechnologies/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@worldapptechnologies/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@worldapptechnologies/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@worldapptechnologies/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@worldapptechnologies/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@worldapptechnologies/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@worldapptechnologies/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@worldapptechnologies/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@worldapptechnologies/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@worldapptechnologies/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@worldapptechnologies\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@worldapptechnologies/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@worldapptechnologies/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@worldapptechnologies/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@worldapptechnologies/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@worldapptechnologies/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@worldapptechnologies/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@worldapptechnologies` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@worldapptechnologies\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@worldapptechnologies/cordis']
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@worldapptechnologies/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@worldapptechnologies/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@worldapptechnologies/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@worldapptechnologies/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@worldapptechnologies/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @worldapptechnologies, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@worldapptechnologies\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@worldapptechnologies/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@worldapptechnologies\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@worldapptechnologies/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
