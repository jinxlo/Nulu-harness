...
import { collections, docsPages, orderedPages, type DocsPage } from '../website/docs.ts'
import {
...
const REPOSITORY_URL = 'https://github.com/worldapptechnologies/nulu-harness'
...
function githubTarget(
  ...
  if (image) return `https://raw.githubusercontent.com/worldapptechnologies/nulu-harness/${repositoryRef}/${path}${suffix}`
...
/**
 * Drop the badge that addresses a canonical page's GitHub reader.
 *
 * The site links the repository from every page, so projecting the badge would
 * repeat that link as the last element of each page.
 *
  ...
  for (const page of entries) {
    ...
    const projected = rewriteMarkdown(markdown, {
      ...
      placeImage: (absPath) => {
        ...
        // Beside the page that references it, under its own basename, so one
        // relative URL is correct from the emitted page.
