...
export function packageReadmeSummaryErrors(file: string, source: string): string[] {
  ...
  const template = kind === undefined
    ? '.agents/skills/nulu-doc/templates/'
    : `.agents/skills/nulu-doc/templates/${kind}.md`
  return [
    `${file}: Summary has ${String(words)} words; the limit is ${String(MAX_PACKAGE_README_SUMMARY_WORDS)}. Read .agents/skills/nulu-doc/SKILL.md and ${template} before rewriting it.`,
  ]
}
