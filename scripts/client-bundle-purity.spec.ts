...
const REQUESTING_PACKAGE = '@worldapptechnologies/nulu-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@worldapptechnologies/nulu-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { NULU_BUILD_FACE: 'client' } })
...
function purityResolveId(id = REQUESTING_PACKAGE): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'nulu-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'nulu-css-modules-inline')
...
function sourceMapPlugin(): SourceMapPlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'nulu-tsc-sourcemap')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@worldapptechnologies/nulu-client-store')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-client-ui-slots')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@worldapptechnologies/nulu-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@worldapptechnologies/nulu-session/surface')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-brand')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-deque')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-util-values')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-token-meter/client')).toBeNull()
    expect(() => resolveId('@worldapptechnologies/nulu-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-token-meter/client/internal')).toThrow(/purity/)
    expect(resolveId('@worldapptechnologies/nulu-host-open-in-app/shared')).toBeNull()
    expect(() => resolveId('@worldapptechnologies/nulu-host-open-in-app')).toThrow(/purity/)
  })

  it('admits only the pure spill notice entry, not its Host policy', () => {
    expect(resolveId('@worldapptechnologies/nulu-spill-policy/notice')).toBeNull()
    expect(resolveId('@worldapptechnologies/nulu-output-retention')).toBeNull()
    expect(() => resolveId('@worldapptechnologies/nulu-spill-policy')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-spill-policy/notice/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@worldapptechnologies/nulu-goal/remote')).toBeNull()
    expect(() => resolveId('@worldapptechnologies/nulu-goal')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @worldapptechnologies leak', () => {
    expect(() => resolveId('@worldapptechnologies/nulu-agent')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@worldapptechnologies/nulu-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@worldapptechnologies/nulu-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@worldapptechnologies/nulu-api-session-controller')
    expect(requesting('@worldapptechnologies/nulu-api-gateway/client')).toBeNull()
    expect(() => resolveId('@worldapptechnologies/nulu-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@worldapptechnologies/nulu-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@worldapptechnologies/nulu-client-store')).toBe(true)
  })
})
...
describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@worldapptechnologies/nulu-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@worldapptechnologies/nulu-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@worldapptechnologies/nulu-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@worldapptechnologies/nulu-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@worldapptechnologies/nulu-client-fixture', { external: 'react' }))
      .toThrow(/nulu\.client\.external must be a string array/)
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('chains emitted tsc maps when the production Client build consumes lib/types', async () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-client-sourcemap-'))
    try {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@worldapptechnologies/nulu-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://nulu.test/plugins/@worldapptechnologies/nulu-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@worldapptechnologies/nulu-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@worldapptechnologies/nulu-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://nulu.test/plugins/@worldapptechnologies/nulu-client-connection/client.js.map')
