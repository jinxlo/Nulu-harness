...
/** Drop the `@worldapptechnologies/nulu-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@worldapptechnologies/nulu-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @worldapptechnologies/nulu-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
