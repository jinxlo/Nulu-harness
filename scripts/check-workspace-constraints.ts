...
export function expectedNuluPackageFiles(manifest: PackageManifest): readonly string[] {
 ...
 */
export function checkNuluFamilyVersion(manifest: PackageManifest, expected: string | undefined): string | undefined {
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const label = manifest.name ?? dir
  const familyVersionError = checkNuluFamilyVersion(manifest, repositoryVersion)
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@worldapptechnologies/nulu-')) {
    ...
    const expectedFiles = expectedNuluPackageFiles(manifest)
