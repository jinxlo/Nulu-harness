...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'nulu-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=nulu-translation-pairing\n')
  return fixture
}
...
// The value matches NULU_COVERAGE_TEST_TIMEOUT_MS, which the Windows coverage
