 ...
 * consumes the briefing is `.agents/skills/nulu-translate-docs/SKILL.md`.
 */
...
function maskCodeSpans(markdown: string, spans: MarkdownSpan[]): string {
  return replaceSpanTexts(markdown, spans, new Map(spans.map(span => [span.index, `NULU_TRANSLATION_CODE_${span.index}\n`])))
}
