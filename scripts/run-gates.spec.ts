...
describe('gate graph validation', () => {
  ...
  it('keeps the hygiene aggregate aligned with the package script checks', () => {
    ...
    expect(ids).toEqual([
      ...
      'nulu-package-licenses', 'package-invariants', 'built-package-invariants', 'node-next-types',
  ...
  it.each(['ci-primary', 'ci-static', 'check-all'] as const)(
    'keeps the NULU package license policy in %s',
    (mode) => {
      const ids = withPnpmEntrypoint(() => gatesForMode(mode).map(subject => subject.id))

      expect(ids).toContain('nulu-package-licenses')
    },
  )
  ...
  it('applies one configured test, polling, and hook timeout to both coverage gates', () => {
    const gates = withEnv('NULU_COVERAGE_TEST_TIMEOUT_MS', '15000', () =>
  ...
  it('keeps Vitest timeout defaults when the coverage override is absent', () => {
    const gates = withEnv('NULU_COVERAGE_TEST_TIMEOUT_MS', undefined, () =>
      withPnpmEntrypoint(() => gatesForMode('ci-windows-complete')))
  ...
  it('rejects an invalid coverage timeout before starting a gate', () => {
    expect(() => withEnv('NULU_COVERAGE_TEST_TIMEOUT_MS', '0', () =>
      withPnpmEntrypoint(() => gatesForMode('ci-windows-complete'))))
      .toThrow('NULU_COVERAGE_TEST_TIMEOUT_MS must be a positive integer')
  })

  it('selects partitioned coverage only when explicitly configured', () => {
    const coverage = withEnv('NULU_COVERAGE_PARTITIONS', '3', () =>
      withPnpmEntrypoint(() => gatesForMode('ci-windows-complete').find(subject => subject.id === 'coverage')))

    expect(coverage).toMatchObject({
      displayCommand: 'NULU_COVERAGE_PARTITIONS=3 pnpm run test:coverage:partitioned',
      args: ['/private/pnpm.cjs', 'run', 'test:coverage:partitioned'],
      env: { NULU_COVERAGE_EXEMPT_HEAVY: '1' },
      streamOutput: true,
    })
  })

  it('rejects an invalid coverage partition count before starting a gate', () => {
    expect(() => withEnv('NULU_COVERAGE_PARTITIONS', '1', () =>
      withPnpmEntrypoint(() => gatesForMode('ci-windows-complete'))))
      .toThrow('NULU_COVERAGE_PARTITIONS must be an integer greater than 1')
  })
...
describe('Oxlint gate', () => {
  it('uses the package script when no worker bound is configured', () => {
    const subject = withEnv('NULU_OXLINT_THREADS', undefined, () =>
  ...
  it('surfaces the configured worker bound on the shared package script', () => {
    const subject = withEnv('NULU_OXLINT_THREADS', '4', () =>
      withPnpmEntrypoint(() => gatesForMode('ci-lint-contracts-ready')[0]))

    expect(subject).toMatchObject({
      id: 'lint',
      displayCommand: 'NULU_OXLINT_THREADS=4 pnpm run lint:contracts-ready',
      command: process.execPath,
      args: ['/private/pnpm.cjs', 'run', 'lint:contracts-ready'],
    })
  })
...
describe('Node 24 lane ownership', () => {
  ...
  it('owns the build and orders its artifact consumers', () => {
    ...
    expect(subject.find(item => item.id === 'snapshot')?.env).toEqual({ NULU_EXAMPLE_MODE: 'lib' })
    expect(subject.find(item => item.id === 'expected-output')?.env).toEqual({ NULU_EXAMPLE_MODE: 'lib' })
    expect(subject.find(item => item.id === 'doc-typecheck')?.env).toEqual({
      NULU_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1',
    })
