 ...
 *
 * Node's built-in `fetch` routes through undici's global dispatcher, which `@worldapptechnologies/nulu-http-proxy`
...
function main(): void {
  ...
  console.error('verify-no-bare-dispatcher: a dispatcher built outside @worldapptechnologies/nulu-http-proxy bypasses the configured proxy.\n')
  for (const violation of violations) {
    console.error(`  ${relative('.', violation.file)}:${String(violation.line)} ${violation.what}`)
    console.error(`    ${violation.text}`)
  }
  console.error('\nUse `proxyRouteFor(url)` from @worldapptechnologies/nulu-http-proxy, or annotate the line')
