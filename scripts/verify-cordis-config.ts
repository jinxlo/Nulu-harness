 ...
 * silently changes composition. Shipped and test-only nulu overlays resolve
...
// These overlays are consumed by the built nulu app, so their bare specifiers
...
const CHOOSER_PACKAGE = '@worldapptechnologies/nulu-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@worldapptechnologies/nulu-host-directory-picker-native',
  '@worldapptechnologies/nulu-host-directory-picker-browse',
  '@worldapptechnologies/nulu-client-ui-directory-picker-browse',
 ...
 * `nulu.client` block, and the node half of a surface plugin is an empty
 ...
 * `nulu.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.nulu?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no nulu.client, so its browser half is never served`
      : `${manifestPath}: declares nulu.client but exports no "./client" entry to serve`]
  })
}
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@worldapptechnologies/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // nulu app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.nulu?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).nulu?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `nulu` source
