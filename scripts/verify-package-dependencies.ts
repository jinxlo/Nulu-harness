...
export function discoverPackageDependencyScope(
  ...
  for (const name of include) {
    ...
    if (pkg !== undefined
      && (pkg.manifestPath.startsWith('packages/client/') || hasClientDeclaration(pkg.manifest.nulu))) {
  ...
  for (const name of exclude) {
    ...
    } else if (pkg !== undefined && !hasClientDeclaration(pkg.manifest.nulu)) {
      violations.push(`clientFaceExclude names ${name}, which declares no nulu.client entry`)
    }
  }
  ...
  for (const pkg of packages) {
    ...
    const clientHost = (hasClientDeclaration(pkg.manifest.nulu) || include.has(pkg.name)) && !exclude.has(pkg.name)
...
export function collectRuntimeSourceExportUses(path: string, source: string): RuntimeSourceExportUse[] {
  ...
  for (const statement of sourceFile.statements) {
    if (!ts.isImportDeclaration(statement)
      ...
      || statement.moduleSpecifier.text !== '@worldapptechnologies/nulu-lazy-require') continue
...
export function readPackageDependencyFacts(
  ...
  const inject = pkg.manifest.nulu?.client?.inject ?? []
...
export function expectedPackageDependencies(
  ...
  for (const name of facts.clientInject) {
    if (facts.workspaceNames.has(name)) add(name, 'devDependencies', 'nulu.client.inject')
  }
