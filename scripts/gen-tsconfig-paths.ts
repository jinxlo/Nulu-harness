 ...
 * `@worldapptechnologies/nulu-*` listed 49 candidate globs and `@worldapptechnologies/nulu-*\/invariant`
...
const PREFIX = '@worldapptechnologies/nulu-'

/** One workspace package the generated region maps. */
interface PackageAlias {
  /** Bare specifier, e.g. `@worldapptechnologies/nulu-session`. */
 ...
 * @returns Every directory whose manifest names a `@worldapptechnologies/nulu-` package and that carries `src`.
 */
 ...
 * `@worldapptechnologies/nulu-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@worldapptechnologies/nulu-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@worldapptechnologies\/nulu-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@worldapptechnologies\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
