/**
 * Enforce nulu profiles as the only supported Node application launcher.
...
interface DemoPolicy {
  readonly kind: 'nulu-direct' | 'nulu-wrapper'
...
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ['apps/cli/package.json', { nulu: 'lib/bin.js' }],
  ['packages/experimental/webworker-packer/package.json', { 'nulu-pack-vfs-image': './bin.js' }],
])
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ['apps/cli/src/bin.ts', 'supported nulu application launcher'],
  ...
  ['packages/subagent/subagent-nulu-sdk/tests/fixtures/loader/driver.ts', 'test-only subprocess driver'],
...
/** Root demos are application wrappers and therefore must visibly select nulu. */
const ROOT_DEMO_POLICIES = new Map<string, DemoPolicy>([
  ['demo:ptc', { kind: 'nulu-wrapper', wrapper: 'scripts/demo-ptc.mjs' }],
  ['demo:inspector', { kind: 'nulu-direct' }],
])
...
function manifestBinViolations(root: string): string[] {
  ...
  for (const rawPath of manifests) {
    ...
    if (expected === undefined) {
      failures.push(`${path}: package bin bypasses the nulu launcher; applications use apps/cli profiles`)
      continue
    }
...
function referencesNuluCli(source: string): boolean {
...
function rootDemoViolations(root: string): string[] {
  ...
  for (const [name, commandValue] of Object.entries(manifest.scripts ?? {}).sort(([left], [right]) => left.localeCompare(right))) {
    ...
    if (policy === undefined) {
      failures.push(`package.json scripts.${name}: demo launcher has no explicit nulu or in-process classification`)
      continue
    }
    if (policy.kind === 'nulu-direct') {
      if (!referencesNuluCli(command)) failures.push(`package.json scripts.${name}: application demo must launch apps/cli/src/bin.ts`)
    ...
    if (!referencesNuluCli(source)) failures.push(`${wrapper}: application demo wrapper must launch apps/cli/src/bin.ts`)
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (failures.length > 0) {
    ...
    console.log('verify-application-entrypoints: nulu is the only supported Node application launcher.')
  }
}
