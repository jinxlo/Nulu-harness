...
describe('repository link policy', () => {
  it('rejects encoded and case-varied references to the unavailable repository', () => {
    const unavailableOwner = ['nulu', 'ai'].join('-')
    const unavailableName = ['nulu', 'harness', 'sdk'].join('-')
    ...
    const source = [
      'https://github.com/worldapptechnologies/nulu-harness',
  ...
  it('preserves frozen archived Agent Notes', () => {
    const unavailableRepository = ['worldapptechnologies', 'nulu-harness-sdk'].join('/')
