...
const TARGET_PACKAGE = '@worldapptechnologies/nulu'
...
export async function resolveNpmPackageLock(
  ...
  const consumer = mkdtempSync(join(tmpdir(), 'nulu-npm-resolution-'))
  try {
    writeFileSync(join(consumer, 'package.json'), `${JSON.stringify({
      name: 'nulu-npm-resolution-benchmark',
      version: '0.0.0',
      private: true,
      dependencies,
    }, null, 2)}\n`)
 ...
 * @param index - Package metadata exposed through the local registry.
 * @param targetVersion - Version of `@worldapptechnologies/nulu` to install.
 * @param timeoutMs - Hard wall-clock limit for the npm child process.
