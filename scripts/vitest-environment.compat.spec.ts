...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('nulu-vitest-storage-probe', 'available')

    expect(localStorage.getItem('nulu-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('nulu-vitest-storage-probe')
  })
})
