...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@worldapptechnologies/nulu-session')
    expect(session).toEqual({
      specifier: '@worldapptechnologies/nulu-session',
    ...
    expect(aliases.some(alias => alias.specifier === '@worldapptechnologies/nulu-typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@worldapptechnologies/nulu-a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@worldapptechnologies/nulu-b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@worldapptechnologies/nulu-a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@worldapptechnologies/nulu-a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@worldapptechnologies/nulu-b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ['@worldapptechnologies/nulu-a', '@worldapptechnologies/nulu-b'],
      new Set(['@worldapptechnologies/nulu-a', '@worldapptechnologies/nulu-a/invariant']),
    )).toEqual(['@worldapptechnologies/nulu-b'])

    expect(uncoveredPackages(['@worldapptechnologies/nulu-a'], new Set(['@worldapptechnologies/nulu-a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@worldapptechnologies/nulu-typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@worldapptechnologies/nulu-*":')
    expect(config).not.toContain('"@worldapptechnologies/nulu-*/invariant":')
  })
})
