...
function gitSupportsObjectFormat(format: 'sha256'): boolean {
  const root = mkdtempSync(join(tmpdir(), 'nulu-git-object-format-'))
  try {
...
describe('translation pairing snapshots', () => {
  it('stores exact uncommitted bytes for later recovery by object ID', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-translation-pairing-'))
    try {
      ...
      expect(execFileSync('git', [
        '-C', root, 'rev-parse', `refs/nulu/translation-pairing/snapshots/${objectId}`,
  ...
  it('reads staged bytes independently of the working tree', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-translation-pairing-index-'))
    try {
      execFileSync('git', ['init', '--quiet', root], {
        env: { ...process.env, GIT_DEFAULT_HASH: 'sha1' },
      })
...
describe('translation pairing switchers', () => {
  ...
  it('accepts only the canonical public URL for an absolute switcher', () => {
    ...
    const canonicalMarkdown = '# README\n\nEnglish | [中文](https://github.com/worldapptechnologies/nulu-harness/blob/master/python/sdk/README.zh.md)\n'
    const canonical = parseTranslationMarkdown(canonicalMarkdown)
    const wrongMarkdown = '# README\n\nEnglish | [中文](https://github.com/worldapptechnologies/nulu-harness/blob/master/other/README.zh.md)\n'
    ...
    expect(translationStructureSignature(wrongPath, targets, {
      ...
      'https://github.com/worldapptechnologies/nulu-harness/blob/master/other/README.zh.md',
    ])
  })

  it('excludes only the header switcher from the structural links', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-translation-switcher-'))
    try {
      ...
      ).links).toEqual(['nulu-translation-target:guide.md'])
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })
...
describe('translation scope discovery', () => {
  ...
  it.each([
    ...
    'python/sdk-runtime/src/nulu_harness_runtime/runtime/nulu-harness-sdk-runtime-macos-arm64/README.md',
    'python/sdk-runtime/src/nulu_harness_runtime/runtime/node/README.md',
...
describe('translation structural signature', () => {
  ...
  it('treats target-locale siblings as one semantic link target', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-translation-structure-'))
    try {
  ...
  it('includes reference-style document links but excludes image-only definitions', () => {
    ...
    try {
      ...
      ).links).toEqual(['nulu-translation-target:reference.md'])
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
  })
  ...
  it('compares the first duplicate reference definition that CommonMark resolves', () => {
    ...
    try {
      ...
      const counterpart = fixtureSignature(root, 'guide.zh.md', counterpartMarkdown, 'guide.md')
      expect(translationStructureDiff(source, counterpart)).toEqual([
        'link target #1 diverges between the pair: "nulu-translation-target:reference.md" vs "nulu-translation-target:different.md"',
      ])
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
