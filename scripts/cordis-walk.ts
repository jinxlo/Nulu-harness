...
const MERGE_HEAD = /declare module ['"](?:@worldapptechnologies\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@worldapptechnologies/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@worldapptechnologies/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@worldapptechnologies/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@worldapptechnologies/cordis'` Context merge
...
/**
 * Every event name a `declare module '@worldapptechnologies/cordis'` Events merge declares in one
