 ...
 * itself (`nulu web`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/web/dist`, which `nulu web`
...
/** Compile-shell workspace whose dist `nulu web` serves. */
const SHELL_PACKAGE = '@worldapptechnologies/nulu-web-frontend'
 ...
 * whose package.json carries `nulu.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      nulu?: { client?: { platform?: unknown } }
    }
    if (manifest.nulu?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `nulu.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.nulu?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
if (isMain) {
  ...
  for (const name of Object.keys(process.env)) {
    if (name === CLIENT_BUILD_PROFILE_SELECTOR || name.startsWith('NULU_CLIENT_')) {
      Reflect.deleteProperty(process.env, name)
    }
  }
  for (const [name, value] of Object.entries(buildEnvironment)) {
    if (name.startsWith('NULU_CLIENT_') && value !== undefined) process.env[name] = value
  }

  const pluginDirs = discoverPluginDirs()
  const libraryDirs = discoverLibraryDirs()
  if (pluginDirs.length === 0) {
    console.error('dev-web: no nulu.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} nulu.client plugin packages`
