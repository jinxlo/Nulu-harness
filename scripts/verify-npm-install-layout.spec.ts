...
import {
  assertDualNuluInstallLayout,
  buildDualNuluRegistry,
...
describe('npm install layout verifier', () => {
  it('creates two incompatible versions of every NULU package', () => {
    ...
    const dual = buildDualNuluRegistry(index, '0.1.1-rc.2')
  ...
  it('accepts isolated NULU releases with one shared Cordis installation', () => {
    expect(assertDualNuluInstallLayout(validLayout())).toEqual({
  ...
  it.each([
    ...
    expect(() => assertDualNuluInstallLayout({ ...layout, packages })).toThrow(
