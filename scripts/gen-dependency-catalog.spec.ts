...
function lockfile() {
  const lock = {
    lockfileVersion: 3,
    packages: {
      '': { dependencies: { '@worldapptechnologies/nulu': 'latest' } },
      ...
      'node_modules/@worldapptechnologies/nulu/node_modules/library': { version: '2.0.0', dependencies: { plugin: '*' } },
...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'nulu-dependency-catalog-test-'))
  roots.push(root)
...
describe('published npm dependency catalog', () => {
  ...
  it('does not silently produce an incomplete catalog from invalid or unrelated resolutions', () => {
    ...
    input.packages[''].dependencies['@worldapptechnologies/nulu'] = 'next'
    expect(() => collectDependencies(input)).toThrow('must request only @worldapptechnologies/nulu@latest')
  ...
  it('isolates scoped registries, resolver settings, and caches from user, global, and environment configuration', async () => {
    ...
    writeFileSync(userConfig, '@worldapptechnologies:registry=https://user-override.invalid/\nstrict-peer-deps=true\n')
    ...
    expect(JSON.parse(before.output)).toMatchObject({
      '@worldapptechnologies:registry': 'https://user-override.invalid/', 'install-strategy': 'nested',
    })
    ...
    expect(settings).toMatchObject({
      registry: 'https://registry.npmjs.org/', '@worldapptechnologies:registry': 'https://registry.npmjs.org/',
