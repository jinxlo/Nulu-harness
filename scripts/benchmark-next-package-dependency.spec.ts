...
describe('next package benchmark graph', () => {
  it('applies a source-derived candidate without changing the filesystem', () => {
    const manifest: PackageDependencyManifest & { version: string } = {
      ...
      version: '1.0.0',
      peerDependencies: {
        '@worldapptechnologies/cordis': 'workspace:^',
        '@f/runtime': 'workspace:^',
        '@f/types': 'workspace:^',
      },
      devDependencies: {
    ...
    const facts: PackageDependencyFacts = {
      ...
      manifest,
      workspaceNames: new Set(['@worldapptechnologies/cordis', '@f/probe', '@f/runtime', '@f/types']),
    ...
    applyFactsToRegistry(index, facts, new Map([
      ['@worldapptechnologies/cordis', '4.0.1'],
      ['@f/probe', '1.0.0'],
      ['@f/runtime', '2.0.0'],
      ['@f/types', '3.0.0'],
    ]))
  ...
  it('finds reachable unconfigured packages with non-Cordis peers', () => {
    const index = new Map([
      ['@worldapptechnologies/nulu', new Map([['1.0.0', {
        name: '@worldapptechnologies/nulu', version: '1.0.0', dependencies: { '@f/a': '^1.0.0', '@f/b': '^1.0.0' },
      }]])],
      ['@f/a', new Map([['1.0.0', {
        name: '@f/a', version: '1.0.0', peerDependencies: { '@f/runtime': '^1.0.0' },
      }]])],
      ['@f/b', new Map([['1.0.0', {
        name: '@f/b', version: '1.0.0', peerDependencies: { '@worldapptechnologies/cordis': '^4.0.0' },
      }]])],
    ...
    expect(discoverBenchmarkCandidates(
      index,
      new Map([['@worldapptechnologies/nulu', '1.0.0'], ['@f/a', '1.0.0'], ['@f/b', '1.0.0']]),
      release,
      new Set(),
    )).toEqual(['@f/a'])
  })
