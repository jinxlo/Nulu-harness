...
function fixture(): string {
  const root = mkdtempSync(join(tmpdir(), 'nulu-clean-'))
  roots.push(root)
  return root
}
...
describe('RepositoryCleaner', () => {
  it('derives live build outputs from project references and removes safe stale package residue', async () => {
    ...
    write(join(root, '.nulu-build/client-build-environment.json'))
    ...
    expect(existsSync(join(root, '.nulu-build'))).toBe(false)
