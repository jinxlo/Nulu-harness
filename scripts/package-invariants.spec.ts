...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'nulu-package-invariants-'))
  roots.push(root)
  const packageDirectory = options.packageDirectory ?? 'packages/core/probe'
  const dir = join(root, packageDirectory)
  mkdirSync(join(dir, 'src'), { recursive: true })
  const packageName = options.packageName ?? '@worldapptechnologies/nulu-probe'
  ...
  const nulu = options.clientDeclaration === true ? { client: {} } : undefined
  ...
  const manifest = {
    ...
    peerDependencies: !invariantDependency || developmentOnlyInvariant ? {} : {
      '@worldapptechnologies/nulu-invariants': 'workspace:^',
    },
...
describe('package invariant gate', () => {
  ...
  it('accepts development-only invariants for configured Host dependencies', () => {
    expect(collectPackageInvariantViolations(fixture({ packageName: '@worldapptechnologies/nulu-llm' }))).toEqual([])
  })

  it('accepts development-only invariants for client packages', () => {
    expect(collectPackageInvariantViolations(fixture({
      packageName: '@worldapptechnologies/nulu-client-probe',
      packageDirectory: 'packages/client/probe',
    }))).toEqual([])
  })

  it('accepts development-only invariants for packages with a nulu.client entry', () => {
  ...
  it('keeps invariant peers for experimental packages with a nulu.client entry', () => {
    expect(collectPackageInvariantViolations(fixture({
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@worldapptechnologies/nulu-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@worldapptechnologies/nulu-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@worldapptechnologies/nulu-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@worldapptechnologies/nulu-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@worldapptechnologies/nulu-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@worldapptechnologies/nulu-probe'
const install = () => {}
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
