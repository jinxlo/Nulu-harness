...
const HOOKS_DIRECTORY = 'nulu-hooks'
const OWNERSHIP_MARKER = '.nulu-lefthook-owned'
const OWNERSHIP_MARKER_VERSION = 1
const OWNERSHIP_MARKER_OWNER = 'nulu-harness worktree-local lefthook hooks'
const INSTALL_LOCK = 'nulu-lefthook-install.lock'
const INSTALL_LOCK_TIMEOUT_MS = 30_000
const INSTALL_LOCK_INITIALIZATION_TIMEOUT_MS = 5_000
const INSTALL_LOCK_POLL_MS = 50
const ALLOW_HOOKS_PATH_OVERRIDE = 'NULU_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE'
const REPOSITORY_EXTENSION_PATTERN = '^extensions\\.'
const PAIRING_MERGE_DRIVER_CONFIG = [
  ['merge.nulu-translation-pairing.name', 'Nulu Harness bilingual pairing records'],
  [
    'merge.nulu-translation-pairing.driver',
  ...
  while (true) {
    try {
      ...
      try {
        ...
        const writeDelay = Number(process.env.NULU_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS ?? 0)
        if (writeDelay > 0) {
