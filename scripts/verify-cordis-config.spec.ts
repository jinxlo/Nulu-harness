...
describe('verify-cordis-config metadata expressions', () => {
  it('accepts a disabled !!js expression', () => {
    const problems = metadataExpressionErrors(
      { id: 'tool-bash', name: '@worldapptechnologies/nulu-tool-bash', disabled: { __jsExpr: "process.platform === 'win32'" } },
      '[0]',
    )
...
describe('workspace Bundle discovery and product dependency closures', () => {
  it('discovers a Bundle outside packages/bundle from its manifest declaration', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'nulu-bundle-discovery-'))
    try {
  ...
  it('allows a Bundle to mount itself but rejects an undeclared plugin package', () => {
    ...
    const self = { file, name: '@worldapptechnologies/nulu-subagent-example' }
    ...
    expect(bundlePluginDependencyErrors(manifestPath, manifest, [
      self,
      { file, name: '@worldapptechnologies/nulu-missing-plugin' },
    ])).toEqual([
      `${file}: @worldapptechnologies/nulu-missing-plugin must be declared in ${manifestPath} dependencies`,
    ])
  })
})
...
describe('package-owned Loader test dependency closures', () => {
  it('requires package test configs to declare each named plugin they load', () => {
    ...
    const manifest = {
      name: '@worldapptechnologies/nulu-owner',
      dependencies: {},
      devDependencies: {
        '@worldapptechnologies/nulu-declared': 'workspace:^',
      },
    }
    expect(packageTestPluginDependencyErrors(manifestPath, manifest, [
      { file, name: '@worldapptechnologies/nulu-owner' },
      { file, name: '@worldapptechnologies/nulu-declared' },
      { file, name: '@worldapptechnologies/nulu-missing' },
    ])).toEqual([
      `${file}: @worldapptechnologies/nulu-missing must be declared in ${manifestPath} dependencies or devDependencies`,
    ])
  })

  it('requires executable package test fixtures to declare their bare imports', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'nulu-package-test-entrypoint-'))
    try {
      ...
      writeFileSync(join(driverDir, 'driver.ts'), [
        "import '@worldapptechnologies/nulu-owner'",
        "import '@worldapptechnologies/nulu-declared'",
        "import '@worldapptechnologies/nulu-missing'",
      ].join('\n'))
      writeFileSync(join(driverDir, 'cordis.yml'), '[]\n')
      writeFileSync(join(driverDir, 'fixture.mjs'), "import '@worldapptechnologies/nulu-declared'\n")
      const unrelatedDir = join(packageDir, 'tests/fixtures/unrelated')
      mkdirSync(unrelatedDir, { recursive: true })
      writeFileSync(join(unrelatedDir, 'driver.ts'), "import '@worldapptechnologies/nulu-unrelated'\n")

      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
        'packages/example/owner/tests/fixtures/loader/driver.ts: '
        + '@worldapptechnologies/nulu-missing must be declared in '
  ...
  it('fails loud when package-owned Loader fixtures disappear from the scan', () => {
    const fixture = mkdtempSync(join(tmpdir(), 'nulu-empty-package-test-entrypoint-'))
    try {
      expect(packageTestFixtureDependencyErrors(fixture)).toEqual([
