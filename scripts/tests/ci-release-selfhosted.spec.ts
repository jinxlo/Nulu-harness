...
const repository = 'nulu-harness/nulu-harness'
...
const trustedPr = {
  'vars.NULU_CI_FAILOVER_LINUX': 'selfhosted',
...
const fallbackCases: Array<[string, Record<string, string | boolean>]> = [
  ['unset switch', { ...trustedPr, 'vars.NULU_CI_FAILOVER_LINUX': '' }],
  ['hosted switch', { ...trustedPr, 'vars.NULU_CI_FAILOVER_LINUX': 'hosted' }],
  ['unknown switch', { ...trustedPr, 'vars.NULU_CI_FAILOVER_LINUX': 'true' }],
  ...
  ['tag push', { ...trustedPush, 'github.ref': 'refs/tags/nulu-v1.0.0' }],
  ...
  ['dispatch on tag', { ...trustedPush, 'github.event_name': 'workflow_dispatch', 'github.ref': 'refs/tags/nulu-v1.0.0' }],
    ...
    for (const jobId of jobIds) {
      describe(jobId, () => {
        ...
        it('routes trusted PRs and master pushes onto the existing Linux pool', () => {
          ...
          expect(evaluate(job['runs-on'], { ...trustedPush, 'vars.NULU_CI_FAILOVER_LINUX': '' })).toBe(hosted)
        })
        ...
        it('retains the dependency and pack verification commands', () => {
          ...
          if (jobId === 'dependencies') {
            ...
            const family = file === 'release.yml' ? 'nulu' : 'vendor'
            const output = family === 'nulu' ? 'dist/npm' : 'dist/npm-vendor'
            expect(job.steps[0]?.with?.['fetch-depth']).toBe(0)
            expect(commands).toContain('pnpm run release:verify --family ' + family)
            expect(commands).toContain('pnpm run ' + (family === 'nulu' ? 'build:official' : 'build:lib:host'))
            ...
            expect(commands).toContain('pnpm run release:verify-packed-install --family ' + family + ' --from ' + output
              + (family === 'nulu' ? ' --from dist/npm-vendor --from dist/npm-landlock' : ''))
