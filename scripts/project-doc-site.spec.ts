...
import { collections, docsPages, landingLink, routeLink, sectionSpec, type DocsPage } from '../website/docs.ts'
import {
...
function fixture(): { root: string; pages: DocsPage[] } {
  const root = mkdtempSync(join(tmpdir(), 'nulu-doc-site-'))
  roots.push(root)
  ...
  return {
    ...
    pages: [
      ...
      { source: 'docs/b.md', route: 'reference-root/b.md', label: 'B', sidebar: 'reference', section: 'Test', order: 2 },
    ],
  }
}
...
describe('documentation site build', () => {
  it.each([
    ...
    const root = mkdtempSync(join(tmpdir(), 'nulu-doc-build-'))
    roots.push(root)
  ...
  it('refuses to remove the site root or an outside directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-doc-build-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'nulu-doc-build-outside-'))
  ...
  it('unlinks a link-shaped output without removing its target', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-doc-build-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'nulu-doc-build-link-target-'))
    roots.push(root, outside)
    const outDir = join(root, '.dist')
  ...
  it('refuses output whose nearest existing parent resolves outside the site root', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-doc-build-parent-link-root-'))
    const outside = mkdtempSync(join(tmpdir(), 'nulu-doc-build-parent-link-target-'))
    roots.push(root, outside)
...
describe('publishableImage', () => {
  ...
  it('refuses a target whose real path escapes the repository', () => {
    ...
    const { root } = fixture()
    const outside = mkdtempSync(join(tmpdir(), 'nulu-doc-site-outside-'))
    roots.push(outside)
...
describe('rewriteMarkdown', () => {
  it('maps published pages and pins unpublished source links', () => {
    ...
    expect(rewriteMarkdown(source, {
      ...
      + '[source](https://github.com/worldapptechnologies/nulu-harness/blob/abc123/packages/tool.ts#L2) '
      + '[web](https://example.com)\n',
    )
  })

  it('maps a source alias to its published page', () => {
    const { root, pages } = fixture()
    writeFileSync(join(root, 'docs/alias.md'), '# Alias\n')
    pages[0] = { ...pages[0]!, sourceAliases: ['docs/alias.md'] }
    expect(rewriteMarkdown('[A](alias.md)\n', {
      sourcePath: 'docs/b.md',
      route: 'reference-root/b.md',
      pages,
      repoRoot: root,
      repositoryRef: 'abc123',
    })).toBe('[A](../a.md)\n')
  ...
  it('uses raw GitHub content for unpublished images when nothing places them', () => {
    ...
    expect(rewriteMarkdown('![logo](../packages/logo.svg)\n', {
      ...
      repoRoot: root,
      repositoryRef: 'abc123',
    })).toBe('![logo](https://raw.githubusercontent.com/worldapptechnologies/nulu-harness/abc123/packages/logo.svg)\n')
  })
  ...
  it('replaces the destination token without changing repeated titles or escapes', () => {
    ...
    expect(rewriteMarkdown(source, {
      ...
      + '[escaped](https://github.com/worldapptechnologies/nulu-harness/blob/abc123/docs/x(y).md)\n',
    )
  })
...
describe('docsPages routes', () => {
  it('redirects the home route to the quick-start page', () => {
    ...
    for (const page of homes) {
      ...
      expect(projected).not.toContain('# Nulu Harness')
    }
  })

  it('publishes every route from one English repository Markdown source', () => {
    const routes = new Set<string>()
    for (const page of docsPages) {
      expect(routes.has(page.route), page.route).toBe(false)
      routes.add(page.route)
      expect(page.source.endsWith('.zh.md'), page.source).toBe(false)
      expect(existsSync(resolve(repositoryRoot, page.source)), page.source).toBe(true)
      for (const alias of page.sourceAliases ?? []) {
        expect(alias.endsWith('.zh.md'), alias).toBe(false)
      }
    }
  })

  it('projects the audited tutorial entry links', () => {
    const entries = [
  ...
  it('indexes every subsystem page in the folder README', () => {
    ...
    expect(missing, 'docs/subsystems/README.md must carry one table row per subsystem page').toEqual([])
  })
  ...
  it('places the shared todo fragment alias on the todo section', () => {
    ...
    expect(catalog.match(/<a id="worldapptechnologiesnulu-tool-todo"><\/a>/g)).toHaveLength(1)
    expect(catalog).toContain(
      '<a id="worldapptechnologiesnulu-tool-todo"></a>\n\n## `@worldapptechnologies/nulu-tool-todo`',
    )
  })

  it('projects every published subsystem page from its English source', () => {
    const rootPages = docsPages.filter(page => page.route.startsWith('reference/subsystems/'))
    expect(rootPages).toHaveLength(48)
    expect(rootPages.every(page => page.source.endsWith('.md') && !page.source.endsWith('.zh.md'))).toBe(true)
  })

  it('publishes the Cordis core API under its reference routes', () => {
  ...
  it('keeps Cordis inherited on the English fallback', () => {
    const pages = docsPages.filter(page => page.route === 'reference/cordis-api/inherited.md')
    expect(pages).toHaveLength(1)
    expect(pages[0]?.source).toBe('docs/cordis-api/inherited.md')
  })

  it('includes persistence event headings in the deep outline', () => {
  ...
  it('projects reviewed generated counterparts into reference routes', () => {
    // module-graph, event-producer-consumer, and graph-atlas are generated but intentionally unpublished.
    const routes = [
...
describe('sidebar ordering', () => {
  ...
  it('refuses a section with no declared placement', () => {
    expect(() => sectionSpec('数据结构'))
      .toThrow('Sidebar section "数据结构" has no placement.')
  })
...
describe('projectedPageContent', () => {
  ...
  it('drops the repository badge every page links from its footer', () => {
    const badge = '[![](https://img.shields.io/badge/powered_by-Nulu_Harness-blue?style=flat-square)](https://github.com/worldapptechnologies/nulu-harness)'
  ...
  it('rejects a home source without frontmatter', () => {
    expect(() => projectedPageContent('# Harness\n', page(null)))
      .toThrow('home source "docs/index.md" must start with YAML frontmatter')
  })
})
...
describe('emitRawMarkdownPages', () => {
  function mirrorDir(): string {
    const out = mkdtempSync(join(tmpdir(), 'nulu-doc-mirror-'))
    roots.push(out)
    return out
  }
  ...
  it('emits a parent-level alias for an index route with links recomputed', () => {
    ...
    pages.push({
      ...
      label: 'C', sidebar: 'reference', section: 'Test', order: 3,
    })
    const out = mirrorDir()
...
describe('raw Markdown projection of the published manifest', () => {
  ...
  beforeAll(() => {
    mirror = mkdtempSync(join(tmpdir(), 'nulu-doc-mirror-real-'))
  ...
  it('emits the home page with its body instead of the frontmatter stub', () => {
    ...
    expect(home).toContain('# Nulu Harness')
  })
...
describe('llmsTxt', () => {
  const site = { base: '/x/', title: 'Nulu Harness', description: 'A plugin-based SDK for building agent harnesses' }
  ...
  it('keeps the collections in navigation order', () => {
    const text = llmsTxt(site)
    const landings = collections.map(collection => text.indexOf(`](/x/${landingLink(collection).slice(1)}`))
    expect(landings).toEqual([...landings].sort((left, right) => left - right))
    expect(landings[0]).toBeGreaterThan(-1)
  })

  it('carries the site identity and the raw-Markdown convention', () => {
    const text = llmsTxt(site)
    expect(text.startsWith('# Nulu Harness\n')).toBe(true)
