...
SDK_DISTRIBUTION = "nulu-harness-sdk"
RUNTIME_DISTRIBUTION = "nulu-harness-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="nulu-python-release-") as temporary:
        staging = Path(temporary) / args.package
        if args.package == "sdk":
            stage_sdk(staging, wheel_version)
            environment = None
            expected = output_dir / f"nulu_harness_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            environment = {"NULU_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"nulu_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"nulu-harness-runtime-bin==[^"]+"',
        f'"nulu-harness-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def verify_wheel(
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        runtime_files = [
            name for name in archive.namelist() if "/runtime/nulu-harness-sdk-runtime-" in name
        ]
        if package == "runtime":
