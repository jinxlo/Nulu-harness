...
describe('npm resolution benchmark', () => {
  ...
  it('combines installed metadata with current publishable workspace fields', () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-npm-registry-index-'))
    roots.push(root)
    ...
    writeJson(root, 'apps/cli/package.json', {
      ...
      version: '0.1.0',
      dependencies: { '@worldapptechnologies/nulu-child': 'workspace:^', external: '^2.0.0' },
    ...
    expect(index.get('@worldapptechnologies/nulu')?.get('0.1.0')).toEqual({
      name: '@worldapptechnologies/nulu',
      version: '0.1.0',
      dependencies: { '@worldapptechnologies/nulu-child': '^0.1.0', external: '^2.0.0' },
    })
  })
  ...
  it('runs npm against the local registry without requesting an archive', async () => {
    const index: RegistryIndex = new Map([[
      ...
      new Map([['0.1.0', { name: '@worldapptechnologies/nulu', version: '0.1.0' }]]),
    ]])
  ...
  it('returns npm placement for two aliased package versions without requesting archives', async () => {
    const index: RegistryIndex = new Map([[
      '@worldapptechnologies/nulu',
      new Map([
        ['0.1.0', { name: '@worldapptechnologies/nulu', version: '0.1.0' }],
        ['0.2.0', { name: '@worldapptechnologies/nulu', version: '0.2.0' }],
      ]),
    ]])

    const result = await resolveNpmPackageLock(index, {
      '@worldapptechnologies/nulu': '0.2.0',
      'nulu-previous': 'npm:@worldapptechnologies/nulu@0.1.0',
    }, 10_000)

    expect(result.archiveRequests).toBe(0)
    expect(result.packageLock.packages['node_modules/@worldapptechnologies/nulu']?.version).toBe('0.2.0')
    expect(result.packageLock.packages['node_modules/nulu-previous']).toMatchObject({
      name: '@worldapptechnologies/nulu',
      version: '0.1.0',
    })
  })
  ...
  it('isolates peer resolution from inherited npm configuration', async () => {
    const root = mkdtempSync(join(tmpdir(), 'nulu-hostile-npm-config-'))
    roots.push(root)
    const userConfig = join(root, 'user.npmrc')
    writeFileSync(userConfig, '@worldapptechnologies:registry=http://127.0.0.1:1/\nlegacy-peer-deps=true\nomit=peer\n')
    const previous = {
    ...
    try {
      const index: RegistryIndex = new Map([
        ['@worldapptechnologies/nulu', new Map([['0.1.0', {
          name: '@worldapptechnologies/nulu',
          version: '0.1.0',
          peerDependencies: { '@worldapptechnologies/nulu-peer': '1.0.0' },
        }]])],
        ['@worldapptechnologies/nulu-peer', new Map([['1.0.0', {
          name: '@worldapptechnologies/nulu-peer',
          version: '1.0.0',
        }]])],
      ])

      const result = await resolveNpmPackageLock(index, { '@worldapptechnologies/nulu': '0.1.0' }, 10_000)

      expect(result.archiveRequests).toBe(0)
      expect(result.packageLock.packages['node_modules/@worldapptechnologies/nulu-peer']?.version).toBe('1.0.0')
    } finally {
