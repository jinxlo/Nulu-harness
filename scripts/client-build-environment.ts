...
const CLIENT_BUILD_ENV_PREFIX = 'NULU_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'NULU_BUILD_CLIENT_PROFILE'

/** Public client environment required by official NULU artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'NULU_CLIENT_COMMIT_HASH'

/** Public variable carrying the repository package version embedded in client artifacts. */
const CLIENT_VERSION_VARIABLE = 'NULU_CLIENT_VERSION'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.nulu-build/client-build-environment.json'
...
export function repositoryClientBuildEnvironment(
  ...
  delete inherited.NULU_CLIENT_COMMIT_HASH
  delete inherited.NULU_CLIENT_GIT_DIRTY
  delete inherited.NULU_CLIENT_VERSION
  const dirty = repositoryGitDirty(root)
  return {
    ...inherited,
    NULU_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    ...(dirty === true ? { NULU_CLIENT_GIT_DIRTY: 'true' } : {}),
    NULU_CLIENT_VERSION: repositoryVersion(root),
  }
}
...
export function officialClientBuildEnvironment(
  root: string,
  environment: NodeJS.ProcessEnv = process.env,
): Readonly<Record<`NULU_CLIENT_${string}`, string>> {
  return {
    NULU_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
    NULU_CLIENT_VERSION: repositoryVersion(root),
 ...
 * @returns defined `NULU_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return {
      NULU_CLIENT_COMMIT_HASH: commitHash,
      NULU_CLIENT_VERSION: version,
      ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT,
    }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`NULU_CLIENT_${string}`, string>>,
): void {
