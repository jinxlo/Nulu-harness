...
const SESSION_PACKAGE = '@worldapptechnologies/nulu-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@worldapptechnologies/nulu-session/types'
 ...
 * top-level declaration (in `@worldapptechnologies/nulu-session`) and any declaration
 * merge inside a `declare module '@worldapptechnologies/nulu-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @worldapptechnologies/nulu-session/known-event-types',
    ' */',
    '',
    '/**',
