...
export interface NuluInstallLayoutSummary {
  readonly nuluPackagesPerVersion: number
  readonly checkedNuluEdges: number
}

function isNuluPackage(name: string): boolean {
...
function cloneForVersion(manifest: object, version: string): MutableRegistryManifest {
  ...
  for (const field of DEPENDENCY_FIELDS) {
    ...
    for (const name of Object.keys(dependencies)) {
      if (isNuluPackage(name)) dependencies[name] = `^${version}`
    }
  }
  return cloned
}
 ...
 */
export function buildDualNuluRegistry(index: RegistryIndex, sourceVersion: string): RegistryIndex {
  const output = new Map(index)
  let nuluPackages = 0
  for (const [name, versions] of index) {
    if (!isNuluPackage(name)) {
 ...
 */
export function assertDualNuluInstallLayout(packageLock: NpmPackageLock): NuluInstallLayoutSummary {
  ...
  for (const [path, manifest] of installed) {
    ...
    if (name === undefined || !isNuluPackage(name)) continue
    ...
    for (const field of DEPENDENCY_FIELDS) {
      for (const dependency of Object.keys(manifest[field] ?? {})) {
        if (!isNuluPackage(dependency)) continue
  ...
  return { nuluPackagesPerVersion: rootNames.size, checkedNuluEdges }
}
...
async function main(): Promise<void> {
  ...
  const index = buildDualNuluRegistry(buildRegistryIndex(root), workspaceVersion(root))
  const [nestedVersion, rootVersion] = SYNTHETIC_NULU_VERSIONS
  ...
  const summary = assertDualNuluInstallLayout(result.packageLock)
  console.log(
    `verify-npm-install-layout: ${String(summary.nuluPackagesPerVersion)} NULU package(s) per release and `
    + `${String(summary.checkedNuluEdges)} internal edge(s) verified in ${(result.durationMs / 1000).toFixed(2)} s; `
