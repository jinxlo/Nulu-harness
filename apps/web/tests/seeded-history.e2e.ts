...
// fixture under the same record discipline as every other: NULU_SNAPSHOT=record drives the turn
...
import { deriveEventMessage, SessionId } from '@worldapptechnologies/nulu-session'
import type { SessionEvent } from '@worldapptechnologies/nulu-session'
import type { TokenMeter } from '@worldapptechnologies/nulu-token-meter'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.NULU_HOME = scaffold.harnessHome
    try {
      const input = page.locator('[data-composer-input]').first()
      ...
      if (previousDshHome === undefined) delete process.env.NULU_HOME
      else process.env.NULU_HOME = previousDshHome
    }
  }, 60_000)
