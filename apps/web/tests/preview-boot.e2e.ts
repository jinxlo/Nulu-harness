...
} from '@worldapptechnologies/nulu-experimental-webworker-packer'
import {
...
function requireVfsAssets(): PreviewAssets {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'nulu-preview-boot-'))
...
async function bootPreview(origin: string, browser: Browser): Promise<void> {
  ...
  try {
    ...
    const exercised = await page.evaluate(async ({ seededSessionId, seededSessionTitle }) => {
      ...
      const transport = (globalThis as typeof globalThis & { __NULU_TRANSPORT__?: PreviewTransport }).__NULU_TRANSPORT__
      ...
      await createDirectory('/nulu/workspace/.agents/skills', 'runtime-created')
  ...
  try {
    ...
    const sessionCount = await page.evaluate(async () => {
      const transport = (globalThis as typeof globalThis & {
        __NULU_TRANSPORT__?: { fetch(input: string, init: RequestInit): Promise<Response> }
      }).__NULU_TRANSPORT__
