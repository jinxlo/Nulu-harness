// Real-host smoke: spawn `nulu web` with a real key, walk the full flow
// list in a real chromium, screenshot every screen into .artifacts/ for the
// figma comparison pass. Self-skips without WORLD_APP_TECHNOLOGIES_API_KEY (repo e2e
...
function authenticatedWeb(launchUrl: string): Promise<{ origin: string; cookie: string }> {
  ...
  const exchange = (async () => {
    ...
    if (response.status !== 303 || setCookie === null) {
      throw new Error(`nulu web authentication returned HTTP ${String(response.status)}`)
    }
    return {
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`nulu web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /nulu web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`nulu web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `nulu web` serves every production manifest plugin; until every UI
...
describe('nulu web keyless CLI smoke', () => {
  it('serves a usable app from immutable plugin batches', async () => {
    requireDist()
    const sessionsDir = mkdtempSync(join(tmpdir(), 'nulu-web-keyless-'))
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...process.env,
          WORLD_APP_TECHNOLOGIES_API_KEY: 'keyless-web-no-call',
          NULU_HOME: join(sessionsDir, '.nulu'),
          NULU_AGENTS_HOME: join(sessionsDir, '.agents'),
    ...
    try {
      ...
      expect(applicationBatches).toHaveLength(2)
      expect(batchPaths).toContainEqual(expect.stringMatching(
        /^\/plugins\/\?\?@worldapptechnologies\/nulu-client-modules\/client\.js&rev=[a-f\d]{12}$/,
      ))
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'nulu-web-workspace-'))
    ...
    const child = spawn(
      ...
      [
        ...
        '--patch', fileURLToPath(new URL('./nulu-default.overlay.yml', import.meta.url)),
        '--no-open', '--port', '0',
      ],
      {
        cwd: workspace,
        env: {
          ...process.env,
          WORLD_APP_TECHNOLOGIES_API_KEY: 'keyless-web-workspace',
          WORLD_APP_TECHNOLOGIES_BASE_URL: `http://127.0.0.1:${address.port}`,
          NULU_HOME: join(workspace, '.nulu'),
          NULU_AGENTS_HOME: join(workspace, '.agents'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
        stdio: ['ignore', 'pipe', 'pipe'],
      },
  ...
  it('retries a partial transport failure through the shipped Web composition', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'nulu-web-retry-'))
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          WORLD_APP_TECHNOLOGIES_API_KEY: 'keyless-web-retry',
          WORLD_APP_TECHNOLOGIES_BASE_URL: `http://127.0.0.1:${address.port}`,
          NULU_HOME: join(workspace, '.nulu'),
          TSX_TSCONFIG_PATH: join(REPO_ROOT, 'tsconfig.json'),
        },
  ...
  it('NULU_TOOLS_MODE=ptc collapses the provider wire tools to run_code with the SDK prompt section', async () => {
    requireDist()
    const workspace = mkdtempSync(join(tmpdir(), 'nulu-web-ptc-'))
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          WORLD_APP_TECHNOLOGIES_API_KEY: 'keyless-web-ptc',
          WORLD_APP_TECHNOLOGIES_BASE_URL: `http://127.0.0.1:${address.port}`,
          NULU_TOOLS_MODE: 'ptc',
          NULU_HOME: join(workspace, '.nulu'),
          NULU_AGENTS_HOME: join(workspace, '.agents'),
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    requireDist()
    sessionsDir = mkdtempSync(join(tmpdir(), 'nulu-web-w5-'))
    const port = await probeFreePort()
    // tsx boot mirrors the runtime half of the root nulu script. Isolate
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $NULU_HOME with the
    // Nulu credential inherited from the environment, so no onboarding
