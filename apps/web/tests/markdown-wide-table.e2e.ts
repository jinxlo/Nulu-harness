...
const LONG_TOKEN = 'workspace/nulu-harness/packages/client/ui-primitives/src/markdown/render.tsx/'.repeat(3)
...
async function awaitTableLayout(target: Page): Promise<void> {
  ...
  await target.waitForFunction(() => {
    ...
    return tracks[0] === 56 && tracks.at(-1) === 0
      ...
      && root !== null
      && root.style.getPropertyValue('--nulu-conversation-column-width') === `${String(root.offsetWidth)}px`
