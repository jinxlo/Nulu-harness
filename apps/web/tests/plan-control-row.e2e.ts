...
// regression the external report asked for (nulu-external/issues#107 →
// nulu-harness#1406): "increase an 800×720 browser regression test and
...
import type {} from '@worldapptechnologies/nulu-plan-mode'
...
describe('web e2e: plan chip click area at the narrow viewport', () => {
  ...
  it('keeps the plan chip and model trigger disjoint and exits plan mode by click', async () => {
    ...
    await expect.poll(() => trigger.getAttribute('aria-label'), { timeout: 10_000 }).toContain('Nulu 5')
