...
import type { Context, Plugin, RegistryService } from '@worldapptechnologies/cordis'
import type { ClientModuleLoader, ClientModuleLoaderTarget } from '@worldapptechnologies/nulu-client-modules/client'
...
function experimentalClientReferences(roster: Awaited<ReturnType<typeof readClientRoster>>): string[] {
  return [
    ...
    ...roster.modules,
  ].filter(name => name.startsWith('@worldapptechnologies/nulu-experimental-'))
}
...
it('activates the actual default Client registry without experimental packages', async (test) => {
  await withDefaultWeb(test, async ({ url, request }) => {
    ...
    try {
        ...
        Object.defineProperty(globalThis, '__ModuleLoader__', {
          ...
          set(target: ClientModuleLoaderTarget) {
            ...
            target.create = function (options) {
              const cordis = options.staticModules['@worldapptechnologies/cordis'] as { RegistryService: typeof RegistryService }
      ...
      expect(roster.plugins.some(plugin => plugin.modules.includes('@worldapptechnologies/nulu-client-ui-layout'))).toBe(true)
