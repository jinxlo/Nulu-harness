...
describe('web e2e: workspace recency', () => {
  ...
  beforeAll(async () => {
    ...
    await page.addInitScript(({ account, ids }) => {
      if (localStorage.getItem('nulu.workspace.view.v5') !== null) return
      localStorage.setItem('nulu.sessions.current', JSON.stringify({ sessionId: ids[0] }))
  ...
  it('pauses current recency in Manual, preserves drags on reload, and resets on Last updated', async () => {
    ...
    await expect.poll(() => page.evaluate(() => {
      const { sessionId } = JSON.parse(localStorage.getItem('nulu.sessions.current')!) as { sessionId: string }
      const { sessionOrderByAccount } = JSON.parse(localStorage.getItem('nulu.workspace.view.v5')!) as {
