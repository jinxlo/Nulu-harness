...
it('isolates replay skill discovery from every ambient host root', async () => {
  const ambient = await mkdtemp(join(tmpdir(), 'nulu-web-ambient-skills-'))
  const nuluHome = join(ambient, 'nulu-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(nuluHome, 'skills'), 'ambient-nulu'),
    writeSkill(join(agentsHome, 'skills'), 'ambient-agents'),
    writeSkill(bundled, 'ambient-bundled'),
  ])

  const originalNuluHome = process.env.NULU_HOME
  const originalAgentsHome = process.env.NULU_AGENTS_HOME
  const originalBundled = process.env.NULU_BUNDLED_SKILL_DIR
  process.env.NULU_HOME = nuluHome
  process.env.NULU_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-nulu')
    ...
    try {
      ...
      if (originalNuluHome === undefined) delete process.env.NULU_HOME
      else process.env.NULU_HOME = originalNuluHome
      if (originalAgentsHome === undefined) delete process.env.NULU_AGENTS_HOME
      else process.env.NULU_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.NULU_BUNDLED_SKILL_DIR
      else process.env.NULU_BUNDLED_SKILL_DIR = originalBundled
