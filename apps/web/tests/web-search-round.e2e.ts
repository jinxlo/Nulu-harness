...
// drives `web_search`; the model stream is replayed while the real Nulu
...
import { WEB_SEARCH_MAX_RESULTS } from '@worldapptechnologies/nulu-tool-web'
import {
...
const QUERIES = ['Nulu Harness snapshot search', 'Nulu Harness multi-query search'] as const
const PROMPT = `Use web_search once with queries ${JSON.stringify(QUERIES)}. Then reply exactly SEARCH_DONE and stop.`
const SEARCH_CREDENTIAL_REF = credentialRef('NULU_WEB_SEARCH_E2E_KEY')
...
/** Start the deterministic Nulu Messages double used by the real provider. */
...
describe('web e2e: shipped default web search', () => {
  ...
  it.skipIf(MODE === 'record')('uses the real provider and persists the capped structured result', () => {
    ...
    const auxiliaryRequests = sessionEvents.filter(
      (event): event is Extract<SessionEvent, { type: 'web/nulu-search-llm-request' }> =>
        event.type === 'web/nulu-search-llm-request',
    )
