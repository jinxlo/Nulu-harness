...
import { composeEntries, loadOverlayPatches } from '@worldapptechnologies/nulu-app-boot'
import { ToolCallId, createUserMessage, LlmAdapter } from '@worldapptechnologies/nulu-llm'
...
} from '@worldapptechnologies/nulu-schedule'
import {
...
describe.skipIf(MODE === 'record')('web e2e: active Schedule catalog', () => {
  ...
  it('replays the overlay-only catalog and sidebar marker, then removes both live', async () => {
    ...
    expect(base.find(entry => entry.id === 'ui-schedule')).toMatchObject({
      name: '@worldapptechnologies/nulu-client-ui-schedule',
      disabled: true,
    })
