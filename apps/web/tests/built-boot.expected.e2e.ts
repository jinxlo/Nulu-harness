...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('NULU_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="32 0 136 24"]')).not.toBeNull()
    expect(screen.queryByText('NULU Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 24 24"]')).not.toBeNull()
    const version = clientBuildValue('NULU_CLIENT_VERSION')
    if (version === undefined) throw new Error('default client build record must carry NULU_CLIENT_VERSION')
    const commit = clientBuildValue('NULU_CLIENT_COMMIT_HASH')
    const buildVersion = version
      + (commit === undefined ? '' : `-${commit}`)
      + (clientBuildValue('NULU_CLIENT_GIT_DIRTY') === 'true' ? '-dirty' : '')
    screen.getByText('NULU Local Build')
  ...
  for (const plugin of ['@worldapptechnologies/nulu-client-ui-layout', '@worldapptechnologies/nulu-client-ui-sidebar', '@worldapptechnologies/nulu-client-ui-conversation', '@worldapptechnologies/nulu-client-ui-tool']) {
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp('?fixture', { exclude: ['@worldapptechnologies/nulu-client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__NULU_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@worldapptechnologies/nulu-client-ui-chat')).toBe(false)
