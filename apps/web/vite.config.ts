...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__NULU_BOOT__. '
  + 'From a repository checkout, run `pnpm nulu web`; an installed package uses `nulu web`. '
  + 'For client-plugin HMR, run `pnpm nulu web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'NULU Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.NULU_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'nulu-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>NULU Local Build</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'nulu-reject-standalone-web-serve',
    config(_config, env) {
