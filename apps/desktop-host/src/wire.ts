...
function encodeFrame(type: ResponseFrameType, streamId: number, payload: Buffer): Buffer {
  ...
  if (payload.byteLength > limit) {
    throw new Error(`nulu desktop: response pipe frame exceeds the ${String(limit)}-byte limit`)
  }
...
export class DesktopHostRequestDecoder {
  ...
  finish(): void {
    if (this.buffer.byteLength !== 0) throw new Error('nulu desktop: Electron request pipe ended inside a frame')
  }

  private next(): DesktopHostRequestFrame | undefined {
    if (this.buffer.byteLength < FRAME_HEADER_BYTES) return undefined
    if (this.buffer.readUInt32BE(0) !== FRAME_MAGIC) throw new Error('nulu desktop: invalid Electron request frame marker')
    ...
    if (payloadLength > limit) {
      throw new Error(`nulu desktop: Electron request frame exceeds the ${String(limit)}-byte limit`)
    }
    ...
    switch (rawType) {
      ...
      case REQUEST_FRAME_END:
        if (payloadLength !== 0) throw new Error('nulu desktop: Electron request end frame carried a payload')
        return { type: 'end', streamId }
      case REQUEST_FRAME_CANCEL:
        if (payloadLength !== 0) throw new Error('nulu desktop: Electron request cancel frame carried a payload')
        return { type: 'cancel', streamId }
      default:
        throw new Error(`nulu desktop: unknown Electron request frame type ${String(rawType)}`)
    }
  }
  ...
  private parseStart(streamId: number, payload: Buffer): DesktopHostRequestFrame {
    ...
    try {
      ...
      throw new Error(`nulu desktop: Electron request start payload is not JSON: ${error instanceof Error ? error.message : String(error)}`)
    }
    if (!isRecord(value) || typeof value.url !== 'string' || typeof value.method !== 'string'
      || !isHeaders(value.headers) || typeof value.hasBody !== 'boolean') {
      throw new Error('nulu desktop: invalid Electron request start payload')
    }
    return {
      type: 'start',
      streamId,
