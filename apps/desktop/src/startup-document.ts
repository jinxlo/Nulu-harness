...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; form-action nulu-recovery:">
...
<form action="nulu-recovery://restart"><button>${escape(locale.messages.restartApplication)}</button></form>
${profileRecovery ? `<form action="nulu-recovery://plugins"><button>${escape(locale.messages.disableThirdPartyPlugins)}</button></form>
<form action="nulu-recovery://reset"><button>${escape(locale.messages.resetConfiguration)}</button></form>` : ''}
</main></html>`
}
