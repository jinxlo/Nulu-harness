...
/** Protocol version implemented by the Electron shell and installed nulu Host. */
...
function assertStreamId(streamId: number): void {
  if (!Number.isInteger(streamId) || streamId < 1 || streamId > 0xffff_ffff) {
    throw new Error(`nulu desktop: invalid pipe stream id ${String(streamId)}`)
  }
}
...
function encodeFrame(type: RequestFrameType, streamId: number, payload: Buffer): Buffer {
  ...
  if (payload.byteLength > limit) {
    throw new Error(`nulu desktop: request pipe frame exceeds the ${String(limit)}-byte limit`)
  }
...
export class DesktopHostResponseDecoder {
  ...
  finish(): void {
    if (this.buffer.byteLength !== 0) throw new Error('nulu desktop: Host response pipe ended inside a frame')
  }

  private next(): DesktopHostResponseFrame | undefined {
    if (this.buffer.byteLength < FRAME_HEADER_BYTES) return undefined
    if (this.buffer.readUInt32BE(0) !== FRAME_MAGIC) throw new Error('nulu desktop: invalid Host response frame marker')
    ...
    if (payloadLength > limit) {
      throw new Error(`nulu desktop: Host response frame exceeds the ${String(limit)}-byte limit`)
    }
    ...
    switch (rawType) {
      ...
      case RESPONSE_FRAME_END:
        if (payloadLength !== 0) throw new Error('nulu desktop: Host response end frame carried a payload')
        return { type: 'end', streamId }
      case RESPONSE_FRAME_ERROR:
        return this.parseError(streamId, payload)
      default:
        throw new Error(`nulu desktop: unknown Host response frame type ${String(rawType)}`)
    }
  }
  ...
  private parseStart(streamId: number, payload: Buffer): DesktopHostResponseFrame {
    ...
    if (!isRecord(value) || !Number.isInteger(value.status) || (value.status as number) < 100
      ...
      throw new Error('nulu desktop: invalid Host response start payload')
    }
    return {
      type: 'start',
      streamId,
  ...
  private parseError(streamId: number, payload: Buffer): DesktopHostResponseFrame {
    ...
    if (!isRecord(value) || typeof value.message !== 'string') {
      throw new Error('nulu desktop: invalid Host response error payload')
    }
  ...
  private parseJson(payload: Buffer, subject: string): unknown {
    try {
      ...
      throw new Error(`nulu desktop: Host response ${subject} payload is not JSON: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
