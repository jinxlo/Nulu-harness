...
function isDesktopHostEvent(message: unknown): message is DesktopHostEvent {
  ...
  switch (candidate.type) {
    case 'ready':
      return candidate.protocolVersion === DESKTOP_HOST_PROTOCOL_VERSION && typeof candidate.nuluVersion === 'string'
    case 'fatal':
...
/** Ready facts reported by one installed nulu child. */
...
/** One nulu backend running under the bundled upstream Node.js executable. */
export class DesktopHostProcess {
  ...
  async start(): Promise<DesktopHostReady> {
    ...
    if (!(requestPipe instanceof Writable) || !(responsePipe instanceof Readable)) {
      child.kill('SIGTERM')
      throw new Error('nulu desktop host did not expose the required byte pipes and IPC channel')
    }
    this.child = child
    ...
    responsePipe.once('end', () => {
      try {
        ...
        this.fail(new Error('nulu desktop host response pipe ended'))
      } catch (error) {
        this.fail(errorOf(error, 'nulu desktop host response pipe failed'))
      }
    })
    ...
    child.on('message', (message: unknown) => {
      if (!isDesktopHostEvent(message)) {
        this.fail(new Error('nulu desktop host sent an invalid IPC event'))
        child.kill('SIGTERM')
        return
      }
    ...
    this.exitPromise = new Promise<void>((resolve) => {
      child.once('close', (code) => {
        ...
        if (code !== 0 && code !== null) this.fail(new Error(`nulu desktop host exited with ${String(code)}${suffix}`))
        else this.fail(new Error(`nulu desktop host stopped${suffix}`))
        resolve()
      })
    })
  ...
  /** Forward one `nulu-app://app` request to the child without buffering its body. */
  async fetch(request: Request): Promise<Response> {
    await this.start()
    const child = this.child
    if (child === undefined || !child.connected || this.requestPipe === undefined) {
      throw new Error('nulu desktop host is unavailable')
    }
    if (this.nextStreamId > 0xffff_ffff) throw new Error('nulu desktop host exhausted its request stream ids')
    ...
    return new Promise<Response>((resolve, reject) => {
      ...
      const abort = (): void => {
        ...
        this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((pipeError: unknown) => {
          this.fail(errorOf(pipeError, 'nulu desktop request pipe failed'))
        })
      ...
      this.pumpRequest(streamId, request, hasBody).catch((error: unknown) => {
        this.failPending(streamId, errorOf(error, 'nulu desktop request upload failed'))
      })
    })
  }
  ...
  async stop(): Promise<void> {
    ...
    if (!await exitsWithin(exited, 5_000)) {
      child.kill('SIGKILL')
      if (!await exitsWithin(exited, 5_000)) {
        throw new Error('nulu desktop host did not exit after SIGKILL')
      }
    }
  ...
  private async pumpRequest(streamId: number, request: Request, hasBody: boolean): Promise<void> {
    ...
    if (body === null) throw new Error('nulu desktop request body disappeared before upload')
  ...
  private enqueueRequestFrame(frame: Buffer): Promise<void> {
    const write = this.requestWriteTail.then(async () => {
      ...
      if (pipe === undefined || pipe.destroyed) throw new Error('nulu desktop host request pipe is unavailable')
  ...
  private send(message: DesktopHostCommand): void {
    const child = this.child
    if (child === undefined || !child.connected) throw new Error('nulu desktop host IPC is unavailable')
  ...
  private handleResponseFrame(frame: DesktopHostResponseFrame): void {
    ...
    if (pending === undefined) {
      if (frame.streamId >= this.nextStreamId) {
        throw new Error(`nulu desktop host responded for unknown stream ${String(frame.streamId)}`)
      }
      return
    }
    switch (frame.type) {
      case 'start': {
        if (pending.responseStarted) throw new Error(`nulu desktop host started stream ${String(frame.streamId)} twice`)
      ...
      case 'data': {
        ...
        if (!pending.responseStarted || controller === undefined) {
          throw new Error(`nulu desktop host sent body data before a body start for stream ${String(frame.streamId)}`)
        }
      ...
      case 'end':
        if (!pending.responseStarted) {
          throw new Error(`nulu desktop host ended stream ${String(frame.streamId)} before its response start`)
        }
  ...
  private cancelResponse(streamId: number, reason: unknown): void {
    ...
    this.enqueueRequestFrame(encodeDesktopRequestCancel(streamId)).catch((error: unknown) => {
      this.fail(errorOf(error, 'nulu desktop request pipe failed'))
    })
    this.finishPending(streamId, false)
  }
