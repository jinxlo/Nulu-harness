 ...
 * @param root - Materialized nulu resources.
...
export async function smokeDesktopRuntime(root: string, node: string, runtime: DesktopRuntimeDescriptor): Promise<void> {
  const home = mkdtempSync(join(tmpdir(), 'nulu-desktop-smoke-'))
  const profile = join(home, 'profiles', 'desktop')
  const host = new DesktopHostProcess(node, root, profile, undefined, { ...process.env, NULU_HOME: home })
  try {
    createPluginProfile(profile)
    const pluginName = 'desktop-runtime-smoke-plugin'
    const plugin = join(profile, 'node_modules', pluginName)
    mkdirSync(plugin, { recursive: true })
    const cordis = runtime.sharedPackages.find(entry => entry.name === '@worldapptechnologies/cordis')
    ...
    writeFileSync(join(plugin, 'package.json'), JSON.stringify({
      ...
      peerDependencies: { '@worldapptechnologies/cordis': cordis.version }, nulu: { bundle: { patch: './bundle.yml' } },
    }))
...
`)
    ...
    manifest.nulu.profile.bundles.push(pluginName)
    ...
    if (ready.nuluVersion !== runtime.release.version) throw new Error('desktop runtime: Host reported another nulu release')
    const response = await host.fetch(new Request('nulu-app://app/'))
