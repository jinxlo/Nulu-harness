...
function target(): { platform: RuntimePlatform; arch: RuntimeArch } {
  const rawPlatform = process.env.NULU_DESKTOP_TARGET_PLATFORM ?? process.env.npm_config_platform ?? process.platform
  const rawArch = process.env.NULU_DESKTOP_TARGET_ARCH ?? process.env.npm_config_arch ?? process.arch
