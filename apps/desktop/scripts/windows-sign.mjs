...
const NSIS_BOOTSTRAP_PATCH = Symbol.for('@worldapptechnologies/nulu-desktop/nsis-bootstrap-signing')
...
const WINDOWS_SIGNING_ENVIRONMENT_PREFIX = 'NULU_DESKTOP_WINDOWS_'
...
function resolveTokenIdentity(input) {
  ...
  if (!keyContainer) {
    throw new Error('NULU_DESKTOP_WINDOWS_KEY_CONTAINER must contain the SafeNet private-key container name')
  }
  if (/["\r\n]/u.test(keyContainer)) {
    throw new Error('NULU_DESKTOP_WINDOWS_KEY_CONTAINER cannot contain quotes or line breaks')
  }
  const tokenPin = input.tokenPin
  if (tokenPin === undefined || tokenPin.length === 0) {
    throw new Error('NULU_DESKTOP_WINDOWS_TOKEN_PIN must contain the SafeNet Token Password')
  }
  if (/[\]"\r\n]/u.test(tokenPin)) {
    throw new Error('NULU_DESKTOP_WINDOWS_TOKEN_PIN cannot contain "]", quotes, or line breaks because the SafeNet key-container syntax uses them as delimiters')
  }
...
function resolveCertificateFile(value) {
  ...
  if (!candidate) {
    throw new Error('NULU_DESKTOP_WINDOWS_CER_FILE must identify the public X.509 leaf certificate file')
  }
  let path
  let certificate
  try {
...
function resolveSignTool(value) {
  const candidate = value?.trim()
  if (!candidate) {
    throw new Error('NULU_DESKTOP_WINDOWS_SIGNTOOL must identify the SafeNet-compatible SignTool executable')
  }
  let path
  try {
    path = realpathSync(candidate)
    ...
    throw new Error(`NULU_DESKTOP_WINDOWS_SIGNTOOL is missing or is not an executable file: ${candidate}`)
  }
  return path
}
...
export function buildWindowsSigningEnvironment(environment, input) {
  return {
    ...
    NULU_DESKTOP_WINDOWS_SIGNTOOL: input.signTool,
    NULU_DESKTOP_WINDOWS_CER_FILE: input.certificateFile,
    NULU_DESKTOP_WINDOWS_TOKEN_PIN: input.tokenPin,
    NULU_DESKTOP_WINDOWS_KEY_CONTAINER: input.keyContainer,
    NULU_DESKTOP_WINDOWS_SIGN_TARGET: input.path,
    NULU_DESKTOP_WINDOWS_SIGN_APPEND: input.isNest ? '1' : '',
  }
}
