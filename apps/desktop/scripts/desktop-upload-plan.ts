...
/**
 * Validate the completed package record, nulu version, update metadata, hashes, and target files.
...
export async function createDesktopUploadPlan(
  ...
  const nuluVersion = await manifestVersion(join(repositoryRoot, 'package.json'), 'nulu package')
  const desktopVersion = await manifestVersion(join(appRoot, 'package.json'), 'desktop package')
  if (nuluVersion !== desktopVersion) {
    throw new Error(`desktop upload: desktop version ${desktopVersion} does not match current nulu version ${nuluVersion}`)
  }
  ...
  if (buildRecord.schemaVersion !== 1
    ...
    || buildRecord.version !== nuluVersion
    || buildRecord.environment !== update.environment
    || buildRecord.publicUrl !== update.publicUrl) {
    throw new Error(`desktop upload: ${targetName} package completion record does not match nulu ${nuluVersion} and ${update.environment} update destination`)
  }

  const metadataFilename = desktopUpdateMetadataFilename(nuluVersion, target.platform)
  ...
  if (metadataVersion !== nuluVersion) {
    throw new Error(`desktop upload: ${metadataFilename} version ${metadataVersion} does not match current nulu version ${nuluVersion}`)
  }
  ...
  const base = `nulu-harness-${nuluVersion}-${target.os}-${target.arch}`
