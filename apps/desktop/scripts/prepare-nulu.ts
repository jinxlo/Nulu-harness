...
const NULU_OUTPUT_ROOT = BUILD_PATHS.nulu
const BUILD_ROOT = mkdtempSync(join(tmpdir(), 'nulu-desktop-runtime-'))
const STORE_ROOT = join(BUILD_ROOT, 'store')
const RUNTIME_ROOT = BUILD_PATHS.runtime
const PNPM_BUILD_STATE = BUILD_PATHS.nuluPnpm
...
function desktopRelease(): DesktopRelease {
  ...
  const nuluVersion = manifestVersion(resolve(APP_ROOT, '..', '..', 'package.json'), 'root nulu package')
  if (version !== nuluVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @worldapptechnologies/nulu, found ${nuluVersion}`)
  }
...
async function main(): Promise<void> {
  rmSync(NULU_OUTPUT_ROOT, { recursive: true, force: true })
  ...
  try {
    ...
    mkdirSync(NULU_OUTPUT_ROOT, { recursive: true })
    cpSync(modules, join(NULU_OUTPUT_ROOT, 'node_modules'), {
      recursive: true, dereference: true,
      filter: source => desktopRuntimeFileExclusion(relative(modules, source), target) === undefined,
    })
    writeFileSync(join(NULU_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@worldapptechnologies/nulu-desktop-runtime', private: true, version: release.version, type: 'module',
    ...
    for (const file of DESKTOP_HOST_RUNTIME_FILES) {
      if (!existsSync(join(NULU_OUTPUT_ROOT, 'node_modules', DESKTOP_HOST_PACKAGE, file))) {
        throw new Error(`desktop runtime: missing private Host file ${file}`)
      }
    }
    if (process.platform === 'darwin') {
      const appId = resolveDesktopAppId(process.env)
      if (process.env.NULU_DESKTOP_UNSIGNED === '1') {
        await adhocSignMacOSRuntime(NULU_OUTPUT_ROOT, appId)
      } else {
        await signMacOSRuntime(NULU_OUTPUT_ROOT, appId, resolveMacOSSigningEnvironment(process.env))
      }
    }
    writeDesktopRuntime(NULU_OUTPUT_ROOT, release, packageSet.packages.map(entry => entry.name), target)
    const descriptor = await verifyDesktopRuntime(NULU_OUTPUT_ROOT, release.version, target)
    await new Promise<void>((accept, reject) => {
      execFile(NODE, [join(APP_ROOT, 'tests/fixtures/runtime-payload-smoke.mjs'), NULU_OUTPUT_ROOT],
    ...
    await smokeDesktopRuntime(NULU_OUTPUT_ROOT, NODE, descriptor)
    await verifyDesktopRuntime(NULU_OUTPUT_ROOT, release.version, target)
  } catch (error) {
    rmSync(NULU_OUTPUT_ROOT, { recursive: true, force: true })
    throw error
  } finally {
