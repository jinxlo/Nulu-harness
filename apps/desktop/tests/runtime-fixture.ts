 ...
 * @param version - Shell and nulu version.
...
export function runtimeFixture(root: string, version = '1.0.0', nodeVersion = '24.17.0'): DesktopRuntimeDescriptor {
  const names = ['@worldapptechnologies/nulu', '@worldapptechnologies/nulu-desktop-host', '@worldapptechnologies/nulu-base', '@worldapptechnologies/nulu-web-app', '@worldapptechnologies/cordis']
  for (const name of names) writePackage(join(root, 'node_modules'), name, { version })
  for (const file of DESKTOP_HOST_RUNTIME_FILES) {
    const path = join(root, 'node_modules', '@worldapptechnologies/nulu-desktop-host', file)
