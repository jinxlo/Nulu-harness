...
describe('Windows token signing', () => {
  it('passes only the validated BAT fields to the signing command interpreter', () => {
    expect(buildWindowsSigningEnvironment({
      ...
      NULU_DESKTOP_WINDOWS_TOKEN_PIN: 'inherited-token-secret',
      WORLD_APP_TECHNOLOGIES_API_KEY: 'api-secret',
      BUILD_PASSWORD: 'build-secret',
    }, {
      certificateFile: CERTIFICATE_FILE,
      signTool: 'C:\\tools\\signtool.exe',
      path: 'C:\\release\\Nulu Harness.exe',
      isNest: false,
      ...
      NULU_DESKTOP_WINDOWS_CER_FILE: CERTIFICATE_FILE,
      NULU_DESKTOP_WINDOWS_TOKEN_PIN: 'token-secret!',
      NULU_DESKTOP_WINDOWS_KEY_CONTAINER: 'te-container',
      NULU_DESKTOP_WINDOWS_SIGN_TARGET: 'C:\\release\\Nulu Harness.exe',
      NULU_DESKTOP_WINDOWS_SIGN_APPEND: '',
    })
  })
  ...
  it('requests an appended signature only for an electron-builder nested task', () => {
    expect(buildWindowsSigningEnvironment({}, {
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    }).NULU_DESKTOP_WINDOWS_SIGN_APPEND).toBe('1')
  })
  ...
  it('keeps the verified SafeNet command in an ASCII CRLF CMD file', async () => {
    ...
    expect(text).toContain('set "NULU_DESKTOP_WINDOWS_CER_FILE="\r\n')
    expect(text).toContain('set "NULU_DESKTOP_WINDOWS_TOKEN_PIN="\r\n')
  ...
  it('rejects incomplete signing identities and non-SHA-256 signing tasks', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'nulu-windows-sign-tool-'))
    ...
    expect(() => createWindowsTokenSigner({
      ...
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toThrow(/NULU_DESKTOP_WINDOWS_CER_FILE/u)
    expect(() => createWindowsTokenSigner({
      certificateFile,
      signTool: undefined,
      tokenPin: 'token-secret!',
      keyContainer: 'te-container',
    })).toThrow(/NULU_DESKTOP_WINDOWS_SIGNTOOL/u)
    ...
    try {
      expect(() => createWindowsTokenSigner({
        ...
        signTool,
        tokenPin: 'token-secret!',
      })).toThrow(/NULU_DESKTOP_WINDOWS_KEY_CONTAINER/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: '',
        keyContainer: 'te-container',
      })).toThrow(/NULU_DESKTOP_WINDOWS_TOKEN_PIN/u)
      expect(() => createWindowsTokenSigner({
        certificateFile,
        signTool,
        tokenPin: 'token]secret',
  ...
  it('clears a certificate table inherited beyond the generated uninstaller', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'nulu-windows-sign-'))
