...
function temporaryRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'nulu-desktop-development-test-'))
  roots.push(root)
  return root
}
...
describe('desktop development project', () => {
  it('projects the built nulu and Desktop Host applications with their dependency graph', () => {
    ...
    mkdirSync(join(dependencies, '@worldapptechnologies', 'nulu'), { recursive: true })
    writeFileSync(join(cli, 'package.json'), '{"name":"@worldapptechnologies/nulu","version":"1.2.3"}\n')
    writeFileSync(join(host, 'package.json'), '{"name":"@worldapptechnologies/nulu-desktop-host","version":"1.2.3"}\n')
    writeFileSync(join(host, 'lib', 'index.js'), '')
    writeFileSync(join(dependencies, '@worldapptechnologies', 'nulu', 'package.json'), '{}\n')
    ...
    expect(realpathSync(join(project, 'node_modules', '@worldapptechnologies', 'nulu'))).toBe(realpathSync(cli))
    expect(realpathSync(join(project, 'node_modules', '@worldapptechnologies', 'nulu-desktop-host'))).toBe(realpathSync(host))
    ...
    expect(manifest.dependencies['@worldapptechnologies/nulu']).toBe('1.2.3')
    expect(manifest.dependencies['@worldapptechnologies/nulu-desktop-host']).toBe('1.2.3')
  })
  ...
  it('rejects a CLI package from another release', () => {
    ...
    writeFileSync(join(cli, 'package.json'), '{"name":"@worldapptechnologies/nulu","version":"2.0.0"}\n')
    writeFileSync(join(host, 'package.json'), '{"name":"@worldapptechnologies/nulu-desktop-host","version":"1.2.3"}\n')
    writeFileSync(join(host, 'lib', 'index.js'), '')
    expect(() => prepareDevelopmentProject({
      ...
      dependencyDir: dependencies,
      release: release(),
    })).toThrow(/must be @worldapptechnologies\/nulu@1\.2\.3/u)
  })
})
