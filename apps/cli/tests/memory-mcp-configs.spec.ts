...
import * as McpClient from '@worldapptechnologies/nulu-mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@worldapptechnologies/nulu-mcp-client')
    ...
    expect(source).not.toContain('WORLD_APP_TECHNOLOGIES_API_KEY')
  })
