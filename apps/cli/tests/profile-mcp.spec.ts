...
import { composeEntries, loadProfile, PROFILE_TEMPLATES } from '@worldapptechnologies/nulu-app-boot'

const installAnchor = fileURLToPath(new URL('../package.json', import.meta.url))
const resourcePackage = '@worldapptechnologies/nulu-mcp-resources'
...
describe('shipped MCP resource composition', () => {
  it.each(Object.keys(PROFILE_TEMPLATES))('%s carries one shared resource consumer without a server', (name) => {
    const home = mkdtempSync(join(tmpdir(), 'nulu-profile-mcp-'))
    try {
      const profile = loadProfile('nulu', name, installAnchor, home)
      ...
      expect(owners.map(owner => owner.packageName)).toEqual([
        name === 'sdk-minimal' ? '@worldapptechnologies/nulu-sdk-minimal' : '@worldapptechnologies/nulu-base',
      ])
    } finally {
