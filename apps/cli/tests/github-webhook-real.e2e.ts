/** Real CLI and Nulu evidence for a GitHub webhook-created Session. */
...
const MARKER = 'NULU_GITHUB_WEBHOOK_REAL_E2E_OK'
...
function observeProcess(child: ChildProcess): ProcessObservation {
  ...
  const timer = setTimeout(() => {
    if (!settled) rejectReady(new Error(`nulu web did not become ready within 90s:\n${output}`))
  }, 90_000)
  timer.unref()
  const append = (chunk: Buffer | string): void => {
    output = `${output}${String(chunk)}`.slice(-100_000)
    const match = /nulu web: (http:\/\/[^\s]+)/u.exec(output)
  ...
  child.once('exit', (code) => {
    if (!settled) rejectReady(new Error(`nulu web exited before readiness (code ${String(code)}):\n${output}`))
  })
...
async function eventually<T>(
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`nulu web exited while waiting for ${label} (code ${String(child.exitCode)}):\n${processOutput()}`)
    }
    try {
...
async function sendGitHubDelivery(origin: string): Promise<Response> {
  const body = JSON.stringify({
    ...
    number: 4242,
    repository: { full_name: 'worldapptechnologies/nulu-harness' },
    pull_request: {
      title: 'Real CLI webhook e2e',
      html_url: 'https://github.com/worldapptechnologies/nulu-harness/pull/4242',
      draft: false,
      user: { login: 'octocat' },
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('GitHub webhook through the real nulu CLI and model', () => {
  it('creates, attaches, prompts, and completes a Workspace Session', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'nulu-github-webhook-real-'))
    ...
    const child = spawn(process.execPath, [
      ...
      env: {
        ...
        NULU_GITHUB_E2E_MARKER: MARKER,
        NULU_GITHUB_E2E_WORKSPACE: workspacePath,
        NULU_GITHUB_WEBHOOK_PORT: String(webhookPort),
        NULU_GITHUB_WEBHOOK_SECRET: SECRET,
    ...
    try {
      ...
      const completed = await eventually(
        child,
        observation.text,
        'a real Nulu assistant response',
        async () => await history(baseUrl, sessionId),
