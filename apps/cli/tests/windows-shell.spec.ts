 ...
 * the REAL shipped bundle layers (nulu-base + nulu-web-app resolved from the
...
import { SHIPPED_PRESET_ROOT } from '@worldapptechnologies/nulu-agent-presets'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@worldapptechnologies/nulu-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL nulu-base/nulu-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'nulu-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@worldapptechnologies/nulu-base', '@worldapptechnologies/nulu-web-app'])
    const profile = loadProfile('nulu', 'web', anchor, home)
    ...
    for (const name of ['@worldapptechnologies/nulu-pwsh-sandbox', '@worldapptechnologies/nulu-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'nulu-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@worldapptechnologies/nulu-base'])
    const profile = loadProfile('nulu', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
