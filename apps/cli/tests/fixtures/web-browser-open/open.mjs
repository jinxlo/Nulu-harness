...
export default async function open(url) {
  ...
  console.log(`nulu browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__NULU_BOOT__'),
    apiKeyPresent: process.env.WORLD_APP_TECHNOLOGIES_API_KEY !== undefined,
    nuluHomePresent: process.env.NULU_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.nulu-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
