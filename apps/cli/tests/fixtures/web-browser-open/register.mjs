...
const exitMarker = join(process.cwd(), `.nulu-browser-open-${process.pid}`)
...
if (process.env.NULU_BROWSER_OPEN_TEST_EXIT_ON_READY === '1') {
  const originalLog = console.log
  console.log = (...args) => {
    originalLog(...args)
    if (typeof args[0] === 'string' && args[0].startsWith('nulu web: ')) {
      setTimeout(() => process.exit(0), 250)
    }
  }
}

if (process.env.NULU_BROWSER_OPEN_TEST_EXIT_ON_FAILURE === '1') {
