...
describe('acp-agent over real stdio (no key required)', () => {
  it('emits only framed JSON-RPC on stdout', async () => {
    ...
    // A dummy key lets the nulu adapter APPLY (it only checks the key is
  ...
  it('session/new succeeds over real stdio (no model call)', async () => {
    ...
    workdir = await mkdtemp(join(tmpdir(), 'acp-e2e-'))
    // A dummy key lets the nulu adapter boot (it only checks presence, not
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('acp-agent e2e: real prompt over ACP', () => {
