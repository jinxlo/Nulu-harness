/** Generic keyless ACP v1 automation-control conformance over the real nulu profile. */
...
describe('standard ACP v1 control surface', () => {
  it('selects, mounts MCP, closes, restarts, resumes, and cancels through the SDK only', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'nulu-acp-control-'))
    const persistenceRoot = join(cwd, '.sessions')
    const env = { NULU_CONFORMANCE_PERSISTENCE_ROOT: persistenceRoot, NULU_TELEMETRY_DISABLED: '1' }
