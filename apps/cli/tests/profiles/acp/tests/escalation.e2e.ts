...
import { bwrapProfileArgs } from '@worldapptechnologies/nulu-sandbox-local/src/profiles.ts'
import { cleanupAcpExampleTest } from './cleanup.ts'

/**
 * The default ACP composition (`cordis.yml`) end to end.
 *
 * Keyless smoke: boot the real profile patch through `nulu --profile acp` as
 ...
 *
 * With-key escalation flow (self-skips without WORLD_APP_TECHNOLOGIES_API_KEY or a usable
...
function launchExampleAcpAgent(
  ...
  const launched = launchAcpTestAgent({
    ...
    env: {
      WORLD_APP_TECHNOLOGIES_API_KEY: process.env.WORLD_APP_TECHNOLOGIES_API_KEY ?? 'sk-dummy-for-boot',
      NULU_PERMISSION_MODE: sandboxMode,
    },
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY || !hasRunner)('default sandbox composition e2e: the live approval loop', () => {
