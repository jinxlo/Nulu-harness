...
async function workspacePtcModeHarness(): Promise<Context> {
  ...
  await harness.plugin(AgentLoop, { agents: [] })
  await harness.plugin(LlmGateway, { models: [{ id: 'nulu-v4-flash' }] })
  await mountRuntime(harness)
  return harness
}
...
describe('PTC mode typed values: keyless real-process contracts', () => {
  ...
  it('returns a background job id, settles the outer run, and polls that id to completion', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-ptc-background-'))
  ...
  it('pre-abort spawns nothing; post-publication abort leaves job_kill as the cancellation owner', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-ptc-task-cancel-'))
    ctx = await backgroundPtcModeHarness(workdir)
  ...
  it('keeps foreground bash coupled to the outer signal', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-ptc-foreground-cancel-'))
    ctx = await backgroundPtcModeHarness(workdir)
...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('PTC mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [run_code], bridges sub-calls, and returns curated output', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-ptc-e2e-'))
    ctx = await ptcModeHarness(workdir)
    const agent = await ctx.agentLoop.create(SessionId('e2e-ptc'), { provider: 'worldapp-gateway', model: 'nulu-5' })
