...
describe.skipIf(!process.env.WORLD_APP_TECHNOLOGIES_API_KEY)('coding task: fix a failing test via bash', () => {
  it('repairs add.js so node add.test.js passes', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'nulu-coding-task-'))
    ...
    const agent = await ctx.agentLoop.create(SessionId('e2e-task'), { provider: 'worldapp-gateway', model: 'nulu-5' })
