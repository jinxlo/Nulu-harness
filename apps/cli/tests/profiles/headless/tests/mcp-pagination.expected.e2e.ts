...
it('warns when MCP discovery exceeds the SDK page limit and completes the headless task', async () => {
  const { stdout, stderr } = await runLoaderSmoke({
    ...
    env: {
      NULU_MCP_PAGINATION_FIXTURE: fileURLToPath(new URL('pagination-limit-server.ts', fixtureRoot)),
  ...
  expect(stderr).toContain('nulu: warning: 1 entry did not activate')
  expect(stderr).toContain('mcp-pagination-limit (@worldapptechnologies/nulu-mcp-client)')
