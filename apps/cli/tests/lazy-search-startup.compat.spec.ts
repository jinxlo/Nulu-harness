 ...
 * web profile (nulu-base + nulu-web-app bundle patches, auto-initialized).
...
const requireBuiltArtifacts = process.env.NULU_REQUIRE_BUILT_CLI_SMOKE === '1'
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      WORLD_APP_TECHNOLOGIES_API_KEY: 'nulu-cli-smoke-dummy-key',
      NULU_HOME: join(cwd, '.nulu'),
    }
    delete env.WORLD_APP_TECHNOLOGIES_BASE_URL
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /nulu web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'nulu-cli-lazy-search-'))
    try {
      const result = await runBuiltWeb(cwd)
      expect(result.stdout).toMatch(/nulu web: http:\/\/127\.0\.0\.1:\d+/u)
