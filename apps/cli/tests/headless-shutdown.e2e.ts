...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@worldapptechnologies/nulu-loader-smoke'
...
markers = [b"nulu-test: never-dispose ready", b"nulu-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'nulu-headless-shutdown-'))
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      private: true,
      dependencies: {},
      nulu: { profile: { bundles: ['@worldapptechnologies/nulu-base', '@worldapptechnologies/nulu-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        WORLD_APP_TECHNOLOGIES_API_KEY: 'keyless-shutdown-no-call',
        NULU_TELEMETRY_DISABLED: '1',
        NULU_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`nulu headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`nulu headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('nulu: observing at ')
    expect(output).toContain('nulu-test: never-dispose ready')
    expect(output).toContain('nulu-test: never-dispose started')
