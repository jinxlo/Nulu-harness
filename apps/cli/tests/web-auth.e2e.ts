/** Real `nulu web` authentication against a temporary Harness home. */
...
const NULU_SOURCE_BIN = join(REPO_ROOT, 'apps/cli/src/bin.ts')
...
function cleanEnvironment(root: string, nuluHome: string): NodeJS.ProcessEnv {
...
async function startWeb(root: string, nuluHome: string, port: number): Promise<RunningWeb> {
  const child = spawn(process.execPath, [
    ...
    cwd: root,
    env: cleanEnvironment(root, nuluHome),
  ...
  const launchUrl = await new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      fail(new Error(`nulu web did not become ready:\n${redact(output)}`))
    }, 90_000)
    ...
    child.once('exit', (code) => {
      fail(new Error(`nulu web exited before readiness (${String(code)}):\n${redact(output)}`))
    })
  })
...
describe('nulu web authentication through the real CLI', () => {
  it('rejects a forged loopback Host and preserves the browser cookie across restart', { timeout: 180_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'nulu-web-auth-real-cli-'))
    const nuluHome = join(root, '.nulu')
    const port = await freePort()
    let first: RunningWeb | undefined
    let second: RunningWeb | undefined
    try {
      first = await startWeb(root, nuluHome, port)
      ...
      await stopWeb(first)
      first = undefined
      second = await startWeb(root, nuluHome, port)
      ...
      const credentialMode = (await stat(join(nuluHome, '.credentials.yaml'))).mode & 0o777
