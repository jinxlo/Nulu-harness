/**
 * Shared profile boot for every `nulu` surface: resolve the profile, stack its
 * patch layers (bundle layers in `nulu.profile.bundles` order, the profile's
 ...
 * @module @worldapptechnologies/nulu/profile-boot
 */
...
/**
 * The home-level user patch layer (`$NULU_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$NULU_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveNuluHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this nulu installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the NULU_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# nulu profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's nulu.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `NULU_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$NULU_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `nulu.profile.bundles` order (a base-backed profile gets the base bundle's
 * platform-gated shell rows), the profile's user layer, the home-level user
 * layer (`$NULU_HOME/cordis.patch.yml` — machine-local preferences that apply
...
async function composeProfile(
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.NULU_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  if (composed.profile.patchReload === 'live'
    ...
    try {
      // Config-only HMR for the live profile patch layer: nulu-base disables
