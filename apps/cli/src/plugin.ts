/**
 * `nulu plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `nulu.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `nulu.bundle` joins the layer stack; a
 ...
 * gained its `nulu.bundle` declaration in a newer version.
 * @module @worldapptechnologies/nulu/plugin
 */
 ...
 * @returns true when the package manifest declares `nulu.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.nulu?.bundle?.patch !== undefined
}

/**
 * Reconcile `nulu.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `nulu.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.nulu?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no nulu.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (nulu-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.nulu = { ...after.nulu, profile: { ...after.nulu?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `nulu` was invoked from.
...
/**
 * Run one `nulu plugin` invocation: init if needed, forward to pnpm, reconcile.
