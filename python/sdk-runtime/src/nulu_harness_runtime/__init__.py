"""Locate and execute the bundled nulu CLI shipped with the Python SDK runtime.
...
- **exe (production)**: single-file Node executables named
  ``nulu-harness-sdk-runtime-<platform>-<arch>`` for Linux/macOS and an
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@worldapptechnologies/nulu/lib/bin.js`` on a
...
Both carriers execute the same nulu command grammar. The Python SDK selects the
``sdk`` profile and requires an explicit Harness home; the installed ``nulu``
console command requires ``NULU_HOME`` for the same reason.
"""
...
PACKAGE_METADATA_FILENAME = "nulu-harness-runtime.json"

RUNTIME_MODE_ENV_VAR = "NULU_RUNTIME_MODE"
...
_EXE_ACQUISITION_HINT = (
    ...
    "nulu-harness checkout, or install the matching `nulu-harness-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"nulu-harness-runtime-bin is missing {metadata}")
    return root
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"nulu-harness-sdk-runtime-{tag}{extension}"
    if not path.is_file():
        raise FileNotFoundError(
            f"nulu-harness-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    ripgrep = (
    ...
    if not ripgrep.is_file():
        raise FileNotFoundError(
            f"nulu-harness-runtime-bin is missing the ripgrep sidecar at {ripgrep}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"nulu-harness-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    ``NULU_RUNTIME_MODE`` environment variable (``exe`` | ``node``), then
    ...
    raise ValueError(
        f"unsupported Nulu Harness runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _current_platform_tag() -> str:
    ...
    if (
        ...
        raise FileNotFoundError(
            "no bundled Nulu Harness SDK runtime exists for this platform "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a nulu-harness "
...
def main() -> None:
    """Launch the CLI with explicit NULU_HOME; wait on Windows, replace the process on POSIX."""
    if not os.environ.get("NULU_HOME", "").strip():
        print(
            "nulu: the Python runtime command requires an explicit NULU_HOME; "
            "it never uses ~/.nulu implicitly",
            file=sys.stderr,
        )
        raise SystemExit(2)
