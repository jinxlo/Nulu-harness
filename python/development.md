...
Trusted pull requests and master pushes also run `--scenario sdk-live --installed-wheel` on each selected native target. That scenario performs two tool-using turns against `https://platform.worldapptechnologies.com/api/v1`: it checks the created file immediately, replaces its content with a host-only random challenge, and requires the second turn to copy the changed content into a fresh receipt without modifying the source. Both turns must complete with the exact sentinel answer and a model-requested tool call; external byte comparisons check the files. Missing repository secrets fail instead of self-skipping. Fork and Dependabot pull requests run the complete keyless installed-wheel path but receive no key.

An interactive smoke test needs `WORLD_APP_TECHNOLOGIES_API_KEY` in the environment or repository-root `.env`:

```python
from nulu_harness import NuluHarness

with NuluHarness(nulu_home="/absolute/path/to/test-nulu-home") as harness:
    print(harness.run("say hi").final_response)
```

Alternatively export a non-empty `NULU_HOME`. The SDK rejects a launch that would silently use `~/.nulu`.

## Run against Node source

Repository contributors can select either development route; both execute the normal `nulu --profile sdk` launcher:

- Set `NULU_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
- Set `nulu_bin` to the absolute built `apps/cli/lib/bin.js` path to exercise the checkout's CLI directly. Supply an explicit `nulu_home`, plus `profile` and ordered `patches` as needed.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `nulu-harness-runtime-bin` version.
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/nulu-harness-sdk-runtime-macos-arm64 --output-dir dist-python
pip install \
  "dist-python/nulu_harness_sdk-$version-py3-none-any.whl" \
  "dist-python/nulu_harness_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
