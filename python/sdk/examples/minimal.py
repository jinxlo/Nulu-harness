...
def main() -> None:
    ...
    configured_home = os.environ.get("NULU_HOME", "")
    ...
    parser.add_argument("--provider", default="worldapp-gateway")
    parser.add_argument("--model", default=os.environ.get("NULU_MODEL", "nulu-5"))
    parser.add_argument("--max-tokens", type=int)
    args = parser.parse_args()
    if args.nulu_home is None:
        parser.error("--nulu-home or a non-empty NULU_HOME is required")

    workspace = args.workspace.resolve()
    nulu_home = args.nulu_home.resolve()
    with NuluHarness(
