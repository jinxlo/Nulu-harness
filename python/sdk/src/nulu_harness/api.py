...
class NuluHarnessConfig:
    ...
    WORLD_APP_TECHNOLOGIES_API_KEY and WORLD_APP_TECHNOLOGIES_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "worldapp-gateway"
    model: str = "nulu-5"
...
class NuluHarness:
    """Reusable synchronous SDK for running Nulu Harness agent turns.
    ...
    def __init__(
        self,
        config: NuluHarnessConfig | None = None,
        *,
        _launch_args: tuple[str, ...] | None = None,
        **kwargs: object,
    ) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either NuluHarnessConfig or keyword options, not both")
        self.config = config or NuluHarnessConfig(**kwargs)
        ...
        if self.config.base_url is not None:
            env["WORLD_APP_TECHNOLOGIES_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["WORLD_APP_TECHNOLOGIES_API_KEY"] = self.config.api_key

        self._client = HarnessClient(
            HarnessConfig(
                nulu_bin=self.config.nulu_bin,
                profile=self.config.profile,
                patches=self.config.patches,
                nulu_home=self.config.nulu_home,
                cwd=runtime_cwd,
                env=env,
    ...
    def __enter__(self) -> "NuluHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: NuluHarness, session_id: str) -> None:
