...
class HarnessConfig:
    """Configuration for launching the local Nulu Harness SDK runtime."""

    nulu_bin: str | None = None
    profile: str = "sdk"
    patches: tuple[str, ...] = ()
    nulu_home: str | None = None
    cwd: str | None = None
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Nulu Harness SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Nulu Harness runtime closed"))
    ...
    def initialize(
        ...
        *,
        ...
        try:
            ...
            self.close()
            raise TimeoutError(f"{error}\nselected nulu profile {self.config.profile!r}") from error
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Nulu Harness runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Nulu Harness runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Nulu Harness runtime") from exc

    def _start_reader_thread(self) -> None:
        self._reader_thread = threading.Thread(target=self._reader_loop, name="nulu-runtime-reader", daemon=True)
        self._reader_thread.start()

    def _start_stderr_thread(self) -> None:
        self._stderr_thread = threading.Thread(target=self._stderr_loop, name="nulu-runtime-stderr", daemon=True)
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Nulu Harness runtime stdout closed"))
    ...
    def _default_launch_args(self, env: dict[str, str]) -> tuple[str, ...]:
        if self.config.nulu_bin is None:
            try:
                from nulu_harness_runtime import resolve_bundled_launch_args
            except ImportError as exc:
                raise FileNotFoundError(
                    "Unable to locate the bundled Nulu Harness nulu runtime. "
                    "Install nulu-harness-runtime-bin."
                ) from exc
            base = resolve_bundled_launch_args()
        else:
            base = (str(Path(self.config.nulu_bin).expanduser().resolve()),)

        if self.config.nulu_home is not None:
            if not self.config.nulu_home.strip():
                raise ValueError("HarnessConfig requires a non-empty nulu_home")
            env["NULU_HOME"] = str(Path(self.config.nulu_home).expanduser().resolve())
        elif not env.get("NULU_HOME", "").strip():
            raise ValueError(
                "HarnessConfig requires an explicit nulu_home or non-empty NULU_HOME; "
                "the Python SDK never uses ~/.nulu implicitly"
            )
