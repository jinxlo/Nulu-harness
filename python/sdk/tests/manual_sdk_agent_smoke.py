"""Drive the repo-source nulu SDK profile through the SDK and a keyless mock SSE server.
...
def run_smoke(repo_root: Path, keep_sessions: bool) -> None:
    nulu_home = Path(tempfile.mkdtemp(prefix="nulu-sdk-smoke-home-"))
    session_root = nulu_home / "sessions"
    ...
    try:
        with NuluHarness(
            ...
            env={
                ...
                "WORLD_APP_TECHNOLOGIES_API_KEY": "sdk-smoke-key",
            },
    ...
    if keep_sessions:
        print(f"kept_nulu_home={nulu_home}")
    else:
        shutil.rmtree(nulu_home)
        print("removed temporary nulu home")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to the nulu-harness checkout.",
    )
