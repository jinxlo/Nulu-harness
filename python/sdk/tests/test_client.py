...
from nulu_harness import NuluHarness, HarnessClient, HarnessConfig, Notification, RunResult, SdkProtocolError
from nulu_harness.errors import JsonRpcError
...
json.dump({
    "WORLD_APP_TECHNOLOGIES_API_KEY": os.environ.get("WORLD_APP_TECHNOLOGIES_API_KEY"),
    "WORLD_APP_TECHNOLOGIES_BASE_URL": os.environ.get("WORLD_APP_TECHNOLOGIES_BASE_URL"),
    "NULU_CWD": os.environ.get("NULU_CWD"),
    "NULU_SESSION_ROOT": os.environ.get("NULU_SESSION_ROOT"),
    "NULU_CORDIS_CONFIG": os.environ.get("NULU_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    with NuluHarness(
        ...
        env={
            ...
            "WORLD_APP_TECHNOLOGIES_API_KEY": "env-key",
            "WORLD_APP_TECHNOLOGIES_BASE_URL": "http://127.0.0.1:4321",
        },
    ) as harness:
    ...
    assert dumped_env["WORLD_APP_TECHNOLOGIES_API_KEY"] == "env-key"
    assert dumped_env["WORLD_APP_TECHNOLOGIES_BASE_URL"] == "http://127.0.0.1:4321"
    assert dumped_env["NULU_CWD"] is None
    assert dumped_env["NULU_SESSION_ROOT"] is None
    assert dumped_env["NULU_CORDIS_CONFIG"] is None
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("NULU_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with NuluHarness(_launch_args=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
for line in sys.stdin:
    ...
    method = msg.get("method")
    if method == "initialize":
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-nulu"}}}), flush=True)
    elif method == "session/prompt":
        params = msg.get("params") or {}
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        init = client.initialize(provider="worldapp-gateway", cwd="/workspace", model="dsagent")
        assert init.serverInfo.name == "fake-nulu"
...
""".strip()
    ...
    with HarnessClient(_launch_args=(sys.executable, str(script))) as client:
        client.initialize(provider="worldapp-gateway", cwd="/workspace", model="dsagent")
        with (
...
""".strip()
    ...
    with pytest.raises(JsonRpcError, match="bad initialize") as excinfo:
        client.initialize(provider="worldapp-gateway", cwd=".", model="dsagent")
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from nulu_harness import NuluHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(NuluHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in NuluHarnessConfig.__dataclass_fields__
    assert "max_tokens" in NuluHarnessConfig.__dataclass_fields__
    assert "reasoning_effort" in NuluHarnessConfig.__dataclass_fields__
    ...
    assert {"nulu_bin", "profile", "patches", "nulu_home"} <= set(
        NuluHarnessConfig.__dataclass_fields__
    )
    assert {"nulu_bin", "profile", "patches", "nulu_home"} <= set(
        HarnessConfig.__dataclass_fields__
    )
    assert "initialize_timeout_seconds" in NuluHarnessConfig.__dataclass_fields__
    assert "initialize_timeout_seconds" in HarnessConfig.__dataclass_fields__
    assert NuluHarnessConfig().initialize_timeout_seconds == 30.0
    ...
    for removed in ("cordis", "session_root", "runtime_bin", "bridge_bin", "launch_args_override"):
        assert removed not in NuluHarnessConfig.__dataclass_fields__
...
def _install_fake_bundled_nulu(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Install a fake runtime package that records nulu argv and serves lifecycle calls."""
    runtime = tmp_path / "nulu.py"
    runtime.write_text(
        """
import json
import os
...
json.dump({
    "argv": sys.argv[1:],
    "NULU_HOME": os.environ.get("NULU_HOME"),
    "NULU_CORDIS_CONFIG": os.environ.get("NULU_CORDIS_CONFIG"),
...
""".strip()
    ...
    module_dir = tmp_path / "nulu_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "nulu_harness_runtime", raising=False)


def test_client_default_launch_uses_bundled_nulu_sdk_profile_and_explicit_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    home = tmp_path / "home"
    ...
    patch.write_text("[]\n")
    _install_fake_bundled_nulu(tmp_path, monkeypatch)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("NULU_HOME", str(tmp_path / "ambient-home"))
    monkeypatch.delenv("NULU_CORDIS_CONFIG", raising=False)

    with HarnessClient(HarnessConfig(
        profile="sdk",
        patches=("sdk.patch.yml",),
        nulu_home=str(home),
        env={"ENV_DUMP": str(env_dump), "NULU_HOME": str(tmp_path / "env-home")},
    )) as client:
        init = client.initialize(provider="worldapp-gateway", cwd="/workspace", model="nulu-5-ultra")

    assert init.serverInfo.name == "bundled-runtime"
    assert json.loads(env_dump.read_text()) == {
        "argv": ["--profile", "sdk", "--patch", str(patch)],
        "NULU_HOME": str(home),
        "NULU_CORDIS_CONFIG": None,
    }


def test_client_accepts_explicit_environment_nulu_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    env_dump = tmp_path / "env.json"
    ...
    with HarnessClient(
        HarnessConfig(profile="custom", env={"ENV_DUMP": str(env_dump), "NULU_HOME": str(home)})
    ) as client:
        client.initialize(provider="worldapp-gateway", cwd="/workspace", model="nulu-5-ultra")
...
def test_client_rejects_an_implicit_default_nulu_home(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _install_fake_bundled_nulu(tmp_path, monkeypatch)
    monkeypatch.delenv("NULU_HOME", raising=False)

    with pytest.raises(ValueError, match="explicit nulu_home or non-empty NULU_HOME"):
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install nulu-harness-runtime-bin"):
        HarnessClient(HarnessConfig(nulu_home="/explicit/home")).start()
