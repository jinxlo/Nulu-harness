...
def test_macos_wheel_tag_does_not_claim_unsupported_node_platforms() -> None:
    ...
    assert build_python_release.PLATFORMS["macos-arm64"][1] == "nulu-harness-sdk-runtime-macos-arm64"
    assert build_python_release.PLATFORMS["macos-x64"][0] == "macosx_14_0_x86_64"
    assert build_python_release.PLATFORMS["macos-x64"][1] == "nulu-harness-sdk-runtime-macos-x64"


def test_windows_wheel_tag_and_payload_are_x64_only() -> None:
    assert build_python_release.PLATFORMS["win-x64"] == (
        "win_amd64",
        "nulu-harness-sdk-runtime-win-x64.exe",
    )
...
def test_stage_sdk_keeps_distribution_module_and_runtime_pin_distinct(tmp_path: Path) -> None:
    ...
    assert 'name = "nulu-harness-sdk"' in pyproject
    assert 'version = "1.2.3"' in pyproject
    assert 'license = "MIT"' in pyproject
    assert '"nulu-harness-runtime-bin==1.2.3"' in pyproject
    assert 'license-files = ["LICENSE"]' in pyproject
    assert (destination / "LICENSE").read_bytes() == (ROOT / "LICENSE").read_bytes()
    assert (destination / "src" / "nulu_harness" / "__init__.py").is_file()
...
def test_stage_runtime_copies_platform_payload(
    ...
    executable = tmp_path / f"nulu-harness-sdk-runtime-{target}"
    ...
    runtime_dir = destination / "src" / "nulu_harness_runtime" / "runtime"
    assert {
        path.name: path.read_bytes()
        for path in runtime_dir.glob("nulu-harness-sdk-runtime-*")
    } == expected
    pyproject = (destination / "pyproject.toml").read_text()
    assert 'license = "MIT"' in pyproject
    assert 'license-files = ["LICENSE", "THIRD_PARTY_NOTICES.md"]' in pyproject
    assert 'nulu = "nulu_harness_runtime:main"' in pyproject
...
def test_stage_runtime_rejects_a_noncanonical_executable_name(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="must be named nulu-harness-sdk-runtime-win-x64.exe"):
